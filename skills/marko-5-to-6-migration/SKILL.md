...
- [`cheatsheet.md`](https://github.com/marko-js/marko/blob/main/packages/runtime-tags/cheatsheet.md) — the Marko 6 syntax reference: a `DON'T` table of exactly the habits a Marko 5 migration leaves behind (`on-click("name")`, `input.renderBody`, `$ ` scriptlets, `class {}`, `{expr}`), plus golden rules for traps none of the files above cover (a top-level `>` silently closing a tag; native inputs staying uncontrolled until their `*Change` handler is added). On disk it ships as `node_modules/marko/cheatsheet.md`; **while the app is still on `marko@5`** read it from `node_modules/@marko/runtime-tags/cheatsheet.md` instead — marko@5 does not ship its own copy.
...
6. **Consider modernizing the bundler.** `@marko/vite`, `@marko/rollup`, and `@marko/run` are major-agnostic (e.g. `@marko/run` declares `"marko": "5 - 6"`), so this ships while still fully on Marko 5. `@marko/webpack` still peer-declares `marko: ^5`, and `@marko/build`/`@marko/serve` apps depend on `marko@^5` directly through `@marko/build` — such an app must replace its build tool **before** Flow A's finish line, not after, or it ends up with two Marko runtimes installed. Lasso also keeps working with Marko 6 — staying on it is viable; you only give up code-splitting of lazy imports (`with { load }` needs the bundler-provided `linkAssets`; without it those imports just load eagerly). `browser.json` component wiring should be retired regardless.
...
3. Swap the dependency to `marko@^6` using the project's package manager (npm/pnpm/yarn — detect via the `packageManager` field or the lockfile; e.g. `npm rm marko && npm i marko@^6`). Marko 6 is the `latest` dist-tag; `@marko/compiler` stays 5.x — that is expected; only the `marko` runtime major changes.
