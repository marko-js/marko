...
3. Swap the dependency to `marko@^6` using the project's package manager (npm/pnpm/yarn — detect via the `packageManager` field or the lockfile; e.g. `npm rm marko && npm i marko@^6`). `@marko/compiler` stays 5.x — that is expected; only the `marko` runtime major changes.
