...
export default {
  translate: {
    exit(program) {
      ...
      traverseFast(program.node, "body", translateNode);
...
function translateNode(node: t.Node, container: t.Node | t.Node[]) {
  return (
    translateReadNode(node) || translateRegisteredFunctionNode(node, container)
  );
}
...
export function translateRegisteredFunctionNode(
  node: t.Node,
