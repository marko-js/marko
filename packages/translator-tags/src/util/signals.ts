...
import { traverseFast } from "./traverse-fast";
...
function getTranslatedExtraArgs(signal: { extraArgs?: t.Expression[] }) {
  if (signal.extraArgs) {
    if (!hasTranslatedExtraArgs.has(signal)) {
      ...
      traverseFast(signal, "extraArgs", translateRenderNode);
    }
...
export function writeSignals(section: Section) {
  ...
  for (const signal of signals) {
    traverseFast(signal, "render", translateRenderNode);

    for (const value of signal.values) {
      traverseFast(value, "value", translateRenderNode);
    }

    let effectDeclarator: t.VariableDeclarator | undefined;
    if (signal.effect.length) {
      traverseFast(signal, "effect", translateEffectNode);
...
export function writeRegisteredFns() {
  ...
  if (registeredFns) {
    for (const { id, extra, node } of registeredFns) {
      let fn: t.FunctionDeclaration;

      if (extra.referencesScope) {
        fn = t.functionDeclaration(
          t.identifier(id),
          extra.referencedBindingsInFunction
            ? [
                scopeIdentifier,
                t.assignmentPattern(
                  createScopeReadPattern(
            ...
            : [scopeIdentifier],
          t.blockStatement(toReturnedFunction(node)),
        );
      } else if (extra.referencedBindingsInFunction) {
        fn = t.functionDeclaration(
          t.identifier(id),
          [
            createScopeReadPattern(
        ...
        if (node.type === "FunctionDeclaration") {
          ...
          fn = t.functionDeclaration(
            t.identifier(id),
            node.params as t.FunctionDeclaration["params"],
    ...
    for (const { id, extra } of registeredFns) {
      statements.push(
        t.expressionStatement(
          callRuntime(
            "register",
...
function translateRenderNode(node: t.Node) {
  return (
    translateAssignmentNode(node) ||
    translateReadNode(node) ||
    translateRegisteredFunctionNode(node)
  );
}

function translateEffectNode(node: t.Node) {
  return translateAssignmentNode(node) || translateReadNode(node);
}

function translateReadNode(node: t.Node) {
  switch (node.type) {
    case "Identifier":
...
function translateAssignmentNode(node: t.Node) {
  switch (node.type) {
...
const registeredFnsForProgram = new WeakMap<
  t.Program,
  {
    id: string;
    node: t.Function;
    extra: RegisteredFnExtra;
  }[]
>();
export function translateRegisteredFunctionNode(node: t.Node) {
  switch (node.type) {
    case "ClassMethod": {
...
function getRegisteredFnExpression(node: t.Function) {
  ...
  if (isRegisteredFnExtra(extra)) {
    ...
    registedFns.push({ id, node, extra });

    if (extra.referencedBindingsInFunction || extra.referencesScope) {
