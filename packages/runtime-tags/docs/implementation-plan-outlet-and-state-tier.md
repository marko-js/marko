...
* **Feature 1 — Compiler‑level outlet extraction**: render *just* the route subtree of a
...
hole *assignment* + control‑flow reconciliation **driven by a serialized discriminant**).
...
* **Translator lowering** — a variant of `visitors/program/dom.ts`. Start from the dom
  ...
  * **Holes** (`_text`/`_attr`/…): keep the *leaf assignment* signals (they apply values
...
| Stripped derivation needed by a hole | Serialize the *final* value (server computes); the chunk only assigns |
