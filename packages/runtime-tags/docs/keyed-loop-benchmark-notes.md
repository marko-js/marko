...
A native event handler reads its referenced values *live* when it fires, so a
...
The decisive evidence is **non-reproducibility**: the *set* of lines where Marko
...
~1% — inside the noise band. (Sanity check: running *Solid against itself* twice
...
- **Per-row label signals for `03_update`** (Solid's model: each row's label is
  ...
  map entry to *every* row at create — regressing the entire create family — and
