...
### A2. Content signal may be missing the reference group of its param defaults — resolved 2026-07-10 (real bug, but a different one; fixed by making defaults first class)

The repro spike surfaced the actual defect behind the suspicion: top-level assignment-pattern params (`<child|a, b = x|>`) never got a binding at all — `createBindingsAndTrackReferences` had no `AssignmentPattern` case — so DOM output referenced an unbound identifier (CSR crash) and never re-applied the default (stale after resume). Rather than extending the pre-analyze rewrite that covered defaults nested in destructures, all default values are now first class in the binding graph: the `AssignmentPattern` case in `references.ts` creates a source binding for the applied value plus a derived binding computed from it, an `AssignmentPattern` translate visitor registers the derivation the way `<const>` does for its variable, with source reads lowered by the shared render node replacement, and the HTML translation materializes the source variable at its emission sites (`strip-default-values.ts`) so it stays serializable for resume. The pre-analyze desugar (`getAssignmentInsertions`) is deleted; `getExprRoot` treats a default value in a binding position as its own expression root so each default keeps a distinct reference group. A companion fix in `writeParamsToSignals` makes the inlined known/define args path match the runtime params applier (attrs object only written into the post-args slot when the generic branch would append one; remaining declared params applied as `undefined` so joined signals settle). Both TODO comments removed; `undefined` at the content site is correct and now documented. Covered by the `param-top-level-default` fixture (known child + inlined define + `<for>` destructured default, defaults reading parent state, explicit falsy arg beating the default); defaulted `${b}` text nodes additionally gained resume markers the old rewrite-based output lacked.

### A3. Shared property aliases ignore destructure defaults — resolved 2026-07-10 (fixed by construction in the defaults unification)

The "intermediate binding" the TODO asked for is exactly the source binding the first class defaults change introduced: the canonical `propertyAliases` root always holds the raw property value (a plain destructure roots the user binding, a defaulted one roots its source binding, never the derived binding), so later aliases of the same property observe the value before any default is applied, and each defaulted alias derives with its own fallback and its own reference group off the shared slot. Verified in both orders (plain first / default first) and with two different defaults via compile probes, and at runtime by the `destructure-alias-default` fixture: withdrawing the property through a state change activates each side's default dynamically, mutating a default's dependency updates only the defaulted texts, and SSR/CSR/resume equivalence holds throughout. The TODO comment is replaced with an explanation of the invariant; the adjacent "should prefer declared properties as alias roots" note is a separate naming preference and stays.
...
2. **Next:** repro fixtures for ~~A2~~ (done 2026-07-09 — real bug found and fixed), ~~A3~~ (done 2026-07-10 — resolved by the defaults unification), A4, A5 — each is a half-day spike that converts a suspicion into either a scoped bug issue or a deleted TODO. File issues for whatever is confirmed.
