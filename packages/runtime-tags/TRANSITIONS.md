...
The problem is what happens when the value an existing `<await>` is waiting on *changes*:
...
Nothing about this requires new rendering machinery. The state has already advanced; the new promise started when the update ran; the queue already knows every DOM change. The only new concept is *who* applies the queue, and *when*.
