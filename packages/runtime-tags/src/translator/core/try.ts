  ...
  translate: translateByTarget({
    html: {
      ...
      exit(tag) {
        ...
        // unique -- filename + section + accessor), the pending-boundary
        // half of the possession echo: runtime scope ids drift between the
        // document and update renders (matched scopes elide), but this
        // compile constant is identical in both, so a later navigation's
        // echo can tell the server "the client still shows this boundary's
        // placeholder" (see `_have`/`_try` in dom/update.ts and
        // html/writer.ts). Persisted only, and only for a `<try>` that can
        // actually show a placeholder; the html runtime stashes it on the
        // parent scope only once a document render's placeholder actually
        // ships (`flushPlaceholder`), so non-persisted output and persisted
        // output with no pending boundaries stay byte-identical.
        const siteId =
          isPersisted() && tag.node.extra?.attributeTags?.["@placeholder"]
            ? t.stringLiteral(
                getUpdateBoundaryRegisterId(section, getScopeAccessor(nodeRef)),
              )
            : undefined;
