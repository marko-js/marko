  ...
  while ((path = pending.pop())) {
    if (path.isConditionalExpression()) {
      ...
      if (binding.kind === "module") {
        ...
        if (decl.extra?.loadImport) continue;
        const request = decl.extra?.tagImport || decl.source.value;
        if (!request.endsWith(".marko") || imports?.includes(request)) {
          continue;
        }
        const childFile = loadFileForImport(file, request);
        if (childFile && childFile.ast.program.extra.featureType !== "class") {
...
export function getEscapedTemplateImports(program: t.NodePath<t.Program>) {
  ...
  for (const statement of program.get("body")) {
    ...
    // `load=` imports pair their `?persisted` entry via their ready channel.
    if (decl.extra?.loadImport) continue;
    const request = decl.extra?.tagImport || decl.source.value;
    if (!request.endsWith(".marko")) continue;
