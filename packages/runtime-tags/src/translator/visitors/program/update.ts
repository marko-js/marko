...
function buildMerge(
  ...
  switch (merge.kind) {
    ...
    case "child": {
      if (merge.load) {
        // `load=` lazy child: no import here -- the child's `?update`
        // module rides its lazy chunks and registers under the
        // compile-constant id both sides derive. `_update_load` merges
        // when it has registered and parks the patch until it loads.
        // The child's trigger-gated loader also registers under its
        // asset/ready id so a fragment delivering the child's markup can
        // start the load and replay its parked keyed resume batch (see
        // `_load_ready` in dom/load.ts).
