 ...
 * `(patch, live) => { ... }` that apply a persisted update-render patch to live
...
function buildMergeStatements(
  ...
  // `UpdateChild:` links; controllable semantics recover from the live
...
function buildMerge(
  ...
  switch (merge.kind) {
    ...
    case "child": {
      ...
      // `UpdateChild:` key instead, and the interpreter call above descends
