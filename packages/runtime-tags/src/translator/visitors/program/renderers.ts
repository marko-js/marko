...
import { forEachSectionReverse, type Section } from "../../util/sections";
...
/** Visits every shell-capable section with the ids it registers under. The
 * client's static registration derives from the same walk, so both sides of
 * a held shell name exactly the same set. */
export function forEachSectionShell(
  rootSection: Section,
  templateId: string,
  visit: (
    section: Section,
  ...
  forEachSectionReverse((section) => {
    ...
    if (template === undefined || walks === undefined) {
      if (process.env.SHELL_DEBUG) {
        console.error(
          ...
          walks === undefined ? "W" : "",
          JSON.stringify(meta.writes)?.slice(0, 300),
          "|",
          JSON.stringify(meta.walks)?.slice(0, 200),
        );
      }
      return;
    }
    const ids = [getResumeRegisterId(section, "update")];
    ...
    visit(section, ids, template, walks);
  });
}
...
export default {
  translate: {
    exit(program: t.NodePath<t.Program>) {
      ...
      forEachSectionShell(
        program.node.extra!.section!,
        program.hub.file.metadata.marko.id,
        (_section, ids, template, walks) => {
          for (const id of ids) shells[id] = [template, walks];
        },
      );
