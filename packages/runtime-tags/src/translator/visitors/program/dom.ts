  ...
  translate: {
    ...
    exit(program) {
          ...
          if (
            ...
            if (getSectionParentIsOwner(childSection)) {
              ...
              let renderer = callRuntime(
                ...
                // update can swap a dynamic tag to a renderer the live page
                // has never rendered (a cross-route navigation's divergence
                // point), resolved from the registry by the serialized id.
                // Fragment-first builds deliver that divergence as a
                // fragment frame instead, so the registration -- the one
                // module-level side effect pinning the section's
                // construction material (and the child imports it pulls)
                // into navigation chunks -- is dropped and the material
                // tree-shakes.
                (
                  isPersistedEntryBuild()
                    ? !isPersistedFragments()
      ...
      if (domExports.updateGeneric) {
        // Parent update entries dispatch this template's patch scopes
        // through the bare generic interpreter based on the analyze phase
        // proving its update module would be exactly that -- so its
        // `?update` module (where the equivalent check lives) may never
        // build. Effects (`_update_pair`) or `$global` re-runs in the
        // render graph mean that proof was wrong and must fail loudly.
      ...
      // `?persisted` entry load, or pre-navigation interactivity reads undefined.
