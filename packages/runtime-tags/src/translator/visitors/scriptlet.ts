  ...
  translate: {
    exit(scriptlet) {
      ...
      if (isHTML) {
        ...
        const ids = Object.keys(scriptlet.getOuterBindingIdentifiers());
        if (ids.length) {
          const selfRequest = `./${basename(
            scriptlet.hub.file.opts.filename as string,
          )}`;
          scriptlet.replaceWith(
            t.importDeclaration(
              ...
              t.stringLiteral(selfRequest),
            ),
          );
          // The entry shares the plain module's single-instance state
          // (census site 33) — the ModuleStateLink mirrors this kind.
          recordPlanImport(scriptlet.hub.file, selfRequest, "module-state");
        } else {
          scriptlet.remove();
        }
      } else {
