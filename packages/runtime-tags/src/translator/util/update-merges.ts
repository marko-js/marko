...
/** Reserves a keyed loop's possession-site id from its analyze path. */
export function recordUpdatePossessionLoop(
  ...
  onFinalizeReferences(() => {
    if (isUpdateRequestDerivedSite(extra)) {
      getUpdateSiteRegisterId(section, "for", getScopeAccessor(binding));
    }
  });
}
