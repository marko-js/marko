...
export type UpdateMerge =
  ...
  | {
       ...
       * (`_load_ready`) so a persisted update delivering the child's
       * markup inside a fragment can start the load and replay the parked
       * keyed resume batch -- the data-driven equivalent of the document's
       * injected trigger script (see custom-tag.ts's load branch and
       * dom/update.ts).
       */
...
// A `<try>` placeholder boundary's per-site id -- same recipe as
// `getUpdateDynamicRegisterId` (a compile constant identical across the
// document and update renders), mirrored under its own suffix rather than
// reused so a try and a dynamic tag can never collide on the same accessor's
// register id. The possession echo (`_have`/`BOUNDARY_SITE_PREFIX` in
// dom/update.ts) keys a still-pending matched boundary by it.
export function getUpdateBoundaryRegisterId(
  section: Section,
  accessor: string | number,
) {
  return getResumeRegisterId(section, undefined, `update_boundary_${accessor}`);
}
