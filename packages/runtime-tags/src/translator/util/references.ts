    ...
    forEachSectionReverse((section) => {
      if (
        ...
        if (isRequestDerivedSerializeReason(branchReason)) {
          ...
          // by re-running the input expression (its value may not exist in
          // the browser at all -- a `server import`ed nav list). Params
          // whose loop expression carries no sources (a stable/render-once
          // set) would otherwise classify render-once and their holes
          // would never capture, leaving fresh-constructed branches with
          // empty param-derived content. Taint them like a `$global` read
          // (markers/captures gated on the persisted flag, no value-signal
          // serialization) and flow it through everything derived from
          // them. State-driven sets are excluded exactly as in the
          // update-merge gates -- the server never pairs into them.
          if (
            section.params &&
            !isStateSerializeReason(
