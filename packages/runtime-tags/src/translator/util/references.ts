...
function resolveDerivedSources(binding: Binding) {
  ...
  if (exprs === undefined || exprs === true) {
    ...
    const valueExprs = filter(
      exprs,
      (expr) =>
        isReferencedExtra(expr) && bindingUtil.has(expr.downstream, binding),
...
function computeBindingSerialization(
  ...
  // A let's change handler serializes under its own accessor; the whole
  // answer carries it so a parent registers what it passes.
  if (binding.reserveSize) {
    reason = mergeSources(
      reason,
      getSerializeReason(
        binding.section,
