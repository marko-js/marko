...
export type MembraneCause = (typeof MembraneCause)[keyof typeof MembraneCause];
...
/** Analyze-time: a same-compile content value (a define body referenced by
 * `content=` or any other render-site expression) renders under this
 * section, so its nuclei count here. */
export function addLocalChildTree(section: Section, root: Section) {
  const record = {
    root,
    // The program's section list is appended in place; by classification
    // time it enumerates the body's descendants.
    sections: getProgram().node.extra.sections || [root],
  };
  const trees = childTrees.get(section);
  if (trees) trees.push(record);
  else childTrees.set(section, [record]);
