...
// Patch analyze work needing resolved sources: runs inside reference
// finalization but BEFORE reason groups freeze and call sites classify.
const [getPatchFinalizers] = createProgramState<(() => void)[]>(() => []);
export function onFinalizePatch(finalize: () => void) {
  getPatchFinalizers().push(finalize);
}
...
export function finalizePatch() {
  ...
  const finalizers = getPatchFinalizers();
