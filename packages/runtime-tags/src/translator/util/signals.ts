...
function inContentSection(section: Section | undefined) {
  while (section?.parent) {
    if (!section.isBranch && !section.isBoundary && section.downstream) {
      return true;
    }
    section = section.parent;
  }
...
export function writeLocalFill(section: Section, binding: Binding) {
  ...
  forEach(binding.sources?.param, (upstream) => {
    ...
    if (bindingUtil.has(section.referencedClosures, upstream)) {
      initIds +=
  ...
  return initIds
    ...
    : t.logicalExpression("&&", owned, write);
}
  ...
  forEach(section.referencedClosures, (closure) => {
    ...
    if (closure.sources && closure.type !== BindingType.constant) {
      if (isDynamicClosure(section, closure)) {
        ...
        // A scriptless patch page or a shell chain registers no subscribe, and
        // one the client cannot change is shaken with its owner's signal.
        const changeable =
  ...
  if (
    ...
    forEach(getPatchFillBindings(section), (binding) => {
      ...
      if (change) {
        ...
        const bindOwned = change.reason.forced
          ? undefined
