      ...
      (boundary.onNext = () => {
        switch (!boundary.count && boundary.flush()) {
          ...
          case FlushStatus.complete: {
            // `consume` can abort (a placeholder flush failing its render or
            // an unsupported-shape guard); the abort listener re-enters this
            // handler and rejects, so only flush html when still live.
  ...
  #read(
    ...
    const onNext = (boundary.onNext = (write?: boolean) => {
      ...
      if (status === FlushStatus.aborted) {
        ...
        head = head.consume();
        // `consume` can abort (a placeholder flush failing its render or an
        // unsupported-shape guard); the abort listener re-enters this handler
        // (reporting through `onAbort` and NOOPing `boundary.onNext`), so
        // bail instead of flushing html into the errored stream.
