...
export function encodePersistedPossession(
  ...
  if (sourceEntries.length > MAX_ITEMS) return "";
  try {
...
function writeNumberSet(writer: Writer, values: number[]) {
  values.sort((a, b) => a - b);
  const candidates = [encodeNumberList(writer.descriptor, values, "F")];
  if (
    values.every(Number.isSafeInteger) &&
    canZigzag(values[0]) &&
    values.every((value, index) => !index || value > values[index - 1])
  ) {
    candidates.push(encodeNumberList(writer.descriptor, values, "L"));
    candidates.push(encodeNumberRanges(writer.descriptor, values));
    const bitset = encodeNumberBitset(writer.descriptor, values);
    if (bitset) candidates.push(bitset);
  }
  let shortest = candidates[0];
  for (let i = 1; i < candidates.length; i++) {
    if (candidates[i].length < shortest.length) shortest = candidates[i];
  }
  writer.add(shortest);
}
...
function readPresenceGroup(reader: Reader, possession: PersistedPossession) {
  ...
  const values =
    reader.peek() === "Q" ? readStringSet(reader) : readNumberSet(reader);
  for (const value of values) {
    last.key = value;
