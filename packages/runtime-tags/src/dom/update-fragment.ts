...
export function applyBoundaryBody(
  ...
  // Owner fills applied before this dispatch resolved the boundary's patch id
  // to its patch-local scope; re-point them at the adopted live branch.
  const patchBranch = context[FragmentContextProp.GetScope](patchBranchId);
  context[FragmentContextProp.Adopt](patchBranchId, tryBranch);
  stampFragmentScopes(context, scopeIds);
  if (scopeIds) {
    for (const id of scopeIds) {
      const scope = context[FragmentContextProp.GetScope](id);
      if (scope[AccessorProp.Owner] === patchBranch) {
        scope[AccessorProp.Owner] = tryBranch;
      }
    }
  }
  const tpl = document.createElement("template");
