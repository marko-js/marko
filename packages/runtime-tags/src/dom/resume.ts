...
// Ready ids whose load settled with failure: they can never flip ready, so
// parked dispatches waiting on them must fail closed instead of hanging.
let failedReadyIds: Set<string> | undefined;
export function readyPersistedFailed(readyId: string) {
  (failedReadyIds ||= new Set()).add(readyId);
  flushReadyUpdates();
}
export function isReadyFailed(readyId: string) {
  return !!failedReadyIds?.has(readyId);
}
...
export function bumpNavEpoch(global: Scope[typeof AccessorProp.Global]) {
