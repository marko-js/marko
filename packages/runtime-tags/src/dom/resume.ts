...
export interface RenderData {
  ...
  // Effect entries parked until their registry id registers (persisted
  // entries register on lazy load).
  pe?: ResumeData;
            ...
            if (typeof serialized === "string") {
              ...
              while (nextToken()) {
                if (/\D/.test(lastToken)) {
                  ...
                  if (!lastEffect) {
                    // Registered later (persisted-entry ids land when the
                    // lazy entry loads); park the rest for a later pass.
                    (render.pe ||= []).push(
