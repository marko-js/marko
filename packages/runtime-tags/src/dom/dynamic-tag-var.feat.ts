import { AccessorProp, RendererProp, type Scope } from "../common/types";
...
installDynamicTagVar((branch: Scope, renderer) => {
  ...
  } else if (!renderer[RendererProp.Returns]) {
    // Content without a `<return>` never calls `_return`.
    branch[AccessorProp.TagVariable]!(undefined);
  }
});
