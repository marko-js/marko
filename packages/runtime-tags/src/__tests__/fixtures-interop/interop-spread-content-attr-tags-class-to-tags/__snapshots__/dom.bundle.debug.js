...
const $walks = " b";
const $setup = () => {};
const $for_content__item__script = _script("__tests__/components/tags-list.marko_1_item#2", ($scope) => _attrs_script($scope, "#div/0"));
...
const $input_id = ($scope, input_id) => _attr($scope["#section/0"], "id", input_id);
const $for = /*@__PURE__*/ _for_of_unkeyed("#section/0", "<div></div>", " ", 0, $for_content__$params);
const $input_item = ($scope, input_item) => $for($scope, [input_item]);
const $input = ($scope, input) => {
	$input_id($scope, input.id);
	$input_item($scope, input.item);
};
var tags_list_default = /*@__PURE__*/ _template("__tests__/components/tags-list.marko", $template, " b", 0, $input);
