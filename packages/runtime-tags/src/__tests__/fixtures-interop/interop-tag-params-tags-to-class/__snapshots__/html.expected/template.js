...
export default _._template("__tests__/template.marko", input => {
  ...
  _._dynamic_tag($scope0_id, "#text/0", _classLayout, {}, _._content_resume("__tests__/template.marko_1_content", (baseCount, message) => {
    ...
    const $scope1_reason = _._scope_reason(),
      $sg__baseCount = _._serialize_guard($scope1_reason, /* baseCount */0);
    _._html(`<h1>${_._escape(message)}${_._el_resume($scope1_id, "#text/0", (_._serialize_guard($scope1_reason, /* message */1)))}</h1><button id=tags>${_._escape(multiplier)}${_._el_resume($scope1_id, "#text/2")} * ${_._sep(($sg__baseCount))}${_._escape(baseCount)}${_._el_resume($scope1_id, "#text/3", ($sg__baseCount))} = <!>${_._escape(multiplier * baseCount)}${_._el_resume($scope1_id, "#text/4")}</button>${_._el_resume($scope1_id, "#button/1")}`);
