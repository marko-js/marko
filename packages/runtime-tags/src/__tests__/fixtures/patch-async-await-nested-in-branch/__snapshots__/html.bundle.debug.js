		...
		if (input.show) {
			...
			_await($scope1_id, "#text/0", input.outer, (outer) => {
				...
				_await($scope2_id, "#text/0", input.inner, (inner) => {
					...
					_subscribe(_unfilled_if($scope0_reason, 4) && $await_content__outer__closures, _scope($scope3_id, { _: _scope_with_id($scope2_id) }, "__tests__/template.marko", "4:8"), _client_guard($scope0_reason, 4) && "__tests__/template.marko_3_outer#2/subscribe");
				...
				}, "__tests__/template.marko", "3:6", { outer: "3:12" }), _client_guard($scope0_reason, 5) && "__tests__/template.marko_2_input_inner#6/subscribe", 0);
