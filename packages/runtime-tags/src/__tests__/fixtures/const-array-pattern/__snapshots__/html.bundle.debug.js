...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	let n = 1;
	_html_opens("__tests__/template.marko:3:1", "__tests__/template.marko:4:1"), _html(`<button>inc <!>${_escape(n)}${_el_resume($scope0_id, "#text/1")}</button>${_el_resume($scope0_id, "#button/0")}<div>${_escape(first)}${_el_resume($scope0_id, "#text/2", $sg__input_list)}|${_sep($sg__input_list)}${_escape(second)}${_el_resume($scope0_id, "#text/3", $sg__input_list)}|${_sep($sg__input_list)}${_escape(others.join(","))}${_el_resume($scope0_id, "#text/4", $sg__input_list)}</div>`);
