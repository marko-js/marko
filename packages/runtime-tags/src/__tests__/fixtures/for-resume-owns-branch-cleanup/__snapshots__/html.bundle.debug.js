...
var child_default = _template("__tests__/tags/child.marko", (input) => {
	...
	_for_to(count, 0, 1, (i) => {
		...
		_html_opens("__tests__/tags/child.marko:3:3"), _html(`<p>item ${_escape(i)}</p>`);
...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	let outer = true;
	let count = 0;
	_html_opens("__tests__/template.marko:3:1", "__tests__/template.marko:4:1", "__tests__/template.marko:5:1"), _html(`<div id=ref>init</div><button id=o>O</button>${_el_resume($scope0_id, "#button/0")}<button id=a>A</button>${_el_resume($scope0_id, "#button/1")}`);
	_if(() => {
		if (outer) {
			const $scope1_id = _scope_id();
