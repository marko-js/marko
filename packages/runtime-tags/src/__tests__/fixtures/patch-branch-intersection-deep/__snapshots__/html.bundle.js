		...
		if (input.show) {
			...
			_if(() => {
				if (input.inner) {
					...
					_subscribe($count__closures, _subscribe(_source_if($scope0_reason, 3) && $input_title__closures, _scope($scope2_id, { _: _scope_with_id($scope1_id) }), _client_guard($scope0_reason, 3) && "a2"), "a3");
					return 0;
				}
