// tags/widget/index.marko
const $template$1 = "<p><!>:<!></p>";
const $walks$1 = "D%c%l";
_shells({ "__tests__/tags/widget/index.marko": "__tests__/tags/widget/index.marko !;D%c%;<p><!>:<!></p>" });
var widget_default = _template_persisted("__tests__/tags/widget/index.marko", (input) => {
	const $scope0_reason = _scope_reason(), $scope0_page = _page_render();
	const $scope0_id = _scope_id();
	let mounted = 0;
	_html(`<p>${_patch_text($scope0_id, "#text/0", input.label, void 0, $scope0_reason, 0)}:${_text_resume($scope0_id, "#text/1", mounted, 2)}</p>`);
	_script($scope0_id, "__tests__/tags/widget/index.marko_0_mounted#5");
	_patch_value($scope0_id, "__tests__/tags/widget/index.marko0", mounted, 1);
	$scope0_page && _scope($scope0_id, { mounted }, "__tests__/tags/widget/index.marko", 0, { mounted: "1:6" });
}, 0, 0);

// template.marko
const $template = "<main><!><button class=outer>+</button></main>";
const $walks = "D%b l";
_shells({ "__tests__/template.marko": "__tests__/template.marko !__tests__/template.marko_0;D%b ;<main><!><button class=outer>+</button></main>" });
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	const $scope0_reason = _scope_reason(), $scope0_page = _page_render();
	const $scope0_id = _scope_id();
	let show = false;
	_html("<main>");
	if ($scope0_page) _if(() => {
		if (show) {
			const $scope1_id = _scope_id();
			const $childScope = _peek_scope_id();
			widget_default({ label: input.label });
			_scope($scope1_id, { "#childScope/0": _existing_scope($childScope) }, "__tests__/template.marko", "3:4");
			return 0;
		}
	}, $scope0_id, "#text/0", 1, 1, 1, 0, 1);
	_html(`<button class=outer>+</button>${_el_resume($scope0_id, "#button/1")}</main>`);
	_script($scope0_id, "__tests__/template.marko_0");
	$scope0_page ? _scope($scope0_id, {
		input_label: input.label,
		show
	}, "__tests__/template.marko", 0, {
		input_label: ["input.label"],
		show: "1:6"
	}) : _filled_guard($scope0_reason, 0) && _patch_value($scope0_id, "__tests__/template.marko0", input.label);
}, 1, () => [widget_default]);
