...
var template_default = _template("a", (input) => {
	...
	_for_of(cfg.options, (opt) => {
		...
		_html(`<option${_attr_option_value(_hole_value($scope1_id, "Nvalue:a", opt.id, _persisted_reason()))}>${_escape(_hole_value($scope1_id, "b", opt.label, _persisted_reason()))}${_el_resume($scope1_id, "b", _persisted_reason())}</option>${_el_resume($scope1_id, "a", _persisted_reason())}`);
