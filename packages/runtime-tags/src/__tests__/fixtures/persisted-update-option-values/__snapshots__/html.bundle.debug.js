...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	_for_of(cfg.options, (opt) => {
		...
		_html(`<option${_attr_option_value(_hole_value($scope1_id, "PatchAttr:value:#option/0", opt.id, _persisted_reason()))}>${_escape(_hole_value($scope1_id, "PatchHole:#text/1", opt.label, _persisted_reason()))}${_el_resume($scope1_id, "#text/1", _persisted_reason())}</option>${_el_resume($scope1_id, "#option/0", _persisted_reason())}`);
		_persisted_reason() && writeScope($scope1_id, {}, "__tests__/template.marko", "5:4");
	}, function(opt) {
		return opt.key;
	}, $scope0_id, "#select/2", _persisted_reason(), _persisted_reason(), _persisted_reason(), "</select>", 1, "__tests__/template.marko_1_update");
