...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	_for_of(cfg.options, (opt) => {
		...
		_html(`<option${_attr_option_value(_hole_value($scope1_id, "UpdateAttr:value:#option/0", opt.id, _persisted_reason()))}>${_escape(_hole_value($scope1_id, "#text/1", opt.label, _persisted_reason()))}${_el_resume($scope1_id, "#text/1", _persisted_reason())}</option>${_el_resume($scope1_id, "#option/0", _persisted_reason())}`);
