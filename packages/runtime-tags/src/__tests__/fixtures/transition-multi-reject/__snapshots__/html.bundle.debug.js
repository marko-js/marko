...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	_try($scope0_id, "#text/2", _content_resume("__tests__/template.marko_1_content", () => {
		...
		_scope_reason();
		_await($scope1_id, "#text/0", count ? rejectAfter(new Error("BOOM")) : resolveAfter(count), (a) => {
			const $scope5_id = _scope_id();
			_html(`a: <!>${_escape(a)}${_el_resume($scope5_id, "#text/0")}`);
			writeScope($scope5_id, {}, "__tests__/template.marko", "8:4");
		});
		...
		catch: attrTag({ content: _content_resume("__tests__/template.marko_4_content", (err) => {
			const $scope4_reason = _scope_reason(), $sg__err_message = _serialize_guard($scope4_reason, 0);
			const $scope4_id = _scope_id();
			_html(`error: ${_sep($sg__err_message)}${_escape(err.message)}${_el_resume($scope4_id, "#text/0", $sg__err_message)}`);
			_serialize_if($scope4_reason, 0) && writeScope($scope4_id, {}, "__tests__/template.marko", "7:4");
		}, $scope0_id) })
	});
	_try($scope0_id, "#text/3", _content_resume("__tests__/template.marko_2_content", () => {
		...
		_await($scope2_id, "#text/0", resolveAfter(count * 10, 2), (b) => {
			...
			_html(`b: <!>${_escape(b)}${_el_resume($scope7_id, "#text/0")}`);
			writeScope($scope7_id, {}, "__tests__/template.marko", "12:4");
		});
	...
	}, $scope0_id), { placeholder: attrTag({ content: _content_resume("__tests__/template.marko_6_content", () => {
		_scope_reason();
