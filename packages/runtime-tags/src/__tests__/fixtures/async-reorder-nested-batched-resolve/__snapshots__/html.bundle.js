				...
				_await($scope3_id, "a", promiseA, (value) => {
					...
					_try($scope6_id, "b", _content_resume("a7", () => {
						...
						_await($scope7_id, "a", promiseB, (value) => {
							...
							}, $scope8_id), { placeholder: attrTag({ content: _content_resume("a11", () => {
								_scope_reason();
								_scope_id();
								_html("LOADING B2");
							}, $scope8_id) }) });
					...
					}, $scope6_id), { placeholder: attrTag({ content: _content_resume("a10", () => {
						_scope_reason();
						_scope_id();
						_html("LOADING B1");
					}, $scope6_id) }) });
