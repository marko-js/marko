...
var grand_child_default = _template("__tests__/grand-child.marko", (input) => {
	...
	let n = input.value;
	_html_opens("__tests__/grand-child.marko:4:1"), _html(`<button id=grand>grand:<!>${_escape(n)}${_el_resume($scope0_id, "#text/1")}:${_sep($sg__input_value)}${_escape(input.value)}${_el_resume($scope0_id, "#text/2", $sg__input_value)}</button>${_el_resume($scope0_id, "#button/0")}`);
	_await($scope0_id, "#text/3", resolveAfter(20, 2), (v) => {
		const $scope1_id = _scope_id();
		_html_opens("__tests__/grand-child.marko:6:3"), _html(`<span id=grand-await>${_escape(v)}</span>`);
	}, 0);
...
var child_default = _template("__tests__/child.marko", (input) => {
	...
	_html_opens("__tests__/child.marko:5:1"), _html(`<button id=child>child:<!>${_escape(count)}${_el_resume($scope0_id, "#text/1")}</button>${_el_resume($scope0_id, "#button/0")}`);
	_await($scope0_id, "#text/2", resolveAfter(10, 1), (value) => {
		const $scope1_id = _scope_id();
		_html_opens("__tests__/child.marko:7:3"), _html(`<span id=child-await>${_escape(value)}</span>`);
