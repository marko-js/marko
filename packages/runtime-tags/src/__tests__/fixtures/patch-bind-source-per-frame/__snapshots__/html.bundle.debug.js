...
var template_default = _template_patch("__tests__/template.marko", (input) => {
	...
	_await($scope0_id, "#text/1", input.two, () => {
		...
		}, "__tests__/template.marko", "7:4", { "EventAttributes:#button/0": ["...{ title: input.title, onClick: handler }", "8:25"] }), _client_guard($scope0_reason, 3) && "__tests__/template.marko_2_input_title#5/subscribe"), _client_guard($scope0_reason, 3) && "__tests__/template.marko_2_handler#9/subscribe");
	}, 1, "__tests__/template.marko_2*content", 1);
	_html(`<em>${_patch_text($scope0_id, "#text/2", count)}</em></main>`);
	$scope0_page ? _scope($scope0_id, {
		input_title: input.title,
		handler: _source_if($scope0_reason, 2) && handler,
		"ClosureScopes:input_title": $input_title__closures,
		"ClosureScopes:handler": $handler2__closures
	}, "__tests__/template.marko", 0, {
		input_title: ["input.title"],
		handler: "2:8"
	}) : _filled_guard($scope0_reason, 3) && _patch_write($scope0_id, "input_title", input.title);
}, 1, 0);
