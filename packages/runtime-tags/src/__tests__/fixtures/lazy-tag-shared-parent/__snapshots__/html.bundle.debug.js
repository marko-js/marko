...
var child_default = _template("__tests__/tags/child.marko", (input) => {
	...
	_html_opens("__tests__/tags/child.marko:1:1"), _html(`<span>${_escape(input.value)}${_el_resume($scope0_id, "#text/0", _serialize_guard($scope0_reason, 0))}</span>`);
...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	const $scope0_id = _scope_id();
	let value = 1;
	_html_opens("__tests__/template.marko:2:1"), _html(`<button>Inc</button>${_el_resume($scope0_id, "#button/0")}`);
