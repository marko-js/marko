...
_renderer_shells({
	"b1": ["<div class=shell><!></div>", "D%l"],
	"b": ["<div class=shell><!></div>", "D%l"]
});
...
var template_default = _template("a", (input) => {
	...
	const PanelA = { content: _content("a2", () => {
		...
		_scope_reason();
		_html(`<p class=a>Panel A: ${_sep(_persisted_reason())}${_escape(getLabel?.($global().topic))}${_el_resume($scope1_id, "a", _persisted_reason())}</p>`);
		_persisted_reason() && writeScope($scope1_id, {});
	}) };
	const PanelB = { content: _content("a3", () => {
		const $scope3_id = _scope_id();
		_scope_reason();
		_html(`<section class=b>Panel B: ${_sep(_persisted_reason())}${_escape(getLabel?.($global().topic))}${_el_resume($scope3_id, "a", _persisted_reason())}</section>`);
