...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	let mounted = false;
	_html_opens("__tests__/template.marko:1:1", "__tests__/template.marko:6:1", "__tests__/template.marko:11:1"), _html(`<script${_attr_nonce()} type=magic>
  A
...
<\/script>${_el_resume($scope0_id, "#script/1")}`);
	_if(() => {
		if (mounted) {
			...
			_html_opens("__tests__/template.marko:18:3"), _html(`<script${_attr_nonce()} type=magic>
    D
  <\/script>`);
