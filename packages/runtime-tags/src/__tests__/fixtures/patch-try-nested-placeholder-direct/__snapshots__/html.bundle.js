...
var template_default = _template_patch("a", (input) => {
	...
	_try($scope0_id, "a", _content_resume("a5", () => {
		...
		_try($scope1_id, "a", _content_resume("a4", () => {
			...
			$scope0_page && _subscribe(_unfilled_if($scope0_reason, 0) && $input_promise__closures, _scope($scope2_id, { _: _scope_with_id($scope1_id) }), _client_guard($scope0_reason, 0) && "a7", 0);
			$scope0_page && _resume_branch($scope2_id);
		}, $scope1_id), {
			placeholder: attrTag({ content: _content_shell("a1", $scope1_id) }),
			catch: attrTag({ content: _content_resume("a6", (err) => {
