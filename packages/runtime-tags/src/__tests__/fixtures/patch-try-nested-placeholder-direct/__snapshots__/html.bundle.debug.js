...
var template_default = _template_patch("__tests__/template.marko", (input) => {
	...
	_try($scope0_id, "#text/0", _content_resume("__tests__/template.marko_1*content", () => {
		...
		_try($scope1_id, "#text/0", _content_resume("__tests__/template.marko_2*content", () => {
			...
			$scope0_page && _subscribe(_unfilled_if($scope0_reason, 0) && $input_promise__closures, _scope($scope2_id, { _: _scope_with_id($scope1_id) }, "__tests__/template.marko", "4:6"), _client_guard($scope0_reason, 0) && "__tests__/template.marko_2_input_promise#3/subscribe", 0);
			$scope0_page && _resume_branch($scope2_id);
		}, $scope1_id), {
			placeholder: attrTag({ content: _content_shell("__tests__/template.marko_4*content", $scope1_id) }),
			catch: attrTag({ content: _content_resume("__tests__/template.marko_5*content", (err) => {
