	...
	_if(() => {
		if (input.show) {
			...
			_await($scope1_id, "#text/0", input.value, (v) => {
				...
				_html(`<b>${_patch_text($scope2_id, "#text/0", v, void 0, $scope0_owned, 2)}</b>`);
				_scope($scope2_id, {}, "__tests__/template.marko", "3:6");
			}, 1, "__tests__/template.marko_1_#text#0/await");
			_html("</section>");
			$scope0_reason && _scope($scope1_id, { _: _scope_with_id($scope0_id) }, "__tests__/template.marko", "1:2");
			return 0;
		}
	}, $scope0_id, "#text/0", 1, $sg__input_show, $sg__input_show, void 0, void 0, ["__tests__/template.marko_1*shell"], $scope0_owned, 1);
	_html(`<p>${_patch_text($scope0_id, "#text/1", input.label, void 0, $scope0_owned, 3)}</p>`);
	$scope0_reason && _scope($scope0_id, { input_value: _source_if($scope0_reason, 1) && input.value }, "__tests__/template.marko", 0, { input_value: ["input.value"] });
}, 1, 0);
