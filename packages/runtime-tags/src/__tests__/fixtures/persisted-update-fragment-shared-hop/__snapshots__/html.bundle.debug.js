...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	const Dashboard = { content: _content("__tests__/template.marko_3_content", () => {
		...
		let tally = 0;
		_html(`<button class=bump>tally <!>${_escape(tally)}${_el_resume($scope3_id, "#text/1")}</button>${_el_resume($scope3_id, "#button/0")}<p class=greeting>hello ${_sep(_persisted_reason())}${_escape(_hole_value($scope3_id, "UpdateHole:#text/2", $global().user, _persisted_reason()))}${_el_resume($scope3_id, "#text/2", _persisted_reason())}</p>`);
