...
var template_default = _template("a", (input) => {
	...
	_for_of(input.children, (child) => {
		...
		_html(`<span${_attr("id", child.id)}>${_escape(child.text)}${_el_resume($scope1_id, "b", $sg__input_children)}</span>${_el_resume($scope1_id, "a", $sg__input_children)}`);
		$si__input_children && writeScope($scope1_id, {});
	}, "id", $scope0_id, "a", $sg__input_children, $sg__input_children, $sg__input_children, "</div>", 1, 1);
