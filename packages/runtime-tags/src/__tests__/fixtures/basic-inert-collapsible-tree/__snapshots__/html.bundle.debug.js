...
const $content = (input) => {
	...
	_for_of(input.comments, (comment, i) => {
		...
		let open = true;
		_html(`<li${_attr("id", id)}${_attr("hidden", !open)}><span>${_text_resume($scope1_id, "#text/1", comment.text, $sg__input_comments)}</span><button>${_text_resume($scope1_id, "#text/3", open ? "[-]" : "[+]")}</button>${_el_resume($scope1_id, "#button/2")}`);
		_if(() => {
			if (comment.comments) {
