...
const $content = (input) => {
	...
	_for_of(input.comments, (comment, i) => {
		...
		let open = true;
		_html(`<li${_attr("id", id)}${_attr("hidden", false)}><span>${_text_resume($scope1_id, "b", comment.text, $sg__input_comments)}</span><button>${_text_resume($scope1_id, "d", "[-]")}</button>${_el_resume($scope1_id, "c")}`);
		_if(() => {
			if (comment.comments) {
