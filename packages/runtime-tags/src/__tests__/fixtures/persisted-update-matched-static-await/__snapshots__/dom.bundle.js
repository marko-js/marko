...
_enable_catch();
const $count = _var_resume("a20", /*@__PURE__*/ _let_persisted(8, ($scope) => _text($scope.c, $scope.i)));
const $setup__script = _script_shared(($scope) => _on($scope.b, "click", function() {
	$count($scope, $scope.i + 1);
}));
const $count_seed = _update_signal("a20");
...
// template.marko
_enable_catch();
const $placeholder_content2 = _content_resume("a10", "loading reviews…", "b");
...
const $setup__script = _script_update("a15", ($scope) => _on($scope.b, "click", function() {
	$count($scope, $scope.i + 1);
}));
