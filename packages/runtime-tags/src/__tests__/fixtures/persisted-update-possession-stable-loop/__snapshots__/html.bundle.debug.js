...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	_for_of($global().groups, (group) => {
		...
		_for_of(ITEMS, (item) => {
			...
			_html(`<li>${_escape(_hole_value($scope3_id, "UpdateHole:#text/0", item.id, _persisted_reason()))}${_el_resume($scope3_id, "#text/0", _persisted_reason())}: `);
		...
		}, "id", $scope2_id, "#ul/0", _persisted_reason(), _persisted_reason(), 0, "</ul>", 1);
