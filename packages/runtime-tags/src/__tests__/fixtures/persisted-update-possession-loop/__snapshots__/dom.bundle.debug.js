...
const $for_content__update = (patch, live) => {
	if ("ConditionalRenderer:#text/1" in patch) _update_dynamic(patch, live, "ConditionalRenderer:#text/1", "BranchScopes:#text/1");
};
...
const $for_content__PanelA__OR__PanelB__OR__item_view = /*@__PURE__*/ _or(5, ($scope) => $for_content__dynamicTag($scope, $scope.item_view === "b" ? $scope._.PanelB : $scope._.PanelA), 2);
...
const $for_content__setup = ($scope) => {
	...
	_text($scope["#text/0"], serverLoopSentinel?.());
};
...
const $for = /*@__PURE__*/ _for_of("#ul/2", "<em>LOOP_ONLY_MARKUP</em><!><!><!>", "b%b%c", $for_content__setup, $for_content__$params);
