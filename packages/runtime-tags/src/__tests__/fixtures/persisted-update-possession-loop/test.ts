...
//
// (Two rows swapping in the SAME navigation still needs multiple fragments per
// update render -- a single capture per update today -- so that case falls back
// to a full nav until fragment capture is extended. This exercises one swap per
// nav.)
