...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	let count = 0;
	_html(`<style${_attr_nonce()}>${_patch_text_content($scope0_id, "#style/0", `.x { color: ${_to_text(input.on ? "red" : "blue")} }`, _escape_style, $scope0_reason, 0)}</style>${_el_resume($scope0_id, "#style/0")}<script${_attr_nonce()}>${_patch_text_content($scope0_id, "#script/1", `window.__label = ${_to_text(JSON.stringify(input.label))};`, _escape_script, $scope0_reason, 1)}<\/script>${_el_resume($scope0_id, "#script/1")}<p class=x>${_patch_text($scope0_id, "#text/2", input.label, void 0, $scope0_reason, 1)} ${_text_resume($scope0_id, "#text/3", count, 2)}</p><button>+</button>${_el_resume($scope0_id, "#button/4")}`);
