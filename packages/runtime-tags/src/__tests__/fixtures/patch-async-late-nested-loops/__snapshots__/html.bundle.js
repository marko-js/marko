...
_shells({
	...
	a1: "a1;D%c%;<em><!>.<!></em>",
	a2: "a2;b%;<!><!><!>",
	a: "a !a6;D%b ;<main><!><button>interactive</button></main>",
...
var template_default = _template_patch("a", (input) => {
	...
	_for_of(input.groups, (group) => {
		...
		const $for_content__group_id__closures = /* @__PURE__ */ new Set();
		_html("<section>");
		_for_of(group.items, (item) => {
			...
			_try($scope2_id, "a", _content_resume("a2", () => {
				...
				_await($scope3_id, "a", item.promise, (v) => {
					...
					_subscribe(_unfilled_if($scope0_reason, 0) && $for_content__group_id__closures, _scope($scope4_id, { _: _scope_with_id($scope3_id) }));
				}, 1, "a0");
				$scope0_page && _subscribe(_unfilled_if($scope0_reason, 0) && $for_content2__item_promise__closures, _scope($scope3_id, { _: _scope_with_id($scope2_id) }));
		...
		$scope0_page && _scope($scope1_id, {
			e: _source_if($scope0_reason, 0) && group?.id,
			f: $for_content__group_id__closures
		});
