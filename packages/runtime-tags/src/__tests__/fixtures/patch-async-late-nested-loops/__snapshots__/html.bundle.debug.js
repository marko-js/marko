...
var template_default = _template_patch("__tests__/template.marko", (input) => {
	...
	_for_of(input.groups, (group) => {
		...
		_for_of(group.items, (item) => {
			...
			_try($scope2_id, "#text/0", _content_resume("__tests__/template.marko_3*content", () => {
				...
				_await($scope3_id, "#text/0", item.promise, (v) => {
					...
					_subscribe(_unfilled_if($scope0_reason, 0) && $for_content__setup__closures, _scope($scope4_id, { _: _scope_with_id($scope3_id) }, "__tests__/template.marko", "6:12"));
		...
		$scope0_page && _scope($scope1_id, {
			"#LoopKey": _source_if($scope0_reason, 0) && group?.id,
			"ClosureScopes:#LoopKey": $for_content__setup__closures
		}, "__tests__/template.marko", "2:4", { "#LoopKey": ["group.id", "2:8"] });
