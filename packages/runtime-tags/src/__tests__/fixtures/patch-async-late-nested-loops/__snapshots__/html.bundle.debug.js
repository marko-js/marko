...
var template_default = _template_patch("__tests__/template.marko", (input) => {
	...
	_for_of(input.groups, (group) => {
		...
		const $for_content__group_id__closures = new Set();
		_html("<section>");
		_for_of(group.items, (item) => {
			...
			_try($scope2_id, "#text/0", _content_resume("__tests__/template.marko_3*content", () => {
				...
				_await($scope3_id, "#text/0", item.promise, (v) => {
					...
					_subscribe(_unfilled_if($scope0_reason, 0) && $for_content__group_id__closures, _scope($scope4_id, { _: _scope_with_id($scope3_id) }, "__tests__/template.marko", "6:12"));
				}, 1, "__tests__/template.marko_4*content");
				$scope0_page && _subscribe(_unfilled_if($scope0_reason, 0) && $for_content2__item_promise__closures, _scope($scope3_id, { _: _scope_with_id($scope2_id) }, "__tests__/template.marko", "5:10"));
		...
		$scope0_page && _scope($scope1_id, {
			group_id: _source_if($scope0_reason, 0) && group?.id,
			"ClosureScopes:group_id": $for_content__group_id__closures
		}, "__tests__/template.marko", "2:4", { group_id: ["group.id", "2:8"] });
