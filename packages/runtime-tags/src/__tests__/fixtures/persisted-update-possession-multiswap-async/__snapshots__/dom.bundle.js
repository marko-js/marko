...
const $placeholder_content = _content_resume("a2", "<p class=loading>loading…</p>", "b");
...
const $PanelAsync_content = _content_resume("a5", "<!><!><!>", "b%c", $PanelAsync_content__setup);
const $PanelB_content__setup = ($scope) => _text($scope.a, getLabel?.($scope.$.topic));
const $PanelB_content = _content_resume("a1", "<section class=b>B: <!></section>", "Db%l", $PanelB_content__setup);
const $PanelA_content__setup = ($scope) => _text($scope.a, getLabel?.($scope.$.topic));
const $PanelA_content = _content_resume("a0", "<span class=a>A: <!></span>", "Db%l", $PanelA_content__setup);
...
const $for_update = _update_for(2, "a8", (branch, args) => $for_content__update(args[0], branch));
...
_update_content("a5", $PanelAsync_content__update);
