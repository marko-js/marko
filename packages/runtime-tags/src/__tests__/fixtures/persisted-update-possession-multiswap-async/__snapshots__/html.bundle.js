...
var template_default = _template("a", (input) => {
	...
	const PanelA = { content: _content_resume("a0", () => {
	...
	const PanelB = { content: _content_resume("a1", () => {
	...
	const PanelAsync = { content: _content_resume("a5", () => {
		const $scope4_id = _scope_id();
		_scope_reason();
		_try($scope4_id, "a", _content_resume("a3", () => {
		...
		}, $scope4_id), { placeholder: attrTag({ content: _content_resume("a2", () => {
			_scope_reason();
			_scope_id();
	...
	_for_of($global().items, (item) => {
		...
		_dynamic_tag($scope2_id, "a", item.view === "b" ? PanelB : item.view === "c" ? PanelAsync : PanelA, {}, 0, 0, _persisted_reason() | _persisted_reason(), "a6");
