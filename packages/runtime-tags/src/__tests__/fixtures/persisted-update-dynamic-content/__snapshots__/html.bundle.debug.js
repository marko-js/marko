...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	let count = 0;
	_html(`<h1>${_escape(_hole_value($scope0_id, "#text/0", input.title, _persisted_reason()))}${_el_resume($scope0_id, "#text/0", $sg__input_title)}</h1><button class=count>clicked <!>${_escape(count)}${_el_resume($scope0_id, "#text/2")}</button>${_el_resume($scope0_id, "#button/1")}`);
	const Overview = { content: _content_resume("__tests__/template.marko_1_content", () => {
		const $scope1_id = _scope_id();
		const $scope1_reason = _scope_reason();
		_html(`<p>Overview of ${_sep($sg__input_title)}${_escape(_hole_value($scope1_id, "#text/0", input.title, _persisted_reason()))}${_el_resume($scope1_id, "#text/0", $sg__input_title)}: ${_sep($sg__input_summary)}${_escape(_hole_value($scope1_id, "#text/1", input.summary, _persisted_reason()))}${_el_resume($scope1_id, "#text/1", $sg__input_summary)}</p>`);
	...
	const Specs = { content: _content_resume("__tests__/template.marko_2_content", () => {
		...
		_for_of(input.specs, (spec) => {
			...
			_html(`<li>${_escape(_hole_value($scope3_id, "#text/0", spec.name, _persisted_reason()))}${_el_resume($scope3_id, "#text/0", $sg__input_specs)} is ${_sep($sg__input_specs)}${_escape(_hole_value($scope3_id, "#text/1", spec.value, _persisted_reason()))}${_el_resume($scope3_id, "#text/1", $sg__input_specs)}</li>`);
