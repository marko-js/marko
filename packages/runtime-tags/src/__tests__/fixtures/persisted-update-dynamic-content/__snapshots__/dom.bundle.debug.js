...
const $for_update = _update_for("#ul/0", "__tests__/template.marko_3_content/update", (branch, args) => _update_scope(args[0], branch));
...
const $Specs_content__input_specs = /*@__PURE__*/ _closure_get("input_specs", ($scope) => {
	if (!updating) {
		if (!updating) $Specs_content__for($scope, [$scope._.input_specs, function(spec) {
			return spec.name;
		}]);
	}
});
