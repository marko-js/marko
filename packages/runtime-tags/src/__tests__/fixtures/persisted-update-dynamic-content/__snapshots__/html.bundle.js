...
var template_default = _template("a", (input) => {
	...
	const Specs = { content: _content_resume("a2", () => {
		...
		_for_of(input.specs, (spec) => {
			...
			_html(`<li>${_escape(_hole_value($scope3_id, "a", spec.name, _persisted_reason()))}${_el_resume($scope3_id, "a", $sg__input_specs)} is ${_sep($sg__input_specs)}${_escape(_hole_value($scope3_id, "b", spec.value, _persisted_reason()))}${_el_resume($scope3_id, "b", $sg__input_specs)}</li>`);
