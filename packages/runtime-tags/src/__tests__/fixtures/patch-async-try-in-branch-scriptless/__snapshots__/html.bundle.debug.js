		...
		if (input.show) {
			...
			_try($scope1_id, "#text/0", _content_resume("__tests__/template.marko_2*content", () => {
				...
				_await($scope2_id, "#text/0", Promise.resolve(input.value), () => {
					...
					}, "__tests__/template.marko", "4:8"), _client_guard($scope0_reason, 2) && "__tests__/template.marko_3_input_value#4/subscribe");
				}, 1, "__tests__/template.marko_3*content");
				$scope0_page && _subscribe(_unfilled_if($scope0_reason, 2) && $input_value__closures, _scope($scope2_id, { _: _scope_with_id($scope1_id) }, "__tests__/template.marko", "3:6"), _client_guard($scope0_reason, 2) && "__tests__/template.marko_2_input_value#4/subscribe", 0);
				$scope0_page && _resume_branch($scope2_id);
			}, $scope1_id), { placeholder: attrTag({ content: _content_shell("__tests__/template.marko_4*content", $scope1_id) }) });
