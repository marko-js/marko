	...
	const Reports = { content: _content("__tests__/template.marko_2_content", () => {
		...
		_try($scope2_id, "#text/0", _content_resume("__tests__/template.marko_3_content", () => {
			...
			_await($scope3_id, "#text/0", getData($global().range), (data) => {
				...
				_html(`<p class=status>${ready ? "ready" : "waiting"}${_el_resume($scope5_id, "#text/1")} of ${_sep(_persisted_reason())}${_escape(_hole_value($scope5_id, "UpdateHole:#text/2", data.total, _persisted_reason()))}${_el_resume($scope5_id, "#text/2", _persisted_reason())}</p>`);
