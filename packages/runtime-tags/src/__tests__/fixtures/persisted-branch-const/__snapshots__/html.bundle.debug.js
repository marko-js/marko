// template.marko
_renderer_shells({ "__tests__/template.marko_1_shell": ",`__tests__/template.marko_1_shell __tests__/template.marko_1_count/init;D%c%;<p><!> <!></p>`" });
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	_if(() => {
		if (input.show) {
			...
			_html(`<p>${_patch_text($scope1_id, "#text/0", label, $scope0_owned, 1)}${_el_resume($scope1_id, "#text/0")} <!>${_escape(doubled)}${_el_resume($scope1_id, "#text/1")}</p>`);
