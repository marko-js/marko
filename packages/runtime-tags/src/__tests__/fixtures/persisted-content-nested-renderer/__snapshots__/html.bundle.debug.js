...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	const $scope0_reason = _scope_reason(), $sg__input_show = _source_guard($scope0_reason, 1), $si__input_show = _source_if($scope0_reason, 1), $scope0_page = _page_render();
	const $scope0_id = _scope_id();
	...
	_if(() => {
		if (input.show) {
			...
			card_default({ content: _content_elide("__tests__/template.marko_2*content", () => {
				...
				_html(`<p>${_patch_text($scope2_id, "#text/0", input.note, void 0, $scope0_reason, 2)}</p>`);
				const $tag = input.inner;
				_dynamic_tag($scope2_id, "#text/1", $tag, {}, 0, 0, _source_guard($scope0_reason, 3), _patch_dynamic_tag($scope2_id, "#text/1", $tag, 0, 0, 0, $scope0_reason, 3));
				_subscribe(_unfilled_if($scope0_reason, 3) && $input_inner__closures, _subscribe(_unfilled_if($scope0_reason, 2) && $input_note__closures, _scope($scope2_id, { _: _scope_with_id($scope1_id) }, "__tests__/template.marko", "3:6")));
			}, $scope1_id) });
			_scope($scope1_id, {
