	...
	_if$1(() => {
		if (input.show) {
			...
			card_default({ content: _content_elide("__tests__/template.marko_2*content", () => {
				...
				_html$1(`<p>${_patch_text($scope2_id, "#text/0", input.note, $scope0_owned, 2)}${_el_resume($scope2_id, "#text/0")}</p>`);
				_patch_dynamic_tag($scope2_id, "#text/1", input.inner, $scope0_owned, 3);
				_dynamic_tag$1($scope2_id, "#text/1", input.inner, {}, 0, 0, _source_guard($scope0_reason, 3), 1);
				_subscribe(_source_if($scope0_reason, 3) && $input_inner__closures, _subscribe(_source_if($scope0_reason, 2) && $input_note__closures, writeScope($scope2_id, { _: _scope_with_id($scope1_id) }, "__tests__/template.marko", "3:6")));
