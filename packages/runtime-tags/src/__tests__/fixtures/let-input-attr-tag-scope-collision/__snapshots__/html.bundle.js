...
var template_default = _template("a", (input) => {
	...
	child_tag_default({ footer: attrTag({ content: _content("a2", () => {
		...
		$si__input_submitLabel__OR__input_label && _subscribe($si__input_label && $input_label__closures, _subscribe($si__input_submitLabel && $input_submitLabel__closures, _scope($scope1_id, { _: _scope_with_id($scope0_id) }), "a0", $sg__input_submitLabel || $sg__input_label), "a1", $sg__input_submitLabel || $sg__input_label);
	...
	$si__input_open__OR__input_openChange__OR__input_submitLabel__OR__input_label && _scope($scope0_id, {
		d: _serialize_if($scope0_reason, 3) && input.open,
		e: _serialize_if($scope0_reason, 2) && input.openChange,
