...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	child_tag_default({ footer: attrTag({ content: _content("__tests__/template.marko_1*content", () => {
		...
		_html(`<button>${_escape(input.submitLabel || "OK")}${_el_resume($scope1_id, "#text/0", _serialize_guard($scope0_reason, 3))}</button><span>${_escape(input.label)}${_el_resume($scope1_id, "#text/1", _serialize_guard($scope0_reason, 4))}</span>`);
