...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	const MyTag = { content: _content("__tests__/template.marko_1_content", (a, ...rest) => {
		...
		_html_opens("__tests__/template.marko:3:3"), _html(`<div>${_escape(a)}${_el_resume($scope1_id, "#text/0", _serialize_guard($scope1_reason, 1))}|${_sep($sg__rest)}${_escape(JSON.stringify(rest))}${_el_resume($scope1_id, "#text/1", $sg__rest)}</div>`);
