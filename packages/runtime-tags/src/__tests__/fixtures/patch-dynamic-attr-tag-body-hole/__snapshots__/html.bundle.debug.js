...
var card_default = _template_patch("__tests__/card.marko", (input) => {
	const $scope0_reason = _scope_reason(), $scope0_page = _page_render();
	const $scope0_id = _scope_id();
	_html(`<section><em>${_patch_text($scope0_id, "#text/0", input.meta.n, void 0, $scope0_reason, 0)}</em>`);
	...
	_dynamic_tag($scope0_id, "#text/2", $tag2, {}, 0, 0, _source_guard($scope0_reason, 2), _patch_dynamic_tag($scope0_id, "#text/2", $tag2, 0, 0, 0, $scope0_reason, 2));
	_html("</section>");
	...
	card_default({
		meta: attrTag({
			...
			content: _content_elide("__tests__/template.marko_1*content", () => {
				...
				_html(_patch_text($scope1_id, "#text/0", input.label, void 0, $scope0_reason, 0));
				_subscribe(_unfilled_if($scope0_reason, 0) && $input_label__closures, _scope($scope1_id, { _: _scope_with_id($scope0_id) }, "__tests__/template.marko", "4:4"));
			}, $scope0_id)
		}),
		content: _content_elide("__tests__/template.marko_2*content", () => {
			const $scope2_reason = _scope_reason();
			const $scope2_id = _scope_id();
			_html(_patch_text($scope2_id, "#text/0", input.label, void 0, $scope0_reason, 0));
