...
var pill_default = _template_persisted("__tests__/tags/pill.marko", (input) => {
	...
	let n = input.start;
	_html$1(`<b class=pill>${_patch_text($scope0_id, "#text/0", input.text, $scope0_owned, 0)}${_el_resume($scope0_id, "#text/0")}:<!>${_escape(n)}${_el_resume($scope0_id, "#text/1")}</b><button class=inc>+</button>${_el_resume($scope0_id, "#button/2")}`);
	_script$1($scope0_id, "__tests__/tags/pill.marko_0");
	_patch_value($scope0_id, "__tests__/tags/pill.marko0", n, 1);
	$scope0_reason && writeScope($scope0_id, { n }, "__tests__/tags/pill.marko", 0, { n: "1:6" });
