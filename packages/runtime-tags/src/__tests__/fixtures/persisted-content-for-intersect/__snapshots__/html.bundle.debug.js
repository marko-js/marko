...
const $walks = "D%b l";
_shells({ "__tests__/template.marko_2*content": "__tests__/template.marko_2*content;b%;<!><!><!>" });
		...
		if (open) {
			...
			box_default({ content: _content_elide("__tests__/template.marko_2*content", () => {
				...
				if ($scope0_reason) forOf(["p", "q"], (item) => {
					...
					_html$1(`<span>${_escape(input.title + ":" + item)}${_el_resume($scope3_id, "#text/0")}</span>`);
					_subscribe(_source_if($scope0_reason, 0) && $input_title__closures, writeScope($scope3_id, {
						item,
