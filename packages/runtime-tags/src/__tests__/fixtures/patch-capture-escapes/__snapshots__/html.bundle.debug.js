...
var template_default = _template_patch("__tests__/template.marko", (input) => {
	...
	_html(`<main><h1>${_patch_text($scope0_id, "#text/0", `a&b${input.name}<c`, void 0, $scope0_reason, 1)}</h1><a${_patch_attr($scope0_id, "#a/1", "title", input.on ? "a\"b" : "c'd", $scope0_reason, 2)}${_patch_attr($scope0_id, "#a/1", "data-x", input.flag && "on", $scope0_reason, 3)}>${_patch_text($scope0_id, "#text/2", input.name, void 0, $scope0_reason, 1)}</a>${_el_resume($scope0_id, "#a/1")}</main>`);
