...
var tabs_default = _template_patch("__tests__/tags/tabs/index.marko", (input) => {
	...
	_if(() => {
		if (input.tab) {
			...
			_html(`<div>a ${_patch_text($scope1_id, "#text/0", input.tab.on, 2, $scope0_reason, 1)}</div>`);
			_scope($scope1_id, { _: _scope_with_id($scope0_id) }, "__tests__/tags/tabs/index.marko", "1:2");
			return 0;
		} else {
