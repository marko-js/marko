...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	let n = 0;
	_html_opens("__tests__/template.marko:3:1"), _html(`<button id=a>A</button>${_el_resume($scope0_id, "#button/0")}`);
	_await($scope0_id, "#text/1", resolveAfter(n, 0), (v) => {
		const $scope1_id = _scope_id();
		_html_opens("__tests__/template.marko:5:3"), _html(`<div id=out>done <!>${_escape(n)}${_el_resume($scope1_id, "#text/0")}</div>`);
