...
export default _._template("__tests__/tags/child.marko", (input, $serialize) => {
  ...
  _._html(`<button>${_._escape(input.value)}${_._el_resume($scope0_id, "#text/1", _._serialize_guard($serialize, /* input.value */0))}|<!>${_._escape(state)}${_._el_resume($scope0_id, "#text/2")}</button>${_._el_resume($scope0_id, "#button/0")}<button>${_._escape(input.value)}${_._el_resume($scope0_id, "#text/4", _._serialize_guard($serialize, /* input.value */0))}|<!>${_._escape(otherState)}${_._el_resume($scope0_id, "#text/5")}</button>${_._el_resume($scope0_id, "#button/3")}`);
  _._script($scope0_id, "__tests__/tags/child.marko_0_otherState");
  _._script($scope0_id, "__tests__/tags/child.marko_0_state");
  _._scope($scope0_id, {
    input_value: _._serialize_if($serialize, /* input.valueChange */1) && input.value,
