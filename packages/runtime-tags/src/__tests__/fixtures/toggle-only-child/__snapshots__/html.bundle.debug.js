...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	_if(() => {
		if (value) {
			...
			_html_opens("__tests__/template.marko:5:5"), _html(`<span>${_escape(value)}${_el_resume($scope1_id, "#text/0")}</span>`);
			writeScope($scope1_id, {}, "__tests__/template.marko", "4:4");
			return 0;
		}
	}, $scope0_id, "#div/0", 1, 1, 1, "</div>", 1);
	_html_opens("__tests__/template.marko:9:1"), _html(`<input${_attr_input_value($scope0_id, "#input/1", value, _resume((_new_value) => {
		value = _new_value;
