...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	const Child = { content: _content("__tests__/template.marko_1_content", (input) => {
		...
		const { a, b } = input;
		_html_opens("__tests__/template.marko:3:3", "__tests__/template.marko:4:3"), _html(`<div>${_escape(a)}${_el_resume($scope1_id, "#text/0", _serialize_guard($scope1_reason, 0))}</div><div>${_escape(b)}${_el_resume($scope1_id, "#text/1", _serialize_guard($scope1_reason, 1))}</div>`);
