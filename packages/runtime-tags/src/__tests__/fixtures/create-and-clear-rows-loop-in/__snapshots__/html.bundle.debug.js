...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	_for_in(input.children, (key, text) => {
		...
		_html_opens("__tests__/template.marko:3:5"), _html(`<p>${_escape(key)}: ${_sep($sg__input_children)}${_escape(text)}${_el_resume($scope1_id, "#text/1", $sg__input_children)}</p>`);
	...
	_for_in(input.children, (key) => {
		...
		_html_opens("__tests__/template.marko:6:5"), _html(`<p>${_escape(key)}</p>`);
