...
var template_default = _template("a", (input) => {
	...
	}, 0, $scope0_id, "a", $sg__input_children, $sg__input_children, $sg__input_children, 0, 1, 1);
	_for_in(input.children, (key) => {
		const $scope2_id = _scope_id();
		_html(`<p>${_escape(key)}</p>`);
		$si__input_children && writeScope($scope2_id, {});
	}, 0, $scope0_id, "b", $sg__input_children, $sg__input_children, $sg__input_children, 0, 1, 1);
	_html("</div>");
