...
const $content = (input) => {
	...
	_html(`<li>${_patch_text($scope0_id, "#text/0", input.node.name, void 0, $scope0_reason, 0)}`);
	_if(() => {
	...
	_html("</li>");
	$scope0_page && _scope($scope0_id, { input_node_children: _source_if($scope0_reason, 1) && input.node?.children }, "__tests__/tags/tree.marko", 0, { input_node_children: ["input.node.children"] });
};
var tree_default = _template_patch("__tests__/tags/tree.marko", $content, 0, () => [$content]);
...
var template_default = _template_patch("__tests__/template.marko", (input) => {
	...
	$scope0_page && _scope($scope0_id, { input_tree: input.tree }, "__tests__/template.marko", 0, { input_tree: ["input.tree"] });
}, 1, () => [tree_default]);
