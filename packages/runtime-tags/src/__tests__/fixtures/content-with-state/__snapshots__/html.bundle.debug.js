...
var outer_default = _template("__tests__/tags/outer.marko", (input) => {
	...
	inner_default({ content: _content("__tests__/tags/outer.marko_1_content", () => {
		...
		_html_opens("__tests__/tags/outer.marko:2:3"), _html(`<button>click</button>${_el_resume($scope1_id, "#button/0")}`);
...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	outer_default({ content: _content("__tests__/template.marko_1_content", () => {
		_scope_reason();
		const $scope1_id = _scope_id();
		_html_opens("__tests__/template.marko:3:3"), _html(`<span>${_escape(count)}${_el_resume($scope1_id, "#text/0")}</span>`);
		_subscribe($count__closures, writeScope($scope1_id, { _: _scope_with_id($scope0_id) }, "__tests__/template.marko", "2:2"));
		_resume_branch($scope1_id);
	}) });
	_html_opens("__tests__/template.marko:5:1"), _html(`<button id=increment>click</button>${_el_resume($scope0_id, "#button/1")}`);
