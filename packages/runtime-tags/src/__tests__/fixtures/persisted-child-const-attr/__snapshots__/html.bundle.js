...
var badge_default = _template_persisted("b", (input) => {
	...
	_if(() => {
		if (input.label) {
			...
			_html(`<b>${_patch_text($scope1_id, "a", input.label, void 0, $scope0_reason, 0)}</b>`);
			_scope($scope1_id, { _: _scope_with_id($scope0_id) });
			return 0;
		}
	}, $scope0_id, "a", 1, $sg__input_label, $sg__input_label, void 0, void 0, ["b0"], $scope0_reason, 0);
	_html(`<i>${_patch_text($scope0_id, "b", input.note, void 0, $scope0_reason, 1)}</i></div>`);
	$scope0_page && _scope($scope0_id, { e: _source_if($scope0_reason, 0) && input.label });
}, 0, 0);
