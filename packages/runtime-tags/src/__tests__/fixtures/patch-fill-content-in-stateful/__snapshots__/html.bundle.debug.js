	...
	if ($scope0_page) _if(() => {
		if (count < 2) {
			...
			wrap_default({ content: _content_elide("__tests__/template.marko_2*content", () => {
				...
				_html(`<span>${_text_resume($scope2_id, "#text/0", input.msg + ":" + count)}</span>`);
				_subscribe($count__closures, _subscribe(_source_if($scope0_reason, 0) && $input_msg__closures, _scope($scope2_id, { _: _scope_with_id($scope1_id) }, "__tests__/template.marko", "4:4")));
			}, $scope1_id) });
