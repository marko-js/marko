...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	_try($scope0_id, "#text/3", _content_resume("__tests__/template.marko_1_content", () => {
		...
		_await($scope1_id, "#text/0", resolveAfter(input.note, 1), (note) => {
			...
			_html(`<p>clicked <!>${_escape(count)}${_el_resume($scope2_id, "#text/0")} times -- ${_sep($sg__input_note)}${_escape(_hole_value($scope2_id, "UpdateHole:#text/1", note, _persisted_reason()))}${_el_resume($scope2_id, "#text/1", $sg__input_note)}</p>`);
