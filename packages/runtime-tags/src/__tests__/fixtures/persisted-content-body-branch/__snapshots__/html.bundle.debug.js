				...
				content: _content_elide("__tests__/template.marko_2*content", () => {
					...
					_if(() => {
						if (input.alt) {
							...
							_subscribe(_unfilled_if($scope0_owned, 7) && $input_note__closures, _scope($scope3_id, { _: _scope_with_id($scope2_id) }, "__tests__/template.marko", "4:8"));
							return 0;
						} else {
					...
					$scope0_reason && _subscribe(_unfilled_if($scope0_owned, 6) && $input_alt__closures, _scope($scope2_id, { _: _scope_with_id($scope1_id) }, "__tests__/template.marko", "3:6"));
