				...
				content: _content_elide("__tests__/template.marko_2*content", () => {
					...
					_if$1(() => {
						if (input.alt) {
							...
							_html$1(`<em>A:<!>${_patch_text($scope3_id, "#text/0", input.note, $scope0_owned, 7)}${_el_resume($scope3_id, "#text/0")}</em>`);
							_subscribe($si__input_note && $input_note__closures, writeScope($scope3_id, { _: _scope_with_id($scope2_id) }, "__tests__/template.marko", "4:8"));
							return 0;
						} else {
							const $scope4_id = _scope_id();
							_html$1(`<strong>B:<!>${_patch_text($scope4_id, "#text/0", input.note, $scope0_owned, 7)}${_el_resume($scope4_id, "#text/0")}</strong>`);
					...
					}, $scope2_id, "#text/0", 1, $sg__input_alt, $sg__input_alt, void 0, void 0, ["__tests__/template.marko_3*shell", "__tests__/template.marko_4*shell"]);
					$scope0_reason && _subscribe(_source_if($scope0_reason, 6) && $input_alt__closures, writeScope($scope2_id, { _: _scope_with_id($scope1_id) }, "__tests__/template.marko", "3:6"));
