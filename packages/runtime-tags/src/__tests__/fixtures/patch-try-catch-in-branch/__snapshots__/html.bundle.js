	...
	_if(() => {
		if (input.show) {
			...
			_try($scope1_id, "a", _content_resume("a0", () => {
				...
				_subscribe(_unfilled_if($scope0_reason, 3) && $input_boom__closures, _subscribe(_unfilled_if($scope0_reason, 2) && $input_message__closures, _scope($scope2_id, { _: _scope_with_id($scope1_id) }), _client_guard($scope0_reason, 2) && "a3"), _client_guard($scope0_reason, 3) && "a4");
			}, $scope1_id), { catch: attrTag({ content: _content_resume("a2", (err) => {
