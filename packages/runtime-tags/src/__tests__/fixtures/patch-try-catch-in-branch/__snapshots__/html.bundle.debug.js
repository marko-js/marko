	...
	_if(() => {
		if (input.show) {
			...
			_try($scope1_id, "#text/0", _content_resume("__tests__/template.marko_2*content", () => {
				...
				_subscribe(_unfilled_if($scope0_reason, 3) && $input_boom__closures, _subscribe(_unfilled_if($scope0_reason, 2) && $input_message__closures, _scope($scope2_id, { _: _scope_with_id($scope1_id) }, "__tests__/template.marko", "6:6"), _client_guard($scope0_reason, 2) && "__tests__/template.marko_2_input_message#4/subscribe"), _client_guard($scope0_reason, 3) && "__tests__/template.marko_2_input_boom#5/subscribe");
