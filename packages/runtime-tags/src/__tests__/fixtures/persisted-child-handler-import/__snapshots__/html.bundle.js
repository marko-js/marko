...
var widget_default = _template_persisted("b", (input) => {
	...
	let last = "";
	_html(`<p>${_patch_text($scope0_id, "a", input.label, $scope0_owned, 0)}${_el_resume($scope0_id, "a")}<!>${_escape(last)}${_el_resume($scope0_id, "b")}</p><button class=run>run</button>${_el_resume($scope0_id, "c")}`);
	_script($scope0_id, "b0");
	_patch_value($scope0_id, "b0", last, 1);
	$scope0_reason ? writeScope($scope0_id, { f: input.label }) : _owned_guard($scope0_owned, 0) && _patch_write($scope0_id, "f", input.label);
