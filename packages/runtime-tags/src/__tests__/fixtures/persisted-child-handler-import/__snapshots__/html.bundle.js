...
var widget_default = _template_persisted("b", (input) => {
	...
	let last = "";
	_html(`<p>${_patch_text($scope0_id, "a", input.label, void 0, $scope0_reason, 0)}${_text_resume($scope0_id, "b", last, 2)}</p><button class=run>run</button>${_el_resume($scope0_id, "c")}`);
	_script($scope0_id, "b0");
	_patch_value($scope0_id, "b0", last, 1);
	$scope0_page ? _scope($scope0_id, { f: input.label }) : _filled_guard($scope0_reason, 0) && _patch_write($scope0_id, "f", input.label);
}, 0, 0);
