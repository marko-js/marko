...
const $input_name__OR__input_write__script = _._script("__tests__/tags/child.marko_0_name_write", $scope => {
  $scope.write(`${$scope.name} mounted`);
  _.$signal($scope, 0).onabort = () => {
    $scope.write(`${$scope.name} destroyed`);
  };
});
const $input_name__OR__input_write = /* @__PURE__ */_._or(7, $scope => {
