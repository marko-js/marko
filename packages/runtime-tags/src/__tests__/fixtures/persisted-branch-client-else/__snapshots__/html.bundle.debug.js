...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	_html(`<main><h2>${_patch_text($scope0_id, "#text/0", input.yes, void 0, $scope0_reason, 1)}</h2>`);
	if ($scope0_page) _if(() => {
		if (on) {
	...
	$scope0_page ? _scope($scope0_id, {
		...
		on: "1:6",
		label: "2:8"
	}) : (_filled_guard($scope0_reason, 1) && _patch_value($scope0_id, "__tests__/template.marko0", input.yes), _filled_guard($scope0_reason, 0) && _patch_value($scope0_id, "__tests__/template.marko1", label));
}, 1, 0);
