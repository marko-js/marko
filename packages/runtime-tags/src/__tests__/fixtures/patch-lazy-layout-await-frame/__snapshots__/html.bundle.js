...
var icon_default = _template_patch("f", (input) => {
	const $scope0_reason = _scope_reason(), $scope0_page = _page_render();
	const $scope0_id = _scope_id();
	_html(`<svg${_patch_attr($scope0_id, "a", "viewBox", input.name, $scope0_reason, 0)}>`);
	_if(() => {
		if (input.name) {
			const $scope1_id = _scope_id();
			_html(`<path${_patch_attr($scope1_id, "a", "d", input.name, $scope0_reason, 0)}></path>${_el_resume($scope1_id, "a")}`);
			_scope($scope1_id, { _: _scope_with_id($scope0_id) });
			return 0;
		}
	}, $scope0_id, "a", 1, 1, _source_guard($scope0_reason, 0), void 0, void 0, ["f0"], $scope0_reason, 0);
...
_shells({ b: "b,<h1>A</h1>" });
var page_a_default = _template_patch("b", (input) => {
	_scope_reason();
	_scope_id();
	_html("<h1>A</h1>");
}, 0, 0);
...
_shells({
	...
	e2: /*@__PURE__*/ ((_w0, _w1) => `e2;${_w0};${_w1}`)(/*@__PURE__*/ ((_w0) => `b%b/${_w0}&b`)($walks), /*@__PURE__*/ ((_w0) => `<!><!>${_w0}<!>`)($template$2)),
	e3: "e3,<p>home</p>",
...
var template_default = _template_patch("e", (input) => {
	...
	root_default({ content: _content_elide("e1", () => {
			...
			if (input.page === 0) {
				...
				$Docs_withLoadAssets({
					...
					content: _content_elide("e0", () => {
						...
						}, $scope3_id, "a", 1, $sg__input_page, $sg__input_page, void 0, void 0, ["e4", "e5"], $scope0_reason, 1);
		...
		}, $scope1_id, "a", 1, $sg__input_page, $sg__input_page, void 0, void 0, ["e3", "e2"], $scope0_reason, 1);
		$scope0_page && _subscribe(_unfilled_if($scope0_reason, 1) && $input_page__closures, _scope($scope1_id, { _: _scope_with_id($scope0_id) }));
		$sg__input_page || $scope0_page && _resume_branch($scope1_id);
	}, $scope0_id) });
	$scope0_page && _scope($scope0_id, {
		e: _source_if($scope0_reason, 1) && input.list,
