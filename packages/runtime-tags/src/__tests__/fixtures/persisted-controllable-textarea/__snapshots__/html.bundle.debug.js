...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	_html(`<main><h1>${_patch_text($scope0_id, "#text/0", input.title, $scope0_owned, 0)}${_el_resume($scope0_id, "#text/0")}</h1><textarea${_patch_bind($scope0_id, "ControlledHandler:#textarea/1", _resume(function(next) {
		document.querySelector("main").dataset.text = next;
	}, "__tests__/template.marko_0/valueChange"))}${_patch_control($scope0_id, "#textarea/1", 2, input.text, $scope0_owned, 1)}>${_attr_textarea_value($scope0_id, "#textarea/1", input.text, _resume(function(next) {
		document.querySelector("main").dataset.text = next;
