...
var add_form_default = _template("b", (input) => {
	...
	let watched = false;
	let quantity = 1;
	_html(`<button class=watch>${_escape(_hole_value($scope0_id, "Qe", "watch", _state_reason()))}${_el_resume($scope0_id, "e")}</button>${_el_resume($scope0_id, "d")}<form class=add><input type=hidden name=productId${_attr("value", productId)} class=pid>${_el_resume($scope0_id, "g", _serialize_guard($scope0_reason, 0))}<input${_attr_input_value($scope0_id, "h", quantity, _resume((_new_quantity) => {
		quantity = Number(_new_quantity);
	}, "b0", $scope0_id))} type=number name=quantity min=1 class=qty>${_el_resume($scope0_id, "h")}<button type=button class=plus>+</button>${_el_resume($scope0_id, "i")}</form>${_el_resume($scope0_id, "f")}`);
	_script($scope0_id, "b3");
	writeScope($scope0_id, {
		m: _seed_fill(_state_reason() && cart),
		n: productId,
		o: _seed_fill(_state_reason() && watched),
		p: _seed_fill(_state_reason() && quantity),
