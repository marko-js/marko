...
var add_form_default = _template("__tests__/tags/add-form.marko", (input) => {
	...
	let watched = false;
	let quantity = 1;
	_html(`<button class=watch>${watched ? "watching" : "watch"}${_el_resume($scope0_id, "#text/3")}</button>${_el_resume($scope0_id, "#button/2")}<form class=add><input type=hidden name=productId${_attr("value", productId)} class=pid>${_el_resume($scope0_id, "#input/5", _serialize_guard($scope0_reason, 0))}<input${_attr_input_value($scope0_id, "#input/6", quantity, _resume((_new_quantity) => {
		quantity = Number(_new_quantity);
	}, "__tests__/tags/add-form.marko_0/valueChange", $scope0_id))} type=number name=quantity min=1 class=qty>${_el_resume($scope0_id, "#input/6")}<button type=button class=plus>+</button>${_el_resume($scope0_id, "#button/7")}</form>${_el_resume($scope0_id, "#form/4")}`);
