...
var template_default = _template("a", (input) => {
	...
	writeScope($scope0_id, { d: _seed_fill(_state_reason() && mirror) });
	_resume_branch($scope0_id);
}, 1);
_renderer_shells({
	"a0": ["<button class=take>take</button><p class=mirror><!> in stock</p><p class=server>server says <!></p>", " bD%lDb%l"],
	"a": ["<button class=take>take</button><p class=mirror><!> in stock</p><p class=server>server says <!></p>", " bD%lDb%l"]
});
