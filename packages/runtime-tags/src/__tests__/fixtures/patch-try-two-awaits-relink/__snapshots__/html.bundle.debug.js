	...
	_try($scope0_id, "#text/0", _content_resume("__tests__/template.marko_1*content", () => {
		...
		_if(() => {
			if (input.show) {
				...
				$scope0_page && _subscribe(_unfilled_if($scope0_reason, 3) && $input_fast__closures, _scope($scope2_id, { _: _scope_with_id($scope1_id) }, "__tests__/template.marko", "4:8"), _client_guard($scope0_reason, 3) && "__tests__/template.marko_2_input_fast#5/subscribe", 0);
				return 0;
			}
		...
		_html("</div>");
		$scope0_page && _subscribe(_unfilled_if($scope0_reason, 4) && $input_slow__closures, _subscribe(_unfilled_if($scope0_reason, 2) && $input_show__closures, _scope($scope1_id, { _: _scope_with_id($scope0_id) }, "__tests__/template.marko", "2:4"), _client_guard($scope0_reason, 2) && "__tests__/template.marko_1_input_show#4/subscribe", $sg__input_show), _client_guard($scope0_reason, 4) && "__tests__/template.marko_1_input_slow#6/subscribe", $sg__input_show);
