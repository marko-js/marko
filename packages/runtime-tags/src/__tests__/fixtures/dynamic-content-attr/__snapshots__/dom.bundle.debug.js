...
const $walks = " b";
let sideEffect = 3;
const $MyThing_content__count = /*@__PURE__*/ _closure_get("count/4", ($scope) => _text($scope["#text/0"], $scope._.count), 0, "__tests__/template.marko_1_count#1/subscribe");
const $MyThing_content__setup = ($scope) => {
	$MyThing_content__count($scope);
	_text($scope["#text/1"], sideEffect++);
};
const $MyThing_content = _content("__tests__/template.marko_1*content", "<!> <!>", "%c%", $MyThing_content__setup);
