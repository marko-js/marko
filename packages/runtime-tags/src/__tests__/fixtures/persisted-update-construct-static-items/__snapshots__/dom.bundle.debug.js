...
const $placeholder_content = /*@__PURE__*/ _content("__tests__/template.marko_4_content", "loading ratings…");
...
const $Ratings_content__try = /*@__PURE__*/ _try("#text/0", $try_content__template, $try_content__walks, $try_content__setup);
const $Ratings_content__setup = ($scope) => $Ratings_content__try($scope, { placeholder: attrTag({ content: $placeholder_content($scope) }) });
const $Ratings_content = /*@__PURE__*/ _content("__tests__/template.marko_2_content", $Ratings_content__template, $Ratings_content__walks, $Ratings_content__setup);
...
_static_shells({
	...
	"__tests__/template.marko_2_update": [$Ratings_content__template, $Ratings_content__walks],
	"__tests__/template.marko_2_content": [$Ratings_content__template, $Ratings_content__walks],
...
const $await_content__for = /*@__PURE__*/ _for_to("#div/0", "<div class=rating><!> for <!></div>", "D%c%", $for_content__setup);
const $await_content__ratings_length = ($scope, ratings_length) => {
	if (!updating) $await_content__for($scope, [
		ratings_length,
		1,
...
const $placeholder_content = _content_resume("__tests__/template.marko_4_content", "loading ratings…");
const $await_content = /*@__PURE__*/ _await_content("#text/0", "<div class=ratings></div>", " ");
const $try_content__await_promise = /*@__PURE__*/ _await_promise("#text/0", $await_content__$params);
const $try_content__setup = ($scope) => {
	$await_content($scope);
	if (!updating) $try_content__await_promise($scope, getRatings($scope.$global.topic));
};
const $Ratings_content__try = /*@__PURE__*/ _try("#text/0", "<!><!><!>", "b%", $try_content__setup);
const $Ratings_content__setup = ($scope) => $Ratings_content__try($scope, { placeholder: attrTag({ content: $placeholder_content($scope) }) });
const $Ratings_content = /*@__PURE__*/ _content("__tests__/template.marko_2_content", "<!><!><!>", "b%", $Ratings_content__setup);
const $Home_content = /*@__PURE__*/ _content("__tests__/template.marko_1_content", "<p class=home>welcome home</p>");
