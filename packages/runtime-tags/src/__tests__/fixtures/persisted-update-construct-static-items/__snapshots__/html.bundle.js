...
var template_default = _template("a", (input) => {
	...
	const Ratings = { content: _content("a8", () => {
			...
			_await($scope3_id, "a", getRatings($global().topic), (ratings) => {
				...
				_region(() => {
					forTo(ratings.length, 1, 1, (i) => {
						...
						_html(`<div class=rating>${_escape(ratings[i - 1])}${_el_resume($scope6_id, "a", _persisted_reason())} for ${_sep(_persisted_reason())}${_escape($global().topic)}${_el_resume($scope6_id, "b", _persisted_reason())}</div>`);
						_persisted_reason() && writeScope($scope6_id, { _: _scope_with_id($scope5_id) });
					});
				}, $scope5_id, "a");
				_html(`</div>${_el_resume($scope5_id, "a", _persisted_reason())}`);
...
_renderer_shells({
	"a4": ["<div class=ratings></div>", " b"],
	"a10": ["<div class=ratings></div>", " b"],
	"a7": ["<!><!><!>", "b%c"],
	"a6": ["<!><!><!>", "b%c"],
	"a11": ["<!><!><!>", "b%c"],
	"a8": ["<!><!><!>", "b%c"],
