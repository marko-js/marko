...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	const Ratings = { content: _content("__tests__/template.marko_2_content", () => {
		...
		_try($scope2_id, "#text/0", _content_resume("__tests__/template.marko_3_content", () => {
			...
			_await($scope3_id, "#text/0", getRatings($global().topic), (ratings) => {
				...
				_region(() => {
					forTo(ratings.length, 1, 1, (i) => {
						const $scope6_id = _scope_id();
						_html(`<div class=rating>${_escape(ratings[i - 1])}${_el_resume($scope6_id, "#text/0", _persisted_reason())} for ${_sep(_persisted_reason())}${_escape($global().topic)}${_el_resume($scope6_id, "#text/1", _persisted_reason())}</div>`);
						_persisted_reason() && writeScope($scope6_id, { _: _scope_with_id($scope5_id) }, "__tests__/template.marko", "18:10");
					});
				}, $scope5_id, "#div/0", "__tests__/template.marko_r0");
				_html(`</div>${_el_resume($scope5_id, "#div/0", _persisted_reason())}`);
	...
	writeScope($scope0_id, { clicks: _seed_fill(_state_reason() && clicks) }, "__tests__/template.marko", 0, { clicks: "3:6" });
