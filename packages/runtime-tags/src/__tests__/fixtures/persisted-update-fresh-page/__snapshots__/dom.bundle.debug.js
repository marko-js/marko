...
const $update2$3 = (_patch, _live) => {
	...
	if ("input_name" in _patch) $input_name_update(_live, _patch["input_name"]);
};
const _merge$3 = _resume("__tests__/tags/shared-list.marko_0_update", $update2$3);
_update_content("__tests__/tags/shared-list.marko", _merge$3);
function _patch2$3() {
	return patch(_merge$3);
}
...
const $update2$2 = (_patch, _live) => {
	_update_pair(_patch, _live);
	if ("added" in _patch) _update_seed(_live, $added_seed, _patch["added"]);
	if ("productId" in _patch) $productId_update(_live, _patch["productId"]);
	if ("#childScope/0" in _patch) _merge$3(_patch["#childScope/0"], _live["#childScope/0"]);
};
const _merge$2 = _resume("__tests__/tags/actions.marko_0_update", $update2$2);
_update_content("__tests__/tags/actions.marko", _merge$2);
function _patch2$2() {
...
const $try_content__product_id = /*@__PURE__*/ _closure_get("product_id", ($scope) => {
	if (!updating) {
		if (!updating) $try_content__await_promise($scope, getRecommendations($scope._._.product_id));
	}
}, ($scope) => $scope._._);
...
const $for_update = _update_for_keyed("#ul/0", ($p, $l) => $for_content3_holes($p, $l));
...
const $else_content2__update = (_patch, _live) => {
	...
	if ("#childScope/3" in _patch) _merge$2(_patch["#childScope/3"], _live["#childScope/3"]);
	if ("BranchScopes:#text/4" in _patch) _update_branch(_patch, _live, "#text/4", $try_content__update);
};
const $Item_content__update = (_patch, _live) => {
	if ("product_image" in _patch) _live["product_image"] = _patch["product_image"];
	if ("product_title" in _patch) _live["product_title"] = _patch["product_title"];
	if ("product_price" in _patch) _live["product_price"] = _patch["product_price"];
	if ("product_id" in _patch) _live["product_id"] = _patch["product_id"];
	if ("ConditionalRenderer:#text/0" in _patch) _update_if(_patch, _live, "ConditionalRenderer:#text/0", "BranchScopes:#text/0", [0, $else_content2__update]);
};
const $Cart_content__update = (_patch, _live) => {
	if ("products" in _patch) _update_seed(_live, $products_seed, _patch["products"]);
	if ("#childScope/0" in _patch) _merge$3(_patch["#childScope/0"], _live["#childScope/0"]);
	if ("BranchScopes:#nav/2" in _patch) _update_for(_patch["BranchScopes:#nav/2"], _live["BranchScopes:#nav/2"], $for_content_holes);
};
