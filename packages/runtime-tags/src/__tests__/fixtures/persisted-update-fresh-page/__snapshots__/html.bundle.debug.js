...
var actions_default = _template("__tests__/tags/actions.marko", (input) => {
	...
	let added = 0;
	_html(`<button class=add>added <!>${_escape(added)}${_el_resume($scope0_id, "#text/3")} of ${_sep($sg__input_id)}${_escape(_hole_value($scope0_id, "UpdateHole:#text/4", productId, _persisted_reason()))}${_el_resume($scope0_id, "#text/4", $sg__input_id)} (<!>${_escape(list.length)}${_el_resume($scope0_id, "#text/5")} in cart)</button>${_el_resume($scope0_id, "#button/2")}`);
...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	const Cart = { content: _content("__tests__/template.marko_1_content", () => {
		...
		_for_of(getTags?.(), (tag) => {
			...
			_html(`<b${_attr_class(_hole_value($scope3_id, "UpdateAttr:class:#b/0", tag === $global().tag && "on", _persisted_reason()))}${_attr("data-tag", _hole_value($scope3_id, "UpdateAttr:data-tag:#b/0", tag, _persisted_reason()))}>${_escape(_hole_value($scope3_id, "UpdateHole:#text/1", label, _persisted_reason()))}${_el_resume($scope3_id, "#text/1", _persisted_reason())}</b>${_el_resume($scope3_id, "#b/0", _persisted_reason())}`);
