...
var shared_list_default = _template("__tests__/tags/shared-list.marko", (input) => {
	...
	writeScope($scope0_id, {
		...
		value: _seed_fill(_state_reason() && value),
...
var actions_default = _template("__tests__/tags/actions.marko", (input) => {
	...
	writeScope($scope0_id, {
		productId,
		list: _seed_fill(_state_reason() && list),
		added: _seed_fill(_state_reason() && added),
...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	const Cart = { content: _content("__tests__/template.marko_1_content", () => {
		...
		}, $scope1_id, "#nav/3", "__tests__/template.marko_r0");
		_html(`</nav>${_el_resume($scope1_id, "#nav/3", _persisted_reason())}`);
		_if(() => {
			if (!entries.length) {
				_group_site("__tests__/template.marko_4_update");
				const $scope4_id = _scope_id();
				_html("<p class=cart>cart is empty</p>");
				writeScope($scope4_id, {}, "__tests__/template.marko", "20:4");
				return 0;
			} else {
		...
		writeScope($scope1_id, {
			...
			entries: _seed_fill(_state_reason() && entries),
			"#childScope/0": _existing_scope($childScope)
	...
	const Item = { content: _content("__tests__/template.marko_6_content", () => {
		...
		_if(() => !product ? 0 : 1, $scope6_id, "#text/0", _persisted_reason(), _persisted_reason(), _persisted_reason(), void 0, void 0, "__tests__/template.marko_6/update_if_#text/0", [() => {
			...
			_persisted_reason() && writeScope($scope9_id, {}, "__tests__/template.marko", "34:4");
		}, () => {
			...
			_try($scope7_id, "#text/5", _content_resume("__tests__/template.marko_8_content", () => {
				...
				_await($scope8_id, "#text/0", getRecommendations(product.id), (recs) => {
					...
					}, $scope11_id, "#ul/0", "__tests__/template.marko_r1");
			...
			}, "__tests__/template.marko", "37:4");
		}], ["__tests__/template.marko_9_update", "__tests__/template.marko_7_update"], "__tests__/template.marko_r2");
		_persisted_reason() && writeScope($scope6_id, { "ClosureScopes:product_id": $Item_content__product_id__closures }, "__tests__/template.marko", "32:2", {
			product_image: ["product.image", "33:10"],
			product_title: ["product.title", "33:10"],
			product_price: ["product.price", "33:10"],
			product_id: ["product.id", "33:10"]
		});
	}) };
	_set_serialize_reason(_persisted_reason());
...
_renderer_shells({
	...
	"__tests__/template.marko_8_content": ["<!><!><!>", "b%c"],
