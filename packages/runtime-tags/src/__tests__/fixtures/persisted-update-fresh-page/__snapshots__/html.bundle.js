...
var template_default = _template("a", (input) => {
	...
	const Cart = { content: _content("a3", () => {
		const $scope1_id = _scope_id();
		_scope_reason();
		const $childScope = _peek_scope_id();
		let list = shared_list_default({ name: "cart" });
		_var($scope1_id, "b", $childScope, "a16");
	...
	const Item = { content: _content("a8", () => {
		...
		_if(() => !product ? 0 : 1, $scope6_id, "a", _persisted_reason(), _persisted_reason(), _persisted_reason(), void 0, void 0, "a0", [() => {
			...
			_try($scope7_id, "e", _content_resume("a10", () => {
				...
				_await($scope8_id, "a", getRecommendations(product.id), (recs) => {
					...
					_for_of(recs, (rec) => {
						...
						return rec.id;
					}, $scope11_id, "a", _persisted_reason(), _persisted_reason(), _persisted_reason(), "</ul>", 1, "a2");
			...
			}, $scope7_id), { placeholder: attrTag({ content: _content_resume("a12", () => {
				_scope_reason();
				_scope_id();
