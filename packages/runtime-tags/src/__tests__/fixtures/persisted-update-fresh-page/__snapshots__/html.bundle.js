...
var actions_default = _template("b", (input) => {
	...
	let added = 0;
	_html(`<button class=add>added <!>${_escape(added)}${_el_resume($scope0_id, "d")} of ${_sep($sg__input_id)}${_escape(_hole_value($scope0_id, "Qe", productId, _persisted_reason()))}${_el_resume($scope0_id, "e", $sg__input_id)} (<!>${_escape(list.length)}${_el_resume($scope0_id, "f")} in cart)</button>${_el_resume($scope0_id, "c")}`);
...
var template_default = _template("a", (input) => {
	...
	const Cart = { content: _content("a1", () => {
	...
	const Item = { content: _content("a5", () => {
		...
		_if(() => {
			if (!product) {
				...
				_try($scope7_id, "e", _content_resume("a3", () => {
				...
				}, $scope7_id), { placeholder: attrTag({ content: _content_resume("a2", () => {
					_scope_reason();
					_scope_id();
		...
		_persisted_reason() && writeScope($scope6_id, { Bf: $Item_content__product_id__closures });
	}) };
