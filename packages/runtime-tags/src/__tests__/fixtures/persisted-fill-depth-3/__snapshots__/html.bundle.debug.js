...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
			...
			_if(() => {
				if (input.inner) {
					...
					_for_of(input.items, (item) => {
						...
						_filled_guard($scope0_owned, 3) ? _patch_value($scope3_id, "__tests__/template.marko1", item) : _patch_init($scope3_id, "__tests__/template.marko_3_input_items#6/init");
					...
					_subscribe(_unfilled_if($scope0_owned, 3) && $input_items__closures, _scope($scope2_id, { _: _scope_with_id($scope1_id) }, "__tests__/template.marko", "4:6"));
					return 0;
				}
	...
	$scope0_reason ? _scope($scope0_id, {
		...
		input_suffix: ["input.suffix"],
		count: "1:6"
	}) : _filled_guard($scope0_owned, 4) && _patch_value($scope0_id, "__tests__/template.marko0", input.suffix);
}, 1, 0);
