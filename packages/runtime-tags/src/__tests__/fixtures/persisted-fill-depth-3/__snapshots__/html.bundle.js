...
var template_default = _template_persisted("a", (input) => {
			...
			_if(() => {
				if (input.inner) {
					...
					_for_of(input.items, (item) => {
						...
						_filled_guard($scope0_owned, 3) ? _patch_value($scope3_id, "a1", item) : _patch_init($scope3_id, "a3");
					...
					_subscribe(_unfilled_if($scope0_owned, 3) && $input_items__closures, _scope($scope2_id, { _: _scope_with_id($scope1_id) }));
					return 0;
				}
	...
	}) : _filled_guard($scope0_owned, 4) && _patch_value($scope0_id, "a0", input.suffix);
}, 1, 0);
