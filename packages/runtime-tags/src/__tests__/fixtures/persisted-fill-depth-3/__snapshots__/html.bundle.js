...
var template_default = _template_persisted("a", (input) => {
			...
			_if(() => {
				if (input.inner) {
					...
					_for_of(input.items, (item) => {
						...
						_filled_guard($scope0_reason, 3) ? _patch_value($scope3_id, "a1", item) : _patch_init($scope3_id, "a3");
	...
	}) : _filled_guard($scope0_reason, 4) && _patch_value($scope0_id, "a0", input.suffix);
}, 1, 0);
