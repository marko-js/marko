...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	child_default({ content: _content("__tests__/template.marko_3*content", (first, ...others) => {
		...
		_html(`<div>${_text_resume($scope3_id, "#text/0", first, $sg__first)}|${_text_resume($scope3_id, "#text/1", others[0], $sg__$params4_ * 2)}|${_text_resume($scope3_id, "#text/2", others[1], $sg__$params4_2 * 2)}|${_text_resume($scope3_id, "#text/3", others.length, $sg__others_length * 2)}</div>`);
	...
	child_default({ content: _content("__tests__/template.marko_4*content", (x, ...[y, z]) => {
		...
		_html(`<div>${_text_resume($scope4_id, "#text/0", x, $sg__x)}-${_text_resume($scope4_id, "#text/1", y, $sg__y * 2)}-${_text_resume($scope4_id, "#text/2", z, $sg__z * 2)}</div>`);
