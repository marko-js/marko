...
var child_default = _template("__tests__/tags/child.marko", (input) => {
	...
	_dynamic_tag($scope0_id, "#text/0", input.content, [
		...
		2,
		3
	], 0, 1, _serialize_guard($scope0_reason, 0), "__tests__/tags/child.marko:1:1");
...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	forOf(list, (item, ...meta) => {
		...
		_html_opens("__tests__/template.marko:3:3"), _html(`<div>${_escape(item)}:${_escape(meta[0])}:${_escape(meta.length)}</div>`);
	});
