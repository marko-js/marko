...
var item_form_default = _template("__tests__/tags/item-form.marko", (input) => {
	...
	_html(`<form><input${_attr_input_value($scope0_id, "#input/0", qty, _resume((_new_qty) => {
		qty = Number(_new_qty);
	}, "__tests__/tags/item-form.marko_0/valueChange", $scope0_id))} type=number class=qty>${_el_resume($scope0_id, "#input/0")}<input${_attr_input_value($scope0_id, "#input/1", _hole_value($scope0_id, "UpdateAttr:value:#input/1", itemId, _persisted_reason()))} type=hidden name=itemId>${_el_resume($scope0_id, "#input/1", _serialize_guard($scope0_reason, 0))}<button class=add>add</button>${_el_resume($scope0_id, "#button/2")}</form>`);
...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	_attr_select_value($scope0_id, "#select/1", _hole_value($scope0_id, "UpdateAttr:value:#select/1", item.category, _persisted_reason()), void 0, () => {
		_html(`<input${_attr_input_value($scope0_id, "#input/0", _hole_value($scope0_id, "UpdateAttr:value:#input/0", item.title, _persisted_reason()))} class=title>${_el_resume($scope0_id, "#input/0", _persisted_reason())}<select class=category><option${_attr_option_value("odd")}>odd</option><option${_attr_option_value("even")}>even</option></select>`);
	});
