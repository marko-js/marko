...
var counter_default = _template_persisted("b", (input) => {
	...
	let spins = 0;
	_html(`<section><p>Value ${_patch_text($scope0_id, "a", input.value, 2, $scope0_owned, 0)} (spun ${_text_resume($scope0_id, "b", spins, 2)})</p><button class=spin>spin</button>${_el_resume($scope0_id, "c")}</section>`);
