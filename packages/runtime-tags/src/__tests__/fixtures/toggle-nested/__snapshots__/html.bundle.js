...
var template_default = _template("a", (input) => {
		...
		if (show) {
			...
			_if(() => {
				if (value1) {
					...
					$si__input_show__OR__input_value && _subscribe($value__closures, writeScope($scope2_id, { _: _scope_with_id($scope1_id) }));
					return 0;
				}
			...
			_if(() => {
				if (value2) {
					...
					$si__input_show__OR__input_value2 && _subscribe($value2__closures, writeScope($scope3_id, { _: _scope_with_id($scope1_id) }));
					return 0;
				}
	...
	$si__input_show__OR__input_value1__OR__input_value && writeScope($scope0_id, {
		...
		Be: _serialize_if($scope0_reason, 5) && $value__closures,
		Bf: _serialize_if($scope0_reason, 6) && $value2__closures
	});
}, 1);
