		...
		if (show) {
			...
			_if(() => {
				if (value1) {
					...
					_serialize_if($scope0_reason, 0) && _subscribe($si__input_value && $value__closures, writeScope($scope2_id, { _: _scope_with_id($scope1_id) }));
					return 0;
				}
			...
			_if(() => {
				if (value2) {
					...
					_serialize_if($scope0_reason, 1) && _subscribe($si__input_value2 && $value2__closures, writeScope($scope3_id, { _: _scope_with_id($scope1_id) }));
					return 0;
				}
