...
var template_default = _template("a", (input) => {
		...
		if (show) {
			...
			_if(() => {
				if (value1) {
					...
					_serialize_if($scope0_reason, 0) && _subscribe($si__input_value && $value__closures, _scope($scope2_id, { _: _scope_with_id($scope1_id) }), "a0", $sg__input_value);
					return 0;
				}
			...
			_if(() => {
				if (value2) {
					...
					_serialize_if($scope0_reason, 1) && _subscribe($si__input_value2 && $value2__closures, _scope($scope3_id, { _: _scope_with_id($scope1_id) }), "a1", $sg__input_value2);
					return 0;
				}
	...
	$si__input_show__OR__input_value1__OR__input_value && _scope($scope0_id, {
		e: $si__input_show && value1,
		f: $si__input_show && value2,
