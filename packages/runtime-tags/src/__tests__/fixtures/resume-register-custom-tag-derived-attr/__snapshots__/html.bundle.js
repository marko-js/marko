...
var wrapper_default = _template("d", (input) => {
	const $scope0_reason = _scope_reason(), $sg__input_kind = _serialize_guard($scope0_reason, 0);
	const $scope0_id = _scope_id();
	_set_serialize_reason($sg__input_kind << 1);
	const $childScope = _peek_scope_id();
	heading_default({ type: components[input.kind] });
