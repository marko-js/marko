...
const $setup__script = _script("__tests__/template.marko_0", ($scope) => {
	_on($scope["#button/0"], "click", function() {
		$count4($scope, ("count" in $scope ? $scope.count : 0) + 1);
	});
	_on($scope["#button/2"], "click", function() {
		$count5($scope, ("$count" in $scope ? $scope.$count : 0) + 1);
	});
	_on($scope["#button/4"], "click", function() {
		$count6($scope, ("$count2" in $scope ? $scope.$count2 : 0) + 1);
	});
	_on($scope["#button/6"], "click", function() {
		$count7($scope, ("$count3" in $scope ? $scope.$count3 : 0) + 1);
	});
});
