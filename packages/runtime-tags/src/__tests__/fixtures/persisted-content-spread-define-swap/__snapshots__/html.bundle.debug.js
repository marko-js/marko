...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	const one = { content: _content_resume("__tests__/template.marko_1*content", () => {
		...
		_subscribe(_unfilled_if($scope0_owned, 0) && $input_title__closures, _scope($scope1_id, { _: _scope_with_id($scope0_id) }, "__tests__/template.marko", "1:2"));
	}, $scope0_id) };
	const two = { content: _content_resume("__tests__/template.marko_2*content", () => {
		...
		_subscribe(_unfilled_if($scope0_owned, 0) && $input_title__closures, _scope($scope2_id, {
	...
	}) : _filled_guard($scope0_owned, 0) && (_content_withheld("__tests__/template.marko_1*content") || _content_withheld("__tests__/template.marko_2*content")) && _patch_value($scope0_id, "__tests__/template.marko0", input.title);
}, 1, 0);
