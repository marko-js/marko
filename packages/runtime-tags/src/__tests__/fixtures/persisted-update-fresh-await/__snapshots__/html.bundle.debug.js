...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	_if(() => {
		if (input.productId) {
			...
			_html(`<h2>Product ${_sep($sg__input_productId)}${_escape(_hole_value($scope1_id, "#text/0", input.productId, _persisted_reason()))}${_el_resume($scope1_id, "#text/0", $sg__input_productId)}</h2>`);
			_try($scope1_id, "#text/1", _content_resume("__tests__/template.marko_2_content", () => {
				...
				_await($scope2_id, "#text/0", getReviews(input.productId), (reviews) => {
					...
					_for_of(reviews, (review) => {
						...
						_html(`<li>${_escape(_hole_value($scope6_id, "#text/0", review.text, _persisted_reason()))}${_el_resume($scope6_id, "#text/0", $sg__input_productId)} rated ${_sep($sg__input_productId)}${_escape(_hole_value($scope6_id, "#text/1", review.stars, _persisted_reason()))}${_el_resume($scope6_id, "#text/1", $sg__input_productId)}</li>`);
