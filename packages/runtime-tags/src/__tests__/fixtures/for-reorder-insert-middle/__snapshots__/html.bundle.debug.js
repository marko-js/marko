...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	_for_of(input.children, (child) => {
		...
		_html_opens("__tests__/template.marko:3:5"), _html(`<span${_attr("id", child.id)}>${_escape(child.text)}${_el_resume($scope1_id, "#text/1", $sg__input_children)}</span>${_el_resume($scope1_id, "#span/0", $sg__input_children)}`);
