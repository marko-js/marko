// template.marko
const $template = "<button>inc <!></button><title></title><title></title><title>&amp; &#38; &#x26; &lt;</title>";
const $walks = " Db%l b c";
const $input_a__OR__n = /*@__PURE__*/ _or(8, ($scope) => {
	_text_content($scope["#title/2"], `& & & < ${_to_text($scope.input_a)} - ${_to_text($scope.n)}`);
	_text_content($scope["#title/3"], `& & & < ${_to_text($scope.input_a)} - ${_to_text($scope.n)}`);
});
const $n = /*@__PURE__*/ _let("n/7", ($scope) => {
	_text($scope["#text/1"], $scope.n);
	$input_a__OR__n($scope);
});
const $setup__script = _script("__tests__/template.marko_0", ($scope) => _on($scope["#button/0"], "click", function() {
	$n($scope, +$scope.n + 1);
}));
function $setup($scope) {
	$n($scope, 1);
	$setup__script($scope);
}
const $input_a = /*@__PURE__*/ _const("input_a", $input_a__OR__n);
const $input = ($scope, input) => $input_a($scope, input.a);
var template_default = /*@__PURE__*/ _template("__tests__/template.marko", $template, $walks, $setup, $input);
