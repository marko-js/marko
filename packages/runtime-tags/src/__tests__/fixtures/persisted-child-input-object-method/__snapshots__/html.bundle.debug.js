...
var code_block_default = _template_persisted("__tests__/tags/code-block.marko", (input) => {
	const $scope0_owned = _persisted_ownership(), $scope0_reason = _persisted_reason(), $si__input_text = _source_if($scope0_reason, 2);
	...
	_html(`<div>${_patch_html($scope0_id, "#text/0", highlight(input.text), void 0, $scope0_owned, 0)}</div>`);
	$scope0_reason ? _scope($scope0_id, {
		...
		highlight: "1:8"
	}) : _filled_guard($scope0_owned, 1) && _patch_write($scope0_id, "input_cursor", input.cursor);
}, 0, 0);
