...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	let id = 1;
	_html(`<div id=outside>${_escape(id)}${_el_resume($scope0_id, "#text/0")}</div>`);
	_try($scope0_id, "#text/1", _content_resume("__tests__/template.marko_1_content", () => {
		...
		_await($scope1_id, "#text/1", resolveAfter({ id }, 4), (data) => {
		...
		_subscribe($id__closures, writeScope($scope1_id, { _: _scope_with_id($scope0_id) }, "__tests__/template.marko", "10:2"));
	...
	writeScope($scope0_id, { "ClosureScopes:id": $id__closures }, "__tests__/template.marko", 0);
