...
var hello_default = _template("b", (input) => {
	...
	_dynamic_tag($scope0_id, "b", input.header.content, {}, 0, 0, $sg__input_header_content);
	_html(`</header>${_el_resume($scope0_id, "a", $sg__input_header_class)}<main>`);
	_dynamic_tag($scope0_id, "c", input.content, {}, 0, 0, $sg__input_content);
	_html(`</main><footer${_attr_class(input.footer.class)}>`);
	_dynamic_tag($scope0_id, "e", input.footer.content, {}, 0, 0, $sg__input_footer_content);
	_html(`</footer>${_el_resume($scope0_id, "d", $sg__input_footer_class)}`);
