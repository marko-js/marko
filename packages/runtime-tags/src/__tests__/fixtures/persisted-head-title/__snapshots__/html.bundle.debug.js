...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	_html(`<html><head><title>${_patch_text_content($scope0_id, "#title/0", `${_to_text(input.title)} | Shop`, _escape, $scope0_reason, 0)}</title>${_el_resume($scope0_id, "#title/0")}<meta name=description${_patch_attr($scope0_id, "#meta/1", "content", input.description, $scope0_reason, 1)}>${_el_resume($scope0_id, "#meta/1")}${_flush_head()}</head><body><main>${_patch_text($scope0_id, "#text/2", input.body, void 0, $scope0_reason, 2)}</main>`), _trailers("</body></html>");
