...
const $walks = "D b b l";
_shells({ "__tests__/template.marko": "__tests__/template.marko !__tests__/template.marko_0;D b b ;<main><ul></ul><button class=add>+</button><button class=inc>c</button></main>" });
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	if ($scope0_reason) _for_of(items, (item) => {
		...
		_html(`<li>${_text_resume($scope1_id, "#text/0", input.note + item + count)}</li>`);
		_scope($scope1_id, { item }, "__tests__/template.marko", "5:6", { item: "5:10" });
