...
var template_default = _template("a", (input) => {
		...
		if (input.a) {
			...
			_if(() => {
				if (input.b) {
					...
					$si__input_c__OR__input_a__OR__input_b && _subscribe($bar2__closures, writeScope($scope2_id, { _: _scope_with_id($scope1_id) }));
					return 0;
				}
	...
	$si__input_c__OR__input_a__OR__input_b && writeScope($scope0_id, {
		...
		Bg: _serialize_if($scope0_reason, 3) && $bar2__closures
	});
}, 1);
