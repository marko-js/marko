...
var template_default = _template("__tests__/template.marko", (input) => {
		...
		if (input.a) {
			...
			_if(() => {
				if (input.b) {
					...
					_html(`<div>${_escape(bar("foo"))}${_el_resume($scope2_id, "#text/0", _serialize_guard($scope0_reason, 1))}</div>`);
					_subscribe($si__input_c && $bar2__closures, writeScope($scope2_id, { _: _scope_with_id($scope1_id) }, "__tests__/template.marko", "6:3"));
					return 0;
				}
	...
	}, $scope0_id, "#text/0", _serialize_guard($scope0_reason, 0), $sg__input_a, $sg__input_a);
	writeScope($scope0_id, {
		input_c: input.c,
		input_b: _serialize_if($scope0_reason, 2) && input.b,
		bar,
