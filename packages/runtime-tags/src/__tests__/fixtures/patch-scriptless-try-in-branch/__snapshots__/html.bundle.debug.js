	...
	_if(() => {
		if (input.show) {
			...
			_try($scope1_id, "#text/0", _content_resume("__tests__/template.marko_2*content", () => {
				...
				$scope0_page && _subscribe(_unfilled_if($scope0_reason, 2) && $input_promise__closures, _scope($scope2_id, { _: _scope_with_id($scope1_id) }, "__tests__/template.marko", "3:6"), _client_guard($scope0_reason, 2) && "__tests__/template.marko_2_input_promise#4/subscribe", 0);
