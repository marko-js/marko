...
// tags/box.marko
var box_default = _template("__tests__/tags/box.marko", (input) => {
	const $scope0_reason = _scope_reason(), $sg__input_as = _serialize_guard($scope0_reason, 0);
	const $scope0_id = _scope_id();
	_dynamic_tag($scope0_id, "#text/0", input.as || "div", { class: "box" }, _content_resume("__tests__/tags/box.marko_1*content", () => {
		const $scope1_id = _scope_id();
		_scope_reason();
		_html("box body");
	}, $scope0_id), 0, $sg__input_as);
	_serialize_if($scope0_reason, 0) && _scope($scope0_id, {}, "__tests__/tags/box.marko", 0);
});
