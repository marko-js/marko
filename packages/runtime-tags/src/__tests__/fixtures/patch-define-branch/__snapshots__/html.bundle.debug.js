...
var template_default = _template_patch("__tests__/template.marko", (input) => {
	...
	const foo = { content: _content("__tests__/template.marko_1*content", () => {
		...
		_subscribe(_unfilled_if($scope0_reason, 0) && $input_x__closures, _scope($scope1_id, { _: _scope_with_id($scope0_id) }, "__tests__/template.marko", "2:2"), _client_guard($scope0_reason, 0) && "__tests__/template.marko_1_input_x#4/subscribe");
	}, $scope0_id) };
	if ($scope0_page) _if(() => {
		if (s) {
			const $scope2_id = _scope_id();
			foo.content({});
			_scope($scope2_id, {}, "__tests__/template.marko", "3:2");
			return 0;
		}
	}, $scope0_id, "#text/0");
