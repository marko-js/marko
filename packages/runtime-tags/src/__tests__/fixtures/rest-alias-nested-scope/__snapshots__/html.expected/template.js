...
export default _._template("__tests__/template.marko", input => {
  const $scope0_reason = _._scope_reason(),
    $sg__input_value_foo = _._serialize_guard($scope0_reason, /* input.value.foo */1),
  ...
  _._if(() => {
    if (value) {
      ...
      _._html(` -- ${_._sep(($sg__input_value_foo))}${_._escape(foo)}${_._el_resume($scope1_id, "#text/0", ($sg__input_value_foo))}<span`);
