...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	_try($scope0_id, "#text/2", _content_resume("__tests__/template.marko_1_content", () => {
		...
		_await($scope1_id, "#text/0", resolveAfter(input.note, input.tick), (note) => {
			...
			writeScope($scope3_id, { taps: _state_reason() && taps }, "__tests__/template.marko", "13:6", { taps: "14:12" });
			_resume_branch($scope3_id);
		});
		$sg__input_note__OR__input_tick && _subscribe($sg__input_tick && $input_tick__closures, _subscribe($sg__input_note && $input_note__closures, writeScope($scope1_id, { _: _scope_with_id($scope0_id) }, "__tests__/template.marko", "11:4")));
