...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	_try($scope0_id, "#text/2", _content_resume("__tests__/template.marko_1_content", () => {
		...
		_await($scope1_id, "#text/0", resolveAfter(input.note, input.tick), (note) => {
			...
			_html(`<p>${_escape(_hole_value($scope3_id, "UpdateHole:#text/0", note, _persisted_reason()))}${_el_resume($scope3_id, "#text/0", $sg__input_note__OR__input_tick)}</p>`);
			$sg__input_note__OR__input_tick && writeScope($scope3_id, {}, "__tests__/template.marko", "14:6");
		}, $sg__input_note__OR__input_tick);
		$sg__input_note__OR__input_tick && _subscribe($sg__input_tick && $input_tick__closures, _subscribe($sg__input_note && $input_note__closures, writeScope($scope1_id, { _: _scope_with_id($scope0_id) }, "__tests__/template.marko", "12:4")));
	...
	writeScope($scope0_id, {
		input_note: (_serialize_if($scope0_reason, 2) || _update_reason()) && input.note,
		input_tick: (_serialize_if($scope0_reason, 1) || _update_reason()) && input.tick,
