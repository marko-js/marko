...
var template_default = _template_patch("__tests__/template.marko", (input) => {
	...
	_try($scope0_id, "#text/0", _content_resume("__tests__/template.marko_3*content", () => {
		...
		$scope0_page && _subscribe(_unfilled_if($scope0_reason, 1) && $input_promise__closures, _scope($scope3_id, { _: _scope_with_id($scope0_id) }, "__tests__/template.marko", "3:4"), _client_guard($scope0_reason, 1) && "__tests__/template.marko_3_input_promise#6/subscribe", 0);
		...
		_if(() => {
			if (input.detail) {
				...
				}, "__tests__/template.marko", "8:8"), _client_guard($scope0_reason, 0) && "__tests__/template.marko_2_input_detail#5/subscribe", $sg__input_detail);
				return 0;
			}
		}, $scope1_id, "#text/0", $sg__input_detail, $sg__input_detail, $sg__input_detail, 0, 1);
		_subscribe($si__input_detail && $input_detail__closures, _scope($scope1_id, { _: _scope_with_id($scope0_id) }, "__tests__/template.marko", "7:6"), _client_guard($scope0_reason, 0) && "__tests__/template.marko_1_input_detail#5/subscribe", $sg__input_detail);
