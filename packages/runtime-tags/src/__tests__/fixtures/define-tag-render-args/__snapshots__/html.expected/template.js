  ...
  const MyTag = {
    content: _._content("__tests__/template.marko_1_content", (a, b, c) => {
      ...
      const $scope1_reason = _._scope_reason(),
        $sg__b = _._serialize_guard($scope1_reason, /* b */2),
        $sg__c = _._serialize_guard($scope1_reason, /* c */3);
      _._html(`<div>${_._escape(a)}${_._el_resume($scope1_id, "#text/0", (_._serialize_guard($scope1_reason, /* a */1)))}|${_._sep(($sg__b))}${_._escape(b)}${_._el_resume($scope1_id, "#text/1", ($sg__b))}|${_._sep(($sg__c))}${_._escape(c)}${_._el_resume($scope1_id, "#text/2", ($sg__c))}</div>`);
