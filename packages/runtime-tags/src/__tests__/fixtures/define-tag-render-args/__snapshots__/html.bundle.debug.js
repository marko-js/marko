...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	const MyTag = { content: _content("__tests__/template.marko_1_content", (a, b, c) => {
		...
		_html_opens("__tests__/template.marko:3:3"), _html(`<div>${_escape(a)}${_el_resume($scope1_id, "#text/0", _serialize_guard($scope1_reason, 1))}|${_sep($sg__b)}${_escape(b)}${_el_resume($scope1_id, "#text/1", $sg__b)}|${_sep($sg__c)}${_escape(c)}${_el_resume($scope1_id, "#text/2", $sg__c)}</div>`);
