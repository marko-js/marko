...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	const ChildA = { content: _content("__tests__/template.marko_1_content", ({ foo, foo: $foo }) => {
		...
		_html_opens("__tests__/template.marko:2:3"), _html(`<div class=a>${_escape(bar)}${_el_resume($scope1_id, "#text/0", $sg__foo)} ${_sep($sg__foo)}${_escape(typeof foo)}${_el_resume($scope1_id, "#text/1", $sg__foo)}</div>`);
	...
	const ChildB = { content: _content("__tests__/template.marko_2_content", (input) => {
		...
		_html_opens("__tests__/template.marko:10:3"), _html(`<div class=b>${_escape(bar)}${_el_resume($scope2_id, "#text/0", $sg__foo2)} ${_sep($sg__foo2)}${_escape(typeof foo)}${_el_resume($scope2_id, "#text/1", $sg__foo2)}</div>`);
