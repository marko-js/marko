...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	const $scope0_reason = _scope_reason(), $scope0_page = _page_render(), $sg__input_kind = _source_guard($scope0_reason, 0), $sg__input_inner = _source_guard($scope0_reason, 1), $si__input_inner = _source_if($scope0_reason, 1);
	const $scope0_id = _scope_id();
	...
	widget_default({ content: _content_elide("__tests__/template.marko_1*content", () => {
		...
		_if(() => {
			if (input.kind === "a") {
				...
				_html("<b>A</b>");
				$scope0_page && _scope($scope6_id, {}, "__tests__/template.marko", "4:6");
				return 0;
		...
		}, $scope1_id, "#text/0", 1, $sg__input_kind, $sg__input_kind, void 0, void 0, ["__tests__/template.marko_6*shell", "__tests__/template.marko_2*shell"], $scope0_reason, 0);
		$scope0_page && _subscribe(_unfilled_if($scope0_reason, 0) && $input_kind__closures, _scope($scope1_id, { _: _scope_with_id($scope0_id) }, "__tests__/template.marko", "3:4"));
			...
			widget_default({ content: _content_elide("__tests__/template.marko_4*content", () => {
				...
				if ($scope0_page) _if(() => {
					if (input.inner === "a") {
						...
						_html("<b>A</b>");
						$scope0_page && _scope($scope7_id, {}, "__tests__/template.marko", "9:8");
						return 0;
