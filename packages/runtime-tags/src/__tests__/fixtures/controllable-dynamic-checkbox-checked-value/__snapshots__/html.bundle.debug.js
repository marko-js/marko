...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	_html(`<input${_attr_input_checkedValue($scope0_id, "#input/2", checkedValue, $checkedValueChange, "c")} type=radio>${_el_resume($scope0_id, "#input/2")}<span>${_text_resume($scope0_id, "#text/3", checkedValue)}</span><button>Toggle</button>${_el_resume($scope0_id, "#button/4")}`);
