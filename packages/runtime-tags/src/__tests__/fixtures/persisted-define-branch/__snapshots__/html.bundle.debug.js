...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	const foo = { content: _content("__tests__/template.marko_1*content", () => {
		...
		_subscribe(_unfilled_if($scope0_reason, 0) && $input_x__closures, _scope($scope1_id, { _: _scope_with_id($scope0_id) }, "__tests__/template.marko", "2:2"));
	}, $scope0_id) };
	if ($scope0_page) _if(() => {
		if (s) {
	...
	}, "__tests__/template.marko", 0, { s: "1:6" }) : _filled_guard($scope0_reason, 0) && _patch_value($scope0_id, "__tests__/template.marko0", input.x);
}, 1, 1);
