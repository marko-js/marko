...
_shells({
	...
	"__tests__/template.marko_3*shell": "__tests__/template.marko_3*shell __tests__/template.marko_3_store_set#8/init!__tests__/template.marko_3;Db%l ;<span>Seen <!></span><button>+</button>"
});
var template_default = _template_patch("__tests__/template.marko", (input) => {
	...
	_for_of(["x", "y"], (name) => {
		...
		_var($scope1_id, "#scopeOffset/1", $childScope, "__tests__/template.marko_1_store#6/var");
		_html(`<p>${_patch_text($scope1_id, "#text/2", name, void 0, 0, 0)}:${_text_resume($scope1_id, "#text/3", store.last, 2)}</p>`);
		_for_of(["y", "x"], (other) => {
			...
			}, "__tests__/template.marko", "4:4", { "#LoopKey": "4:8" }), _client_guard($scope0_reason, 0) && "__tests__/template.marko_2_input_show#3/subscribe", $sg__input_show);
	...
	$scope0_page && _scope($scope0_id, { "ClosureScopes:input_show": $input_show__closures }, "__tests__/template.marko", 0);
}, 1, () => [store_default]);
