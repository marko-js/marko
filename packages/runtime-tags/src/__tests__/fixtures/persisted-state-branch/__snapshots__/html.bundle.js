...
var template_default = _template("a", (input) => {
	...
	let count = 0;
	let list = ["a", "b"];
	_html(`<h1>${_escape(_hole_value($scope0_id, "a", input.title, _persisted_reason()))}${_el_resume($scope0_id, "a", _serialize_guard($scope0_reason, 0))}</h1><button class=toggle>toggle</button>${_el_resume($scope0_id, "b")}<button class=count>clicked <!>${_escape(count)}${_el_resume($scope0_id, "d")}</button>${_el_resume($scope0_id, "c")}`);
	_if(() => {
		{
