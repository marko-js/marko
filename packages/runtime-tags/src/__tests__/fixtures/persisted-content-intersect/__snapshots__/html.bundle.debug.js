	...
	if ($scope0_reason) _if$1(() => {
		if (open) {
			...
			box_default({ content: _content$1("__tests__/template.marko_2*content", () => {
				...
				_html$1(`<p>${_escape(input.a + ":" + input.b)}${_el_resume($scope2_id, "#text/0", _source_guard($scope0_reason, 0))}</p>`);
				_subscribe(_source_if($scope0_reason, 2) && $input_b__closures, _subscribe(_source_if($scope0_reason, 1) && $input_a__closures, writeScope($scope2_id, { _: _scope_with_id($scope1_id) }, "__tests__/template.marko", "4:6")));
