...
var template_default = _template("a", (input) => {
	...
	_if(() => {
		if (input.err) {
			...
			_html(`<h1>${_escape(_hole_value($scope1_id, "a", input.title, _persisted_reason()))}${_el_resume($scope1_id, "a", _serialize_guard($scope0_reason, 2))}</h1><button>${_escape(count)}${_el_resume($scope1_id, "c")}</button>${_el_resume($scope1_id, "b")}`);
