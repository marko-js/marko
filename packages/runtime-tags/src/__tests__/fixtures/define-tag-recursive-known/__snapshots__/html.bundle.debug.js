	...
	const Foo = { content: _content("__tests__/template.marko_1*content", (input) => {
		...
		_if(() => {
			if (input.bar) {
				...
				_html(_text_resume($scope3_id, "#text/0", JSON.stringify(input.message), $sg__input_message));
		...
		$si__input_bar && _scope($scope1_id, { input_message: input.message }, "__tests__/template.marko", "1:1", { input_message: ["input.message", "1:12"] });
	}, $scope0_id) };
	Foo.content({ bar: "hi" });
}, 1);
