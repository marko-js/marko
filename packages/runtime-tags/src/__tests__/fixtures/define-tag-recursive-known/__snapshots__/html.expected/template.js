  ...
  const Foo = {
    content: _._content("__tests__/template.marko_1_content", input => {
      ...
      const $scope1_reason = _._scope_reason(),
        $sg__input_bar = _._serialize_guard($scope1_reason, /* input.bar */1),
        $si__input_bar = _._serialize_if($scope1_reason, /* input.bar */1);
      _._if(() => {
        if (input.bar) {
          const $scope2_id = _._scope_id();
          const $childScope = _._peek_scope_id();
          _._set_serialize_reason({
            /* input.bar, input.message */0: ($sg__input_bar),
            /* input.message */2: ($sg__input_bar)
          });
          Foo.content({
            message: input.bar
          });
          ($si__input_bar) && _._scope($scope2_id, {
      ...
      }, $scope1_id, "#text/0", (_._serialize_guard($scope1_reason, /* input.bar, input.message */0)) || $sg__input_bar, $sg__input_bar, $sg__input_bar);
      ($si__input_bar) && _._scope($scope1_id, {
        input_bar: input.bar,
