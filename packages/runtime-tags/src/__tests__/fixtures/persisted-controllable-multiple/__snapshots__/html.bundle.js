...
var template_default = _template_persisted("a", (input) => {
	...
	_attr_select_value($scope0_id, "b", input.choice, _resume(function(next) {
		...
		_html(`<select${_patch_bind($scope0_id, "Eb", _resume(function(next) {
			document.querySelector("main").dataset.choice = String(next);
		}, "a0"), 0, 0)}${_patch_control($scope0_id, "b", 3, input.choice, $scope0_owned, 2)}${_patch_attr($scope0_id, "b", "multiple", input.many, $scope0_owned, 1)}><option${_attr_option_value("a")}>A</option><option${_attr_option_value("b")}>B</option></select>`);
	});
