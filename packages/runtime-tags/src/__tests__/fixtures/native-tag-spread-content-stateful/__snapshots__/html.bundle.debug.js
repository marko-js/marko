...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	my_box_default({
		...
		content: _content("__tests__/template.marko_1_content", () => {
			...
			let count = 0;
			_html_opens("__tests__/template.marko:3:3", "__tests__/template.marko:4:3"), _html(`<button type=button class=inc>increment</button>${_el_resume($scope1_id, "#button/0")}<span class=count>${_escape(count)}${_el_resume($scope1_id, "#text/1")}</span>`);
