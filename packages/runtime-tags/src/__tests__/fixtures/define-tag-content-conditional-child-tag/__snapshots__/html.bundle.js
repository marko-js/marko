...
var template_default = _template("a", (input) => {
	...
	({ content: _content("a1", (input) => {
		...
		child_default({ content: _content_resume("a0", () => {
			_scope_reason();
			const $scope3_id = _scope_id();
			_dynamic_tag($scope3_id, "a", input.content, {}, 0, 0, $sg__input_content);
			_subscribe($si__input_content && $Box_content2__input_content__closures, _scope($scope3_id, { _: _scope_with_id($scope2_id) }));
	...
	}, $scope0_id) }).content({ content: _content_resume("a2", () => {
		_scope_reason();
