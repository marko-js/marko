		...
		content: _content_elide("a0", () => {
			...
			_if(() => {
				if (input.show) {
					...
					_html(`<em>${_patch_text($scope2_id, "a", input.note, void 0, $scope0_reason, 3)}</em>`);
					_subscribe(_unfilled_if($scope0_reason, 3) && $input_note__closures, _scope($scope2_id, { _: _scope_with_id($scope1_id) }));
					return 0;
				}
