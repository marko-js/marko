		...
		content: _content_elide("a0", () => {
			...
			_if(() => {
				if (input.show) {
					...
					_html(`<em>${_patch_text($scope2_id, "a", input.note, $scope0_owned, 3)}${_el_resume($scope2_id, "a")}</em>`);
					_subscribe(_source_if($scope0_reason, 3) && $input_note__closures, writeScope($scope2_id, { _: _scope_with_id($scope1_id) }));
					return 0;
				}
			}, $scope1_id, "a", 1, $sg__input_show, $sg__input_show, void 0, void 0, ["a1"]);
			$scope0_reason && _subscribe(_source_if($scope0_reason, 2) && $input_show__closures, writeScope($scope1_id, { _: _scope_with_id($scope0_id) }));
