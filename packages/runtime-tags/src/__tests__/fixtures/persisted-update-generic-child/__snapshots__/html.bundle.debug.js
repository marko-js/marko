...
var toggle_default = _template("__tests__/tags/toggle.marko", (input) => {
	...
	const $scope0_id = _scope_id();
	let on = false;
	_html(`<button class=toggle>${_escape(_hole_value($scope0_id, "PatchHole:#text/1", on ? "on" : "off", _state_reason()))}${_el_resume($scope0_id, "#text/1")}</button>${_el_resume($scope0_id, "#button/0")}<em>${_escape(_hole_value($scope0_id, "PatchHole:#text/2", input.name, _persisted_reason()))}${_el_resume($scope0_id, "#text/2", _serialize_guard($scope0_reason, 0))}</em>`);
	_script($scope0_id, "__tests__/tags/toggle.marko_0");
	writeScope($scope0_id, { on: _seed_fill(_state_reason() && on) }, "__tests__/tags/toggle.marko", 0, { on: "1:6" });
...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	}, $scope0_id, "#text/3", "__tests__/template.marko_r0");
	_set_serialize_reason({
	...
	writeScope($scope0_id, {
		...
		"#childScope/4": $sg__input_panel_title__OR__input_panel_body__OR__input_panel_expanded | _persisted_reason() && _existing_scope($childScope2),
		"#childScope/6": $sg__input_name | _persisted_reason() && _existing_scope($childScope3)
