	...
	_if(() => {
		if (input.show) {
			...
			_try($scope1_id, "#text/0", _content_resume("__tests__/template.marko_3*content", () => {
				...
				$scope0_page && _subscribe(_unfilled_if($scope0_reason, 3) && $input_promise__closures, _scope($scope3_id, { _: _scope_with_id($scope1_id) }, "__tests__/template.marko", "4:4"), _client_guard($scope0_reason, 3) && "__tests__/template.marko_3_input_promise#7/subscribe", 0);
				...
				_subscribe(_source_if($scope0_reason, 2) && $input_title__closures, _scope($scope2_id, { _: _scope_with_id($scope1_id) }, "__tests__/template.marko", "6:6"), _client_guard($scope0_reason, 2) && "__tests__/template.marko_2_input_title#6/subscribe", $sg__input_title || $sg__err_message);
