...
var card_default = _template_patch("b", (input) => {
	...
	_html(`<b>${_patch_text($scope0_id, "a", input.title, void 0, $scope0_reason, 0)}</b>`);
...
var template_default = _template_patch("a", (input) => {
	...
	_for_of(input.items, (item) => {
		...
		card_default({
			...
			content: _content_elide("a0", () => {
				...
				_html(`<em>${_patch_text($scope2_id, "a", item.n, void 0, $scope0_reason, 0)}</em>`);
				_subscribe(_unfilled_if($scope0_reason, 0) && $for_content__item_n__closures, _scope($scope2_id, { _: _scope_with_id($scope1_id) }));
			}, $scope1_id)
		});
		_html("</li>");
		_scope($scope1_id, {
	...
	}, 0, $scope0_id, "a", 1, 1, _source_guard($scope0_reason, 0), void 0, void 0, "a1", $scope0_reason, 0);
