...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	_for_of(input.rows, (row) => {
		...
		_for_of(row.cells, (cell) => {
			...
			_filled_guard($scope0_owned, 1) ? _patch_value($scope2_id, "__tests__/template.marko1", cell) : _patch_init($scope2_id, "__tests__/template.marko_2_input_rows#6/init");
	...
	$scope0_reason ? _scope($scope0_id, {
		...
		input_suffix: ["input.suffix"],
		count: "1:6"
	}) : _filled_guard($scope0_owned, 2) && _patch_value($scope0_id, "__tests__/template.marko0", input.suffix);
}, 1, 0);
