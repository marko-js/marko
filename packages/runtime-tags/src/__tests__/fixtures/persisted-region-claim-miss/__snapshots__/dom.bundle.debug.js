...
const $if = /*@__PURE__*/ _if("#text/2", "<aside class=banner>system maintenance at midnight</aside>");
const $input_banner = ($scope, input_banner) => {
	if (!updating) $if($scope, input_banner ? 0 : 1);
};
const $input = ($scope, input) => $input_banner($scope, input.banner);
