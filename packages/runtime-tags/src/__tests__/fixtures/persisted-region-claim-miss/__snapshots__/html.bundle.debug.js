...
var template_default = _template("__tests__/template.marko", (input) => {
	const $scope0_reason = _scope_reason(), $sg__input_banner = _serialize_guard($scope0_reason, 0);
	const $scope0_id = _scope_id();
	let count = 1;
	_html(`<button class=bump>${_escape(count)}${_el_resume($scope0_id, "#text/1")}</button>${_el_resume($scope0_id, "#button/0")}`);
	_if(() => input.banner ? 0 : undefined, $scope0_id, "#text/2", $sg__input_banner, $sg__input_banner, $sg__input_banner, 0, 1, "__tests__/template.marko_0/update_if_#text/2", [() => {
		const $scope1_id = _scope_id();
		_html("<aside class=banner>system maintenance at midnight</aside>");
		$sg__input_banner && writeScope($scope1_id, {}, "__tests__/template.marko", "3:2");
