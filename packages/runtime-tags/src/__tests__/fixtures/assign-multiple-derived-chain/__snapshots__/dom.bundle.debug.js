...
const $index__OR__filtered = /*@__PURE__*/ _or(10, ($scope) => $selected($scope, $scope.filtered["index" in $scope ? "index" in $scope ? $scope.index : 0 : 0]));
const $filtered = /*@__PURE__*/ _const("filtered", ($scope) => {
	_text($scope["#text/0"], $scope.filtered.join(" "));
	$index__OR__filtered($scope);
});
const $items__OR__min__OR__max = /*@__PURE__*/ _or(7, ($scope) => $filtered($scope, $scope.items.filter((item) => item >= ("min" in $scope ? "min" in $scope ? $scope.min : 0 : 0) && item <= ("max" in $scope ? "max" in $scope ? $scope.max : 4 : 4))), 2);
