	...
	_if(() => {
		if (input.show) {
			...
			_html(`<input${_attr_input_value($scope1_id, "a", input.value, _resume(function(next) {
				...
				document.querySelector("main").dataset.got = next;
			}, "a0"), 0, 0)}${_patch_control($scope1_id, "a", 2, input.value, $scope0_reason, 3)}>${_el_resume($scope1_id, "a")}`);
			_script($scope1_id, "a2");
			_scope($scope1_id, { _: _scope_with_id($scope0_id) });
			return 0;
		}
	}, $scope0_id, "b", 1, $sg__input_show, $sg__input_show, void 0, void 0, ["a1"], $scope0_reason, 2);
	_html("</main>");
