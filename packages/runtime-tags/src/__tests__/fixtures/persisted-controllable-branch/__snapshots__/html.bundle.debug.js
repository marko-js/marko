// template.marko
_renderer_shells({ "__tests__/template.marko_1_shell": ",`__tests__/template.marko_1_shell __tests__/template.marko_1; ;<input>`" });
	...
	_if(() => {
		if (input.show) {
			...
			_html(`<input${_attr_input_value($scope1_id, "#input/0", input.value, _resume(function(next) {
				...
				document.querySelector("main").dataset.got = next;
			}, "__tests__/template.marko_1/valueChange"))}${_patch_control($scope1_id, "#input/0", 2, input.value)}>${_el_resume($scope1_id, "#input/0")}`);
