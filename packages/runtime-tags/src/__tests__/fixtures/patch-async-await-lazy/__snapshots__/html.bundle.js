...
var template_default = _template_patch("a", (input) => {
	...
	let count = 0;
	_html(`<main><h1>${_patch_text($scope0_id, "a", input.title, void 0, $scope0_reason, 3)}</h1><section>`);
	_try($scope0_id, "b", _content_resume("a4", () => {
		...
		$scope0_page && _subscribe(_unfilled_if($scope0_reason, 5) && $input_slow__closures, _subscribe(_unfilled_if($scope0_reason, 4) && $input_related__closures, _scope($scope1_id, { _: _scope_with_id($scope0_id) })));
