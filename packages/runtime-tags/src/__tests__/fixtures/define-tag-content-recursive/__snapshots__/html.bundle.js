...
var template_default = _template("a", (input) => {
	...
	({ content: _content("a1", (input) => {
		...
		_serialize_guard($scope2_reason, 1);
		const $si__input_depth = _serialize_if($scope2_reason, 0), $si__input_content = _serialize_if($scope2_reason, 1);
		let open = false;
		...
		_scope($scope2_id, {
			...
			g: input.content,
			h: open,
			i: $si__input_depth && $Tree_content2__input_depth__closures,
			j: $si__input_content && $Tree_content2__input_content__closures
		});
	}, $scope0_id) }).content({
		depth: 2,
