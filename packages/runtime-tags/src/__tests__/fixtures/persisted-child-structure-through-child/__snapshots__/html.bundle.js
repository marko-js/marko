...
var leaf_default = _template_persisted("b", (input) => {
	...
	_if(() => {
		if (input.flag) {
			...
			_html(`<em>${_patch_text($scope1_id, "a", input.label, void 0, $scope0_owned, 2)}</em>`);
			_scope($scope1_id, { _: _scope_with_id($scope0_id) });
			return 0;
		}
	}, $scope0_id, "a", 1, $sg__input_flag, $sg__input_flag, void 0, void 0, ["b0"], $scope0_owned, 1);
	$scope0_reason ? _scope($scope0_id, { e: input.label }) : _filled_guard($scope0_owned, 2) && _client_guard($scope0_owned, 1) && _patch_value($scope0_id, "b0", input.label);
}, 0, 0);
...
var mid_default = _template_persisted("c", (input) => {
	...
	}, $scope0_id, "a", 1, $sg__input_show, $sg__input_show, void 0, void 0, ["c0"], $scope0_owned, 2);
	$scope0_reason ? _scope($scope0_id, {
		e: input.flag,
		f: input.label
	}) : (_filled_guard($scope0_owned, 3) && _client_guard($scope0_owned, 2) && _patch_value($scope0_id, "c0", input.flag), _filled_guard($scope0_owned, 4) && _client_guard($scope0_owned, 2) && _patch_value($scope0_id, "c1", input.label));
}, 0, () => [leaf_default]);
...
var template_default = _template_persisted("a", (input) => {
	...
	$scope0_reason ? _scope($scope0_id, {
		g: count,
		a: _existing_scope($childScope)
	}) : _filled_guard($scope0_owned, 2) && _patch_value($scope0_id, "a0", input.label);
}, 1, () => [mid_default]);
