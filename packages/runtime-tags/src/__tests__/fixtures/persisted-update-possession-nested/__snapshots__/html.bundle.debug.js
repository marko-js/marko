...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	const PanelA = { content: _content("__tests__/template.marko_1_content", () => {
		...
		_html(`<p class=a>Panel A: ${_sep(_persisted_reason())}${_escape(_hole_value($scope1_id, "UpdateHole:#text/0", getLabel?.($global().topic), _persisted_reason()))}${_el_resume($scope1_id, "#text/0", _persisted_reason())}</p>`);
	...
	const PanelB = { content: _content("__tests__/template.marko_3_content", () => {
		...
		_html(`<section class=b>Panel B: ${_sep(_persisted_reason())}${_escape(_hole_value($scope3_id, "UpdateHole:#text/0", getLabel?.($global().topic), _persisted_reason()))}${_el_resume($scope3_id, "#text/0", _persisted_reason())}</section>`);
