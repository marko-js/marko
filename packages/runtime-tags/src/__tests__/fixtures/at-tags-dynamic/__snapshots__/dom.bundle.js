// tags/hello/index.marko
const $for_content3__attrs__script = _script("b2", ($scope) => _attrs_script($scope, "a"));
const $for_content2__attrs__script = _script("b1", ($scope) => _attrs_script($scope, "a"));
const $for_content__attrs__script = _script("b0", ($scope) => _attrs_script($scope, "a"));

// template.marko
const $row_content2 = _content_resume("a3", "Outside", "b");
const $row_content = /*@__PURE__*/ _content_closures(_content_resume("a2", " ", " b"), { 2($scope) {
	_text($scope.a, $scope.c);
} });
const $item_content2 = _content_resume("a1", "bar", "b");
const $item_content = _content_resume("a0", "foo", "b");
