...
# Update
```js
_strict.default.deepEqual(lineupItems(container), expected);
_strict.default.equal(container.querySelector("h2.headline").textContent, `${expected.length} on stage`);
```
