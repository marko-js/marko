...
var template_default = _template_persisted("c", (input) => {
	...
	_dynamic_tag($scope0_id, "a", $tag, {}, _content_elide("c1", () => {
		...
		box_default({
			...
			content: _content_elide("c0", () => {
				...
				_html(_patch_text($scope2_id, "a", input.label, void 0, $scope0_owned, 2));
				_subscribe(_unfilled_if($scope0_owned, 2) && $input_label__closures, _scope($scope2_id, { _: _scope_with_id($scope1_id) }));
			}, $scope1_id)
		});
	...
	}, $scope0_id), 0, _source_guard($scope0_reason, 1), _patch_dynamic_tag($scope0_id, "a", $tag, 0, "c1", 0, $scope0_owned, 1));
