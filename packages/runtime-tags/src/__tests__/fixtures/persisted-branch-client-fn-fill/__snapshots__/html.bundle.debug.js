...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	if ($scope0_reason) _if(() => {
		if (show) {
			...
			_html(`<p>${_escape(getTitle())}${_el_resume($scope1_id, "#text/0")}</p>`);
	...
	$scope0_reason ? writeScope($scope0_id, {
		...
		input_title: ["input.title"],
		getTitle: "1:8"
	}) : (_owned_guard($scope0_owned, 0) && _patch_value($scope0_id, "__tests__/template.marko0", getTitle), _owned_guard($scope0_owned, 0) && _patch_write($scope0_id, "input_title", input.title));
