...
var card_default = _template_persisted("b", (input) => {
	...
	_html(`<section${_patch_attr_class($scope0_id, "a", input.klass, $scope0_owned, 0)}><h2>${_patch_text($scope0_id, "b", input.title, $scope0_owned, 1)}${_el_resume($scope0_id, "b")}</h2><p>${_patch_text($scope0_id, "c", input.note, $scope0_owned, 2)}${_el_resume($scope0_id, "c")}</p></section>${_el_resume($scope0_id, "a")}`);
