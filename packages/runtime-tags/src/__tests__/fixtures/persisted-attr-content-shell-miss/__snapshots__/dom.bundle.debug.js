...
const $if_content__n = _var_resume("__tests__/template.marko_2_n/var", /*@__PURE__*/ _let_persisted("n/3", ($scope) => _text($scope["#text/1"], $scope.n)));
...
const $if = /*@__PURE__*/ _if("#text/2", "<button class=tap>tap <!></button><div class=host></div>", " Db%l ", $if_content__setup);
const $input_show = ($scope, input_show) => {
	if (!updating) $if($scope, input_show ? 0 : 1);
};
const $input = ($scope, input) => $input_show($scope, input.show);
