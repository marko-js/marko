...
const $try_content__input_note = /*@__PURE__*/ _closure_get(8, ($scope) => {
	if (!updating) $try_content__await_promise($scope, resolveAfter($scope._._.g, 1));
}, ($scope) => $scope._._);
...
const $collapsed = _var_resume("a10", /*@__PURE__*/ _let_persisted(7, ($scope) => $if($scope, !$scope.h ? 0 : 1)));
const $setup__script = _script_shared(($scope) => _on($scope.b, "click", function() {
	$collapsed($scope, !$scope.h);
}));
_static_shells({
...
const $collapsed_seed = _update_signal("a10");
...
const $update2 = ($patch, $live) => {
	...
	if ("h" in $patch) _update_seed($live, $collapsed_seed, $patch["h"]);
	if ("g" in $patch) {
		$live["g"] = $patch["g"];
...
const $if_content__setup = ($scope) => $if_content__try($scope, { placeholder: attrTag({ content: $placeholder_content($scope) }) });
const $if = /*@__PURE__*/ _if(2, "<!><!><!>", "b%", $if_content__setup);
const $collapsed = /*@__PURE__*/ _let_persisted(7, ($scope) => $if($scope, !$scope.h ? 0 : 1));
