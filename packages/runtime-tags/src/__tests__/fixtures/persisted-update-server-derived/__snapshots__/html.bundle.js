...
var template_default = _template("a", (input) => {
	...
	_if(() => {
		if (input.detailId) {
			...
			_html(`<section><h2>${_escape(_hole_value($scope1_id, "a", details.name, _persisted_reason()))}${_el_resume($scope1_id, "a", $sg__input_detailId)}</h2><p>costs ${_sep($sg__input_detailId)}${_escape(_hole_value($scope1_id, "b", details.price, _persisted_reason()))}${_el_resume($scope1_id, "b", $sg__input_detailId)}</p><button class=copy>use price</button>${_el_resume($scope1_id, "c")}</section>`);
