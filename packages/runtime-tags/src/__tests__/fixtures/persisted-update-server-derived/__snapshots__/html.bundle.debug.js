...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	_if(() => input.detailId ? 0 : 1, $scope0_id, "#text/2", $sg__input_detailId, $sg__input_detailId, $sg__input_detailId, 0, 1, "__tests__/template.marko_0/update_if_#text/2", [() => {
		...
		_html(`<section><h2>${_escape(_hole_value($scope1_id, "PatchHole:#text/0", details.name, _persisted_reason()))}${_el_resume($scope1_id, "#text/0", $sg__input_detailId)}</h2><p>costs ${_sep($sg__input_detailId)}${_escape(details.price)}${_el_resume($scope1_id, "#text/1", $sg__input_detailId)}</p><button class=copy>use price</button>${_el_resume($scope1_id, "#button/2")}</section>`);
	...
	}], ["__tests__/template.marko_1_update", "__tests__/template.marko_2_update"], "__tests__/template.marko_r0");
	_script($scope0_id, "__tests__/template.marko_0");
	writeScope($scope0_id, {
		input_detailId: (_serialize_if($scope0_reason, 0) || _patch_reason()) && input.detailId,
...
_renderer_shells({
	"__tests__/template.marko_2_update": ["<p>no selection</p>", "b"],
	"__tests__/template.marko_2_content": ["<p>no selection</p>", "b"],
	"__tests__/template.marko_1_update": ["<section><h2> </h2><p>costs <!></p><button class=copy>use price</button></section>", "E lDb%l l"],
	"__tests__/template.marko_1_content": ["<section><h2> </h2><p>costs <!></p><button class=copy>use price</button></section>", "E lDb%l l"],
