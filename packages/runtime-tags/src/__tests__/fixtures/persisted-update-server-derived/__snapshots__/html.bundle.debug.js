...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	_if(() => {
		if (input.detailId) {
			...
			_html(`<section><h2>${_escape(_hole_value($scope1_id, "UpdateHole:#text/0", details.name, _persisted_reason()))}${_el_resume($scope1_id, "#text/0", $sg__input_detailId)}</h2><p>costs ${_sep($sg__input_detailId)}${_escape(_hole_value($scope1_id, "UpdateHole:#text/1", details.price, _persisted_reason()))}${_el_resume($scope1_id, "#text/1", $sg__input_detailId)}</p><button class=copy>use price</button>${_el_resume($scope1_id, "#button/2")}</section>`);
