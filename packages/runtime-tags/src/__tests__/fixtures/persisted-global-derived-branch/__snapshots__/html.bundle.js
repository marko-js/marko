// collect.ts
function collect(global, item) {
	(global.seen ??= []).push(item);
	return item;
}
function collected(global) {
	const seen = global.seen;
	return seen?.length ? seen.splice(0).join("+") : "";
}

// template.marko
_shells({
	a: "a;D%b%;<main><!><!></main>",
	a0: "a0;D ;<p> </p>",
	a1: "a1; ;<style></style>"
});
var template_default = _template_persisted("a", (input) => {
	const $scope0_owned = _persisted_ownership(), $scope0_reason = _persisted_reason(), $sg__input_items = _source_guard($scope0_reason, 0);
	const $scope0_id = _scope_id();
	const $global$1 = $global();
	_html("<main>");
	_for_of(input.items, (item) => {
		const $scope1_id = _scope_id();
		_html(`<p>${_patch_text($scope1_id, "a", collect($global$1, item), void 0, $scope0_owned, 0)}</p>`);
		_scope($scope1_id, {});
	}, 0, $scope0_id, "a", 1, $sg__input_items, $sg__input_items, void 0, void 0, "a0", $scope0_owned, 0);
	const summary = collected($global$1);
	_if(() => {
		if (summary) {
			const $scope2_id = _scope_id();
			_html(`<style${_attr_nonce()}>${_patch_text_content($scope2_id, "a", summary, _escape_style)}</style>${_el_resume($scope2_id, "a")}`);
			_scope($scope2_id, {});
			return 0;
		}
	}, $scope0_id, "b", 1, $scope0_reason, $scope0_reason, void 0, void 0, ["a1"]);
	_html("</main>");
	$scope0_reason && _scope($scope0_id, {});
}, 1, 1);
