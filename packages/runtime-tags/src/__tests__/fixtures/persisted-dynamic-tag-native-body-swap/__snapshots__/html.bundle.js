...
var template_default = _template_persisted("b", (input) => {
	...
	_dynamic_tag($scope0_id, "a", $tag, {}, _content_elide("b0", () => {
		...
		_persisted_reason();
		_html(`body ${_patch_text($scope1_id, "a", input.x, 2, $scope0_owned, 2)}`);
		_subscribe(_unfilled_if($scope0_owned, 2) && $input_x__closures, _scope($scope1_id, { _: _scope_with_id($scope0_id) }));
	}, $scope0_id), 0, $sg__input_wrap, _patch_dynamic_tag($scope0_id, "a", $tag, 0, "b0", 0, $scope0_owned, 1));
	$scope0_reason && _scope($scope0_id, {
