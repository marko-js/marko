...
var template_default = _template_persisted("b", (input) => {
	...
	_dynamic_tag($scope0_id, "a", $tag, {}, _content_elide("b0", () => {
		...
		_subscribe(_unfilled_if($scope0_owned, 1) && $input_x__closures, _scope($scope1_id, { _: _scope_with_id($scope0_id) }));
	}, $scope0_id), 0, $sg__input_wrap, _patch_dynamic_tag($scope0_id, "a", $tag, 0, "b0", 0, $scope0_owned, 0));
	$scope0_reason && _scope($scope0_id, {
		e: input.x,
		f: $input_x__closures
	});
