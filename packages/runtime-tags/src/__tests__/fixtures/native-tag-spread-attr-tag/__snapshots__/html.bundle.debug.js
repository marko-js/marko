...
var my_box_default = _template("__tests__/tags/my-box.marko", (input) => {
	...
	_html_opens("__tests__/tags/my-box.marko:1:1"), _html("<section");
	_attrs_content(input.head, "#section/0", $scope0_id, "section");
	_html_opens("__tests__/tags/my-box.marko:2:1"), _html(`</section>${_el_resume($scope0_id, "#section/0")}<footer`);
	...
	my_box_default({
		head: attrTag({
			...
			content: _content("__tests__/template.marko_1_content", () => {
				...
				let n = 0;
				_html_opens("__tests__/template.marko:4:5"), _html(`<button>${_escape(n)}${_el_resume($scope1_id, "#text/1")}</button>${_el_resume($scope1_id, "#button/0")}`);
