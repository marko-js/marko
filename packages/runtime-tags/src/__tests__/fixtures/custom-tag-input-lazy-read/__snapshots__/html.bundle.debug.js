...
var press_button_default = _template("__tests__/tags/press-button/index.marko", (input) => {
	...
	_html_opens("__tests__/tags/press-button/index.marko:1:1"), _html(`<button class=act>press</button>${_el_resume($scope0_id, "#button/0")}`);
...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	let count = 0;
	let log = "";
	_html_opens("__tests__/template.marko:6:1"), _html(`<button class=inc>inc</button>${_el_resume($scope0_id, "#button/0")}`);
	press_button_default({ onPress: _resume(function() {
		log = `${log}[${count}]`;
	}, "__tests__/template.marko_0/onPress", $scope0_id) });
	_html_opens("__tests__/template.marko:9:1"), _html(`<div class=log>${_escape(log)}${_el_resume($scope0_id, "#text/2")}</div>`);
