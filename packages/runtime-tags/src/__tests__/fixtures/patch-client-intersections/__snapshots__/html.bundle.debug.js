...
var promo_tag_default = _template_patch("__tests__/tags/promo-tag.marko", (input) => {
	const $scope0_reason = _scope_reason(), $scope0_page = _page_render();
	const $scope0_id = _scope_id();
	let seen = 0;
	...
	$scope0_page ? _scope($scope0_id, {
		...
		input_text: ["input.text"],
		seen: "1:6"
	}) : _filled_guard($scope0_reason, 0) && _patch_value($scope0_id, "__tests__/tags/promo-tag.marko0", input.text);
}, 0, 0);
...
var site_footer_default = _template_patch("__tests__/tags/site-footer.marko", (input) => {
	const $scope0_reason = _scope_reason(), $scope0_page = _page_render();
	const $scope0_id = _scope_id();
	let frozen = 0;
...
var template_default = _template_patch("__tests__/template.marko", (input) => {
	...
	_html(`<main><h1>${_patch_text($scope0_id, "#text/0", input.title, void 0, $scope0_reason, 0)}</h1><p${_patch_attr($scope0_id, "#p/1", "title", $global$1.locale)}>${_patch_text($scope0_id, "#text/2", $global$1.brand)}</p>${_el_resume($scope0_id, "#p/1")}`);
