...
var price_card_default = _template_patch("b", (input) => {
...
var promo_tag_default = _template_patch("c", (input) => {
	const $scope0_reason = _scope_reason(), $scope0_page = _page_render();
	const $scope0_id = _scope_id();
	let seen = 0;
...
var site_footer_default = _template_patch("d", (input) => {
	const $scope0_reason = _scope_reason(), $scope0_page = _page_render();
	const $scope0_id = _scope_id();
	let frozen = 0;
...
var template_default = _template_patch("a", (input) => {
	...
	_html(`<main><h1>${_patch_text($scope0_id, "a", input.title, void 0, $scope0_reason, 0)}</h1><p${_patch_attr($scope0_id, "b", "title", $global$1.locale)}>${_patch_text($scope0_id, "c", $global$1.brand)}</p>${_el_resume($scope0_id, "b")}`);
