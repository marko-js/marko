...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	_if(() => input.id ? 0 : 1, $scope0_id, "#text/2", $sg__input_id, $sg__input_id, $sg__input_id, 0, 1, "__tests__/template.marko_0/update_if_#text/2", [() => {
		...
		_html(`<p>${_escape(_hole_value($scope1_id, "PatchHole:#text/0", label, _persisted_reason()))}${_el_resume($scope1_id, "#text/0", $sg__input_id)} / ${_sep($sg__input_id)}${_escape(_hole_value($scope1_id, "PatchHole:#text/1", live(input.id), _persisted_reason()))}${_el_resume($scope1_id, "#text/1", $sg__input_id)}</p>`);
