...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	const Panel = { content: _content("__tests__/template.marko_1_content", () => {
		...
		_html(`<p>${_escape(_hole_value($scope1_id, "UpdateHole:#text/0", input.title, _persisted_reason()))}${_el_resume($scope1_id, "#text/0", $sg__input_title)}</p>`);
	...
	_if(() => input.kind === "native" ? 0 : input.kind === "dynamic" ? 1 : 2, $scope0_id, "#div/2", $sg__input_kind__OR__input_id | $sg__input_kind__OR__input_id__OR__input_tag, $sg__input_kind__OR__input_id__OR__input_tag, _serialize_guard($scope0_reason, 4), "</div>", void 0, "__tests__/template.marko_0/update_if_#div/2", [
		() => {
			...
			_html(`<h2>${_escape(_hole_value($scope2_id, "UpdateHole:#text/0", title, _persisted_reason()))}${_el_resume($scope2_id, "#text/0", $sg__input_id)}</h2>`);
		...
		() => {
			...
			_dynamic_tag($scope3_id, "#text/0", input.tag, {}, _content_resume("__tests__/template.marko_5_content", () => {
				...
				_html(`${_escape(_hole_value($scope5_id, "UpdateHole:#text/0", title, _persisted_reason()))}${_el_resume($scope5_id, "#text/0", $sg__input_id)}`);
	...
	writeScope($scope0_id, {
		input_id: ($si__input_kind || _update_reason()) && input.id,
		input_tag: ($si__input_kind || _update_reason()) && input.tag,
