...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	const MyTag = { content: _content("__tests__/template.marko_1_content", () => {
		...
		_scope_reason();
		_html_opens("__tests__/template.marko:3:3"), _html(`<div>${_escape(x)}${_el_resume($scope1_id, "#text/0")}</div>`);
	...
	_html_opens("__tests__/template.marko:10:1"), _html(`<button>${_escape(x)}${_el_resume($scope0_id, "#text/3")}</button>${_el_resume($scope0_id, "#button/2")}`);
