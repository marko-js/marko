...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	let a = 0;
	let b = 0;
	_html_opens("__tests__/template.marko:3:1"), _html(`<button></button>${_el_resume($scope0_id, "#button/0")}`);
	_try($scope0_id, "#text/1", _content_resume("__tests__/template.marko_1_content", () => {
		const $scope1_id = _scope_id();
		_scope_reason();
		if (true) {
			const $scope2_id = _scope_id();
			_html_opens("__tests__/template.marko:9:5", "__tests__/template.marko:10:5"), _html(`<div>${_escape(a)}${_el_resume($scope2_id, "#text/0")}</div><div>${_escape(b)}${_el_resume($scope2_id, "#text/1")}</div>`);
