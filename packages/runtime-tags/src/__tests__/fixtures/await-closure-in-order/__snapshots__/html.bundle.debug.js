...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	_await($scope0_id, "#text/2", resolveAfter(0, 4), () => {
		...
		_html_opens("__tests__/template.marko:5:3"), _html("<span>Hello</span>");
	}, 0);
	_if(() => {
		if (value) {
			const $scope1_id = _scope_id();
			_html_opens("__tests__/template.marko:7:3"), _html(`<span>${_escape(value)}${_el_resume($scope1_id, "#text/0")}</span>`);
