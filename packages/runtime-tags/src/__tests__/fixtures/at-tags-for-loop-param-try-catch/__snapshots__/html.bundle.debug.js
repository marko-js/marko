...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	forOf(["render"], (label) => {
		$catch = attrTags($catch, { content: _content_resume("__tests__/template.marko_3*content", (err) => {
			...
			_html(`caught ${_text_resume($scope3_id, "#text/0", label, 2)}: ${_text_resume($scope3_id, "#text/1", err.message, _serialize_guard($scope3_reason, 0) * 2)}`);
			_scope($scope3_id, {}, "__tests__/template.marko", "7:8");
		}, $scope0_id, { label }) });
	});
