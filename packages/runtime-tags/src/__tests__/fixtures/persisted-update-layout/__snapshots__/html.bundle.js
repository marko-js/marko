...
var template_default = _template("a", (input) => {
	...
	layout_default({ content: _content("a1", () => {
		...
		_html(`<h1>${_escape(_hole_value($scope1_id, "a", input.title, $sg__input_title))}${_el_resume($scope1_id, "a", $sg__input_title)}</h1><button class=inc>${_escape(count)}${_el_resume($scope1_id, "c")}</button>${_el_resume($scope1_id, "b")}`);
