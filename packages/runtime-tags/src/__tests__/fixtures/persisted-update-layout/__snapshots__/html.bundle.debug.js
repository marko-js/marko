...
var layout_default = _template("__tests__/tags/layout/layout.marko", (input) => {
	...
	writeScope($scope0_id, { open }, "__tests__/tags/layout/layout.marko", 0, { open: "1:6" });
...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	layout_default({ content: _content("__tests__/template.marko_1_content", () => {
		...
		_html(`<h1>${_escape(_hole_value($scope1_id, "#text/0", input.title, $sg__input_title))}${_el_resume($scope1_id, "#text/0", $sg__input_title)}</h1><button class=inc>${_escape(count)}${_el_resume($scope1_id, "#text/2")}</button>${_el_resume($scope1_id, "#button/1")}`);
