...
var layout_default = _template("__tests__/tags/layout/layout.marko", (input) => {
	...
	writeScope($scope0_id, { open: _seed_fill(_state_reason() && open) }, "__tests__/tags/layout/layout.marko", 0, { open: "1:6" });
	_resume_branch($scope0_id);
});
_renderer_shells({
	"__tests__/tags/layout/layout.marko_0_update": ["<header><button> </button></header><main><!></main>", "D D mD%l"],
	"__tests__/tags/layout/layout.marko": ["<header><button> </button></header><main><!></main>", "D D mD%l"]
});
...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	layout_default({ content: _content("__tests__/template.marko_1_content", () => {
		...
		_html(`<h1>${_escape(_hole_value($scope1_id, "PatchHole:#text/0", input.title, _persisted_reason()))}${_el_resume($scope1_id, "#text/0", $sg__input_title)}</h1><button class=inc>${_escape(count)}${_el_resume($scope1_id, "#text/2")}</button>${_el_resume($scope1_id, "#button/1")}`);
