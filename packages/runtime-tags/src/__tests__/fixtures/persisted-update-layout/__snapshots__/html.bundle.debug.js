...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	layout_default({ content: _content("__tests__/template.marko_1_content", () => {
		...
		_html(`<h1>${_escape(_hole_value($scope1_id, "UpdateHole:#text/0", input.title, _persisted_reason()))}${_el_resume($scope1_id, "#text/0", $sg__input_title)}</h1><button class=inc>${_escape(count)}${_el_resume($scope1_id, "#text/2")}</button>${_el_resume($scope1_id, "#button/1")}`);
