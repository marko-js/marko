...
const $for_update = _update_for_keyed("#ul/1", ($p, $l) => $for_content_holes($p, $l));
...
const $await_content__update = (_patch, _live) => {
	...
	if ("BranchScopes:#ul/1" in _patch) $for_update(_live, [_patch["BranchScopes:#ul/1"], "#LoopKey"]);
};
...
const $Reports_content__update = (_patch, _live) => {
	if ("profile_plan" in _patch) _live["profile_plan"] = _patch["profile_plan"];
