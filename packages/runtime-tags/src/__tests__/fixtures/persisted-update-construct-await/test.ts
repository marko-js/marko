...
// Async content inside a constructed hop: frame 1 shows the placeholder
// in its markup and the awaited body arrives as its own boundary-body frame.
export const config: TestConfig = {
  // Replacement-first (the run router's contract): async boundary bodies arrive
  ...
  steps: [
    ...
    clickCount,
    // Cross-route construction: frame 1 = sync fills + placeholder; frame 2 =
    ...
    clickWidget,
    clickToggle,
    // Same-route update, matched: the await re-renders server-side and its
    ...
    clickWidget,
    // Cross-route back: the constructed swap destroys the try/body subtree.
    navigate({
      $global: {
        persisted: true,
        persistedCrossRoute: true,
