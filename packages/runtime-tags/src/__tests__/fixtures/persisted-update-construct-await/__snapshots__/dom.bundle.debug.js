...
_static_shells({
	"__tests__/tags/widget.marko_0_update": [$template$2, $walks$2],
	"__tests__/tags/widget.marko": [$template$2, $walks$2]
});
...
_construct("__tests__/tags/widget.marko_0_update", $construct$2);
const $merge$2 = _resume("__tests__/tags/widget.marko_0_update", $update2$2);
_update_content("__tests__/tags/widget.marko", $merge$2, $construct$2);
...
const $placeholder_content = /*@__PURE__*/ _content("__tests__/template.marko_5_content", "<p class=loading>crunching numbers…</p>");
...
const $Reports_content__profile_plan = /*@__PURE__*/ _const_persisted("profile_plan", $Reports_content__profile_plan__closure);
const $Reports_content__try = /*@__PURE__*/ _try("#text/1", $try_content__template, $try_content__walks, $try_content__setup);
...
const $await_content__construct = ($scope) => {
	_construct_child($scope, "#childScope/0", "__tests__/tags/widget.marko_0_update");
};
...
const $Reports_content__update = ($patch, $live) => {
	...
	if ("BranchScopes:#text/1" in $patch) _update_branch($patch, $live, "#text/1", $try_content__update, "__tests__/template.marko_3_update", "__tests__/template.marko_5_content");
};
...
_construct("__tests__/template.marko_4_update", $await_content__construct);
...
_update_content("__tests__/template.marko_4_update", $await_content__update);
...
const $placeholder_content = _content_resume("__tests__/template.marko_5_content", "<p class=loading>crunching numbers…</p>");
const $await_content__profile_plan = /*@__PURE__*/ _closure_get("profile_plan", ($scope) => {
	if (!updating) {
		$input_label($scope["#childScope/0"], $scope._._.profile_plan);
	}
...
const $await_content__for = /*@__PURE__*/ _for_of("#ul/2", "<li><!>: <!></li>", " D%c%", 0, $for_content__$params);
const $await_content__reports = ($scope, reports) => {
	if (!updating) $await_content__for($scope, [reports]);
};
const $await_content__$params = ($scope, $params2) => $await_content__reports($scope, $params2[0]);
const $await_content = /*@__PURE__*/ _await_content("#text/0", /*@__PURE__*/ ((_w0) => `${_w0}<!><ul class=reports></ul>`)($template$2), /*@__PURE__*/ ((_w0) => `/${_w0}&%b b`)($walks$2), $await_content__setup);
const $try_content__await_promise = /*@__PURE__*/ _await_promise("#text/0", $await_content__$params);
const $try_content__setup = ($scope) => {
	$await_content($scope);
	if (!updating) $try_content__await_promise($scope, getReports($scope.$global.range));
...
const $Reports_content__profile_plan__closure = /*@__PURE__*/ _closure($await_content__profile_plan);
const $Reports_content__profile_plan = /*@__PURE__*/ _const_persisted("profile_plan", $Reports_content__profile_plan__closure);
const $Reports_content__try = /*@__PURE__*/ _try("#text/1", "<!><!><!>", "b%", $try_content__setup);
const $Reports_content__setup = ($scope) => {
	_text($scope["#text/2"], $scope.$global.stamp);
	if (!updating) $Reports_content__profile($scope, getProfile?.($scope.$global.user));
	$Reports_content__try($scope, { placeholder: attrTag({ content: $placeholder_content($scope) }) });
};
const $Reports_content = /*@__PURE__*/ _content("__tests__/template.marko_2_content", "<h2 class=greeting> </h2><!><p class=footer>updated <!></p>", "D l%bDb%", $Reports_content__setup);
const $Home_content = /*@__PURE__*/ _content("__tests__/template.marko_1_content", "<p class=home>welcome home</p>");
