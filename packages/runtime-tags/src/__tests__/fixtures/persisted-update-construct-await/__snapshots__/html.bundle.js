	...
	const Reports = { content: _content("a9", () => {
		...
		_try($scope2_id, "b", _content_resume("a7", () => {
			...
			_await($scope3_id, "a", getReports($global().range), (reports) => {
				...
				}, 0, $scope4_id, "b", _persisted_reason(), _persisted_reason(), _persisted_reason(), "</ul>", 1, "a3");
...
_renderer_shells({
	"a3": ["<li><!>: <!></li>", " D%c%l"],
	"a11": ["<li><!>: <!></li>", " D%c%l"],
	"a12": ["<p class=loading>crunching numbers…</p>", "b"],
	"a6": ["<p class=loading>crunching numbers…</p>", "b"],
	"a5": [[["c"], "<ul class=reports></ul>"], [
		"/",
		["c"],
		"& b"
	]],
	"a13": [[["c"], "<ul class=reports></ul>"], [
		"/",
		["c"],
		"& b"
	]],
	"a8": ["<!><!><!>", "b%c"],
	"a7": ["<!><!><!>", "b%c"],
	"a14": ["<h2 class=greeting> </h2><!><p class=footer>updated <!></p>", "D l%bDb%l"],
