...
## Console
```
ERROR "navigate() document fallback: Error: A persisted update depends on a lazy module that failed to load, so the navigation cannot complete client-side: Error: simulated chunk load failure: ./v:panel.marko.setup.mjs"
```
