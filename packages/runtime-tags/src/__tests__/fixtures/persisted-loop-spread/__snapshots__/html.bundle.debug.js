...
const $walks = " b D l";
_shells({ "__tests__/template.marko_1*shell": "__tests__/template.marko_1*shell !__tests__/template.marko_1_item_attrs#4; D ;<li> </li>" });
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	_for_of(input.items, (item) => {
		...
		_html(`<li${_patch_attrs(item.attrs, "#li/0", $scope1_id, "li", $scope0_owned, 0)}>${_patch_text($scope1_id, "#text/1", item.id, $scope0_owned, 0)}${_el_resume($scope1_id, "#text/1")}</li>${_el_resume($scope1_id, "#li/0")}`);
		_script($scope1_id, "__tests__/template.marko_1_item_attrs#4");
		writeScope($scope1_id, {}, "__tests__/template.marko", "3:4", { "EventAttributes:#li/0": ["...item.attrs", "4:12"] });
