...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	_html(`<fieldset><input${_attr_input_checkedValue($scope0_id, "#input/0", input.picked, _resume(function(next) {
		...
		note = next;
	}, "__tests__/template.marko_0/checkedValueChange", $scope0_id), 0, 0)}${_patch_control($scope0_id, "#input/0", 1, [input.picked, "a"], $scope0_reason, 0)} type=radio class=a>${_el_resume($scope0_id, "#input/0")}<input${_attr_input_checkedValue($scope0_id, "#input/1", input.picked, _resume(function(next) {
		note = next;
		...
		note = next;
	}, "__tests__/template.marko_0/checkedValueChange2", $scope0_id), 0, 0)}${_patch_control($scope0_id, "#input/1", 1, [input.picked, "b"], $scope0_reason, 0)} type=radio class=b>${_el_resume($scope0_id, "#input/1")}</fieldset><p>${_text_resume($scope0_id, "#text/2", note)}</p>`);
