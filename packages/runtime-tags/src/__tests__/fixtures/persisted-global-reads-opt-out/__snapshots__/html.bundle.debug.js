...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	_if(() => $global().params.sale ? 0 : undefined, $scope0_id, "#section/4", _persisted_reason(), 1 | _persisted_reason(), _persisted_reason(), "</section>", 1, "__tests__/template.marko_0/update_if_#section/4", [() => {
		...
		_html(`<em>Sale ${_sep(_persisted_reason())}${_escape(_hole_value($scope1_id, "PatchHole:#text/0", $global().params.sale, _persisted_reason()))}${_el_resume($scope1_id, "#text/0", _persisted_reason())}% off</em>`);
...
_renderer_shells({
	"__tests__/template.marko_1_update": ["<em>Sale <!>% off</em>", "Db%l"],
	"__tests__/template.marko_1_content": ["<em>Sale <!>% off</em>", "Db%l"],
