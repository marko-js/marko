...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	_await($scope0_id, "#text/2", resolveAfter(`R${n}`), (result) => {
		...
		_html(`<span>#<!>${_escape(n)}${_el_resume($scope1_id, "#text/0")}</span>`);
		_if(() => {
			if (n % 2) {
				const $scope2_id = _scope_id();
				_html(`<em>odd <!>${_escape(n)}${_el_resume($scope2_id, "#text/0")}</em>`);
				_subscribe($n__closures, writeScope($scope2_id, { "ClosureSignalIndex:n": 1 }, "__tests__/template.marko", "8:6"));
				return 0;
			}
		}, $scope1_id, "#text/1", 1, 1, 1, 0, 1);
		_html(`<p>${_escape(result)}${_el_resume($scope1_id, "#text/2")}</p>`);
