...
var template_default = _template("a", (input) => {
	...
	_await($scope0_id, "c", resolveAfter(`R${n}`), (result) => {
		...
		_html(`<span>#<!>${_escape(n)}${_el_resume($scope1_id, "a")}</span>`);
		_if(() => {
			if (n % 2) {
				const $scope2_id = _scope_id();
				_html(`<em>odd <!>${_escape(n)}${_el_resume($scope2_id, "a")}</em>`);
				_subscribe($n__closures, writeScope($scope2_id, { Ce: 1 }));
				return 0;
			}
		}, $scope1_id, "b", 1, 1, 1, 0, 1);
		_html(`<p>${_escape(result)}${_el_resume($scope1_id, "c")}</p>`);
