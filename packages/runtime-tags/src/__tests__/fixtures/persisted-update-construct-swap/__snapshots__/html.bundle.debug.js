...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	const PanelA = { content: _content("__tests__/template.marko_1_content", () => {
		...
		_html(`<p class=a>Panel A: ${_sep(_persisted_reason())}${_escape(getLabel?.($global().topic))}${_el_resume($scope1_id, "#text/0", _persisted_reason())}</p><span class=a2>alpha detail</span><em class=a3>${_escape(getMotto?.())}</em>`);
	...
	const PanelB = { content: _content("__tests__/template.marko_2_content", () => {
		...
		_html(`<section class=b>Panel B: ${_sep(_persisted_reason())}${_escape(getLabel?.($global().topic))}${_el_resume($scope2_id, "#text/0", _persisted_reason())}</section><span class=b2>beta detail</span>`);
