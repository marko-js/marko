...
# Update
```js
_assert.default.equal(document.querySelector(selector)?.textContent, text);
_assert.default.equal(document.querySelector("button.count").textContent, `clicked ${count}`);
```
...
# Update
```js
_assert.default.equal(document.querySelector("em.a3").textContent, "always in stock");
```
