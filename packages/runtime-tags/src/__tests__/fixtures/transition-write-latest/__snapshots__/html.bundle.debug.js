...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	let id = 1;
	_html(`<button id=inc>${_escape(id)}${_el_resume($scope0_id, "#text/1")}</button>${_el_resume($scope0_id, "#button/0")}<button id=back>back</button>${_el_resume($scope0_id, "#button/2")}`);
	_try($scope0_id, "#text/3", _content_resume("__tests__/template.marko_1_content", () => {
		const $scope1_id = _scope_id();
		_scope_reason();
		_await($scope1_id, "#text/0", getData(id), (data) => {
		...
		_subscribe($id__closures, writeScope($scope1_id, { _: _scope_with_id($scope0_id) }, "__tests__/template.marko", "5:2"));
