...
// A keyed loop owned by an `<if>` body: its possession site must be reserved
...
export const config: TestConfig = {
  ...
  steps: [
    ...
    clickCount,
    // Adds a new head key (fragment), reorders the two live keys, and
    // updates their etas (fills into the moved branches).
    navigate({
      $global: { persisted: true, lane: "active", day: "tue" },
    }),
    clickCount,
    // Removes the fragment-built key; the survivors reorder and refill.
    navigate({
