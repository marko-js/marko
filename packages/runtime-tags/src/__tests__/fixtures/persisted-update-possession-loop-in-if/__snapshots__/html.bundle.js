...
var template_default = _template("a", (input) => {
	...
	_if(() => $global().lane === "active" ? 0 : 1, $scope0_id, "c", _persisted_reason(), _persisted_reason(), _persisted_reason(), 0, 1, "a1", [() => {
		const $scope1_id = _scope_id();
		_html("<ol class=lane>");
		_for_of(getTasks?.($global().day), (task) => {
			const $scope3_id = _scope_id();
			_html(`<li><em>TASK_ROW_MARKUP</em>${_escape(serverTaskSentinel?.())}<span class=name>${_escape(_hole_value($scope3_id, "Qb", task.name, _persisted_reason()))}${_el_resume($scope3_id, "b", _persisted_reason())}</span>: <span class=eta>${_escape(_hole_value($scope3_id, "Qc", task.eta, _persisted_reason()))}${_el_resume($scope3_id, "c", _persisted_reason())}</span></li>`);
			_persisted_reason() && writeScope($scope3_id, {});
		}, function(task) {
			return task.id;
		}, $scope1_id, "a", _persisted_reason(), _persisted_reason(), _persisted_reason(), "</ol>", 1, "a0");
