...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	const row = { content: _content_elide("__tests__/template.marko_1*content", (label) => {
		...
		_subscribe(_unfilled_if($scope0_reason, 1) && $input_suffix__closures, _scope($scope1_id, { _: _scope_with_id($scope0_id) }, "__tests__/template.marko", "1:2"));
	}, $scope0_id) };
	_html("<ul>");
	_if(() => {
		if (input.show) {
	...
	$scope0_page && _scope($scope0_id, { "ClosureScopes:input_suffix": $input_suffix__closures }, "__tests__/template.marko", 0);
}, 1, 1);
