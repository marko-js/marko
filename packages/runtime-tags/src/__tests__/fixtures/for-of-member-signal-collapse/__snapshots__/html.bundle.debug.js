...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	_html_opens("__tests__/template.marko:1:1"), _html("<ul>");
	_for_of(input.users, (user) => {
		const $scope1_id = _scope_id();
		_html_opens("__tests__/template.marko:3:5"), _html(`<li>${_escape(user.name)}${_el_resume($scope1_id, "#text/0", $sg__input_users)} (${_sep($sg__input_users)}${_escape(user.role)}${_el_resume($scope1_id, "#text/1", $sg__input_users)})</li>`);
