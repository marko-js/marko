...
_shells({
	...
	"__tests__/template.marko_1*shell": "__tests__/template.marko_1*shell __tests__/template.marko_1_count#6/init;D ;<p> </p>"
});
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	_if(() => {
		if (input.show) {
			...
			_filled_guard($scope0_reason, 1) ? _patch_value($scope1_id, "__tests__/template.marko0", label) : _patch_init($scope1_id, "__tests__/template.marko_1_input_title#5/init");
			_html(`<p>${_text_resume($scope1_id, "#text/0", label + " #" + count)}</p>`);
			_scope($scope1_id, {
				label,
				_: _scope_with_id($scope0_id)
			}, "__tests__/template.marko", "3:4", { label: "4:12" });
			return 0;
		}
