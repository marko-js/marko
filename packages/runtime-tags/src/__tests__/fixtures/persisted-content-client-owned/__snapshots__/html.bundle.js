		...
		if (open) {
			...
			card_default({
				...
				content: _content_elide("a0", () => {
					...
					_html(`<em>${_escape(input.note)}${_el_resume($scope2_id, "a")}</em>`);
					_subscribe(_source_if($scope0_reason, 1) && $input_note__closures, writeScope($scope2_id, { _: _scope_with_id($scope1_id) }));
