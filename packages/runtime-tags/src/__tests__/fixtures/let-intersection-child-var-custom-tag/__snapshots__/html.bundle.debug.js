...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	let b = a + 1;
	_html_opens("__tests__/template.marko:4:1", "__tests__/template.marko:5:1", "__tests__/template.marko:7:1"), _html(`<div>${_escape(a)}${_el_resume($scope0_id, "#text/2")}</div><div>${_escape(b)}${_el_resume($scope0_id, "#text/3")}</div><button>${_escape(`${a}`)},${_escape(`${b}`)}${_el_resume($scope0_id, "#text/5")}</button>${_el_resume($scope0_id, "#button/4")}`);
