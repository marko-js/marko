...
var template_default = _template("a", (input) => {
	...
	let count = 0;
	const Child = { content: _content_resume("a6", () => {
	...
	_dynamic_tag($scope0_id, "c", input.outer, { class: "outer" }, _content_resume("a4", () => {
		const $scope1_id = _scope_id();
		_scope_reason();
		_dynamic_tag($scope1_id, "a", input.child === "component" ? Child : input.child === "native" ? "span" : void 0, {}, 0, 0, $sg__input_child | _persisted_reason(), "a3");
		_subscribe($sg__input_child && $input_child__closures, writeScope($scope1_id, { _: _scope_with_id($scope0_id) }));
		_resume_branch($scope1_id);
	}, $scope0_id), 0, _serialize_guard($scope0_reason, 0) | _persisted_reason(), "a1");
