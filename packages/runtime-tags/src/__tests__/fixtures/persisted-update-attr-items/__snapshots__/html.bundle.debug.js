...
var chip_list_default = _template("__tests__/tags/chip-list.marko", (input) => {
	...
	_for_of(categories, (cat) => {
		...
		_html(`<span${_attr_class(_hole_value($scope1_id, "UpdateAttr:class:#span/0", ["chip", { "chip--active": search.category === cat }], _persisted_reason()))}>${_escape(_hole_value($scope1_id, "UpdateHole:#text/1", cat, _persisted_reason()))}${_el_resume($scope1_id, "#text/1", _persisted_reason())}</span>${_el_resume($scope1_id, "#span/0", _persisted_reason())}`);
