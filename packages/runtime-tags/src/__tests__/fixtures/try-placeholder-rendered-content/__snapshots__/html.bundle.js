	...
	_try($scope0_id, "a", _content_resume("c3", () => {
		...
		_if(() => {
			if (show) {
				...
				_subscribe($si__rest && $rest__closures, _scope($scope2_id, { _: $si__input_show__OR__rest && _scope_with_id($scope1_id) }), "c1");
				return 0;
			}
		}, $scope1_id, "a", $sg__input_show, $sg__input_show, $sg__input_show, 0, 1);
		$si__input_show__OR__rest && _subscribe($si__input_show && $show__closures, _scope($scope1_id, { _: _scope_with_id($scope0_id) }), "c4", $sg__input_show);
...
var template_default = _template("a", (input) => {
	...
	const Content = { content: _content_resume("a0", () => {
		const $scope2_id = _scope_id();
		_scope_reason();
		_await($scope2_id, "a", resolveAfter("defined"), (v) => {
			_scope_id();
			_html(_escape(v));
		}, 0);
	}, $scope0_id) };
