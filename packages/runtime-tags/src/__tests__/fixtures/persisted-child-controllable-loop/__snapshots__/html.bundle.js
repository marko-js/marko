...
_shells({
	...
	a1: /*@__PURE__*/ ((_w0, _w1) => `a1 !;${_w0};${_w1}`)(/*@__PURE__*/ ((_w0) => `D%c%l/${_w0}&b`)("b%c"), /*@__PURE__*/ ((_w0) => `<p><!> hit <!></p>${_w0}<!>`)($template))
});
var template_default = _template_persisted("a", (input) => {
	...
	_for_of(["a", "b"], (name) => {
		const $scope1_id = _scope_id();
		let hits = 0;
		_html(`<p>${_patch_text($scope1_id, "a", name)} hit ${_text_resume($scope1_id, "b", hits, 2)}</p>`);
	...
	}, 0, $scope0_id, "b", 1, _source_guard($scope0_reason, 1), 0, void 0, void 0, "a1");
	_html("</main>");
