...
const $Reports_content__await_promise = /*@__PURE__*/ _await_promise("#text/0", $await_content__$params);
const $Reports_content__setup = ($scope) => {
	$await_content($scope);
	if (!updating) $Reports_content__await_promise($scope, getReport($scope.$global.range));
};
