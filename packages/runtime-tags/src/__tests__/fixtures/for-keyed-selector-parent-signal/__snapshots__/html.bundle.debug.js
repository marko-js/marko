...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	_for_of(rows, (row) => {
		...
		_html(`<tr${_attr_class(enabled && selected === row.id && "danger")}><td><button class=select>${_escape(row.label)}</button>${_el_resume($scope1_id, "#button/1")}</td></tr>${_el_resume($scope1_id, "#tr/0")}`);
