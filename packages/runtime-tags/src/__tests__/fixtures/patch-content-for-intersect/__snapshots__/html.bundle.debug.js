...
var template_default = _template_patch("__tests__/template.marko", (input) => {
	const $scope0_reason = _scope_reason(), $sg__input_title = _source_guard($scope0_reason, 0), $si__input_title = _source_if($scope0_reason, 0), $scope0_page = _page_render();
	const $scope0_id = _scope_id();
	...
	if ($scope0_page) _if(() => {
		if (open) {
			...
			box_default({ content: _content_elide("__tests__/template.marko_2*content", () => {
				...
				if ($scope0_page) forOf(["p", "q"], (item) => {
					const $scope3_id = _scope_id();
					_html(`<span>${_text_resume($scope3_id, "#text/0", input.title + ":" + item, $sg__input_title)}</span>`);
