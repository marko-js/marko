...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	let count = 0;
	_dynamic_tag($scope0_id, "#text/0", false || child_default, {}, _content_resume("__tests__/template.marko_1*content", () => {
		const $scope1_id = _scope_id();
		_scope_reason();
		_html(`<button>${_text_resume($scope1_id, "#text/1", count)}</button>${_el_resume($scope1_id, "#button/0")}`);
		_script($scope1_id, "__tests__/template.marko_1");
		_subscribe($count__closures, _scope($scope1_id, { _: _scope_with_id($scope0_id) }, "__tests__/template.marko", "4:4"), "__tests__/template.marko_1_count#1/subscribe");
	}, $scope0_id), 0, 0);
	_scope($scope0_id, {
		count,
