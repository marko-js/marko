	...
	_if$1(() => {
		if (input.show) {
			...
			widget_default({ content: _content_elide("__tests__/template.marko_2*content", () => {
				...
				_html$1(`<em>${_patch_text($scope2_id, "#text/0", input.text, $scope0_owned, 2)}${_el_resume($scope2_id, "#text/0")}</em>`);
				_subscribe(_source_if($scope0_reason, 2) && $input_text__closures, writeScope($scope2_id, { _: _scope_with_id($scope1_id) }, "__tests__/template.marko", "3:6"));
