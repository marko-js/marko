	...
	_if(() => {
		if (input.show) {
			...
			widget_default({ content: _content_elide("a0", () => {
				...
				_html(`<em>${_patch_text($scope2_id, "a", input.text, void 0, $scope0_owned, 2)}</em>`);
				_subscribe(_unfilled_if($scope0_owned, 2) && $input_text__closures, _scope($scope2_id, { _: _scope_with_id($scope1_id) }));
			}, $scope1_id) });
			_scope($scope1_id, {
