...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	_for_of(categories, (cat) => {
		...
		_html(`<span${_attr_class(_hole_value($scope2_id, "UpdateAttr:class:#span/0", cat === $global().params.pick && "active", _persisted_reason()))}>${_escape(cat)}</span>${_el_resume($scope2_id, "#span/0", _persisted_reason())}`);
	...
	_if(() => {
		if (categories.length) {
			...
			_html(`<em${count && $global().params.tag ? " class=hot" : ""}>pick:${_sep(_persisted_reason())}${_escape(_hole_value($scope1_id, "#text/1", $global().params.pick, _persisted_reason()))}${_el_resume($scope1_id, "#text/1", _persisted_reason())}</em>${_el_resume($scope1_id, "#em/0", 1 | _persisted_reason())}`);
