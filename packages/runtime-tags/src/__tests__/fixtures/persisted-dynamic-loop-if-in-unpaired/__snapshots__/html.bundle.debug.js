			...
			_if(() => {
				if (input.on) {
					...
					_for_of(input.items, (x) => {
						...
						_html(`<li>${_patch_text($scope3_id, "#text/0", x, void 0, $scope0_owned, 3)}:${_patch_text($scope3_id, "#text/1", input.label, 2, $scope0_owned, 4)}</li>`);
						_subscribe(_unfilled_if($scope0_owned, 4) && $input_label__closures, _scope($scope3_id, { _: _scope_with_id($scope2_id) }, "__tests__/template.marko", "5:21"));
					}, 0, $scope2_id, "#ul/0", 1, $sg__input_items, $sg__input_items, void 0, void 0, "__tests__/template.marko_3*shell", $scope0_owned, 3);
					_html(`</ul>${_el_resume($scope2_id, "#ul/0", $sg__input_items)}`);
					$scope0_reason && _subscribe(_unfilled_if($scope0_owned, 3) && $input_items__closures, _scope($scope2_id, { _: _scope_with_id($scope1_id) }, "__tests__/template.marko", "5:4"));
					return 0;
				}
			}, $scope1_id, "#text/0", 1, $sg__input_on, $sg__input_on, void 0, void 0, ["__tests__/template.marko_2*shell"], $scope0_owned, 2);
			$scope0_reason && _subscribe(_unfilled_if($scope0_owned, 2) && $input_on__closures, _scope($scope1_id, { _: _scope_with_id($scope0_id) }, "__tests__/template.marko", "3:4"));
