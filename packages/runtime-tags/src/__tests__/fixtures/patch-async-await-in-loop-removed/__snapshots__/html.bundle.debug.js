...
var rows_default = _template_patch("__tests__/tags/rows.marko", (input) => {
	const $scope0_reason = _scope_reason(), $scope0_page = _page_render(), $sg__input_items = _source_guard($scope0_reason, 1);
	...
	_for_of(input.items, (item) => {
		...
		_await($scope1_id, "#text/0", input.promise, (v) => {
			...
			_subscribe(_unfilled_if($scope0_reason, 1) && $for_content__item_id__closures, _scope($scope2_id, { _: _scope_with_id($scope1_id) }, "__tests__/tags/rows.marko", "3:6"));
		}, 1, "__tests__/tags/rows.marko_2*content");
		_html("</div>");
		$scope0_page && _scope($scope1_id, {
			item_id: _source_if($scope0_reason, 2) && item?.id,
			_: _scope_with_id($scope0_id),
			"ClosureScopes:item_id": $for_content__item_id__closures
		}, "__tests__/tags/rows.marko", "1:2", { item_id: ["item.id", "1:6"] });
	}, "id", $scope0_id, "#text/0", 1, _source_guard($scope0_reason, 0), $sg__input_items, void 0, void 0, "__tests__/tags/rows.marko_1*shell", $scope0_reason, 1);
	$scope0_page ? _scope($scope0_id, { input_promise: input.promise }, "__tests__/tags/rows.marko", 0, { input_promise: ["input.promise"] }) : _filled_guard($scope0_reason, 2) && _client_guard($scope0_reason, 1) && _patch_value($scope0_id, "__tests__/tags/rows.marko0", input.promise);
}, 0, 0);
