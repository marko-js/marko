...
var template_default = _template_persisted("c", (input) => {
	...
	_dynamic_tag$1($scope0_id, "a", input.mode === "a" ? box_a_default : box_b_default, {}, _content_elide("c0", () => {
		const $scope1_id = _scope_id();
		_persisted_reason();
		_html(`${_patch_text($scope1_id, "a", input.text, $scope0_owned, 1)}${_el_resume($scope1_id, "a")}`);
		_subscribe(_source_if($scope0_reason, 1) && $input_text__closures, writeScope($scope1_id, { _: _scope_with_id($scope0_id) }));
