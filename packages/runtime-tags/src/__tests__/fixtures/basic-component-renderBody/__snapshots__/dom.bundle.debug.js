// tags/my-button.marko
const $template$1 = "<button><!></button>";
const $walks$1 = " D%l";
const $setup$1 = () => {};
const $content_direct = /*@__PURE__*/ _dynamic_tag_content("#text/1");
const $onClick__script = _script("__tests__/tags/my-button.marko_0_onClick#4", ($scope) => _on($scope["#button/0"], "click", $scope.onClick));
const $onClick$1 = /*@__PURE__*/ _const("onClick", $onClick__script);
const $dynamicTag = /*@__PURE__*/ _dynamic_tag("#text/1");
const $content = $dynamicTag;
const $input = ($scope, input) => {
	$onClick$1($scope, input.onClick);
	$content($scope, input.content);
};
var my_button_default = /*@__PURE__*/ _template("__tests__/tags/my-button.marko", $template$1, $walks$1, 0, $input);

// template.marko
const $template = $template$1;
const $walks = /*@__PURE__*/ ((_w0) => `/${_w0}&`)($walks$1);
const $mybutton_content__clickCount = /*@__PURE__*/ _closure_get("clickCount/2", ($scope) => _text($scope["#text/0"], $scope._.clickCount), 0, "__tests__/template.marko_1_clickCount#1/subscribe");
const $mybutton_content__setup = $mybutton_content__clickCount;
const $mybutton_content = /*@__PURE__*/ _content("__tests__/template.marko_1*content", " ", " ", $mybutton_content__setup);
const $clickCount__closure = /*@__PURE__*/ _closure($mybutton_content__clickCount);
const $clickCount = /*@__PURE__*/ _let("clickCount/1", $clickCount__closure);
function $setup($scope) {
	$content_direct($scope["#childScope/0"], $mybutton_content($scope));
	$onClick$1($scope["#childScope/0"], $onClick($scope));
	$clickCount($scope, 0);
}
const $onClick = ($scope) => function() {
	$clickCount($scope, +$scope.clickCount + 1);
};
_resumed["__tests__/template.marko_0/onClick"] = $onClick;
var template_default = /*@__PURE__*/ _template("__tests__/template.marko", $template, $walks, $setup);
