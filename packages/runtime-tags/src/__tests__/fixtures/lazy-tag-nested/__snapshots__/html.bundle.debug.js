...
var grand_child_default = _template("__tests__/grand-child.marko", (input) => {
	...
	_html_opens("__tests__/grand-child.marko:1:1"), _html(`<span>${_escape(input.value)}${_el_resume($scope0_id, "#text/0", _serialize_guard($scope0_reason, 0))}</span>`);
	_script($scope0_id, "__tests__/grand-child.marko_0");
	_serialize_if($scope0_reason, 0) && writeScope($scope0_id, {}, "__tests__/grand-child.marko", 0);
});
