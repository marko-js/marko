...
var template_default = _template("a", (input) => {
	...
	_if(() => input.missing ? 0 : 1, $scope0_id, "d", $sg__input_title__OR__input_missing | $sg__input_missing, $sg__input_title__OR__input_missing, $sg__input_missing, void 0, void 0, "a0", [() => {
		...
		_if(() => input.title.length ? 0 : 1, $scope1_id, "a", $sg__input_title, $sg__input_title, _serialize_guard($scope0_reason, 4), void 0, void 0, "a1", [() => {
			...
			_html(` 5> <h2>long ${_sep($sg__input_title)}${_escape(input.title)}${_el_resume($scope2_id, "a", $sg__input_title)}</h2>`);
			...
			_html(`<h2>short ${_sep($sg__input_title)}${_escape(input.title)}${_el_resume($scope3_id, "a", $sg__input_title)}</h2>`);
		...
		_html("<section>");
		_try($scope1_id, "b", _content_resume("a8", () => {
			const $scope4_id = _scope_id();
			_scope_reason();
			_await($scope4_id, "a", resolveAfter(0, 1), () => {
				const $scope5_id = _scope_id();
				$sg__input_title__OR__input_missing && _script($scope5_id, "a5");
			...
			}, $sg__input_title__OR__input_missing, "a6");
			$sg__input_title__OR__input_missing && writeScope($scope4_id, { _: _scope_with_id($scope1_id) });
		}, $scope1_id), { placeholder: attrTag({ content: _content_resume("a7", () => {
			_scope_reason();
			_scope_id();
	...
	writeScope($scope0_id, {
		...
		j: _seed_fill(_state_reason() && count),
...
_renderer_shells({
	"a11": ["<p>gone</p>", "b"],
	"a14": ["<p>gone</p>", "b"],
	"a6": ["<p>extras for <!></p>", "Db%l"],
	"a15": ["<p>extras for <!></p>", "Db%l"],
	"a9": ["<!><!><!>", "b%c"],
	"a8": ["<!><!><!>", "b%c"],
	"a10": ["<!><!><section><!></section>", "b%bD%l"],
	"a16": ["<!><!><section><!></section>", "b%bD%l"],
