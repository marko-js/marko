...
var template_default = _template("a", (input) => {
	...
	_if(() => input.missing ? 0 : 1, $scope0_id, "d", $sg__input_title__OR__input_missing | $sg__input_missing, $sg__input_title__OR__input_missing, $sg__input_missing, void 0, void 0, "a0", [() => {
		...
		_html("<section>");
		_try($scope1_id, "b", _content_resume("a7", () => {
			const $scope4_id = _scope_id();
			_scope_reason();
			_await($scope4_id, "a", resolveAfter(0, 1), () => {
				const $scope5_id = _scope_id();
				$sg__input_title__OR__input_missing && _script($scope5_id, "a4");
			...
			}, $sg__input_title__OR__input_missing, "a5");
			$sg__input_title__OR__input_missing && writeScope($scope4_id, { _: _scope_with_id($scope1_id) });
		}, $scope1_id), { placeholder: attrTag({ content: _content_resume("a6", () => {
			_scope_reason();
			_scope_id();
...
_renderer_shells({
	"a10": ["<p>gone</p>", "b"],
	"a12": ["<p>gone</p>", "b"],
	"a5": ["<p>extras for <!></p>", "Db%l"],
	"a13": ["<p>extras for <!></p>", "Db%l"],
	"a8": ["<!><!><!>", "b%c"],
	"a7": ["<!><!><!>", "b%c"],
	"a9": ["<!><!><section><!></section>", "b%bD%l"],
	"a14": ["<!><!><section><!></section>", "b%bD%l"],
