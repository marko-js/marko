...
var template_default = _template("a", (input) => {
	...
	_if(() => $global().show ? 0 : 1, $scope0_id, "c", _persisted_reason(), _persisted_reason(), _persisted_reason(), void 0, void 0, "a0", [() => {
		const $scope1_id = _scope_id();
		let pair = `${$global().info.a}-${$global().info.b}`;
		_html(`<p class=pair>${_escape(pair)}${_el_resume($scope1_id, "a")}</p><button class=bump>bump</button>${_el_resume($scope1_id, "b")}<p class=combo>${_escape(_hole_value($scope1_id, "Qc", settings.prefix, _persisted_reason()))}${_el_resume($scope1_id, "c", _persisted_reason())}:${_sep(_persisted_reason())}${_escape(_hole_value($scope1_id, "Qd", $global().info.a, _persisted_reason()))}${_el_resume($scope1_id, "d", _persisted_reason())}</p>`);
		_script($scope1_id, "a2");
		writeScope($scope1_id, {
			e: _seed_fill(_state_reason() && pair),
...
_renderer_shells({
	"a3": ["<p class=empty>hidden</p>", "b"],
	"a7": ["<p class=empty>hidden</p>", "b"],
	"a4": ["<p class=pair> </p><button class=bump>bump</button><p class=combo><!>:<!></p>", "D l bD%c%l"],
	"a8": ["<p class=pair> </p><button class=bump>bump</button><p class=combo><!>:<!></p>", "D l bD%c%l"],
