...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	_for_of(input.children, (child) => {
		...
		_html(`<span>${_escape(child.text)}${_el_resume($scope1_id, "#text/0", $sg__input_children)}</span>`);
		$si__input_children && writeScope($scope1_id, {}, "__tests__/template.marko", "2:4");
	}, "id", $scope0_id, "#div/0", $sg__input_children, $sg__input_children, $sg__input_children, "</div>", 1, 1);
