...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	_for_of(input.items, (item) => {
		...
		widget_default({ content: _content_elide("__tests__/template.marko_2*content", () => {
			...
			_html(`<b>${_patch_text($scope2_id, "#text/0", item.text, void 0, $scope0_reason, 0)}</b>`);
			_subscribe(_unfilled_if($scope0_reason, 0) && $for_content__item_text__closures, _scope($scope2_id, { _: _scope_with_id($scope1_id) }, "__tests__/template.marko", "3:10"));
		}, $scope1_id) });
		_html("</li>");
		_scope($scope1_id, {
