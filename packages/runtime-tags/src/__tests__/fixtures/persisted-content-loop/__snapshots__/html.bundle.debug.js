...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	_for_of$1(input.items, (item) => {
		...
		widget_default({ content: _content_elide("__tests__/template.marko_2*content", () => {
			...
			_html$1(`<b>${_patch_text($scope2_id, "#text/0", item.text, $scope0_owned, 0)}${_el_resume($scope2_id, "#text/0")}</b>`);
			_subscribe($scope0_reason && $for_content__item_text__closures, writeScope($scope2_id, { _: _scope_with_id($scope1_id) }, "__tests__/template.marko", "3:10"));
