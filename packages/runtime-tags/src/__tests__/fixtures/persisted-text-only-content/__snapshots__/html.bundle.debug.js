...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	let x = 1;
	_html(`<script${_attr_nonce()}>${_escape_script(`window.log = [${_to_text(x)}, "${_to_text(input.title)}"]`)}<\/script>${_el_resume($scope0_id, "#script/0")}<style${_attr_nonce()}>${_patch_text_content($scope0_id, "#style/1", `.a { color: ${_to_text(input.color)} }`, _escape_style, $scope0_reason, 1)}</style>${_el_resume($scope0_id, "#style/1")}<title>${_patch_text_content($scope0_id, "#title/2", `${_to_text(input.title)} | site`, _escape, $scope0_reason, 0)}</title>${_el_resume($scope0_id, "#title/2")}<!--${_patch_text_content($scope0_id, "#comment/3", input.title, _escape_comment, $scope0_reason, 0) || " "}-->${_el_resume($scope0_id, "#comment/3")}<button>+</button>${_el_resume($scope0_id, "#button/4")}`);
