...
var card_default = _template_patch("b", (input) => {
	...
	}, $scope0_id), { placeholder: attrTag({ content: _content_resume("b4", () => {
		_scope_reason();
...
var template_default = _template_patch("a", (input) => {
	...
	card_default({
		...
		content: _content_resume("a1", () => {
			...
			_subscribe(_unfilled_if($scope0_reason, 1) && $input_note__closures, _scope($scope1_id, { _: _scope_with_id($scope0_id) }), _client_guard($scope0_reason, 1) && "a0");
		}, $scope0_id)
	});
	_html("</main>");
	$scope0_page && _scope($scope0_id, {
