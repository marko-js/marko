...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	const $scope0_reason = _scope_reason(), $sg__input_show = _source_guard($scope0_reason, 3), $scope0_page = _page_render();
	const $scope0_id = _scope_id();
	let count = 0;
	_html(`<main><a${_patch_attrs({
		href: "/static",
		...input.attrs
	}, "#a/0", $scope0_id, "a", void 0, $scope0_reason, 1)}>${_patch_text($scope0_id, "#text/1", input.label, void 0, $scope0_reason, 2)}</a>${_el_resume($scope0_id, "#a/0")}`);
	_if(() => {
		if (input.show) {
			...
			_html(`<img${_patch_attrs({
				alt: "hero",
				...input.img
			}, "#img/0", $scope1_id, "img", void 0, $scope0_reason, 4)}>${_el_resume($scope1_id, "#img/0")}`);
	...
	}, $scope0_id, "#text/2", 1, $sg__input_show, $sg__input_show, void 0, void 0, ["__tests__/template.marko_1*shell"], $scope0_reason, 3);
