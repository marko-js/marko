...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	const Dashboard = { content: _content("__tests__/template.marko_2_content", () => {
		...
		_html(`<h2 class=greeting>${_escape(_hole_value($scope2_id, "UpdateHole:#text/0", session.greeting, _persisted_reason()))}${_el_resume($scope2_id, "#text/0", _persisted_reason())}</h2>`);
		...
		_for_of(getMetrics?.($global().range), (metric) => {
			...
			_html(`<li${_attr_class(_hole_value($scope3_id, "UpdateAttr:class:#li/0", metric.name === $global().focus && "focus", _persisted_reason()))}>${_escape(_hole_value($scope3_id, "UpdateHole:#text/1", metric.name, _persisted_reason()))}${_el_resume($scope3_id, "#text/1", _persisted_reason())}: ${_sep(_persisted_reason())}${_escape(_hole_value($scope3_id, "UpdateHole:#text/2", metric.value, _persisted_reason()))}${_el_resume($scope3_id, "#text/2", _persisted_reason())}</li>${_el_resume($scope3_id, "#li/0", _persisted_reason())}`);
