...
var template_default = _template("a", (input) => {
	...
	_if(() => results.total ? 0 : 1, $scope0_id, "c", _persisted_reason(), _persisted_reason(), _persisted_reason(), void 0, void 0, "a0", [() => {
		const $scope1_id = _scope_id();
		_html("<ul class=items>");
		_for_of(results.items, (item) => {
			const $scope5_id = _scope_id();
			_html(`<li>${_escape(item.name)}${_el_resume($scope5_id, "a", _persisted_reason())}</li>`);
		...
		_for_to(results.totalPages, 1, 1, (page) => {
			...
			_if(() => {
				if (page === search.page) {
					...
					_html(`<a${_attr("href", `/search?page=${page}&q=${search.q}`)}>${_escape(page)}</a>${_el_resume($scope3_id, "a", _persisted_reason())}`);
