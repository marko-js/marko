...
const $for_content__search_page = /*@__PURE__*/ _closure_get("search_page", ($scope) => {
	if (!updating) {
		$for_content__if($scope, $scope["#LoopKey"] === $scope._._.search_page ? 0 : 1);
	}
}, ($scope) => $scope._._);
...
const $else_content_holes = /*@__PURE__*/ _update_scopes({ "PatchAttr:href:#a/0": /*@__PURE__*/ _update_named_attr("#a/0", "href") });
const $for_update = _update_for_keyed("#ul/0", ($p, $l) => $for_content2_holes($p, $l));
const $for_update2 = _update_for_keyed("#nav/1", ($p2, $l2) => $for_content__update($p2, $l2));
const $count_seed = _update_signal("__tests__/template.marko_0_count/var");
const $for_content__update = (_patch, _live) => {
	if ("ConditionalRenderer:#text/0" in _patch) _update_if(_patch, _live, "ConditionalRenderer:#text/0", "BranchScopes:#text/0", [0, $else_content_holes]);
};
const $if_content__update = (_patch, _live) => {
	if ("BranchScopes:#ul/0" in _patch) $for_update(_live, [_patch["BranchScopes:#ul/0"], "#LoopKey"]);
	if ("BranchScopes:#nav/1" in _patch) $for_update2(_live, [_patch["BranchScopes:#nav/1"], "#LoopKey"]);
};
const $update2 = (_patch, _live) => {
	_update_pair(_patch, _live);
	if ("count" in _patch) _update_seed(_live, $count_seed, _patch["count"]);
	if ("search_page" in _patch) _live["search_page"] = _patch["search_page"];
	if ("search_q" in _patch) _live["search_q"] = _patch["search_q"];
	if ("results_items" in _patch) _live["results_items"] = _patch["results_items"];
	if ("results_totalPages" in _patch) _live["results_totalPages"] = _patch["results_totalPages"];
