// PATCH
[`packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_1*content packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_1_page#5/init packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_1_go_pending#8/init!packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_1; b%bD%b%l%;<button class=next>next</button><!><span class=of><!><!></span><!><!>`, `packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_3*content,failed`, `packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_4_#text#0/await;Db%;<span class=limit>of <!></span>`, `packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_4*content;b%;<!><!><!>`, `packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_5*content,loading`, `packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/template.marko_3*shell;b%b/b%c&b;<!><!><!><!><!><!>`, {
  "PatchReady:ready:packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko": {
    "PatchChild:#childScope/0": {
      "PatchChild:BranchScopes:#text/2": {
        "PatchBranch:#text/0": [1, {
          "PatchChild:#childScope/1": {
            1: "packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_0/go",
            "PatchChild:BranchScopes:#text/0": [{
              "PatchBranch:#text/1": 0,
              "PatchChild:BranchScopes:#text/4": [{
                "PatchPending:#text/0": "packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_4_#text#0/await",
                "PatchChild:BranchScopes:#text/0": {
                  "PatchText:#text/0": "9"
                }
              }, "packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_4*content", $, "packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_5*content"],
              "PatchSetup:": {
                "PatchInit:": "!packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_1"
              }
            }, "packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_1*content", "packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_3*content"],
            "PatchSetup:": {
              "PatchValue:packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko1": 1,
              "PatchValue:packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko2": bind(1)
            },
            "PatchValue:packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko0": 1
          }
        }, "packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/template.marko_3*shell"]
      }
    }
  }
}]
"DgMBAAAABA"

// PATCH holding DgMBAAAABA
[`packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_2*shell;Db%;<span class=prev>prev <!></span>`, {
  "PatchReady:ready:packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko": {
    "PatchChild:#childScope/0": {
      "PatchChild:BranchScopes:#text/2": {
        "PatchBranch:#text/0": [1, {
          "PatchChild:#childScope/1": {
            1: "packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_0/go",
            "PatchChild:BranchScopes:#text/0": [{
              "PatchBranch:#text/1": [{
                "PatchText:#text/0": "1"
              }, "packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_2*shell"],
              "PatchChild:BranchScopes:#text/4": [{
                "PatchPending:#text/0": "packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_4_#text#0/await",
                "PatchChild:BranchScopes:#text/0": {
                  "PatchText:#text/0": "9"
                }
              }, "packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_4*content", $, "packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_5*content"],
              "PatchSetup:": {
                "PatchInit:": "!packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_1"
              }
            }, "packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_1*content", "packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_3*content"],
            "PatchSetup:": {
              "PatchValue:packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko1": 2,
              "PatchValue:packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko2": bind(1)
            },
            "PatchValue:packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko0": 2
          }
        }, "packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/template.marko_3*shell"]
      }
    }
  }
}]
"DgMAAAAAAAQ"

// PATCH holding DgMAAAAAAAQ
{
  "PatchReady:ready:packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko": {
    "PatchChild:#childScope/0": {
      "PatchChild:BranchScopes:#text/2": {
        "PatchBranch:#text/0": [1, {
          "PatchChild:#childScope/1": {
            1: "packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_0/go",
            "PatchChild:BranchScopes:#text/0": [{
              "PatchBranch:#text/1": [{
                "PatchText:#text/0": "4"
              }, "packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_2*shell"],
              "PatchChild:BranchScopes:#text/4": [{
                "PatchPending:#text/0": "packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_4_#text#0/await",
                "PatchChild:BranchScopes:#text/0": {
                  "PatchText:#text/0": "9"
                }
              }, "packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_4*content", $, "packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_5*content"],
              "PatchSetup:": {
                "PatchInit:": "!packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_1"
              }
            }, "packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_1*content", "packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_3*content"],
            "PatchSetup:": {
              "PatchValue:packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko1": 5,
              "PatchValue:packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko2": bind(1)
            },
            "PatchValue:packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko0": 5
          }
        }, "packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/template.marko_3*shell"]
      }
    }
  }
}

// PATCH holding DgMAAAAAAAQ
[`packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/template.marko_2*shell;b%b/ Db%l&b;<!><!><button class=home>home:<!></button><!>`, {
  "PatchReady:ready:packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/home.marko": {
    "PatchChild:#childScope/0": {
      "PatchChild:BranchScopes:#text/2": {
        "PatchBranch:#text/0": [{
          "PatchChild:#childScope/1": {
            "PatchSetup:": {
              "PatchInit:": "!packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/home.marko_0",
              "PatchValue:packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/home.marko0": 0
            }
          }
        }, "packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/template.marko_2*shell"]
      }
    }
  }
}]
"DgMAAAAAAAMA"

// PATCH holding DgMAAAAAAAMA
{
  "PatchReady:ready:packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko": {
    "PatchChild:#childScope/0": {
      "PatchChild:BranchScopes:#text/2": {
        "PatchBranch:#text/0": [1, {
          "PatchChild:#childScope/1": {
            1: "packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_0/go",
            "PatchChild:BranchScopes:#text/0": [{
              "PatchBranch:#text/1": [{
                "PatchText:#text/0": "2"
              }, "packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_2*shell"],
              "PatchChild:BranchScopes:#text/4": [{
                "PatchPending:#text/0": "packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_4_#text#0/await",
                "PatchChild:BranchScopes:#text/0": {
                  "PatchText:#text/0": "9"
                }
              }, "packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_4*content", $, "packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_5*content"],
              "PatchSetup:": {
                "PatchInit:": "!packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_1"
              }
            }, "packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_1*content", "packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko_3*content"],
            "PatchSetup:": {
              "PatchValue:packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko1": 3,
              "PatchValue:packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko2": bind(1)
            },
            "PatchValue:packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/page.marko0": 3
          }
        }, "packages/runtime-tags/src/__tests__/fixtures/patch-app-chain-draft/template.marko_3*shell"]
      }
    }
  }
}
