...
_enable_catch();
const $await_content__taps = _var_resume("a10", /*@__PURE__*/ _let_persisted(5, ($scope) => _text($scope.c, $scope.f)));
const $await_content__setup__script = _script_shared(($scope) => _on($scope.a, "click", function() {
	$await_content__taps($scope, $scope.f + 1);
}));
const $clicks = _var_resume("a11", /*@__PURE__*/ _let_persisted(7, ($scope) => _text($scope.b, $scope.h)));
const $setup__script = _script_shared(($scope) => _on($scope.a, "click", function() {
	$clicks($scope, $scope.h + 1);
}));
const $taps_seed = _update_signal("a10");
const $await_content_holes = /*@__PURE__*/ _update_scopes({ "Qb": /*@__PURE__*/ _update_text("b") });
const $clicks_seed = _update_signal("a11");
