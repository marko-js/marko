...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	_if(() => {
		if (input.show) {
			...
			_html(`<a${_patch_attr($scope1_id, "#a/0", "href", input.href, $scope0_owned, 4)}${_patch_attr($scope1_id, "#a/0", "hidden", input.hidden, $scope0_owned, 5)}>${_patch_text($scope1_id, "#text/1", input.label, void 0, $scope0_owned, 6)}</a>${_el_resume($scope1_id, "#a/0")}`);
