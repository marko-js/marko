...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	_for_of(getItems?.($global().filter), (item) => {
		...
		writeScope($scope1_id, { watched: _seed_fill(_state_reason() && watched) }, "__tests__/template.marko", "4:4", { watched: "5:10" });
	}, function(item) {
		return item.id;
	}, $scope0_id, "#ul/0", _persisted_reason(), _persisted_reason(), _persisted_reason(), "</ul>", 1, "__tests__/template.marko_1_update");
	_persisted_reason() && writeScope($scope0_id, {}, "__tests__/template.marko", 0);
}, 1);
_renderer_shells({
	"__tests__/template.marko_1_update": ["<li><span> </span><button> </button></li>", "E l D m"],
	"__tests__/template.marko_1_content": ["<li><span> </span><button> </button></li>", "E l D m"],
	"__tests__/template.marko_0_update": ["<ul></ul>", " b"],
	"__tests__/template.marko": ["<ul></ul>", " b"]
});
