...
const $for_content__walks = "E l D m", $for_content__template = "<li><span> </span><button> </button></li>";
...
const $for = /*@__PURE__*/ _for_of("#ul/0", "<li><span> </span><button> </button></li>", "E l D ", $for_content__setup, $for_content__$params);
function $setup($scope) {
	if (!updating) $for($scope, [getItems?.($scope.$global.filter), function(item) {
		return item.id;
	}]);
