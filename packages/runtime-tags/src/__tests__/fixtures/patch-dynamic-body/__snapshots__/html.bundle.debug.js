...
var box_a_default = _template_patch("__tests__/box-a.marko", (input) => {
	...
	_html("</div>");
	$scope0_page && _scope($scope0_id, {}, "__tests__/box-a.marko", 0);
}, 0, 0);
...
var box_b_default = _template_patch("__tests__/box-b.marko", (input) => {
	const $scope0_reason = _scope_reason(), $sg__input_content = _source_guard($scope0_reason, 0), $scope0_page = _page_render();
	const $scope0_id = _scope_id();
	_html("<p class=b>");
	const $tag = input.content;
	_dynamic_tag($scope0_id, "#text/0", $tag, {}, 0, 0, $sg__input_content, _patch_dynamic_tag($scope0_id, "#text/0", $tag, 0, 0, 0, $scope0_reason, 0));
	_html("</p>");
	$scope0_page && _scope($scope0_id, {}, "__tests__/box-b.marko", 0);
}, 0, 0);
...
var template_default = _template_patch("__tests__/template.marko", (input) => {
	...
	_dynamic_tag($scope0_id, "#text/0", $tag, {}, _content_elide("__tests__/template.marko_1*content", () => {
		...
		_html(_patch_text($scope1_id, "#text/0", input.text, void 0, $scope0_reason, 2));
		_subscribe(_unfilled_if($scope0_reason, 2) && $input_text__closures, _scope($scope1_id, { _: _scope_with_id($scope0_id) }, "__tests__/template.marko", "4:6"));
	}, $scope0_id), 0, $sg__input_mode, _patch_dynamic_tag($scope0_id, "#text/0", $tag, 0, "__tests__/template.marko_1*content", 0, $scope0_reason, 1));
	_html("</main>");
	$scope0_page && _scope($scope0_id, { "ClosureScopes:input_text": $input_text__closures }, "__tests__/template.marko", 0);
}, 1, 1);
