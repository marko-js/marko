...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	_for_of($global().items, (item) => {
		...
		_for_of(CELLS, (cell) => {
			...
			_html(`<span>${_escape(_hole_value($scope2_id, "UpdateHole:#text/0", item.id, _persisted_reason()))}${_el_resume($scope2_id, "#text/0", _persisted_reason())}:${_sep(_persisted_reason())}${_escape(_hole_value($scope2_id, "UpdateHole:#text/1", item.view, _persisted_reason()))}${_el_resume($scope2_id, "#text/1", _persisted_reason())}:${_sep(_persisted_reason())}${_escape(_hole_value($scope2_id, "UpdateHole:#text/2", cell.id, _persisted_reason()))}${_el_resume($scope2_id, "#text/2", _persisted_reason())}</span>`);
