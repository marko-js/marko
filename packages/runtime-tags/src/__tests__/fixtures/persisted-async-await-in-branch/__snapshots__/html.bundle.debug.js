	...
	_if(() => {
		if (input.show) {
			...
			_await($scope1_id, "#text/0", input.promise, (value) => {
				...
				_html(`<em>${_patch_text($scope3_id, "#text/0", value, void 0, $scope0_reason, 3)}</em>`);
				_scope($scope3_id, {}, "__tests__/template.marko", "5:6");
			}, 1, "__tests__/template.marko_1_#text#0/await");
			$scope0_page && _scope($scope1_id, { _: _scope_with_id($scope0_id) }, "__tests__/template.marko", "4:4");
			return 0;
		} else {
			const $scope2_id = _scope_id();
			_html("<em>closed</em>");
			$scope0_page && _scope($scope2_id, {}, "__tests__/template.marko", "9:4");
			return 1;
		}
	}, $scope0_id, "#text/1", 1, $sg__input_show, $sg__input_show, void 0, void 0, ["__tests__/template.marko_1*shell", "__tests__/template.marko_2*shell"], $scope0_reason, 2);
