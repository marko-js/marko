...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	let items = ["a"];
	_html("<main><ul>");
	if ($scope0_reason) _for_of(items, (entry) => {
		const $scope1_id = _scope_id();
		_html(`<li>${_text_resume($scope1_id, "#text/0", entry)}</li>`);
		_scope($scope1_id, {}, "__tests__/template.marko", "4:6");
	}, input.key, $scope0_id, "#ul/0", 1, 1, 1, "</ul>", 1);
	...
	$scope0_reason ? _scope($scope0_id, {
		...
		input_key: ["input.key"],
		items: "1:6"
	}) : _filled_guard($scope0_owned, 0) && _patch_value($scope0_id, "__tests__/template.marko0", input.key);
}, 1, 0);
