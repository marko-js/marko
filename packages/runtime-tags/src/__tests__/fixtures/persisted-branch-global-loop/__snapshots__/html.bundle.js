...
var row_default = _template_persisted("b", (input) => {
	...
	let clicks = 0;
	_html(`<p>${_patch_text($scope0_id, "a", input.label, void 0, $scope0_owned, 0)}<button>${_text_resume($scope0_id, "c", clicks)}</button>${_el_resume($scope0_id, "b")}</p>`);
	_script($scope0_id, "b0");
	_patch_value($scope0_id, "b0", clicks, 1);
	$scope0_reason && _scope($scope0_id, { g: clicks });
}, 0, 0);
...
var template_default = _template_persisted("a", (input) => {
	...
	_if(() => {
		if (issues) {
			...
			}, "id", $scope1_id, "a", 1, $scope0_reason, $scope0_reason, void 0, void 0, "a2");
			_html(`</div>${_el_resume($scope1_id, "a", $scope0_reason)}`);
			$scope0_reason && _scope($scope1_id, {});
			return 1;
		}
	}, $scope0_id, "a", 1, $scope0_reason, $scope0_reason, void 0, void 0, ["a1", "a0"]);
