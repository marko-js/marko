...
_shells({ "__tests__/tags/card/index.marko": "__tests__/tags/card/index.marko;D lD ;<h2> </h2><p> </p>" });
var card_default = _template_persisted("__tests__/tags/card/index.marko", (input) => {
	const $scope0_owned = _persisted_ownership(), $scope0_reason = _persisted_reason();
	const $scope0_id = _scope_id();
	const { label, ...rest } = input;
	_html(`<h2>${_patch_text($scope0_id, "#text/0", label, $scope0_owned, 0)}${_el_resume($scope0_id, "#text/0")}</h2><p>${_patch_text($scope0_id, "#text/1", JSON.stringify(rest), $scope0_owned, 1)}${_el_resume($scope0_id, "#text/1")}</p>`);
