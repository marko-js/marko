...
const $try_content__input_related = /*@__PURE__*/ _closure_get("input_related", ($scope) => {
	if (!updating) {
		if (!updating) $try_content__await_promise($scope, resolveAfter($scope._.input_related, 1));
	}
});
...
const $update2 = (_patch, _live) => {
	...
	if ("BranchScopes:#text/4" in _patch) _update_branch(_patch, _live, "#text/4", $await_content2_holes);
};
