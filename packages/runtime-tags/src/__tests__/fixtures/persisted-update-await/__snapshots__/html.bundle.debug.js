...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	let count = 0;
	_html(`<h1>${_escape(_hole_value($scope0_id, "#text/0", input.title, _persisted_reason()))}${_el_resume($scope0_id, "#text/0", _serialize_guard($scope0_reason, 0))}</h1><button>clicked <!>${_escape(count)}${_el_resume($scope0_id, "#text/2")}</button>${_el_resume($scope0_id, "#button/1")}<section>`);
	_try($scope0_id, "#text/3", _content_resume("__tests__/template.marko_1_content", () => {
		...
		_await($scope1_id, "#text/0", resolveAfter(input.related, 1), (related) => {
			...
			_for_of(related, (item) => {
				...
				_html(`<li>${_escape(_hole_value($scope4_id, "#text/0", item.name, _persisted_reason()))}${_el_resume($scope4_id, "#text/0", $sg__input_related)} costs ${_sep($sg__input_related)}${_escape(_hole_value($scope4_id, "#text/1", item.price, _persisted_reason()))}${_el_resume($scope4_id, "#text/1", $sg__input_related)}</li>`);
	...
	_await($scope0_id, "#text/4", resolveAfter(input.note, 2), (note) => {
		...
		_html(`<em>${_escape(_hole_value($scope5_id, "#text/0", note, _persisted_reason()))}${_el_resume($scope5_id, "#text/0", $sg__input_note)}</em>`);
