...
var card_default = _template_persisted("__tests__/tags/card/index.marko", (input) => {
	const $scope0_owned = _persisted_ownership(), $scope0_reason = _persisted_reason(), $sg__input_count__OR__input_content = _source_guard($scope0_reason, 0), $sg__input_count = _source_guard($scope0_reason, 1);
	...
	_for_of(input.count, (i) => {
		...
		_dynamic_tag($scope1_id, "#text/1", $tag, {}, 0, 0, _source_guard($scope0_reason, 2), _patch_dynamic_tag($scope1_id, "#text/1", $tag, 0, 0, 0, $scope0_owned, 2));
		_html("</li>");
		_scope($scope1_id, { _: _scope_with_id($scope0_id) }, "__tests__/tags/card/index.marko", "2:4");
	}, 0, $scope0_id, "#ul/0", 1, $sg__input_count__OR__input_content, $sg__input_count, void 0, void 0, "__tests__/tags/card/index.marko_1*shell", $scope0_owned, 1);
	_html(`</ul>${_el_resume($scope0_id, "#ul/0", $sg__input_count__OR__input_content)}`);
	$scope0_reason ? _scope($scope0_id, { input_content: input.content }, "__tests__/tags/card/index.marko", 0, { input_content: ["input.content"] }) : _filled_guard($scope0_owned, 2) && _client_guard($scope0_owned, 1) && _patch_value($scope0_id, "__tests__/tags/card/index.marko0", input.content);
}, 0, 0);
...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	card_default({
		...
		content: _content_elide("__tests__/template.marko_1*content", () => {
			...
			_subscribe(_unfilled_if($scope0_owned, 2) && $input_note__closures, _scope($scope1_id, { _: _scope_with_id($scope0_id) }, "__tests__/template.marko", "2:4"));
		}, $scope0_id)
	});
	_html("</main>");
