...
const $walks$1 = "E l%l";
_shells({ e: "e;E l%;<section><h2> </h2><!></section>" });
var card_default = _template_persisted("e", (input) => {
...
var page_b_default = _template_persisted("c", (input) => {
	...
	const $p__closures = /* @__PURE__ */ new Set();
	let count = 0;
	const p = input.note;
	_html("<h1>B</h1>");
	_for_of([1, 2], (item) => {
		...
		card_default({
			...
			content: _content_elide("c0", () => {
				_persisted_reason();
				const $scope2_id = _scope_id();
				_html(`<span>${_patch_text($scope2_id, "a", p, void 0, $scope0_owned, 0)}/${_text_resume($scope2_id, "b", count, 2)}/${_patch_text($scope2_id, "c", item, 2)}</span>`);
				_subscribe(_unfilled_if($scope0_owned, 0) && $p__closures, _subscribe($count__closures, _scope($scope2_id, { _: _scope_with_id($scope1_id) })));
			}, $scope1_id)
		});
		_scope($scope1_id, {
...
_shells({
	...
	d1: /*@__PURE__*/ ((_w0, _w1) => `d1;${_w0};${_w1}`)(/*@__PURE__*/ ((_w0) => `b/${_w0}&b`)($walks$3), /*@__PURE__*/ ((_w0) => `<!>${_w0}<!>`)($template$3)),
	d2: /*@__PURE__*/ ((_w0, _w1) => `d2;${_w0};${_w1}`)(/*@__PURE__*/ ((_w0) => `b%b/${_w0}&b`)($walks$2), /*@__PURE__*/ ((_w0) => `<!><!>${_w0}<!>`)($template$2)),
...
var template_default = _template_persisted("d", (input) => {
	const $scope0_owned = _persisted_ownership(), $scope0_reason = _persisted_reason(), $sg__input_page = _source_guard($scope0_reason, 4), $si__input_page = _source_if($scope0_reason, 4);
	const $scope0_id = _scope_id();
		...
		if (input.page === 0) {
			...
			$Layout_withLoadAssets({
				...
				content: _content_elide("d0", () => {
					...
					_if(() => {
						if (input.page === 1) {
							...
							_set_serialize_reason({
								0: _mask_group($scope0_owned, 7),
								1: _mask_group($scope0_owned, 6)
							});
							const $childScope = _peek_scope_id();
							...
							_subscribe(_unfilled_if($scope0_owned, 7) && $input_note__closures, _subscribe(_unfilled_if($scope0_owned, 6) && $input_wide__closures, _scope($scope3_id, {
							...
							_set_serialize_reason({ 0: _mask_group($scope0_owned, 7) });
							const $childScope2 = _peek_scope_id();
							_patch_child($scope4_id, "b", $childScope2);
							$PageB_withLoadAssets({ note: input.note });
							_subscribe(_unfilled_if($scope0_owned, 7) && $input_note__closures, _scope($scope4_id, {
								_: _scope_with_id($scope2_id),
					...
					}, $scope2_id, "a", 1, $sg__input_page, $sg__input_page, void 0, void 0, ["d2", "d3"], $scope0_owned, 4);
					$scope0_reason && _subscribe(_unfilled_if($scope0_owned, 4) && $input_page__closures, _scope($scope2_id, { _: _scope_with_id($scope1_id) }));
	...
	}, $scope0_id, "a", 1, $sg__input_page, $sg__input_page, void 0, void 0, ["d4", "d1"], $scope0_owned, 4);
