...
var child_default = _template_persisted("a", (input) => {
	...
	_html(`<div${_patch_attr_class($scope0_id, "a", input.name, $scope0_owned, 0)}></div>${_el_resume($scope0_id, "a")}`);
	_script($scope0_id, "a0");
	_patch_effect($scope0_id, "a0", "d e");
	$scope0_reason ? _scope($scope0_id, {
		d: input.name,
		e: input.item
	}) : (_filled_guard($scope0_owned, 0) && _patch_write($scope0_id, "d", input.name), _filled_guard($scope0_owned, 1) && _patch_write($scope0_id, "e", input.item));
}, 0, 0);
...
var template_default = _template_persisted("b", (input) => {
	...
	_try($scope0_id, "c", _content_resume("b3", () => {
		...
		_await($scope1_id, "a", input.promise, (value) => {
			...
			_html(`<span>${_patch_text($scope2_id, "a", value, void 0, $scope0_owned, 2)}</span>`);
			_set_serialize_reason({ 1: _mask_group($scope0_owned, 1) });
		...
		$scope0_reason && _subscribe(_unfilled_if($scope0_owned, 2) && $input_promise__closures, _scope($scope1_id, { _: _scope_with_id($scope0_id) }));
