...
_shells({
	...
	"__tests__/template.marko_1*shell": "__tests__/template.marko_1*shell __tests__/template.marko_1_items#5/init; ;<ul></ul>"
});
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	_if(() => {
		if (input.show) {
			...
			if ($scope0_page) _for_of(items, (item) => {
				...
				_html(`<li>${_text_resume($scope2_id, "#text/0", item)}</li>`);
				_scope($scope2_id, {}, "__tests__/template.marko", "5:8");
	...
	$scope0_page && _scope($scope0_id, { items }, "__tests__/template.marko", 0, { items: "1:6" });
}, 1, 0);
