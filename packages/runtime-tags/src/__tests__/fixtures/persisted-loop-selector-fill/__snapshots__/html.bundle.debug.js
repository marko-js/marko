...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	if ($scope0_reason) _for_of(items, (item) => {
		...
		_html("</li>");
		_scope($scope1_id, { item_id: _source_if($scope0_reason, 0) && item?.id }, "__tests__/template.marko", "4:6", { item_id: ["item.id", "4:10"] });
	...
	$scope0_reason ? _scope($scope0_id, {
		...
		input_selected: ["input.selected"],
		items: "1:6"
	}) : _filled_guard($scope0_owned, 0) && _patch_value($scope0_id, "__tests__/template.marko0", input.selected);
}, 1, 0);
