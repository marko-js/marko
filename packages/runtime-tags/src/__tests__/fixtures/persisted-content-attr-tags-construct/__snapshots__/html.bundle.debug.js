...
var card_default = _template_persisted("__tests__/tags/card/index.marko", (input) => {
	...
	_dynamic_tag$1($scope0_id, "#text/0", input.header, {}, 0, 0, _source_guard($scope0_reason, 0), 1);
	_html$1(`</header><h2>${_patch_text($scope0_id, "#text/1", input.title, $scope0_owned, 1)}${_el_resume($scope0_id, "#text/1")}</h2>`);
	_patch_dynamic_tag($scope0_id, "#text/2", input.content, $scope0_owned, 2);
	_dynamic_tag$1($scope0_id, "#text/2", input.content, {}, 0, 0, _source_guard($scope0_reason, 2), 1);
	_html$1("</section>");
...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	$scope0_reason && writeScope($scope0_id, { input_title: input.title }, "__tests__/template.marko", 0, { input_title: ["input.title"] });
}, 1, () => [card_default]);
