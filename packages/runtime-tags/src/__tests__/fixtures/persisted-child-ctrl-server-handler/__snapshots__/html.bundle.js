...
var field_default = _template_persisted("b", (input) => {
	...
	_html(`<input${_attr_input_value($scope0_id, "a", input.value, input.valueChange)}${_patch_bind($scope0_id, "Ea", input.valueChange, 0, 0)}${_patch_control($scope0_id, "a", 2, input.value, $scope0_reason, 0)}>${_el_resume($scope0_id, "a")}`);
...
var template_default = _template_persisted("a", (input) => {
	...
	$scope0_page ? _scope($scope0_id, {
		...
		f: handle,
		g: open
	}) : (_filled_guard($scope0_reason, 0) && _patch_value($scope0_id, "a0", handle), _filled_guard($scope0_reason, 0) && _patch_write($scope0_id, "e", input.prefix));
}, 1, () => [field_default]);
