...
var template_default = _template_patch("a", (input) => {
	...
	wrap_default({ content: _content_resume("a2", () => {
		_scope_reason();
		...
		_try($scope1_id, "a", _content_resume("a1", () => {
			...
			_subscribe(_source_if($scope0_reason, 2) && $input_x__closures, _subscribe(_source_if($scope0_reason, 1) && $input_fail__closures, _scope($scope2_id, { _: _scope_with_id($scope1_id) })));
		}, $scope1_id), { catch: attrTag({ content: _content_elide("a0", (err) => {
