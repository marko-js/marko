...
// template.marko
_shells({ a: /*@__PURE__*/ ((_w0, _w1) => `a !a3;${_w0};${_w1}`)(((_w0) => `D/${_w0}& l`)("b%c"), ((_w0) => `<main>${_w0}<button>+</button></main>`)($template)) });
var template_default = _template_patch("a", (input) => {
	...
	if ($scope0_page || _must_render(panel_default)) {
		...
		panel_default({
			open: true,
			body: attrTag({ content: _content_resume("a2", () => {
				_scope_reason();
				const $scope1_id = _scope_id();
				_html(`<em>${_patch_text($scope1_id, "a", $global$1.brand)}</em>`);
				_global_subscribe("a0", $scope1_id);
				_subscribe(_unfilled_if() && $global_brand__closures, _scope($scope1_id, { _: _scope_with_id($scope0_id) }), "a1");
			}, $scope0_id) })
		});
	}
