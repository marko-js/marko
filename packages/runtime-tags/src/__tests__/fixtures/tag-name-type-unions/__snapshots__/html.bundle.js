...
var template_default = _template("a", (input) => {
	...
	_dynamic_tag($scope0_id, "b", "div", { id: "d1" }, _content_resume("a1", () => {
		_scope_id();
		_scope_reason();
		_html("u");
	}, $scope0_id));
	_dynamic_tag($scope0_id, "c", "div", {}, _content_resume("a2", () => {
		_scope_id();
		_scope_reason();
		_html("n");
	}, $scope0_id));
	_dynamic_tag($scope0_id, "d", navigator, {}, _content_resume("a3", () => {
		_scope_id();
		_scope_reason();
		_html("g");
	}, $scope0_id), 0, 0);
	_dynamic_tag($scope0_id, "e", a_default, { label: "ab" });
	_dynamic_tag($scope0_id, "f", "div", { label: "ad" }, _content_resume("a4", () => {
		_scope_id();
		_scope_reason();
		_html("m");
	}, $scope0_id));
