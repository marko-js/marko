...
var template_default = _template_persisted("a", (input) => {
	const $scope0_reason = _scope_reason(), $sg__input_show = _serialize_guard($scope0_reason, 2), $sg__input_items = _serialize_guard($scope0_reason, 3);
	const $scope0_id = _scope_id();
	_patch_unprovable();
	_html(`<div class=wrap><h1>Hello <!>${_escape(input.name)}${_el_resume($scope0_id, "a")}${_patch_hole($scope0_id, "a", input.name)}</h1>`);
	_if(() => {
		if (input.show) {
