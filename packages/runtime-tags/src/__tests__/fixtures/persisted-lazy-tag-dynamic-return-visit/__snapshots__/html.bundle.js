...
var child_default = _template_persisted("a", (input) => {
	...
	_html(`<button>${_patch_text($scope0_id, "a", input.label, void 0, $scope0_owned, 0)}</button>`);
...
var template_default = _template_persisted("b", (input) => {
	...
	_patch_dynamic_tag($scope0_id, "c", $tag, $input2, 0, 0, $scope0_owned, 0);
	_dynamic_tag($scope0_id, "c", $tag, $input2, 0, 0, _source_guard($scope0_reason, 0), 1);
	_html("</main>");
