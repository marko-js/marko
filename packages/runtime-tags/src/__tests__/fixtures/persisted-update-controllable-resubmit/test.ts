...
const typeQty = (value: string) => (document: Document) => {
  const input = document.querySelector<HTMLInputElement>("input.qty")!;
...
const submit = (document: Document) => {
  const form = document.querySelector<HTMLFormElement>("form.order")!;
