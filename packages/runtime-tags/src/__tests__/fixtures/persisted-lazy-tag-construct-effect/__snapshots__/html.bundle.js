...
var template_default = _template_persisted("b", (input) => {
	...
	_if(() => {
		if (input.show) {
			...
			_html(`<div${_patch_attrs(input.attrs, "a", $scope1_id, "div", void 0, $scope0_reason, 2)}>x</div>${_el_resume($scope1_id, "a")}`);
	...
	$scope0_page && _scope($scope0_id, { f: _source_if($scope0_reason, 1) && input.label });
