...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	_for_of(input.items, (item) => {
		...
		_filled_guard($scope0_reason, 0) ? _patch_value($scope1_id, "__tests__/template.marko1", item?.id) : _patch_init($scope1_id, "__tests__/template.marko_1_input_items#4/init");
		const { name } = item;
		_filled_guard($scope0_reason, 0) ? _patch_value($scope1_id, "__tests__/template.marko0", name) : _patch_init($scope1_id, "__tests__/template.marko_1_input_items#4/init");
		const same = item;
