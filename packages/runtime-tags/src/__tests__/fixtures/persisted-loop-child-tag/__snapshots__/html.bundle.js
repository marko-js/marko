...
var row_default = _template_persisted("b", (input) => {
	...
	_html(`<li>${_patch_text($scope0_id, "a", input.item.name, $scope0_owned, 0)}${_el_resume($scope0_id, "a")}<!>${_patch_text($scope0_id, "b", input.item.hot ? " 🔥" : "", $scope0_owned, 1)}${_el_resume($scope0_id, "b")}</li>`);
	$scope0_reason && writeScope($scope0_id, {});
}, 0, 0);

// template.marko
_shells({ a0: /*@__PURE__*/ ((_w0, _w1) => `a0;${_w0};${_w1}`)(/*@__PURE__*/ ((_w0) => `/${_w0}&`)($walks), $template) });
