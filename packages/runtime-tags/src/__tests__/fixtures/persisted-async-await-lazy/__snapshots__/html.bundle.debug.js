...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	let count = 0;
	_html(`<main><h1>${_patch_text($scope0_id, "#text/0", input.title, void 0, $scope0_reason, 4)}</h1><section>`);
	_try($scope0_id, "#text/1", _content_resume("__tests__/template.marko_1*content", () => {
		...
		$scope0_page && _subscribe(_unfilled_if($scope0_reason, 6) && $input_slow__closures, _subscribe(_unfilled_if($scope0_reason, 5) && $input_related__closures, _scope($scope1_id, { _: _scope_with_id($scope0_id) }, "__tests__/template.marko", "7:6")));
