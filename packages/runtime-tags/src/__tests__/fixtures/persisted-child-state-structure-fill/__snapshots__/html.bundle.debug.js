...
var toggle_panel_default = _template_persisted("__tests__/tags/toggle-panel/index.marko", (input) => {
	...
	_if(() => {
		if (input.show) {
			...
			_html(`<em>${_patch_text($scope1_id, "#text/0", input.title, void 0, $scope0_reason, 2)}</em>`);
	...
	$scope0_page ? _scope($scope0_id, { input_title: input.title }, "__tests__/tags/toggle-panel/index.marko", 0, { input_title: ["input.title"] }) : _filled_guard($scope0_reason, 2) && _client_guard($scope0_reason, 1) && _patch_value($scope0_id, "__tests__/tags/toggle-panel/index.marko0", input.title);
}, 0, 0);
