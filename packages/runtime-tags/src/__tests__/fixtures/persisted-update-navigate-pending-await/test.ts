...
//    captures a navigation epoch at its own (per-render) init and drops a
//    placeholder completion whole (cleanup walk, `runtime.j` callbacks, and
//    final swap) once a persisted apply has bumped that epoch (`dom/
//    resume`'s `bumpNavEpoch`, called from `dom/update`'s `createUpdate`
//    before any frame applies, scoped to the navigated render) -- confirmed
//    below: the late chunk's content lands inside its own still-hidden
//    `<t>` wrapper (`t > p::text("pre-nav-data")`) instead of being spliced
//    into the live `<section>`, so no `pre-nav-data` ever appears in the
//    rendered page. (Here the navigation's boundary-body apply already
//    replaced the placeholder -- and detached its markers -- before the
//    stale chunk arrived; sibling fixture persisted-update-stale-reorder-
//    cleanup covers the aborted-navigation case where the markers survive
//    and the gated cleanup walk matters.)
