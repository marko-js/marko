...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	let opened = false;
	const on = n % 2 === 0;
	const Chunk = { content: _content_resume("__tests__/template.marko_1_content", () => {
...
`)}<\/script>${_el_resume($scope0_id, "#script/8")}<div>${_unescaped(`<b>html ${n}</b>`)}${_el_resume($scope0_id, "#text/9")}</div><div`);
	...
	_html(`</div>${_el_resume($scope0_id, "#div/14")}<input${_attr_input_checked($scope0_id, "#input/15", on, _resume(function(v) {
		...
		hidden = v;
	}, "__tests__/template.marko_0/valueChange2", $scope0_id))} type=hidden>${_el_resume($scope0_id, "#input/22")}`);
	...
	_script($scope0_id, "__tests__/template.marko_0_spread");
