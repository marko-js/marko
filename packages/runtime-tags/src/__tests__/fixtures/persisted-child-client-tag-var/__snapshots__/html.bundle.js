...
var widget_default = _template_persisted("b", (input) => {
	...
	let count = 1;
	_html(`<em>${_patch_text($scope0_id, "a", input.label, $scope0_owned, 0)}${_el_resume($scope0_id, "a")} x<!>${_escape(count)}${_el_resume($scope0_id, "b")}</em><button class=bump>+</button>${_el_resume($scope0_id, "c")}`);
	const $return = count;
	...
	if ($scope0_reason) _if(() => {
		{
			...
			_html(`<p class=echo>${_escape(w)}${_el_resume($scope1_id, "c")}</p>`);
