...
var widget_default = _template_persisted("b", (input) => {
	...
	let count = 1;
	_html(`<em>${_patch_text($scope0_id, "a", input.label, void 0, $scope0_reason, 0)} x${_text_resume($scope0_id, "b", count, 2)}</em><button class=bump>+</button>${_el_resume($scope0_id, "c")}`);
	const $return = count;
