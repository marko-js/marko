...
var widget_default = _template_persisted("__tests__/tags/widget/index.marko", (input) => {
	...
	let count = 1;
	_html(`<em>${_patch_text($scope0_id, "#text/0", input.label, $scope0_owned, 0)}${_el_resume($scope0_id, "#text/0")} x<!>${_escape(count)}${_el_resume($scope0_id, "#text/1")}</em><button class=bump>+</button>${_el_resume($scope0_id, "#button/2")}`);
	const $return = count;
	_script($scope0_id, "__tests__/tags/widget/index.marko_0");
	_patch_value($scope0_id, "__tests__/tags/widget/index.marko0", count, 1);
	$scope0_reason && writeScope($scope0_id, { count }, "__tests__/tags/widget/index.marko", 0, { count: "1:6" });
...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	if ($scope0_reason) _if(() => {
		if (show) {
			...
			_html(`<p class=echo>${_escape(w)}${_el_resume($scope1_id, "#text/2")}</p>`);
