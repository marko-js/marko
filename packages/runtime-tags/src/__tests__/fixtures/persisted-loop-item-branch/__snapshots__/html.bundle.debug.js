...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	_for_of(input.items, (item) => {
		...
		_html(`<li>${_patch_text($scope1_id, "#text/0", item.id, void 0, $scope0_reason, 0)}`);
		_if(() => {
			if (item.children.length) {
				...
				_for_of(item.children, (child) => {
					...
					_html(`<li>${_patch_text($scope3_id, "#text/0", child, void 0, $scope0_reason, 0)}</li>`);
					_scope($scope3_id, {}, "__tests__/template.marko", "7:12");
				}, 0, $scope2_id, "#ul/0", 1, $sg__input_items, $sg__input_items, void 0, void 0, "__tests__/template.marko_3*shell", $scope0_reason, 0);
				_html(`</ul>${_el_resume($scope2_id, "#ul/0", $sg__input_items)}`);
				$scope0_page && _scope($scope2_id, { _: _scope_with_id($scope1_id) }, "__tests__/template.marko", "5:8");
				return 0;
			}
