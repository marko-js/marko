...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	_try($scope0_id, "#text/0", _content_resume("__tests__/template.marko_2*content", () => {
		...
		_await($scope2_id, "#text/0", attempt === 1 ? resolveAfter("body", 3) : Promise.reject(new Error("nope")), (value) => {
			...
			_html(`<p>${_escape(value)}${_el_resume($scope4_id, "#text/0")}</p>`);
			writeScope($scope4_id, {}, "__tests__/template.marko", "12:4");
		});
		...
		placeholder: attrTag({ content: _content_resume("__tests__/template.marko_1*content", () => {
			...
			_html(`<button>retry <!>${_escape(attempt)}${_el_resume($scope1_id, "#text/1")}</button>${_el_resume($scope1_id, "#button/0")}`);
			_script($scope1_id, "__tests__/template.marko_1");
			_subscribe($attempt__closures, writeScope($scope1_id, { _: _scope_with_id($scope0_id) }, "__tests__/template.marko", "5:4"));
			_resume_branch($scope1_id);
		}, $scope0_id) }),
		catch: attrTag({ content: _content_resume("__tests__/template.marko_3*content", (err) => {
			const $scope3_reason = _scope_reason();
			const $scope3_id = _scope_id();
			_html(`<b>${_escape(err.message)}${_el_resume($scope3_id, "#text/0", _serialize_guard($scope3_reason, 0))}</b>`);
			_serialize_if($scope3_reason, 0) && writeScope($scope3_id, {}, "__tests__/template.marko", "15:4");
		}, $scope0_id) })
	});
