...
const $try_content__attempt = /*@__PURE__*/ _closure_get("attempt/2", ($scope) => $try_content__await_promise($scope, $scope._.attempt === 1 ? resolveAfter("body", 3) : Promise.reject(new Error("nope"))), 0, "__tests__/template.marko_2_attempt#1/subscribe");
const $try_content__setup = ($scope) => {
	$try_content__attempt($scope);
	$await_content($scope);
};
const $placeholder_content__attempt = /*@__PURE__*/ _closure_get("attempt/2", ($scope) => _text($scope["#text/1"], $scope._.attempt), 0, "__tests__/template.marko_1_attempt#1/subscribe");
