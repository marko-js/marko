...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	const Row = { content: _content("__tests__/template.marko_1_content", (input) => {
		...
		_html_opens("__tests__/template.marko:2:3"), _html(`<div>${_escape(input.cell.label)}${_el_resume($scope1_id, "#text/0", _serialize_guard($scope1_reason, 0))}</div>`);
	...
	_html_opens("__tests__/template.marko:5:1"), _html(`<div>added=${_sep($sg__input_obj)}${_escape(Object.getOwnPropertySymbols(input.obj).length)}${_el_resume($scope0_id, "#text/1", $sg__input_obj)}</div>`);
