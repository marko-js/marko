...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	const Row = { content: _content("__tests__/template.marko_1*content", (input) => {
		...
		_html(`<div>${_text_resume($scope1_id, "#text/0", input.cell.label, _serialize_guard($scope1_reason, 0))}</div>`);
	...
	_html(`<div>added=${_text_resume($scope0_id, "#text/1", Object.getOwnPropertySymbols(input.obj).length, _serialize_guard($scope0_reason, 0) * 2)}</div>`);
