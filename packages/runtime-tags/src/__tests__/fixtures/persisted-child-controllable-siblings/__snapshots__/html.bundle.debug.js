...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	let first = 0;
	let second = 0;
	_html(`<main><h1>${_patch_text($scope0_id, "#text/0", input.title, $scope0_owned, 0)}${_el_resume($scope0_id, "#text/0")}</h1><p>First <!>${_escape(first)}${_el_resume($scope0_id, "#text/1")} Second <!>${_escape(second)}${_el_resume($scope0_id, "#text/2")}</p>`);
