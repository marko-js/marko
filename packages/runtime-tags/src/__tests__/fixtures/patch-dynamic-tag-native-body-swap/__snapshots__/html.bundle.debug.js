...
var outer_default = _template_patch("__tests__/outer.marko", (input) => {
	...
	_html("</section>");
	$scope0_page && _scope($scope0_id, {}, "__tests__/outer.marko", 0);
}, 0, 0);
...
var template_default = _template_patch("__tests__/template.marko", (input) => {
	...
	_dynamic_tag($scope0_id, "#text/0", $tag, {}, _content_resume("__tests__/template.marko_1*content", () => {
		...
		_html(`body ${_patch_text($scope1_id, "#text/0", input.x, 2, $scope0_reason, 2)}`);
		_subscribe(_unfilled_if($scope0_reason, 2) && $input_x__closures, _scope($scope1_id, { _: _scope_with_id($scope0_id) }, "__tests__/template.marko", "2:4"), _client_guard($scope0_reason, 2) && "__tests__/template.marko_1_input_x#4/subscribe");
	}, $scope0_id, ($scope) => [{ input_x: input.x }]), 0, $sg__input_wrap, _patch_dynamic_tag($scope0_id, "#text/0", $tag, 0, "__tests__/template.marko_1*content", 0, $scope0_reason, 1));
	$scope0_page && _scope($scope0_id, {
		input_x: _source_if($scope0_reason, 1) && input.x,
