...
var outer_default = _template_patch("a", (input) => {
...
var template_default = _template_patch("b", (input) => {
	...
	_dynamic_tag($scope0_id, "a", $tag, {}, _content_resume("b0", () => {
		const $scope1_id = _scope_id();
		_scope_reason();
		_html(`body ${_patch_text($scope1_id, "a", input.x, 2, $scope0_reason, 2)}`);
		_subscribe(_unfilled_if($scope0_reason, 2) && $input_x__closures, _scope($scope1_id, { _: _scope_with_id($scope0_id) }), _client_guard($scope0_reason, 2) && "b1");
	}, $scope0_id, ($scope) => [{ e: input.x }]), 0, $sg__input_wrap, _patch_dynamic_tag($scope0_id, "a", $tag, 0, "b0", 0, $scope0_reason, 1));
	$scope0_page && _scope($scope0_id, {
