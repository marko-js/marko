...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	let count = 0;
	_html(`<main><h1>${_patch_text($scope0_id, "#text/0", input.heading, void 0, $scope0_owned, 1)}</h1>`);
	_if(() => {
		if (input.show) {
			...
			_if(() => {
				if (input.inner) {
					...
					_subscribe($count__closures, _subscribe(_source_if($scope0_reason, 4) && $input_title__closures, _scope($scope2_id, { _: _scope_with_id($scope1_id) }, "__tests__/template.marko", "5:6")));
					return 0;
				}
	...
	$scope0_reason ? _scope($scope0_id, {
		...
		input_title: ["input.title"],
		count: "1:6"
	}) : _filled_guard($scope0_owned, 4) && _patch_value($scope0_id, "__tests__/template.marko0", input.title);
}, 1, 0);
