...
var panel_default = _template("__tests__/tags/panel.marko", (input) => {
	...
	}, 0, $scope0_id, "#ul/2", $sg__input_item, $sg__input_item, $sg__input_item, "</ul>", 1, "__tests__/tags/panel.marko_0/update_for_#ul/2");
...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	if (input.show) {
		$body = attrTag({ content: _content("__tests__/template.marko_2_content", () => {
			...
			inner_default({ part: attrTag({ content: _content("__tests__/template.marko_3_content", () => {
				...
				_html(`<p>${_escape(_hole_value($scope3_id, "UpdateHole:#text/0", getTitle(input.id), _persisted_reason()))}${_el_resume($scope3_id, "#text/0", $sg__input_id)}</p>`);
	...
	forOf(getRows(input.id), (row) => {
		$item = attrTags($item, {
			...
			content: _content("__tests__/template.marko_4_content", () => {
				...
				_html(`${_escape(row)}${_el_resume($scope4_id, "#text/0")}`);
	...
	panel_default({
		...
		header: attrTag({ content: _content("__tests__/template.marko_1_content", () => {
			...
			const $scope1_id = _scope_id();
			_html(`<h2>${_escape(_hole_value($scope1_id, "UpdateHole:#text/0", getTitle(input.id), _persisted_reason()))}${_el_resume($scope1_id, "#text/0", $sg__input_id)}</h2>`);
