...
export default _._template("__tests__/template.marko", input => {
  const $scope0_reason = _._scope_reason(),
    $sg__input_c__OR__input_d = _._serialize_guard($scope0_reason, /* input.c, input.d */0);
  ...
  _._html(`<div${_._attr_class(["a", {
    b: c,
    d
  }])}></div>${_._el_resume($scope0_id, "#div/0", ($sg__input_c__OR__input_d))}<div class="a b"></div><div class="a b c"></div>`);
  const $childScope = _._peek_scope_id();
  _._set_serialize_reason({
    /* input.class, input.test */0: ($sg__input_c__OR__input_d),
    /* input.class */2: $sg__input_c__OR__input_d
  });
  _customTag({
    class: ["a", {
      b: c,
