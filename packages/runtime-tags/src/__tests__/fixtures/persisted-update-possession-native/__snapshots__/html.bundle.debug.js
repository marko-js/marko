...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	const Widget = { content: _content("__tests__/template.marko_1_content", () => {
		...
		_html(`<p class=widget>widget: ${_sep(_persisted_reason())}${_escape(_hole_value($scope1_id, "UpdateHole:#text/0", getNote?.($global().topic), _persisted_reason()))}${_el_resume($scope1_id, "#text/0", _persisted_reason())}</p>`);
	...
	_dynamic_tag($scope0_id, "#text/2", $global().view === "plain" ? "blockquote" : Widget, { class: "hop" }, _content_resume("__tests__/template.marko_2_content", () => {
		...
		_html(`plain: ${_sep(_persisted_reason())}${_escape(_hole_value($scope2_id, "UpdateHole:#text/0", getNote?.($global().topic), _persisted_reason()))}${_el_resume($scope2_id, "#text/0", _persisted_reason())}`);
