...
var custom_tag_default = _template("__tests__/tags/custom-tag.marko", (input) => {
	...
	_dynamic_tag($scope0_id, "#text/2", input.content, {
		count: x,
		name: input.name
	}, void 0, void 0, void 0, "__tests__/tags/custom-tag.marko:6:1");
...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	custom_tag_default({
		...
		content: _content_resume("__tests__/template.marko_1_content", ({ count, name }) => {
			...
			_html_opens("__tests__/template.marko:2:3"), _html(`<div>Count (${_sep($sg__name)}${_escape(name)}${_el_resume($scope1_id, "#text/0", $sg__name)}): ${_sep($sg__count)}${_escape(count)}${_el_resume($scope1_id, "#text/1", $sg__count)}</div>`);
