...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	let items = ["a"];
	_html("<main><ul>");
	if ($scope0_page) _for_of(items, (item, i) => {
		const $scope1_id = _scope_id();
		_html(`<li>${_text_resume($scope1_id, "#text/0", item)}<button>x</button>${_el_resume($scope1_id, "#button/1")}</li>`);
		_script($scope1_id, "__tests__/template.marko_1");
		_scope($scope1_id, { "#LoopKey": i }, "__tests__/template.marko", "4:6", { "#LoopKey": "4:16" });
