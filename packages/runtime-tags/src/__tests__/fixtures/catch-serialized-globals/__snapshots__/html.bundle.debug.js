...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	_try($scope0_id, "#text/0", _content_resume("__tests__/template.marko_1_content", () => {
		...
		let clicked = false;
		_html_opens("__tests__/template.marko:10:5"), _html(`<button>${clicked ? _escape(message) : _escape(error.message)}${_el_resume($scope2_id, "#text/1")}</button>${_el_resume($scope2_id, "#button/0")}`);
