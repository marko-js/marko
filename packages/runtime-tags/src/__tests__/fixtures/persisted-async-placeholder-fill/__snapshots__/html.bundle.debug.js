...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	_try($scope0_id, "#text/0", _content_resume("__tests__/template.marko_2*content", () => {
		...
		_html(`<p>${_escape(input.label)}${_el_resume($scope1_id, "#text/0")}</p>`);
		_subscribe(_source_if($scope0_reason, 0) && $input_label__closures, writeScope($scope1_id, { _: _scope_with_id($scope0_id) }, "__tests__/template.marko", "7:6"));
