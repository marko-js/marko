...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	_if(() => {
		if ($global().params.sale) {
			...
			_html(`<em>Sale ${_sep(_persisted_reason())}${_escape(_hole_value($scope1_id, "UpdateHole:#text/0", $global().params.sale, _persisted_reason()))}${_el_resume($scope1_id, "#text/0", _persisted_reason())}% off</em><button class=buy>buy</button>${_el_resume($scope1_id, "#button/1")}`);
