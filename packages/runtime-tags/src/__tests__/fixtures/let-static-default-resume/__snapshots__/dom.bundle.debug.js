...
const $for_content__item_name__OR__open = /*@__PURE__*/ _or(8, ($scope) => _text($scope["#text/2"], $scope.open && $scope.item_name));
const $for_content__open = /*@__PURE__*/ _let("open/7", $for_content__item_name__OR__open);
const $for_content__setup__script = _script("__tests__/template.marko_1", ($scope) => {
	$scope.open ??= false;
...
const $setup__script = _script("__tests__/template.marko_0", ($scope) => {
	...
	_on($scope["#button/2"], "click", function() {
		$label($scope, $scope.label === "none" ? "some" : "none");
	});
	_on($scope["#button/4"], "click", function() {
		$fromInput($scope, $scope.fromInput + $scope.count);
	});
});
