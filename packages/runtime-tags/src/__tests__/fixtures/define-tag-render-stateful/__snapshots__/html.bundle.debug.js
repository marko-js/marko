...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	const MyTag = { content: _content("__tests__/template.marko_1_content", ({ name, count }) => {
		...
		_html_opens("__tests__/template.marko:7:3"), _html(`<div>Hello ${_sep($sg__name)}${_escape(name)}${_el_resume($scope1_id, "#text/0", $sg__name)} ${_sep($sg__count)}${_escape(count)}${_el_resume($scope1_id, "#text/1", $sg__count)}</div>`);
