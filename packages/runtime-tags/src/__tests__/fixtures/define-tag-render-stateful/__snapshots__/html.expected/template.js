  ...
  const MyTag = {
    content: _._content("__tests__/template.marko_1_content", ({
      ...
      const $scope1_reason = _._scope_reason(),
        $sg__name = _._serialize_guard($scope1_reason, /* name */1),
        $sg__count = _._serialize_guard($scope1_reason, /* count */2);
      _._html(`<div>Hello ${_._sep(($sg__name))}${_._escape(name)}${_._el_resume($scope1_id, "#text/0", ($sg__name))} ${_._sep(($sg__count))}${_._escape(count)}${_._el_resume($scope1_id, "#text/1", ($sg__count))}</div>`);
