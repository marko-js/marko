...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	const MyTag = { content: _content("__tests__/template.marko_1_content", (a, b) => {
		...
		_html_opens("__tests__/template.marko:4:3"), _html(`<div>${_escape(a)}${_el_resume($scope1_id, "#text/0", _serialize_guard($scope1_reason, 1))}|${_sep($sg__b)}${_escape(b)}${_el_resume($scope1_id, "#text/1", $sg__b)}|${_sep($sg__input)}${_escape(JSON.stringify(input))}${_el_resume($scope1_id, "#text/2", $sg__input)}</div>`);
	...
	_html_opens("__tests__/template.marko:18:1"), _html(`<button>inc <!>${_escape(x)}${_el_resume($scope0_id, "#text/4")}</button>${_el_resume($scope0_id, "#button/3")}`);
