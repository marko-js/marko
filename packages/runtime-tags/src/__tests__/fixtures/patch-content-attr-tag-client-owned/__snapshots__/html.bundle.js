...
const $template = "<section><!><button>+</button></section>";
const $walks = "D%b l";
...
var card_default = _template_patch("b", (input) => {
	...
	$scope0_page ? _scope($scope0_id, {
		e: input.header,
		f: open
	}) : _filled_guard($scope0_reason, 0) && _patch_value($scope0_id, "b0", input.header);
}, 0, 0);

// template.marko
_shells({ a: /*@__PURE__*/ ((_w0, _w1) => `a !;${_w0};${_w1}`)(((_w0) => `D/${_w0}&l`)($walks), ((_w0) => `<main>${_w0}</main>`)($template)) });
var template_default = _template_patch("a", (input) => {
	...
	card_default({ header: attrTag({ content: _content_resume("a1", () => {
		_scope_reason();
		const $scope1_id = _scope_id();
		_html(`<em>${_text_resume($scope1_id, "a", input.note, $sg__input_note)}</em>`);
		_subscribe(_source_if($scope0_reason, 0) && $input_note__closures, _scope($scope1_id, { _: _scope_with_id($scope0_id) }), _client_guard($scope0_reason, 0) && "a0", $sg__input_note);
