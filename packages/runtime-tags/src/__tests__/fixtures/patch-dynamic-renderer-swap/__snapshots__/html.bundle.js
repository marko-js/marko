// card-a.marko
_shells({ a: "a;D ;<section class=a> </section>" });
var card_a_default = _template_patch("a", (input) => {
	const $scope0_reason = _scope_reason(), $scope0_page = _page_render();
	const $scope0_id = _scope_id();
	_html(`<section class=a>${_patch_text($scope0_id, "a", input.label, void 0, $scope0_reason, 0)}</section>`);
	$scope0_page && _scope($scope0_id, {});
}, 0, 0);

// card-b.marko
_shells({ b: "b;D%;<article class=b><!>!</article>" });
var card_b_default = _template_patch("b", (input) => {
	const $scope0_reason = _scope_reason(), $scope0_page = _page_render();
	const $scope0_id = _scope_id();
	_html(`<article class=b>${_patch_text($scope0_id, "a", input.label, void 0, $scope0_reason, 0)}!</article>`);
	$scope0_page && _scope($scope0_id, {});
}, 0, 0);

// template.marko
_shells({ c: "c !c0;D%b D ;<main><!><button> </button></main>" });
var template_default = _template_patch("c", (input) => {
	const $scope0_reason = _scope_reason(), $scope0_page = _page_render();
	const $scope0_id = _scope_id();
	let n = 0;
	_html("<main>");
	const $tag = input.mode === "a" ? card_a_default : card_b_default;
	const $input2 = { label: input.label };
	_dynamic_tag($scope0_id, "a", $tag, $input2, 0, 0, _source_guard($scope0_reason, 0), _patch_dynamic_tag($scope0_id, "a", $tag, $input2, 0, 0, $scope0_reason, 0));
	_html(`<button>${_text_resume($scope0_id, "c", n)}</button>${_el_resume($scope0_id, "b")}</main>`);
	_script($scope0_id, "c0");
	$scope0_page && _scope($scope0_id, {
		f: _source_if($scope0_reason, 2) && input.mode,
		g: _source_if($scope0_reason, 1) && input.label,
		i: n
	});
}, 1, 1);
