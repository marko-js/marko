...
var box_default = _template_patch("__tests__/box.marko", (input) => {
	const $scope0_reason = _scope_reason(), $scope0_page = _page_render();
	const $scope0_id = _scope_id();
...
var template_default = _template_patch("__tests__/template.marko", (input) => {
	...
	_dynamic_tag($scope0_id, "#text/0", $tag, {}, _content_elide("__tests__/template.marko_1*content", () => {
		...
		box_default({
			...
			content: _content_elide("__tests__/template.marko_2*content", () => {
				...
				_html(_patch_text($scope2_id, "#text/0", input.label, void 0, $scope0_reason, 1));
				_subscribe(_unfilled_if($scope0_reason, 1) && $input_label__closures, _scope($scope2_id, { _: _scope_with_id($scope1_id) }, "__tests__/template.marko", "5:6"));
			}, $scope1_id)
		});
	...
	}, $scope0_id), 0, _source_guard($scope0_reason, 0), _patch_dynamic_tag($scope0_id, "#text/0", $tag, 0, "__tests__/template.marko_1*content", 0, $scope0_reason, 0));
