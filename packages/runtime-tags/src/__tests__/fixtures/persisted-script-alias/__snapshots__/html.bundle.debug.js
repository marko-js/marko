...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	const alias = copy;
	_html(`<h1>${_patch_text($scope0_id, "#text/0", input.title, void 0, $scope0_reason, 1)}</h1>`);
	_script($scope0_id, "__tests__/template.marko_0_alias#5");
	_patch_effect($scope0_id, "__tests__/template.marko_0_alias#5", "copy");
	$scope0_page ? _scope($scope0_id, { copy }, "__tests__/template.marko", 0, { copy: "1:8" }) : _filled_guard($scope0_reason, 0) && _patch_write($scope0_id, "copy", copy);
}, 1, 0);
