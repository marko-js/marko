...
var custom_tag_default = _template("__tests__/tags/custom-tag/index.marko", (input) => {
	...
	const { x, y } = input;
	_html(`<div>x: ${_sep($sg__input_x_value)}${_escape(x?.value)}${_el_resume($scope0_id, "#text/0", $sg__input_x_value)} y: ${_sep($sg__input_y_value)}${_escape(y?.value)}${_el_resume($scope0_id, "#text/1", $sg__input_y_value)}</div>`);
