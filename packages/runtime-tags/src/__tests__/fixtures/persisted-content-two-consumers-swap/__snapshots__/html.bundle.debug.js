...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	card_default({
		...
		content: _content_elide("__tests__/template.marko_1*content", () => {
			...
			_html$1(`<em>${_patch_text($scope1_id, "#text/0", input.note, $scope0_owned, 4)}${_el_resume($scope1_id, "#text/0")}</em>`);
			_subscribe($si__input_note && $input_note__closures, writeScope($scope1_id, { _: _scope_with_id($scope0_id) }, "__tests__/template.marko", "2:4"));
		...
		if (input.show) {
			...
			card_default({
				...
				content: _content_elide("__tests__/template.marko_3*content", () => {
					...
					_html$1(`<em>${_patch_text($scope3_id, "#text/0", input.note, $scope0_owned, 4)}${_el_resume($scope3_id, "#text/0")}</em>`);
