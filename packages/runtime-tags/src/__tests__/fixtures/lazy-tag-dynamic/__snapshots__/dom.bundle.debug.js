...
const $show__OR__value = /*@__PURE__*/ _or(5, ($scope) => $dynamicTag($scope, ("show" in $scope ? "show" in $scope ? $scope.show : true : true) ? Child : null, () => ({
	label: "x",
	value: "value" in $scope ? "value" in $scope ? $scope.value : 1 : 1
})));
...
const $setup__script = _script("__tests__/template.marko_0", ($scope) => {
	...
	_on($scope["#button/1"], "click", function() {
		$value($scope, ("value" in $scope ? $scope.value : 1) + 1);
	});
});
