...
const $inputtype_content = _content("__tests__/tags/heading.marko_2*content", "depth <!>", "b%", $inputtype_content__setup);
_content_resume($inputtype_content);
const $if_content__dynamicTag = /*@__PURE__*/ _dynamic_tag("#text/2", $inputtype_content);
const $if_content__input_type = /*@__PURE__*/ _if_closure("#text/0", 0, ($scope) => $if_content__dynamicTag($scope, $scope._.input_type));
const $if_content__setup__script = _script("__tests__/tags/heading.marko_1", ($scope) => _on($scope["#button/0"], "click", function() {
	$count($scope._, +$scope._.count + 1);
}));
const $if_content__setup = ($scope) => {
	$if_content__input_type._($scope);
...
const $if = /*@__PURE__*/ _if("#text/0", "<button id=inc> </button><!><!>", " D l%", $if_content__setup);
