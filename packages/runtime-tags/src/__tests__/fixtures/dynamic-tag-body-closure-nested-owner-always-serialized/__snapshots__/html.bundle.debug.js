...
var heading_default = _template("__tests__/tags/heading.marko", (input) => {
	const $scope0_reason = _scope_reason(), $sg__input_depth = _serialize_guard($scope0_reason, 4), $si__input_show__OR__input_type__OR__input_depth = _serialize_if($scope0_reason, 1), $sg__input_show = _serialize_guard($scope0_reason, 2), $si__input_depth = _serialize_if($scope0_reason, 4);
	const $scope0_id = _scope_id();
	...
	_if(() => {
		if (input.show) {
			...
			_dynamic_tag($scope1_id, "#text/2", input.type, {}, _content_resume("__tests__/tags/heading.marko_2*content", () => {
				const $scope2_id = _scope_id();
				const $scope2_reason = _scope_reason();
				_html(`depth ${_text_resume($scope2_id, "#text/0", input.depth, $sg__input_depth * 2)}`);
				$si__input_show__OR__input_type__OR__input_depth && _subscribe($si__input_depth && $input_depth__closures, _scope($scope2_id, { _: _scope_with_id($scope1_id) }, "__tests__/tags/heading.marko", "4:6"), "__tests__/tags/heading.marko_2_input_depth#5/subscribe", $sg__input_depth);
				$sg__input_depth || $si__input_show__OR__input_type__OR__input_depth && _resume_branch($scope2_id);
			}, $scope1_id, ($scope) => [{ input_depth: input.depth }, { _: $scope($scope0_id) }]), 0, _serialize_guard($scope0_reason, 3));
			_script($scope1_id, "__tests__/tags/heading.marko_1");
			_scope($scope1_id, { _: _scope_with_id($scope0_id) }, "__tests__/tags/heading.marko", "2:2");
			return 0;
		}
