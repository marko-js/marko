...
var template_default = _template_patch("__tests__/template.marko", (input) => {
	...
	let count = 0;
	_html(`<main><h1>${_patch_text($scope0_id, "#text/0", input.heading, void 0, $scope0_reason, 1)}</h1>`);
	_if(() => {
		if (input.show) {
			...
			_if(() => {
				if (input.inner) {
					...
					_subscribe($count__closures, _subscribe(_source_if($scope0_reason, 4) && $input_title__closures, _scope($scope2_id, { _: _scope_with_id($scope1_id) }, "__tests__/template.marko", "5:6"), _client_guard($scope0_reason, 4) && "__tests__/template.marko_2_input_title#8/subscribe"), "__tests__/template.marko_2_count#9/subscribe");
					return 0;
				}
