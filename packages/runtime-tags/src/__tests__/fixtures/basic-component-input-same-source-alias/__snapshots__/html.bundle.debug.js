...
var my_button_default = _template("__tests__/tags/my-button.marko", (input) => {
	...
	_html(`<button>${_escape(text)}${_el_resume($scope0_id, "#text/1", $sg__input_text)} ${_sep($sg__input_text)}${_escape(textAlias)}${_el_resume($scope0_id, "#text/2", $sg__input_text)}</button>${_el_resume($scope0_id, "#button/0")}`);
