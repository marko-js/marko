		...
		if (input.show) {
			...
			card_default({
				...
				content: _content_elide("__tests__/template.marko_2*content", () => {
					...
					_html$1(`<em>${_patch_text($scope2_id, "#text/0", input.note, $scope0_owned, 5)}${_el_resume($scope2_id, "#text/0")}</em>`);
					_subscribe(_source_if($scope0_reason, 5) && $input_note__closures, writeScope($scope2_id, { _: _scope_with_id($scope1_id) }, "__tests__/template.marko", "3:6"));
