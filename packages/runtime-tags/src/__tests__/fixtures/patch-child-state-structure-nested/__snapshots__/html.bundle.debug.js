...
var panel_default = _template_patch("__tests__/tags/panel/index.marko", (input) => {
	const $scope0_reason = _scope_reason(), $sg__input_inner = _source_guard($scope0_reason, 3), $scope0_page = _page_render(), $sg__input_show = _source_guard($scope0_reason, 2);
		...
		if (input.show) {
			...
			_if(() => {
				if (input.inner) {
					...
					_subscribe(_unfilled_if($scope0_reason, 4) && $input_title__closures, _scope($scope2_id, { _: _scope_with_id($scope1_id) }, "__tests__/tags/panel/index.marko", "3:6"), _client_guard($scope0_reason, 4) && "__tests__/tags/panel/index.marko_2_input_title#5/subscribe");
					return 0;
				}
			}, $scope1_id, "#section/0", 1, $sg__input_inner, $sg__input_inner, void 0, void 0, ["__tests__/tags/panel/index.marko_2*shell"], $scope0_reason, 3);
	...
	}) : (_filled_guard($scope0_reason, 3) && _client_guard($scope0_reason, 2) && _patch_value($scope0_id, "__tests__/tags/panel/index.marko0", input.inner), _filled_guard($scope0_reason, 4) && (_client_guard($scope0_reason, 3) || _client_guard($scope0_reason, 2)) && _patch_value($scope0_id, "__tests__/tags/panel/index.marko1", input.title));
}, 0, 0);
...
var template_default = _template_patch("__tests__/template.marko", (input) => {
	...
	let count = 0;
	_html("<main>");
	_set_serialize_reason(62 | (_mask_group($scope0_reason, 0) << 7 | _mask_group($scope0_reason, 1) << 9));
	...
	}, "__tests__/template.marko", 0, { count: "1:6" }) : _filled_guard($scope0_reason, 1) && _patch_value($scope0_id, "__tests__/template.marko0", input.title);
}, 1, () => [panel_default]);
