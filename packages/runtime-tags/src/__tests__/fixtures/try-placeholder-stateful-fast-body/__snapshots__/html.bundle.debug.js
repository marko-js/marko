...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	_try($scope0_id, "#text/0", _content_resume("__tests__/template.marko_2*content", () => {
		...
		_await($scope2_id, "#text/0", resolveAfter("done", 1), () => {
			...
			_html(`<button>loaded <!>${_escape(clicks)}${_el_resume($scope3_id, "#text/1")}</button>${_el_resume($scope3_id, "#button/0")}`);
		...
		_html(`<button>loading <!>${_escape(clicks)}${_el_resume($scope1_id, "#text/1")}</button>${_el_resume($scope1_id, "#button/0")}`);
