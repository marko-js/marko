...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
		...
		if (count > 1) {
			...
			if ($scope0_reason) _if(() => {
				if (on) {
					...
					_subscribe(_source_if($scope0_reason, 1) && $input_inner__closures, _scope($scope2_id, {}, "__tests__/template.marko", "6:6"));
					return 0;
				}
	...
	$scope0_reason ? _scope($scope0_id, {
		...
		count: "1:6",
		on: "2:6"
	}) : (_filled_guard($scope0_owned, 0) && _patch_value($scope0_id, "__tests__/template.marko0", input.outer), _filled_guard($scope0_owned, 1) && _patch_value($scope0_id, "__tests__/template.marko1", input.inner));
}, 1, 0);
