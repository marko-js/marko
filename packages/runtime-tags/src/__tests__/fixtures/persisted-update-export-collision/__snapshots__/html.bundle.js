...
var template_default = _template("a", (input) => {
	...
	_if(() => input.id ? 0 : 1, $scope0_id, "c", $sg__input_id, $sg__input_id, $sg__input_id, 0, 1, "a1", [() => {
		...
		_html(`<p>${_escape(_hole_value($scope1_id, "Qa", createPatch(input.id), _persisted_reason()))}${_el_resume($scope1_id, "a", $sg__input_id)} / ${_sep($sg__input_id)}${_escape(_hole_value($scope1_id, "Qb", have(input.id), _persisted_reason()))}${_el_resume($scope1_id, "b", $sg__input_id)}</p>`);
