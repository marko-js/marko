...
var template_default = _template_persisted("a", (input) => {
	...
	_if(() => {
		if (input.show) {
			...
			_filled_guard($scope0_reason, 2) && _patch_write($scope1_id, "c", label);
			_html(`<p>${_patch_text($scope1_id, "a", label, void 0, $scope0_reason, 2)}</p><button>c</button>${_el_resume($scope1_id, "b")}`);
