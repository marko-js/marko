...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	_if(() => {
		if (input.show) {
			...
			_filled_guard($scope0_reason, 2) && _patch_write($scope1_id, "label", label);
			_html(`<p>${_patch_text($scope1_id, "#text/0", label, void 0, $scope0_reason, 2)}</p><button>c</button>${_el_resume($scope1_id, "#button/1")}`);
