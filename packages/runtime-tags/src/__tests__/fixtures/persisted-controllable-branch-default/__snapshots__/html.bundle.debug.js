...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	_if(() => {
		if (input.show) {
			...
			_html(`<input${_attr_input_value($scope1_id, "#input/0", input.value)}${_patch_control($scope1_id, "#input/0", 2, input.value, $scope0_reason, 3)}>${_el_resume($scope1_id, "#input/0")}`);
	...
	_html("</main>");
	$scope0_page && _scope($scope0_id, { input_value: _source_if($scope0_reason, 2) && input.value }, "__tests__/template.marko", 0, { input_value: ["input.value"] });
}, 1, 0);
