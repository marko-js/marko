...
var template_default = _template_persisted("a", (input) => {
	...
	_if(() => {
		if (input.show) {
			...
			_html(`<input${_attr_input_value($scope1_id, "a", input.value)}${_patch_control($scope1_id, "a", 2, input.value, $scope0_owned, 3)}>${_el_resume($scope1_id, "a")}`);
			_scope($scope1_id, { _: _scope_with_id($scope0_id) });
			return 0;
		}
	}, $scope0_id, "b", 1, $sg__input_show, $sg__input_show, void 0, void 0, ["a0"], $scope0_owned, 2);
	_html("</main>");
	$scope0_reason && _scope($scope0_id, { g: _source_if($scope0_reason, 2) && input.value });
}, 1, 0);
