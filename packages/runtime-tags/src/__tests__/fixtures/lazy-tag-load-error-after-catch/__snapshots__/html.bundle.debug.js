...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	let show = false;
	_html(`<button id=toggle>toggle</button>${_el_resume($scope0_id, "#button/0")}<button id=load>load</button>`);
	_try($scope0_id, "#text/1", _content_resume("__tests__/template.marko_1*content", () => {
		...
		_if(() => {
			if (show) {
				...
				_await($scope4_id, "#text/2", rejectAfter(new Error("rejected"), 1), (value) => {
		...
		_subscribe($show__closures, _scope($scope1_id, { _: _scope_with_id($scope0_id) }, "__tests__/template.marko", "7:2"), "__tests__/template.marko_1_show#2/subscribe");
	}, $scope0_id), {
