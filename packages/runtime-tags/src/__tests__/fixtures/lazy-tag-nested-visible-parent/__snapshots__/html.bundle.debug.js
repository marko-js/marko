...
var grand_child_default = _template("__tests__/grand-child.marko", (input) => {
	...
	let copy = input.obj;
	_html_opens("__tests__/grand-child.marko:2:1"), _html(`<button class=grand>grand:<!>${_escape(copy.name)}${_el_resume($scope0_id, "#text/1")}</button>${_el_resume($scope0_id, "#button/0")}`);
...
var child_default = _template("__tests__/child.marko", (input) => {
	...
	const $scope0_id = _scope_id();
	let obj = { name: "shared" };
	_html_opens("__tests__/child.marko:4:1"), _html(`<button class=child>child:<!>${_escape(obj.name)}${_el_resume($scope0_id, "#text/1")}</button>${_el_resume($scope0_id, "#button/0")}`);
