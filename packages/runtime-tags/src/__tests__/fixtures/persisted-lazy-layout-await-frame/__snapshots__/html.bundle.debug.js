...
var icon_default = _template_persisted("__tests__/tags/icon.marko", (input) => {
	...
	const $scope0_id = _scope_id();
	_html(`<svg${_patch_attr($scope0_id, "#svg/0", "viewBox", input.name, $scope0_owned, 0)}>`);
	_if(() => {
		if (input.name) {
			const $scope1_id = _scope_id();
			_html(`<path${_patch_attr($scope1_id, "#path/0", "d", input.name, $scope0_owned, 0)}></path>${_el_resume($scope1_id, "#path/0")}`);
			_scope($scope1_id, { _: _scope_with_id($scope0_id) }, "__tests__/tags/icon.marko", "2:4");
			return 0;
		}
	}, $scope0_id, "#svg/0", 1, 1, _source_guard($scope0_reason, 0), void 0, void 0, ["__tests__/tags/icon.marko_1*shell"], $scope0_owned, 0);
	_html(`</svg>${_el_resume($scope0_id, "#svg/0")}`);
	$scope0_reason && _scope($scope0_id, { input_name: _source_if($scope0_reason, 0) && input.name }, "__tests__/tags/icon.marko", 0, { input_name: ["input.name"] });
}, 0, 0);
...
var docs_default = _template_persisted("__tests__/docs.marko", (input) => {
	...
	_await($scope0_id, "#text/0", input.list, (list) => {
		...
		_for_of(list, (item) => {
			...
			_html(`<a>${_patch_text($scope3_id, "#text/0", item, void 0, $scope0_owned, 0)}</a>`);
			_scope($scope3_id, {}, "__tests__/docs.marko", "3:6");
		}, 0, $scope1_id, "#text/0", 1, $sg__input_list, $sg__input_list, void 0, void 0, "__tests__/docs.marko_3*shell", $scope0_owned, 0);
		_if(() => {
			if (list[0]) {
		...
		}, $scope1_id, "#text/1", 1, $sg__input_list, $sg__input_list, void 0, void 0, ["__tests__/docs.marko_2*shell"], $scope0_owned, 0);
		$scope0_reason && _scope($scope1_id, { list_0: _source_if($scope0_reason, 0) && list?.[0] }, "__tests__/docs.marko", "2:4", { list_0: ["list[0]", "2:10"] });
...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	root_default({ content: _content_elide("__tests__/template.marko_1*content", () => {
			...
			if (input.page === 0) {
				...
				$Docs_withLoadAssets({
					...
					content: _content_elide("__tests__/template.marko_3*content", () => {
						...
						}, $scope3_id, "#text/0", 1, $sg__input_page, $sg__input_page, void 0, void 0, ["__tests__/template.marko_5*shell", "__tests__/template.marko_6*shell"], $scope0_owned, 1);
		...
		}, $scope1_id, "#text/0", 1, $sg__input_page, $sg__input_page, void 0, void 0, ["__tests__/template.marko_4*shell", "__tests__/template.marko_2*shell"], $scope0_owned, 1);
		$scope0_reason && _subscribe(_unfilled_if($scope0_owned, 1) && $input_page__closures, _scope($scope1_id, { _: _scope_with_id($scope0_id) }, "__tests__/template.marko", "6:2"));
