...
var child_default = _template("__tests__/child.marko", (input) => {
	...
	_html_opens("__tests__/child.marko:2:1"), _html(`<button>${_escape(input.label)}${_el_resume($scope0_id, "#text/1", _serialize_guard($scope0_reason, 0))}: <!>${_escape(count)}${_el_resume($scope0_id, "#text/2")}</button>${_el_resume($scope0_id, "#button/0")}`);
