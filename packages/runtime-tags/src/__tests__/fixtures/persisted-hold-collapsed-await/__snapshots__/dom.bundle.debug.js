...
const $if_content__walks = "b%c", $if_content__template = "<!><!><!>", $try_content__walks = "b%c", $try_content__template = "<!><!><!>", $await_content__walks = "D l", $await_content__template = "<p class=note> </p>";
const $template = "<h1> </h1><button class=collapse>toggle</button><!><!>";
const $walks = "D l b%c";
_enable_catch();
...
const $try_content__input_note = /*@__PURE__*/ _closure_get("input_note", ($scope) => {
	if (!updating) $try_content__await_promise($scope, resolveAfter($scope._._.input_note, 1));
}, ($scope) => $scope._._);
...
const $collapsed = _var_resume("__tests__/template.marko_0_collapsed/var", /*@__PURE__*/ _let_persisted("collapsed/7", ($scope) => $if($scope, !$scope.collapsed ? 0 : 1)));
const $setup__script = _script_shared(($scope) => _on($scope["#button/1"], "click", function() {
	$collapsed($scope, !$scope.collapsed);
}));
...
const $collapsed_seed = _update_signal("__tests__/template.marko_0_collapsed/var");
...
const $collapsed = /*@__PURE__*/ _let_persisted("collapsed/7", ($scope) => $if($scope, !$scope.collapsed ? 0 : 1));
