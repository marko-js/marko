...
export default _._template("__tests__/template.marko", input => {
  ...
  _._try($scope0_id, "#text/0", _._content_resume("__tests__/template.marko_1_content", () => {
    ...
    _._await($scope1_id, "#text/0", rejectAfter(new Error("failed"), 1), data => {
      const $scope3_id = _._scope_id();
      _._html(`<div>Should not see this: ${_._escape(data)}</div>`);
    }, 0);
  }, $scope0_id), {
    catch: _.attrTag({
      content: _._content_resume("__tests__/template.marko_2_content", err => {
        ...
        _._serialize_if($scope2_reason, /* err.message */0) && _._scope($scope2_id, {}, "__tests__/template.marko", "10:6");
      }, $scope0_id)
    })
  });
