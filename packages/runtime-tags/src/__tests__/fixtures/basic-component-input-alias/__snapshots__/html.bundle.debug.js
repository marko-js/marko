...
var my_button_default = _template("__tests__/tags/my-button.marko", (input) => {
	...
	const { text } = input;
	const { onClick } = attrs;
	_html_opens("__tests__/tags/my-button.marko:4:1"), _html(`<button>${_escape(text)}${_el_resume($scope0_id, "#text/1", _serialize_guard($scope0_reason, 0))}</button>${_el_resume($scope0_id, "#button/0")}`);
