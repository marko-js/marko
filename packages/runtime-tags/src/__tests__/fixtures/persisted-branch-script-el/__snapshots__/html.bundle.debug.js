...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	_if(() => {
		if (input.show) {
			...
			_html(`<span>${_patch_text($scope1_id, "#text/1", input.label, void 0, $scope0_reason, 3)}</span>${_el_resume($scope1_id, "#span/0")}`);
	...
	_html("</main>");
	$scope0_page && _scope($scope0_id, { input_label: _source_if($scope0_reason, 2) && input.label }, "__tests__/template.marko", 0, { input_label: ["input.label"] });
}, 1, 0);
