	...
	if ($scope0_page) _if(() => {
		if (open) {
			...
			box_default({ content: _content_elide("__tests__/template.marko_2*content", () => {
				...
				_html(`<p>${_text_resume($scope2_id, "#text/0", "t:" + input.title, $sg__input_title)}</p>`);
				_subscribe(_source_if($scope0_reason, 0) && $input_title__closures, _scope($scope2_id, { _: _scope_with_id($scope1_id) }, "__tests__/template.marko", "4:6"));
