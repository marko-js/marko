...
var template_default = _template_persisted("a", (input) => {
	...
	_for_of(input.items, (item) => {
		...
		card_default({ content: _content_elide("a0", () => {
			...
			_subscribe(_unfilled_if($scope0_owned, 1) && $for_content__item__closures, _subscribe(_unfilled_if() && $global_brand__closures, _subscribe(_unfilled_if($scope0_owned, 2) && $input_prefix__closures, _scope($scope2_id, { _: _scope_with_id($scope1_id) }))));
		}, $scope1_id) });
		_scope($scope1_id, {
