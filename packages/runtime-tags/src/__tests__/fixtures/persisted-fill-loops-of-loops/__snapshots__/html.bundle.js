...
var template_default = _template_persisted("a", (input) => {
	...
	_for_of(input.rows, (row) => {
		...
		_for_of(row.cells, (cell) => {
			...
			_filled_guard($scope0_reason, 0) ? _patch_value($scope2_id, "a1", cell) : _patch_init($scope2_id, "a2");
	...
	$scope0_page ? _scope($scope0_id, {
		...
		i: $count__closures
	}) : _filled_guard($scope0_reason, 1) && _patch_value($scope0_id, "a0", input.suffix);
}, 1, 0);
