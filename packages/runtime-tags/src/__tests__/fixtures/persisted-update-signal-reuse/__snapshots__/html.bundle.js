...
var template_default = _template("a", (input) => {
	...
	let highlight = false;
	_html(`<button>toggle</button>${_el_resume($scope0_id, "a")}<div${_attr_class(highlight)}>${_escape(input.label)}${_el_resume($scope0_id, "c", _serialize_guard($scope0_reason, 0))}</div>${_el_resume($scope0_id, "b", 1 | _persisted_reason())}<span>${_escape(_hole_value($scope0_id, "d", input.count, _persisted_reason()))}${_el_resume($scope0_id, "d", _serialize_guard($scope0_reason, 1))} items</span>`);
