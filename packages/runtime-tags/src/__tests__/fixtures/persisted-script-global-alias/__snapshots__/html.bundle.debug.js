...
var template_default = _template_persisted("__tests__/template.marko", (input) => {
	...
	_html(`<main><h1>${_patch_text($scope0_id, "#text/0", brand)}${_el_resume($scope0_id, "#text/0")}</h1></main>`);
	_script($scope0_id, "__tests__/template.marko_0_brand#1");
	_patch_effect($scope0_id, "__tests__/template.marko_0_brand#1", "brand");
	$scope0_reason ? writeScope($scope0_id, { brand }, "__tests__/template.marko", 0, { brand: "1:8" }) : _patch_write($scope0_id, "brand", brand);
}, 1, 1);
