...
var custom_tag_default = _template("__tests__/tags/custom-tag.marko", (input) => {
	...
	_html_opens("__tests__/tags/custom-tag.marko:1:1"), _html(`<div>Child: ${_sep($sg__input)}${_escape(input)}${_el_resume($scope0_id, "#text/0", $sg__input)}</div>`);
	const $return = input;
...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	let y = _dynamic_tag($scope0_id, "#text/2", tags[0], [x], 0, 1, void 0, "__tests__/template.marko:5:1");
	_var($scope0_id, "#scopeOffset/3", $tags0_scope, "__tests__/template.marko_0_y/var");
	_html_opens("__tests__/template.marko:6:1"), _html(`<div>Parent: <!>${_escape(y)}${_el_resume($scope0_id, "#text/4")}</div>`);
