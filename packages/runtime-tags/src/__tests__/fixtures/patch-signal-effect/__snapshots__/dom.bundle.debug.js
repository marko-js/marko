...
const $walks = " b b";
const $input_label__OR__count__script = _script("__tests__/template.marko_0_input_label#4_count#5", ($scope) => {
	{
		const ac = new AbortController();
		_el_read($scope["#div/0"]).dataset.v = `${$scope.input_label}:${$scope.count}`;
		$signal($scope, 0).addEventListener("abort", () => {
			_el_read($scope["#div/0"]).dataset.v = "gone";
		}, { signal: ac.signal });
	}
});
