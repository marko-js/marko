...
var template_default = _template_patch("__tests__/template.marko", (input) => {
	...
	const row = { content: _content("__tests__/template.marko_1*content", (label) => {
		const $scope1_id = _scope_id();
		const $scope1_reason = _scope_reason();
		_html(`<li>${_patch_text($scope1_id, "#text/0", label)}: ${_patch_text($scope1_id, "#text/1", input.suffix, 2, $scope0_reason, 1)}</li>`);
		_subscribe(_unfilled_if($scope0_reason, 1) && $input_suffix__closures, _scope($scope1_id, { _: _scope_with_id($scope0_id) }, "__tests__/template.marko", "1:2"), _client_guard($scope0_reason, 1) && "__tests__/template.marko_1_input_suffix#3/subscribe");
	}, $scope0_id) };
	_html("<ul>");
	_if(() => {
		if (input.show) {
	...
	}, $scope0_id, "#ul/0", 1, $sg__input_show, $sg__input_show, void 0, void 0, ["__tests__/template.marko_2*shell"], $scope0_reason, 2);
	_html(`</ul>${_el_resume($scope0_id, "#ul/0", $sg__input_show)}`);
	$scope0_page && _scope($scope0_id, {
		input_suffix: _source_if($scope0_reason, 2) && input.suffix,
		"ClosureScopes:input_suffix": $input_suffix__closures
	}, "__tests__/template.marko", 0, { input_suffix: ["input.suffix"] });
}, 1, 1);
