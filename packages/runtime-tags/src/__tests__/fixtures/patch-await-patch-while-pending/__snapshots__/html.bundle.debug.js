...
var template_default = _template_patch("__tests__/template.marko", (input) => {
	...
	_try($scope0_id, "#text/2", _content_resume("__tests__/template.marko_1*content", () => {
		...
		_await($scope1_id, "#text/0", input.promise, () => {
			...
			_subscribe(_unfilled_if($scope0_reason, 2) && $input_msg__closures, _scope($scope2_id, { _: _scope_with_id($scope1_id) }, "__tests__/template.marko", "4:4"), _client_guard($scope0_reason, 2) && "__tests__/template.marko_2_input_msg#6/subscribe");
		}, 1, "__tests__/template.marko_2*content", 1);
		$scope0_page && _subscribe(_unfilled_if($scope0_reason, 1) && $input_promise__closures, _scope($scope1_id, { _: _scope_with_id($scope0_id) }, "__tests__/template.marko", "3:2"), _client_guard($scope0_reason, 1) && "__tests__/template.marko_1_input_promise#5/subscribe", 0);
