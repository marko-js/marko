...
var child_default = _template_persisted("a", (input) => {
	...
	_for_of([1, 2], (i) => {
		...
		_html(`<span>${_patch_text($scope1_id, "a", input.label, void 0, $scope0_owned, 0)}:${_text_resume($scope1_id, "b", count + i, 2)}</span>`);
		_scope($scope1_id, {
			d: i,
...
var template_default = _template_persisted("b", (input) => {
	...
	}, $scope0_id, "c", 1, $sg__input_show, $sg__input_show, void 0, void 0, ["b0"], $scope0_owned, 1);
	_html(`</main>${_el_resume($scope0_id, "c", $sg__input_show)}<p>${_patch_text($scope0_id, "d", input.label, void 0, $scope0_owned, 2)}</p>`);
