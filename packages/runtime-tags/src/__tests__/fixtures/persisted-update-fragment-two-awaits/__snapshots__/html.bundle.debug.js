	...
	const Reports = { content: _content("__tests__/template.marko_2_content", () => {
		...
		_try($scope2_id, "#text/0", _content_resume("__tests__/template.marko_3_content", () => {
			...
			_await($scope3_id, "#text/0", getReport($global().topic), (report) => {
				...
				_html(`<p class=report>${_escape(_hole_value($scope5_id, "UpdateHole:#text/0", report, _persisted_reason()))}${_el_resume($scope5_id, "#text/0", _persisted_reason())}</p>`);
			...
			_await($scope3_id, "#text/1", getSummary($global().topic), (summary) => {
				...
				_html(`<p class=summary>${_escape(_hole_value($scope6_id, "UpdateHole:#text/0", summary, _persisted_reason()))}${_el_resume($scope6_id, "#text/0", _persisted_reason())}</p>`);
