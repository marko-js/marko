...
var price_default = _template("__tests__/tags/price.marko", (input) => {
	...
	_html(`<span class=price${_attr("title", _hole_value($scope0_id, "PatchAttr:title:#span/0", `$${input.amount.toFixed(2)}`, _persisted_reason()))}>$${_sep($sg__input_amount)}${_escape(_hole_value($scope0_id, "PatchHole:#text/1", input.amount.toFixed(2), _persisted_reason()))}${_el_resume($scope0_id, "#text/1", $sg__input_amount)}</span>${_el_resume($scope0_id, "#span/0", $sg__input_amount)}`);
	$sg__input_amount && writeScope($scope0_id, {}, "__tests__/tags/price.marko", 0);
});
_renderer_shells({
	"__tests__/tags/price.marko_0_update": ["<span class=price>$<!></span>", " Db%l"],
	"__tests__/tags/price.marko": ["<span class=price>$<!></span>", " Db%l"]
});
...
var template_default = _template("__tests__/template.marko", (input) => {
	...
	_if(() => input.product.sale ? 0 : undefined, $scope0_id, "#section/4", $sg__input_product_sale, 1 | _persisted_reason(), $sg__input_product_sale, "</section>", 1, "__tests__/template.marko_0/update_if_#section/4", [() => {
		...
		_html(`<em>Save ${_sep($sg__input_product_sale_percent)}${_escape(_hole_value($scope1_id, "PatchHole:#text/0", input.product.sale.percent, _persisted_reason()))}${_el_resume($scope1_id, "#text/0", $sg__input_product_sale_percent)}%</em>`);
	...
	_for_of(input.related, (item) => {
		...
		return item.id;
	}, $scope0_id, "#ul/5", $sg__input_related, $sg__input_related, $sg__input_related, "</ul>", 1, "__tests__/template.marko_2_update");
...
_renderer_shells({
	...
	"__tests__/template.marko_1_update": ["<em>Save <!>%</em>", "Db%l"],
	"__tests__/template.marko_1_content": ["<em>Save <!>%</em>", "Db%l"],
