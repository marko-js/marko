...
var price_default = _template("b", (input) => {
	...
	_html(`<span class=price${_attr("title", _hole_value($scope0_id, "Ntitle:a", `$${input.amount.toFixed(2)}`, _persisted_reason()))}>$${_sep($sg__input_amount)}${_escape(_hole_value($scope0_id, "b", input.amount.toFixed(2), _persisted_reason()))}${_el_resume($scope0_id, "b", $sg__input_amount)}</span>${_el_resume($scope0_id, "a", $sg__input_amount)}`);
...
var template_default = _template("a", (input) => {
	...
	const $scope0_id = _scope_id();
	let expanded = false;
	_html(`<h1>${_escape(_hole_value($scope0_id, "a", input.product.name, _persisted_reason()))}${_el_resume($scope0_id, "a", _serialize_guard($scope0_reason, 0))}</h1><a${_attr("href", _hole_value($scope0_id, "Nhref:b", `/products/${input.product.slug}/specs`, _persisted_reason()))}>specs</a>${_el_resume($scope0_id, "b", _serialize_guard($scope0_reason, 1))}<button>Show${_el_resume($scope0_id, "d")}</button>${_el_resume($scope0_id, "c")}<section>`);
	_if(() => {
		if (input.product.sale) {
			const $scope1_id = _scope_id();
			_html(`<em>Save ${_sep($sg__input_product_sale_percent)}${_escape(_hole_value($scope1_id, "a", input.product.sale.percent, _persisted_reason()))}${_el_resume($scope1_id, "a", $sg__input_product_sale_percent)}%</em>`);
	...
	_for_of(input.related, (item) => {
		...
		_html(` ${_sep($sg__input_related)}${_escape(_hole_value($scope2_id, "b", item.name, _persisted_reason()))}${_el_resume($scope2_id, "b", $sg__input_related)}</li>`);
