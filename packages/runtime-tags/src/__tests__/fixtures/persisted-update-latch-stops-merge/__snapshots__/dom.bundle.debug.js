...
const $count = _var_resume("__tests__/template.marko_0_count/var", /*@__PURE__*/ _let_persisted("count/12", ($scope) => _text($scope["#text/1"], $scope.count)));
...
const $if = /*@__PURE__*/ _if("#text/4", "<p class=flagged>flag on</p>");
const $input_flag = ($scope, input_flag) => {
	if (!updating) $if($scope, input_flag ? 0 : 1);
};
...
const $template = "<button class=count>clicked <!></button><h1> </h1><!><!><!>";
const $walks = " Db%lD l%b%c";
const $count = /*@__PURE__*/ _let_persisted("count/12", ($scope) => _text($scope["#text/1"], $scope.count));
