	...
	if ($scope0_page || _must_render(panel_default)) {
		...
		panel_default({
			...
			body: attrTag({ content: _content_resume("__tests__/template.marko_1*content", () => {
				...
				_subscribe(_unfilled_if($scope0_reason, 0) && $input_title__closures, _scope($scope1_id, { _: _scope_with_id($scope0_id) }, "__tests__/template.marko", "4:6"), _client_guard($scope0_reason, 0) && "__tests__/template.marko_1_input_title#4/subscribe");
			}, $scope0_id) })
		});
	}
