...
var icon_default = _template_patch("__tests__/tags/icon.marko", (input) => {
	const $scope0_reason = _scope_reason(), $scope0_page = _page_render();
	const $scope0_id = _scope_id();
	_html(`<svg viewBox="0 0 1 1"><title>${_patch_text_content($scope0_id, "#title/0", input.name, _escape, $scope0_reason, 0)}</title>${_el_resume($scope0_id, "#title/0")}</svg>`);
	$scope0_page && _scope($scope0_id, {}, "__tests__/tags/icon.marko", 0);
}, 0, 0);
...
var template_default = _template_patch("__tests__/template.marko", (input) => {
	const $scope0_reason = _scope_reason(), $sg__input_items__OR__input_path = _source_guard($scope0_reason, 0), $sg__input_page = _source_guard($scope0_reason, 4), $sg__input_items = _source_guard($scope0_reason, 2), $scope0_page = _page_render();
	const $scope0_id = _scope_id();
	_html("<nav>");
	...
	}, 0, $scope0_id, "#nav/0", 1, $sg__input_items__OR__input_path, $sg__input_items, void 0, void 0, "__tests__/template.marko_1*shell", $scope0_reason, 2);
