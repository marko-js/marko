...
- **Optional feature enablement** (tree-shakable runtime API) has two patterns:
  - **`src/{dom,html}/**/*.feat.ts`** are compiler-injected side-effect modules for behavior a referenced import cannot keep alive (catch handling, controllable registration). The build emits them as extra entries of their runtime's bundle — a shared chunk keeps one state instance, so they import runtime internals directly. A feature body is direct registry/property assignments (plus at most one installer call, e.g. `installCatch`); the compiler emits it once per program via `importRuntimeFeature` (typed by `DOMRuntimeFeature`). Keep that a bare top-level `import "…"`, and never mark `.feat` modules side-effect free in `package.json`: `@marko/vite`'s linked build treats a browser module as side-effect free unless a template imports it bare, and bundlers still drop a bare import of a module its package marks side-effect free, silently losing features like `<try>` catching and controllables in production.
