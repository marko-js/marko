...
- **Hot paths** (SSR writer, serializer, resume): never `delete`, copy with spread/rest, `unshift`/`reverse`, take rest params per tag, or create a promise or closure unconditionally. Never post-process output with string `replace`/`split`/`indexOf`; the serializer appends valid JavaScript. Keep object shapes consistent; put cheap checks first. A module-level `/g` escape regex keeps `lastIndex` after a `test` hit: follow each hit with `replace` (which resets it) or `lastIndex = 0`, or the next call starts scanning there and writes an earlier unsafe character raw.
