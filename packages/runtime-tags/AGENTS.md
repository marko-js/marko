...
- Each template's analysis stays in its own terms. Never store another template's bindings or sections in this template's metadata, nor bindings from different templates in one list (`bindingUtil` orders by per-template ids, so they collide). A call site maps a child's facts into its own through the known tag's exprs (`mapParamReason`, `mapParamReasonToExpr`, `mapParamBindingToExpr`); only the call-site links (`NodeExtra.downstream`, `Section.downstream`) point at a child's bindings. What must persist across templates is template-neutral, such as a `ResolvedExport` or a child's `ProgramExtra`. This is the pattern for any fact a child exposes about its params: keep it keyed by the child's params, and map it at each call site (as `finalizeKnownTags` does for serialize reasons and `finalizeKnownTagRenders` for the content a param may name), so what lands on the caller's own params rises to its callers.
