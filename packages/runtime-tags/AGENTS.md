...
- **Optional feature enablement** (tree-shakable runtime API) has two patterns:
  ...
  - **Definition-site wrappers** gate behavior on a helper's own retention: `export const _if = /*@__PURE__*/ withBranches(...)`. One may also install dispatch wrappers, as `withPending` does on `addAwaitCounter`.
  - Latches are `let`s written only by their enabler (`branchesEnabled`, `pendingEnabled`) so bundlers fold latch and guarded code away together; object-property flags defeat that analysis and re-inflate resume bundles.
