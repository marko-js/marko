...
export function translateLoadTag(path, tagName, relativePath) {
  ...
  if (markoOpts.output === "dom") {
    if (importDecl) {
      // The lazy wrapper pretends to be the template, so every reference
      // not already translated as a tag uses it.
      for (const ref of importBinding.referencePaths) {
        if (!(t.isMarkoTag(ref.parent) && ref.parent.extra?.tagNameLoad)) {
          ref.replaceWith(t.cloneNode(loadId));
        }
      }
      importDecl.remove();
    }
