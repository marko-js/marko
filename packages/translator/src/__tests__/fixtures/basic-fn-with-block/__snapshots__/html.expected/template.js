...
const _renderer = _register((input, _tagVar, _scope0_) => {
  ...
  const count = 0;
  _write(`<button>${_escapeXML(count)}${_markHydrateNode(_scope0_id, "#text/1")}</button>${_markHydrateNode(_scope0_id, "#button/0")}`);
  _writeHydrateCall(_scope0_id, "packages/translator/src/__tests__/fixtures/basic-fn-with-block/template.marko_0_count");
