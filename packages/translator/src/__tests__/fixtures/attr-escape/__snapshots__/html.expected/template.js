...
const _renderer = _register((input, _tagVar, _scope0_) => {
  ...
  _write(`<div${_classAttr(input.foo)}${_attr("foo", 'a' + input.foo + 'b')}${_attr("bar", `a ${input.bar} b`)}${_attr("nested", `a ${input.foo + ` nested ${input.bar}`} b`)}></div>${_markHydrateNode(_scope0_id, "#div/0")}`);
