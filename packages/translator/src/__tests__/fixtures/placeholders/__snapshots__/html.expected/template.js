...
const _renderer = _register(({
  ...
  _write(`${_escapeXML(x)}${_markHydrateNode(_scope0_id, "#text/0")}<span>${_escapeXML(x)}${_markHydrateNode(_scope0_id, "#text/1")}<div></div></span><div><div>a</div>${_escapeXML(x)}${_markHydrateNode(_scope0_id, "#text/2")}Hello Text &lt;a/>${_toString(x)}${_markHydrateNode(_scope0_id, "#text/3")}Hello HTML <a/><script>
    <!>Hello &lt;b> &lt;/script>
  </script></div>`);
