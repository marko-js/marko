...
const _hydrate_expr_id_items = _register("packages/translator/src/__tests__/fixtures/basic-push-pop-list/template.marko_0_id_items", _scope => _on(_scope[1], "click", function () {
  const id = _scope[3],
    items = _scope[4];
...
const _expr_id_items = /* @__PURE__ */_subscriber([], 2, (_scope, id = _scope[3], items = _scope[4]) => _queueHydrate(_scope, _hydrate_expr_id_items));
const _for = /* @__PURE__ */_loop(0, 1, _forBody, [_item$forBody], (_scope, [item]) => _setSource(_scope, _item$forBody, item), (_scope, items = _scope[4]) => [items, null]);
const _hydrate_items = _register("packages/translator/src/__tests__/fixtures/basic-push-pop-list/template.marko_0_items", _scope => _on(_scope[2], "click", function () {
  const items = _scope[4];
  _queueSource(_scope, _items, items.slice(0, -1));
}));
const _items = /* @__PURE__ */_source(4, [_for, _expr_id_items], (_scope, items) => _queueHydrate(_scope, _hydrate_items));
const _id = /* @__PURE__ */_source(3, [_expr_id_items]);
const _setup = _scope => {
