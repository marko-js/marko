...
const _renderer = _register((input, _tagVar, _scope0_) => {
  ...
  const a = [0];
  const b = 1;
  _write(`<button>${_escapeXML(a.join(""))}${_markHydrateNode(_scope0_id, "#text/1")}</button>${_markHydrateNode(_scope0_id, "#button/0")}`);
  _writeHydrateCall(_scope0_id, "packages/translator/src/__tests__/fixtures/basic-handler-multi-ref-nested/template.marko_0_a_b");
