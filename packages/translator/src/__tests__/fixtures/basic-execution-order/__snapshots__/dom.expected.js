...
const _message$ifBody = _closure(1, 7, [], (_scope, message) => _data(_scope[0], message.text));

const _ifBody = _createRenderer(" ",
/* get */
" ", null, [_message$ifBody]);

const _if = _conditional(1, 1, (_scope, show = _scope[8]) => show ? _ifBody : null);

const _show = _source(8, [_if]);

const _message = _source(7, [_inConditionalScope(_message$ifBody, 1)]);
...
const _hydrate_setup = _register("packages/translator/src/__tests__/fixtures/basic-execution-order/template.marko_0", _scope => _on(_scope[0], "click", _bind(_scope, _temp3)));
