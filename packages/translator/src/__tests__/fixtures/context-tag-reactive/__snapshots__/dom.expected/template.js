...
const _put = /* @__PURE__ */_derivation("0:", 1, [_dynamicSubscribers("0:")], (_scope, x = _scope[3]) => x);
const _hydrate_x = _register("packages/translator/src/__tests__/fixtures/context-tag-reactive/template.marko_0_x", _scope => _on(_scope[1], "click", function () {
  const x = _scope[3];
  _queueSource(_scope, _x, x + 1);
}));
const _x = /* @__PURE__ */_source(3, [_put], (_scope, x) => {
  _data(_scope[2], x);
  _queueHydrate(_scope, _hydrate_x);
});
const _setup = _scope => {
  _setSource(_scope, _x, 123);
  _initContextProvider(_scope, 0, "0:", "packages/translator/src/__tests__/fixtures/context-tag-reactive/template.marko", _putBody);
};
