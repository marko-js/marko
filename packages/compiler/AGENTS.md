...
- `src/config.js` — config defaults, and the `globalConfig` that `configure()` sets (both re-exported by `index.js`); an omitted translator in `getRuntimeEntryFiles`, `getRuntimeVersion` and `taglib.buildLookup` reads it. Key options: `output` (`"html"` | `"dom"` | `"source"` | `"migrate"`), `optimize`, `translator`, `modules` (`"esm"`/`"cjs"`), `optimizeKnownTemplates`, `errorRecovery`, `cache`, `fileSystem`.
