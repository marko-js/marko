...
const fixtures = [
  ...
  "persisted-update-fragment",
  "persisted-update-pending-await-streamed",
  "persisted-update-possession-loop",
  "persisted-update-possession-swap",
];
...
const expectedCoverage: Record<string, { payloads: number; frames: number }> = {
  ...
  "persisted-update-fragment:cross": { payloads: 2, frames: 2 },
  "persisted-update-fragment:same": { payloads: 1, frames: 1 },
  "persisted-update-pending-await-streamed:same": {
    payloads: 2,
    frames: 3,
  },
  "persisted-update-possession-loop:same": { payloads: 3, frames: 3 },
  "persisted-update-possession-swap:same": { payloads: 2, frames: 2 },
};

const names: Record<string, string> = {
  "persisted-update-title": "same-route sparse value/attribute update",
  "persisted-update-fragment": "cross-route fragment hop",
  "persisted-update-pending-await-streamed": "async multi-frame update",
  "persisted-update-possession-loop": "keyed loop same/add/remove/reorder",
  "persisted-update-possession-swap": "conditional branch divergence",
};
...
const rows = [...grouped.entries()].map(([key, group]) => {
  ...
  for (const record of group) {
    ...
    if (record.have) totals.have.push(record.have);
  }
...
process.stdout.write(
  "| Case | Payloads / frames | Raw bytes | Gzip bytes | Brotli bytes | x-marko-have raw bytes |\n| --- | ---: | ---: | ---: | ---: | ---: |\n",
);
for (const row of rows) {
  const haveBytes = row.totals.have.reduce(
    (sum, have) => sum + Buffer.byteLength(have),
    0,
  );
  process.stdout.write(
    `| ${row.name} (${row.key.endsWith(":cross") ? "cross-route" : "same-route"}) | ${row.payloads} / ${row.totals.frames} | ${row.totals.raw} | ${row.totals.gzip} | ${row.totals.brotli} | ${haveBytes} |\n`,
  );
}
process.stdout.write(
  `| **Total** | ${records.length} / ${overall.frames} | **${overall.raw}** | **${overall.gzip}** | **${overall.brotli}** | — |\n\n`,
);
process.stdout.write("## Possession request header\n\n");
process.stdout.write("| Case | x-marko-have raw bytes |\n| --- | ---: |\n");
for (const row of rows) {
  const raw = row.totals.have.reduce(
    (sum, have) => sum + Buffer.byteLength(have),
    0,
  );
  process.stdout.write(
    `| ${row.name} (${row.key.endsWith(":cross") ? "cross-route" : "same-route"}) | ${raw} |\n`,
  );
}
process.stdout.write("\n");
...
function markEntry(
  ...
  if (
    ...
    (t.isStringLiteral(values[1]) || t.isNumericLiteral(values[1])) &&
    t.isStringLiteral(values[2]) &&
    t.isStringLiteral(body)
  ) {
    mark(owners, source, first, "fragment metadata");
    mark(owners, source, values[1], "fragment metadata");
    mark(owners, source, values[2], "fragment metadata");
    mark(owners, source, body, "fragment HTML");
    markFragmentMarkers(owners, source, body);
    if (t.isArrayExpression(values[4])) markDeps(owners, source, values[4]);
    return;
  }
  if (t.isNumericLiteral(first)) {
    // A ready batch's first number is the protocol wrapper/id. Its remaining
    // entries are ordinary fills, effects, fragments, or dependency markers.
    ...
    for (const value of values.slice(1)) {
      if (t.isNumericLiteral(value))
        ...
        if (isFragmentEntry(value)) markEntry(owners, source, value);
        else markDeps(owners, source, value);
      }
    }
    return;
  }
...
function isFragmentEntry(entry: BabelTypes.ArrayExpression) {
  const values = entry.elements.filter(
    (value): value is BabelTypes.Expression => !!value && t.isExpression(value),
  );
  return (
    t.isNumericLiteral(values[0]) &&
    (t.isStringLiteral(values[1]) || t.isNumericLiteral(values[1])) &&
    t.isStringLiteral(values[2]) &&
...
function markFragmentMarkers(
  owners: Array<Category | undefined>,
  source: string,
  literal: BabelTypes.StringLiteral,
) {
  const start = requireRange(literal).start;
  const end = requireRange(literal).end;
  for (let index = source.indexOf("<!--", start); index >= 0 && index < end;) {
...
function assertReadyBatchSelfTest() {
  const measured = measureFrame(
    '[[7,_=>[2,{value:42}],"effect",[3,0,"m","<p>x<!--m* a--></p>",[2,4]],[5,6]]]',
  );
  assertEqual(
