...
Two things need to change before it ships as _the blessed optimistic
pattern_, and neither is a change to the primitive:

1. **The flagship example is subtly wrong.** `by=input.cartCount` (the
   self-value key) cannot converge when the server _rejects_ a mutation —
   ...
   pending-confirmation state wants to be _derived at rest_ with an
...
- **The key structural claim** — that the client half is a small runtime
  helper, not new compiler wiring — is _stronger_ than the doc states:
...
- **A free correctness win the doc doesn't claim**: a keyed let inside a
  _positional_ loop fixes today's stale-state hazard — when rows shift,
...
- **Delivery-less reconciliation coherence**: a keyed let inside a
  ...
  persisted path degrades _into_ the client path, not into nothing.
...
(which the router _explicitly supports_ as an in-place patch — non-2xx
...
overwritten either way" only holds when the key _changes_.
...
- **In-progress input** (the draft): local writes are the user's property
  until the _subject_ changes. `by=item.id` — narrow key, exactly as
  specced. `<let by>` nails this.
- **Pending confirmation** (the counter): a local write is a _guess about
  the server's next answer_; the very next mutation response is
...
_forked at rest_ — a durable local copy, which is exactly what `<let>`
is, and identity (`by=`) is the right rule for when the fork re-bases.
Pending confirmation wants to be **derived at rest** — between
interactions the cell simply _is_ the server value, reactively fresh
...
primitive that just _is_ one:

**Expressible today (userland version key — works, fragile).** Key on a
server-side counter that bumps whenever the server _processes_ a
...
hand-maintained invariant that must fire on _every_ processing path
...
counter's annoying property (changes even when data doesn't) _is_ the
...
- **Derived at rest.** `<optimistic/cart=$global.data.cart/>` reads as a
  ...
  be `input`/`$global` (persisted canonical) _or_ an await boundary's
...
- **Settle = the transition's async work committed**, abstracted over
  ...
  then _is_ the authoritative answer. A rejected mutation re-derives to
...
- **Association: explicit cell, implicit write-capture, `for=` as the
  escape hatch.** _(Superseded — kept as the record of how the design
  evolved. The adversarial review killed write-capture: the final
  model, in optimistic.md's "Association, precisely," binds cells
  statically to their confirmation **channel** and never ties writes to
  interactions at all — settle is a property of the channel, made
  sufficient by F2's mutation ordering.)_ The sketch's `for=ref` is
  precise but strains exactly where the feature matters: loops (N tab
  links, one cell — per-iteration tag vars can't name it) and
  cross-template shells (the cart cell lives in the layout provider;
  the forms live elsewhere — refs don't cross templates). The original
  resolution — associate any write occurring during a router-converted
  event dispatch — looked free but claimed knowledge the client lacks
  and machinery (dispatch-chain tagging) the reviews showed leaking.
  `for=` stays rejected on the same grounds either way. A write with no
  resolvable channel degrades to "overlay until the next delivery" —
  `let-global`'s old semantics as the fallback rung; that part
  survived.
- **Honest edges to design**: which client boundaries belong to a
  ...
  with no server round trip behind them are _not_ optimistic — that's
  plain `<let>`, and the primitive should say so rather than absorb it.

_Considered and demoted — a run-owned settle token_ (`$global.settled`,
...
else never bumps _your_ token, so the cell goes stale until your next
...
- **Salvage the dropped response** (read superseded mutation streams and
  ...
  superseding GET may have rendered _before_ the mutation committed, so
...
- **Order navigations behind in-flight mutations** (recommended): while a
  ...
  read and applied, in order; and because the queued GET is _fetched_
...
describes the _persisted_ driver. The client runtime does the opposite:
...
body detaches and the skeleton re-shows on _every_ re-await, identity
...
against a stored live object is _always false_: an object-keyed
...
every object key collapses to `"[object Object]"` and compares _equal_ —
...
currently gives _no feedback at all_ between intent and first frame.
...
render _even when the key matches and the value will be discarded_ — for
...
choice)" under _also considered_ — the interim attribute name was
...
must answer _somehow_; the table says where this design answers it, and
...
compiled granularity doesn't pay — so the considerations that _survive_
...
primitive encodes: the overlay holds while _that cell's_ pending
...
by>`) is the right teaching story. The genuine shape split — `<for by>`
  ...
  directly — is fine: "for needs a key _per item_; everything else keys
  _itself_" is one sentence.
- **Seed-once-per-key is the right default**, and the prior-art section's
  ...
  though per F1, teach it for _derived-overridable_ cells (the color
...
- **Implementation notes** (for whoever builds it): (a) when the key
  ...
  _next_ delivery misclassifies; (b) the key slot must serialize whenever
...
- **The ownership analysis as the optimistic dividing line.** The
  ...
  cell gets the visible behavior with zero transport coupling, _given_
  F1's recipe).
...
- **Context's identity model** (template identity via taglib discovery,
  ...
  set — it makes cross-template state _improve_ under the wire-economics
...
2. **Router mutation ordering** (F2) — prerequisite for any settle
   ...
   races; its queue state is the persisted settle predicate. Placement
   pinned by review round 2: queue-insert happens synchronously in the
   submit listener, before `navigate()`'s first `await` (the entry
   import is a task boundary unrelated frames can otherwise slip
   through).
3. ~~Early-input stamping for GET navigations~~ — **moved** (review
   round 2): the surviving mechanism is the run nav context (`nav.url`
   stamped at click through the provider), which requires `<context>`
   and the nav provider, so it now rides step 7. The original
   router-only synthetic-frame-zero version died in round 1
   (R-F5/R-F7), and with it the free interaction-time `$global`
   claims — see the re-priced link-driven section below.
...
7. **The optimistic primitive** (F1's target) — the gate cell with
   channel-bound settle (deliberately _not_ write-capture, which the
   adversarial review's graveyard rejects; cells bind statically to
   their confirmation channel, and the boundary-membership analysis
   shares the cross-template reason-threading spike with `<context>`),
   plus the pending surfaces and the run nav context — which also
   delivers early-input stamping, folded in from old step 3. The
   version-key recipe is the documented interim until here.
...
Applying the input early _meant_, in this section's original form:
stamp the URL-derived globals (`$global.url`, and eventually matched
`params`/raw `search`) into the live page at click and let the
existing reactive promotion do the rest — chip highlights, tab moves,
URL-keyed `<let by>` re-seeds, all in the click's own frame, while the
fetch streams the content. This is not a violation of the "templates react to
applied input, not transport state" non-goal — the URL is the
navigation's _input_, not its transport state, and the whole persisted
...
zero**: the router already loads the `?update` entry _before_ the fetch
...
expressions are _skipped_, not miscomputed client-side — those holes
...
What this buys beyond chip highlighting — **re-priced by review
round 2 (F4)**, because the surviving nav-context mechanism stamps
`nav.url` and leaves `$global` untouched until delivery:

- **Interaction-time recede is no longer free.** The recede doc cites
  Unpoly's lesson that placeholders "should appear at interaction
  time, not at first-response time" — the synthetic-frame version
  delivered that automatically for `$global`-keyed boundaries, and
  died with it. Under the nav context,
  `<@placeholder by=$global.search.tag>` recedes at **first frame**
  (when the delivered `$global` merges); click-time recede means
  keying the boundary off a nav-context value the author explicitly
  consumes — possible, but authored. Whether to bless that authoring
  pattern as the documented recipe, or eventually restore a free
  mechanism, is an open question for the nav-provider design.
- **URL-keyed state resets land at first frame, not at click.**
  `<let/expanded=false by=$global.params.id/>` collapses when the new
  `$global` merges — still an authoring-free reset, one round trip
  _later_ than the dead mechanism promised. Same nav-context escape
  hatch where click-time matters.
...
_stay_ there — the stamp may lead `history.pushState` by a beat without
...
| input latency     | none — the input is already local                                                                             | nav-context stamp at click (proposed; re-priced by round 2): `nav.url` reacts at interaction time for authored consumers; `$global`-derived state and `by=` comparisons react at first frame; content keeps stale behind the pending signal until frames land |

One deliberate omission to keep, one reversed. Kept: no client data
layer, and no transport _detail_ in templates (no fetch promises, no
...
to optimistically drive _any_ update, not only respond to attribute
...
9. (Benchmark-suite smoke, run-side; rewritten for the revised
   mechanism) Early input via the nav context: a filter-chip click
   stamps `nav.url` before any frame arrives — a nav-context consumer
   (`chip--loading`) reacts at click while the `$global`-derived active
   state flips only on delivery; a superseding click re-stamps and the
   aborted navigation leaves no residue. Companion assertion pinning
   round-2 F4: a `$global`-keyed site does _not_ react before first
   frame.

---

_Also recorded while reviewing, out of band: the workspace benchmark
