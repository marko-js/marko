...
layering on top of it. The trade it makes explicit: patches stop requiring a
stateless server (product constraint 7) in exchange for value-level deltas,
a constant-size request header at any page size, and a substantially smaller
protocol surface.

## Motivation

The shipped token prunes structure (branches, renderers, keys) but not
values: every request-derived hole re-serializes on every patch. Measured
same-route payloads are dominated by unchanged values (search route ~61 kB
raw for a few changed rows; the 5k keyed benchmark ships 391 kB with full
possession because all item values ride along). Separately, the token's
size ceiling forced real machinery — graduated prefix claims, range and
stem-suffix codecs, canonical re-encode — all of which exists only because
the client must describe its state inside a ~4 KiB header.
...
- Sites are build-static ordinals into the compiled route (no strings).
...
- Dense arrays, varint-packed. Typical page ≈ 1.5–3 kB; capped (over-cap
  loops degrade to fragment delivery, as over-budget claims do today).
...
the stored tree and suppress unchanged values. Structure falls out of the
same compare (renderer hash differs at a site → fragment; key present both
sides → fills), so no separate possession vocabulary crosses the wire.
...
Illustrative grammar; fills/fragments are unchanged from the shipped wire
...
Cross-route with a divergent branch: identical to today — matched sites
arrive as (now value-pruned) fills, the divergent site as a fragment entry;
...
loop — individually boring pieces. The compiled possession-site identity
(analyze-owned site ordinals, loop keys) survives as the server-internal
...
tier keeps everything this deletes. Client applier, merges, fragments,
...
- Whether the descriptor deletion breaks any planned feature that wanted a
  client-readable site vocabulary (none known today).
