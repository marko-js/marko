...
> **Status (2026-07-21):** re-measured on the end-state wire (fills + shells
> only; the possession token and replacement delivery are deleted). The
> token-era sections below are retained as the recorded history of the
> scaling cliff that motivated construction; their mechanisms no longer
> exist.
>
> Current numbers (`NODE_OPTIONS=--experimental-vm-modules node -r ~ts
scripts/measure-persisted-scaling.ts`, optimized bundles, jsdom apply):
>
> | Case       | Doc ms | Patch ms | Patch raw | Patch gzip | Frames | Apply ms |
> | ---------- | -----: | -------: | --------: | ---------: | -----: | -------: |
> | keyed-100  |    1.6 |      2.1 |     6,152 |      1,376 |      1 |     10.4 |
> | keyed-1000 |    5.8 |     12.1 |    64,574 |     12,864 |      1 |    124.2 |
> | keyed-5000 |   23.5 |     41.2 |   340,206 |     64,506 |      1 |  4,447.9 |
> | awaits-1   |    0.7 |      0.5 |       134 |        121 |      2 |      1.0 |
> | awaits-10  |    0.9 |      1.3 |       874 |        299 |     11 |      4.0 |
> | awaits-25  |    0.9 |      2.3 |     2,149 |        549 |     26 |      3.4 |
>
> There is no token tier and no cliff: every keyed item ships the same sparse
> fills whether matched or fresh (matched keys merge locally; fresh keys
> construct from the body's once-per-response shell), so payload scales with
> the item data itself — 340 kB raw at 5,000 items versus the token era's
> 391 kB all-replacement collapse, with zero request-header bytes. Patch
> render time stays linear (41 ms at 5,000 items). The keyed-5000 apply time
> is dominated by jsdom's O(position) mutation accounting (see "Superlinear
> pipeline stages"), not runtime work real browsers repeat.
...
These measurements predate shell delivery. Under the landed
architecture a constructible keyed body never consults possession: matched
keys merge locally, fresh keys construct client-side from the body's wire
shell, and the token cliff and claim-density machinery below apply only to
keyed bodies with no shell (`noConstruct` content). The benchmark must be
re-run to characterize the constructible path; the record below stands for
the fallback tier.
...
- **The possession token collapses between 100 and 1,000 items.** At 100 items
  ...
  Losing possession converts every matched keyed item into a replacement, so the
...
- **Server patch render goes superlinear on the all-replacement path**: 10× items
  cost 12× (3.2 → 38.9 ms) but the next 5× costs 16× (38.9 → 621.9 ms).
- **Client apply is the ceiling breaker**: 12 → 390 → 5,825 ms. Parsing and
  stamping thousands of replacement entries plus the keyed diff dominates; a
...
anchors that fits the budget (a 64-entry sample estimates bytes per entry, then
...
items, so very large lists still ship mostly replacements). Document render pays
...
server's per-key membership question (`possessed[anchorKey]`), and any
...
replacements. 5,000 items: 783 → 391 kB raw, 96 → 67 kB gzip; the document
...
- **Server patch render** spent ~70% of its time in `takeReplacementScopeIds`
  (`html/writer.ts`), which filtered every replacement scope id with a
  ...
  replacements. `replacementMarkerScopeIds` is now a `Set<number>`; the patch render
  is linear again (3.8 → 28 → 118 ms for 100/1k/5k, measured post-claims where
  ~10% of items are replacements).
- **Client apply** spent ~67% under `_for_keyed`'s placement loop, which
  ...
  branches into a `DocumentFragment` cut the 5,000-item all-replacement apply
...
iframe state). Replacement `innerHTML` parsing (~23% pre-claims) is real linear
work proportional to replacement bytes and shrinks with claim density.
...
cannot be shown to preserve parent→child structural dispatch (replacement
...
2. ~~Profile the all-replacement apply path~~ — profiled and fixed ("Superlinear
   pipeline stages").
