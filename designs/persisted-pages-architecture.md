...
- **Optimized register ids are pre-allocated at analyze time**
  (`translator/util/preallocate-register-ids.ts`): analyze enumerates the
  full key union once per cached file and allocates every child id in a
  canonical order, so translates only look up existing slots and compile
  scheduling cannot permute ids. Every compile of a persisted template
  (html, dom, `?update`, `?persisted`) must still share one
  `optimizeKnownTemplates` array (or `getTemplateId` option) so lookups
  resolve in the same registry — but the order those compiles run in no
  longer matters. A translate-time request for a key analyze failed to
  enumerate throws an internal error (`getResumeRegisterId`'s tripwire).
...
**A. Pure mainline improvements (backported ahead of the feature):**

1. **Compiler/translator hardening** (behavior-neutral on `main` by
   construction; extracted as PR #3332): `$signal` abort-ids pre-allocated
   in analyze and stamped on the expression root's extra so every translate
   reads the same id (the old section-keyed module `WeakMap` leaked
   translate-phase state onto cached-analysis objects);
   ...
   references.
2. Mainline-relevant `agent-feedback/` entries and cspell additions riding
   the above.

**Deferred, not backported (recorded as `agent-feedback/perf.md` entries,
which are the source of truth for redoing them):** the runtime packaging
work (analyzable purity so bundlers tree-shake the runtime with no package
metadata, preserved dist modules behind the `dom.mjs` facade, and a
bundle-size classifier that understands the preserved layout) and the
runtime module splits (`dom/spread.ts` out of `dom/dom.ts`; a catch split
moving `_enable_catch` installation out of the render queue; a controllable
per-kind split). Both were validated behavior-neutral on an earlier
iteration of this branch — the packaging is where the measured
non-persisted wins live (163 fixtures shrank at the time of measurement) —
but they restructure runtime file hosting, so they were pulled out to keep
the backport reviewable and this feature's diff on `main`'s file layout.
...
(dropped when the vite branch rebased onto upstream), and it means library
packages need no `sideEffects` declaration of their own in Marko apps.
