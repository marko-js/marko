...
deferred escape-site loader in that module's persisted entry, so a
same-renderer dynamic dispatch with no registered merge starts the child's
`?persisted` entry on demand and drains through the same parked-update
machinery. A renderer never referenced as a compile-time template value
anywhere (constructed at runtime or interop-wrapped) has no escape site to
register, and its dispatch fails loudly through the transport's document
fallback rather than parking silently.
...
| New structure server-rendered and resumable                                        | `persisted-update-fragment`, `persisted-update-fresh-page`, the `persisted-update-possession-*` family (swap, native, loop, nested, multiswap, positional), and `persisted-update-fragment-body-scope` (dom-less scope stamping). Interactivity of arrived fragments (effects run, later frames fill them) is asserted by clicks inside every fragment fixture.                                                                                                                                                                                                                                                                          |
