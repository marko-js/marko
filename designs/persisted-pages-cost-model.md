...
Companion to [single-page-server-updates-handoff.md](single-page-server-updates-handoff.md).
...
- **A1 — node markers** (the patch addresses). Was 82% of a +54.4% gzip
  delta; the same-scope continuation encoding (landed — see the handoff
  marker-cost entry) cut the page to **+19.1%** (`/item` +16.4% →
...
twice (see the handoff's corrections):
...
3. **Id consistency across compiles**: html/dom/`?update`/`?persisted`
   ...
   `optimizeKnownTemplates` array). Everything registry-shared rests on
   this.
...
2. **`x-marko-have` T2 digests + tombstones** (B; adds I-c). The only
   ...
   shape. Decision-ready design in the handoff (canonicalization
   approach + per-route cache policy are the open forks). Pair with a
   server CPU probe (D).
...
as swap-construction material. The parked wire-delivery direction
(fragment `templates` frames + client content store — see the handoff)
is exactly that: server-only components' static parts and graphs arrive
as navigation payload for the subtree being constructed instead of
shipping ahead-of-time as JS for the whole library, turning
O(components) JS into O(navigation) bytes. C2 dedup (main ↔ `?persisted`
graph sharing) is the tactical complement for interactive components,
which must stay in JS.
...
2. **Wire-delivered fragment content** (the at-scale lever): promote
   ...
   [persisted-pages-at-scale.md](./persisted-pages-at-scale.md)
   (options, the "server-only components are JS-free" recommendation,
   phasing, open forks).
