...
exports.attrs = function(widget) {
    ...
    if (widgetConfig) {
        attrs['data-rwidget-config'] = new WrappedString(stringify(widgetConfig, STRINGIFY_OPTIONS));
    }
    ...
    if (extend && extend.length) {
        ...
        for (var i=0, len=extend.length; i<len; i+=2) {
            var cureExtend = getWidgetRequireInfo(extend[i]).path;
            var curExtendConfig = stringify(extend[i+1], STRINGIFY_OPTIONS);
            extendParts.push(cureExtend);
            extendParts.push(curExtendConfig);
        }
        attrs['data-rwidget-extend'] = new WrappedString(extendParts.join('%'));
    }
