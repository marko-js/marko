...
exports.attrs = function(widget) {
    ...
    var customEvents = widget.customEvents;
    if (customEvents) {
        attrs['data-w-events'] = widget.scope + ',' + customEvents.join(',');
    }
    ...
    if (extend && extend.length) {
        ...
        for (i=0, len=extend.length; i<len; i+=4) {
            var curExtend = extend[i];
            // If a config was associated with the extend then add it to the list as JSON,
            // otherwise push an empty string (we will need the placeholder)

            var curExtendConfig = extend[i+1];
            if (curExtendConfig) {
                curExtendConfig = stringify(curExtendConfig, STRINGIFY_OPTIONS);
            } else {
                curExtendConfig = '';
            }

            var curExtendState = extend[i+2];
            if (curExtendState) {
                curExtendState = stringify(curExtendState, STRINGIFY_OPTIONS);
            } else {
                curExtendState = '';
            }

            var curExtendPreserve = extend[i+3];
            if (curExtendPreserve) {
                curExtendPreserve = curExtendPreserve.join(',');
            } else {
                curExtendPreserve = '';
            }
            ...
            extendParts.push(curExtendState);
            extendParts.push(curExtendPreserve);
        }
        attrs['data-w-extend'] = new WrappedString(extendParts.join('%'));
    }

    var preserve = widget.preserve;

    if (preserve && preserve.length) {
        attrs['data-w-preserve'] = preserve.join(',');
    }
