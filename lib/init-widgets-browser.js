...
function initWidget(
    ...
    if (existingWidget) {
        ...
        if (widget.__preserver) {
            widget.__preserver.restore(widget);
            widget.__preserver = null;
        }
    ...
    if (widget._isWidget) {
        ...
        if (extendList) {
            ...
            // that we created
            for (i=0, len=extendList.length; i<len; i+=4) {
                var extendPath = extendList[i];
                var extendConfig = extendList[i+1];
                var extendState = extendList[i+2];
                var extendPreserve = extendList[i+3];

                if (extendState) {
                    if (typeof extendState === 'string') {
                        extendState = parseJSON(extendState);
                    }

                    extend(widget.state, extendState);
                }

                if (extendPreserve) {
                    if (typeof extendPreserve === 'string') {
                        extendPreserve = extendPreserve.split(',');
                    }
                    ...
                    preserve = preserve.concat(extendPreserve);
                }
                ...
                if (!existingWidget) {
                    ...
                    if (extendConfig && typeof extendConfig === 'string') {
                        extendConfig = parseJSON(extendConfig);
                    }
...
function initWidgetFromEl(el) {
    ...
    var preserve = el.getAttribute('data-w-preserve');
    if (preserve) {
        preserve = preserve.split(',');
        el.removeAttribute('data-w-preserve');
    }
