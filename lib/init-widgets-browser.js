...
function addCustomEventListener(widget, targetWidget, eventType, targetMethodName) {
    widget.subscribeTo(targetWidget)
        .on(eventType, function() {
            var args = arrayFromArguments(arguments);
            args.push(targetWidget);
    ...
    if (widget._isWidget) {
        ...
        if (events) {
            ...
            for (i=0, len=events.length; i<len; i+=3) {
                ...
                var targetMethodName = events[i+1];
                ...
                } else if (eventElId.charAt(0) === '@') {
                    // We are add a listener for custom event on another widget
                    targetWidgetId = eventElId.substring(1);
                } else {
                    eventEl = document.getElementById(eventElId);
                }

                if (targetWidgetId) {
                    // The event mapping is for a custom event (not a DOM event)
                    targetWidget = widgetCollection[targetWidgetId];
                    addCustomEventListener(widget, targetWidget, eventType, targetMethodName);
                } else {
