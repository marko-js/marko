...
var promises = require.find('raptor/promises');
var logger = require('raptor-logging').logger('raptor/render-context/Context');
var DATA_PROVIDERS_CONTEXT_KEY = 'dataProviders';
var renderContext;
...
function getOrCreateDataProviders(context) {
    ...
    return contextDataProviders || (attributes[DATA_PROVIDERS_CONTEXT_KEY] = dataProviders.create());
}
...
require('raptor-util').extend(require('./Context'), {
    beginAsyncFragment: function (callback, timeout) {
        ...
        try {
            ...
            if (promise) {
                promises.immediateThen(promise, function (result) {
                    if (result != null) {
