...
describe('AsyncStream', function () {
    ...
    it('should support chaining', function (done) {
        var errors = [];
        var out = new AsyncStream().on('error', function (e) {
            errors.push(e);
    ...
    it('should support writing to a through2 stream', function (done) {
        ...
        var output = '';
        var through2 = require('through2')(function write(data, encoding, callback) {
            output += data;
            callback(null, data);
        });

        var errors = [];
        var out = createAsyncStream(through2).on('error', function (e) {
            errors.push(e);
        }).on('finish', function () {
    ...
    it('should support writing to a through stream', function (done) {
        ...
        var errors = [];
        var out = createAsyncStream(through).on('error', function (e) {
            errors.push(e);
        }).on('end', function () {
            expect(errors.length).to.equal(0);
            expect(output).to.equal('123');
    ...
    it('should support writing to a file output stream', function (done) {
        ...
        out = createAsyncStream(out).on('error', function (e) {
            errors.push(e);
        }).on('finish', function () {}).write('1');
    ...
    it('should support piping to an async writer', function (done) {
        ...
        var helloReadStream = fs.createReadStream(nodePath.join(__dirname, 'fixtures/hello.txt'), fsReadOptions);
