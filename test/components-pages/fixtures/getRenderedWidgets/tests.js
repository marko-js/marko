...
describe(path.basename(__dirname), function() {
    it("should serialize component config down to the browser", function() {
        ...
        expect(window.simpleComponents[0].state.type).to.equal(
            "component state"
        );
        ...
        expect(window.simpleComponents[0].componentConfig.name).to.equal(
            "John"
        );
        expect(
            window.simpleComponents[0].componentConfig.messageCount
        ).to.equal(10);

        expect(window.simpleComponents[1].state.type).to.equal(
            "component state"
        );
        ...
        expect(window.simpleComponents[1].componentConfig.name).to.equal(
            "Frank"
        );
        expect(
