...
A fed renderer change rejects to navigation exactly when optimize shook the dispatcher (`patch-content-root-swap` asserts the debug-mode apply via `skip_optimize`), but nothing asserts the optimized rejection because `expect_rejection` is mode-global and there is no `skip_debug`. A `skip_debug` (or per-mode `expect_rejection`) would let that fail-closed path pin its behavior.
