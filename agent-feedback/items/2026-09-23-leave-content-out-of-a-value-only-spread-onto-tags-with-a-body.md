...
Check: `pnpm run compile -- -o html -d template.marko` on `<let/count=0><child><@item class="a">One ${count}</@item></child><button onClick() { count++ }/>` with `tags/child.marko` `<let/show=true/><button onClick() { show = !show }/><if=show><div ...input.item>Body</div></if>` emits `_content_resume(` in `template.marko.js`.
