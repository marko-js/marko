...
resume/serialization — does not mention the patch protocol at all: no shells
(`src/translator/util/shell.ts`), frames/`PatchState`
(`src/html/patch.ts`), ready-channel deferral (`src/dom/patch-ready.feat.ts`),
...
mode and the admission guard must be revisited. Add a patch protocol
...
Check: search RESUMABILITY.md for "patch", "patch", "shell" — no hits.
