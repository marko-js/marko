...
`rendererKey` (and its server twin `html/writer.ts › rendererKey`) returns a native tag name unchanged and an ownerless renderer by its bare id, and a `_template` renderer has no owner. Under `optimizeKnownTemplates`, `encodeTemplateId` gives short ids such as `a`, `b`, `i`, `p`, `q`, `s`, `u` (and `em`, `br`, `h1` at larger indexes), so `<${cond ? Child : "b"}>` computes the same key for both values when Child's id is `"b"`: `_dynamic_tag` keeps the native branch and calls Child's Params on the native branch scope. Give ownerless renderers a key no tag name can take in both runtimes (the owner-bound key already joins with a space, which tag names cannot contain), and fix the comment in `rendererKey`, which assumes only Class-API interop renderers are ownerless.
