...
Check: `pnpm run compile -- -o html -d packages/runtime-tags/src/__tests__/fixtures/import-tag-cycle/tags/cyc-b.marko` (cyc-b analyzed first) calls `CycA({ depth: input.depth + 1 })` inline, while that fixture's `__snapshots__/html.bundle.debug.js` (cyc-b analyzed after cyc-a through `template.marko`) renders the same edge as `_dynamic_tag($scope0_id, "#text/1", cyc_a_default, …)`; `-o dom` likewise inlines `_CycA_template`/`_CycA_walks` where the fixture's dom bundle uses `_dynamic_tag("#text/1")`.
