...
## Serialize `Temporal` values so they survive resume

`packages/runtime-tags/src/html/serializer.ts` › `writeUnknownObject` | 2026-07-23 | impact:med | effort:low

`Date` serializes, but every `Temporal` type aborts SSR with `Unable to serialize` when reached from browser-referenced content, so the modern date API cannot be held in a `<const>`/`<let>` that interactive code reads. Unlike the `Intl` formatters — whose `resolvedOptions()` is only a projection of internal state — `Temporal` round-trips exactly through its string form: every type parses back from `toString()` via `Temporal.<Type>.from(...)`, so a writer alongside `writeDate` is a small, lossless addition covering `Instant`, `Duration`, `PlainDate`, `PlainDateTime`, `PlainMonthDay`, `PlainTime`, `PlainYearMonth`, and `ZonedDateTime`. Note these are `globalThis.Temporal` members, so the dispatch entries need the same optional-chained form the other non-universal constructors already use. Re-verify: pass `Temporal.Instant.from("2024-01-05T00:00:00Z")` through `Serializer#stringifyScopes`; the payload omits the value, and under MARKO_DEBUG the boundary aborts.
...
## Serialize the remaining unsupported built-ins reachable from browser code
...
## Stop the pre-commit hook from failing when the only staged JSON is generated fixture `sizes.json`

`.lintstagedrc.json` › `*.{json,md,css}` | 2026-07-24 | impact:med | effort:low

`.lintstagedrc.json` routes `*.{json,md,css}` to `oxfmt --with-node-modules`, but `.oxfmtrc.json` `ignorePatterns` excludes `**/fixtures/*/*`, which covers every `packages/*/src/__tests__/fixtures/*/sizes.json`. A commit whose staged JSON/md/css set is entirely fixture `sizes.json` therefore hands oxfmt a list in which every file is ignored, and it exits non-zero with "Expected at least one target file. All matched files may have been excluded by ignore rules.", so lint-staged reverts and the commit is refused. This is easy to hit because `pnpm run test:update` regenerates many `sizes.json` at once: a runtime change committed together with only its regenerated sizes fails, while the same change committed alongside any non-ignored `.md` (a changeset, for instance) succeeds, which makes the failure look random. Suggested direction: narrow the lint-staged glob to exclude fixture output (or add `--allow-empty`-style tolerance by wrapping the command so an empty target list is a no-op). Re-verify by staging only a `.ts` file plus several `packages/runtime-tags/src/__tests__/fixtures/*/sizes.json` and running `pnpm exec lint-staged`.
