...
## Warn (or error, like `<let>` does) when a native tag has duplicate attributes instead of silently keeping the last

`packages/runtime-tags/src/translator/visitors/tag/native-tag.ts` › `analyze.enter` | 2026-07-19 | impact:med | effort:low

Two or more attributes with the same name on a native element silently collapse to the LAST occurrence with no compile-time or dev-server warning: `<div class="card primary" class="override">` emits `<div class=override>` (first class dropped), `<button onClick(){a()} onClick(){b()}>` binds only the second handler, `<div id=a id=b id=c>` keeps `id=c` — all compile cleanly in both `html` and `dom`, and a real `marko-run build`/`dev` renders the collapsed result. `native-tag.ts` iterates attributes in reverse (`for (let i = attributes.length; i--;)`, :129) into a `seen` record and, on a repeat name, just `dropNodes(attr.value); continue` (:134-137) — the earlier value is discarded with no `buildError`/warn. A duplicate class/id/value/onClick is almost always a merge or refactor mistake, and Marko already errors for the analogous cases: `<let>`/`<const>` throw "Invalid duplicate value attribute." (`translator/core/let.ts:48`), `<script>` throws the same (`core/script.ts:79`), and the parser throws "Cannot have shorthand id and id attribute." (`compiler/src/babel-plugin/parser.js:549`). It also diverges from HTML parsing, which keeps the FIRST duplicate, so the same literal markup renders differently in Marko vs a browser. At `native-tag.ts:134` emit a compile warning (or `MARKO_DEBUG` error) naming the dropped attribute, exempting the mergeable cases already handled elsewhere (class `.foo` shorthand merges deliberately into `class=` at `parser.js:505-535`; spreads). No existing entry covers duplicate native attributes.
