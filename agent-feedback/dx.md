...
## Give the removed `class {}` component block a named migration error, like the sibling `<% %>`/`$` scriptlet cases, instead of htmljs-parser's generic "Invalid attribute name."

`packages/compiler/src/babel-plugin/parser.js` › `onOpenTagName` | 2026-07-19 | impact:med | effort:med

A Marko 5 `class { onCreate(){...} }` block compiled with the tags translator surfaces htmljs-parser's generic `Invalid attribute name.` with the caret on the class body, reading like an HTML-attribute typo and giving zero signal that the class-component API was removed. `class` is invisible to the translator-agnostic parser: `getTagDefForTagName(file, "class")` yields no parse options, so `class` is scanned as an ordinary concise tag name in `onOpenTagName` (`parser.js:314-350`) and the following `{` fails attribute parsing, relayed verbatim through `onError` (`parser.js:141`) — the same concise-line-scanned-as-tag path that surfaces "Invalid attribute name." for the `error-agent-fix-guide` fixture and dx.md:80's cases. The removed-syntax precedent sits in the same function: `parser.js:325-328` throws a named migration error for `literalTagName === "%"` (`<% scriptlets %> are no longer supported.`), and the translator's `scriptlet.ts:18` throws `Scriptlets are not supported when using the tags api.` for `$`. Because the class block is the dominant structure in Marko 5 code and training data, a Marko-5-trained agent writes it constantly, and the generic message steers it to fix the `{` rather than convert to `<let>`/`<const>`/lifecycle. The agent-fix-guide (`compiler/src/util/agent-fix-guide.js`) does append `READ cheatsheet.md`, whose line 156 maps `class {}` → `<let/x=0>`, which partially rescues a diligent agent, but the error itself never signals a class block is involved. Add a guard for `literalTagName === "class"` (followed by `{`) at `parser.js:325` mirroring the `%` case. Distinct from dx.md:80 (structural remap for stray close tags / unwrapped concise prose): this is a removed-API migration diagnostic for a specific reserved word, matching the `%`/`$` precedent.
...
## Fail loudly when a fixture's `test.ts` errors at load instead of silently falling back to the default `{}` config

`packages/runtime-tags/src/__tests__/main.test.ts` › `testFixtures` | 2026-07-20 | impact:med | effort:low

The per-fixture config loader wraps `require(resolve("test.ts")).config` in a bare `try { ... } catch { return {} }` (main.test.ts:113-119) to make `test.ts` optional, but the catch cannot tell an absent file (require throws MODULE_NOT_FOUND) from a present-but-broken one: a syntax error, a module-scope `throw`, or a bad `import` all collapse to the default `{}`, so the fixture silently runs with empty input, no steps, and default flags (`equivalent`, `skip_*`, `runtime_id`, `error_compiler`, etc. all ignored). Confirmed by adding `throw new Error("boom")` at module scope of `fixtures/assign-live-read/test.ts`: in normal mode the swallowed config drops the click steps and the `ssr` test fails with a misleading render-diff that never names `test.ts`, while under `test:update` (UPDATE_EXPECTATIONS=1) all 7 tests pass and `render.md` is silently rewritten from the full 3-click interaction down to the initial render — committing corrupted, coverage-stripped snapshots with zero error. 770 of 878 fixtures ship a `test.ts`, so this loader runs on nearly every fixture, and it sits directly beside the explicit `if (!fs.existsSync(templateFile))` guard at :126 that it could mirror. Mirror that guard: check `fs.existsSync(resolve("test.ts"))` for optionality and let a present file's require errors propagate (or narrow the catch to a MODULE_NOT_FOUND whose failing path is `test.ts` itself). Re-verify: add a module-scope `throw` (or syntax error) to any fixture's `test.ts`, run that fixture under `UPDATE_EXPECTATIONS=1` scoped with `--grep`, and confirm it passes while rewriting that fixture's `render.md` instead of failing with the thrown error.

## Fix the double extension-strip in the snapshot `.actual` filename that mangles names and collapses ssr/csr debug diffs into one clobbered `.actual.md`

`packages/runtime-tags/src/__tests__/utils/snap.ts` › `snap` | 2026-07-20 | impact:low | effort:low

In `snap`'s mismatch branch (`snap.ts:87-93`), the debug `.actual` file's base is `(uniqueName ?? file.slice(0, -ext.length)).slice(0, -ext.length)` — the extension length is subtracted twice. With no `uniqueName` this mangles ordinary names (`dom.bundle.js` -> `dom.bun.actual.js`, `writes.html` -> `writes..actual.html`), but the real damage is that `snapMode` only ever passes `uniqueName` as `"ssr"`/`"csr"` (`main.test.ts:207-210`, extensionless so the debug `.replace` is a no-op) — both length 3, and the render snapshot's `.md` ext is also length 3, so both `.slice(0,-3)` to `""` and a mismatching SSR render and a mismatching CSR render both write to `__snapshots__/.actual.md` (a hidden dotfile), the later csr run clobbering ssr. That defeats the exact ssr-vs-csr disambiguation `uniqueName` was added for — worst in `equivalent` mode where a single `render.md` is shared and divergence is precisely what you want to diff — and it also collides in non-equivalent mode (`render-ssr.md`/`render-csr.md` still both emit `.actual.md`). The `.actual` files are gitignored (`.gitignore` `*actual*`), so nothing committed is harmed; only the local debug aid degrades. Drop the trailing `.slice(0, -ext.length)` so the base is just `uniqueName ?? file.slice(0, -ext.length)`. Re-verify: `node -e 'const p=require("path");const f=(file,u)=>{const e=p.extname(file);return (u??file.slice(0,-e.length)).slice(0,-e.length)+".actual"+e};console.log(f("render.md","ssr"),f("render.md","csr"),f("dom.bundle.js"))'` prints `.actual.md .actual.md dom.bun.actual.js`.
