...
## Don't retry replacing `test:parallel` with mocha's built-in `--parallel`

`scripts/test-parallel.js` › `SLICED_FILES` | 2026-07-30 | impact:low | effort:high

Mocha's own `--parallel` dispatches work per _spec file_ (`lib/nodejs/parallel-buffered-runner.js` hands one filepath at a time to a `workerpool` process), so it cannot split `packages/runtime-tags/src/__tests__/main.test.ts`, which is ~100s of the ~150s serial run in a single file — the 4-job floor would be ~100s against the ~40s this runner achieves. Making it work requires N checked-in shard spec files that each call into the suite with a `(shard, total)` pair, and a prototype of that (16 shards) measured 38.6s vs 40.9s at 4 jobs: a few percent, in exchange for the shard files plus rewriting the `import type { TestConfig } from "../../main.test"` line in ~900 fixture `test.ts` files. Two non-obvious traps if anyone revisits: a `.mocharc.*.cjs` config must spread `.mocharc.json` itself (mocha's `extends` is not applied to JS configs, so `node-option` silently drops and every fixture fails with `vm.SourceTextModule is not a constructor`), and `utils/snap.ts`'s top-level `after(cleanupSnapshots)` must become a `mochaHooks` root hook because parallel workers reuse the module across files and would only ever run it for the worker's first file. Re-verify the floor: `time npx mocha --config .mocharc.parallel.json --reporter dot packages/runtime-tags/src/__tests__/main.test.ts`.
