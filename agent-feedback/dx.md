...
## Guided hint candidate: `by=` referencing the loop's own parameter

`packages/runtime-tags/src/translator/core/for.ts:1` | 2026-07-11 | impact:med | effort:low

Capability-sweep evidence (llm/experiment RESULTS5): 2/2 sonnet subjects keyed a primitive list with `by=city` on `<for|city| of=results>` — but the `by=` value evaluates in the surrounding scope, so the loop param is not visible and SSR dies with a bare `city is not defined` at render time (no code frame pointing at the mistake). An analyze-stage check for a `by=` value that is a lone identifier matching one of the tag's own params could raise a guided error: "The `by=` value is evaluated outside the loop, so the loop variable is not in scope; use a property name (`by=\"id\"`) or a function (`by=(item) => key`)." An outer binding with the same name as the param is pathological enough that flagging it unconditionally seems safe. The docs-side fix shipped in `packages/runtime-tags/llms.md` (v8 row, validated in llm/experiment/runs/exp5-v8check); this entry is the error-message half.
