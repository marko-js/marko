...
Corroboration (two-round repair loop, 2026-07-11): with only one error visible per round, a weak LLM subject applied each newly surfaced message correctly but sometimes un-fixed a previous round's repair in the same edit (removed JSX braces in round 1, reintroduced them in round 2 while fixing `on:click` → `onClick`) — multi-error reporting would expose such conflicts within a single round.
