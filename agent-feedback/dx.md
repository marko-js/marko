...
## Lazy-load fixture teardown flakes when a `visible`/`idle` trigger fires against an already-closed jsdom window

`packages/runtime-tags/src/__tests__/main.test.ts` › fixture `after()` browser-cleanup hook (`main.test.ts:151`) | 2026-07-22 | impact:med | effort:med

The `lazy-tag-shared-parent-conflicting-triggers` debug fixture intermittently fails its "after all" hook with `done() called multiple times in hook <… "after all" hook in "debug">; in addition, done() received error: TypeError: Cannot read properties of null (reading '_location')`. It is non-deterministic: in one CI run (PR #3551, commit 4b056789eb) node@24 failed while node@22 and node@26 passed on identical code, main's own CI for the commit that added the fixture (08e5ef8a6e) was green, and a plain job re-run of the failed node@24 passed. The teardown `after()` closes each cached jsdom browser with `browser.window.close()` (`main.test.ts:151`); `_location` is a jsdom `Window` internal, so the null read is a still-pending lazy-load trigger firing its callback into a window teardown already closed — this fixture's `tags/parent-a.marko` imports its child `with { load: "visible body" }` (IntersectionObserver) and `tags/parent-b.marko` `with { load: "idle" }` (requestIdleCallback), both of which schedule async work not drained before close. The async throw then surfaces as a second `done()` and crashes the hook. Direction: cancel/drain the fixture's pending load-trigger timers and observers (or no-op post-close callbacks) before `browser.window.close()`, so teardown is ordered rather than racing the fixture's own loads. Distinct from the `bugs.md` entry "Make conflicting load triggers for one shared asset deterministic", which concerns the runtime's trigger-dedup behavior, not test teardown. Re-verify by running the fixture in a loop — `pnpm test -- --grep "lazy-tag-shared-parent-conflicting-triggers"` — or across node@22/24/26 on CI, where only some versions flake per run.
