...
`WalkCode` reserves char codes 67..91 for `next` walks (`Next=67`, `NextEnd=91`, a 25-code span the `// 67 through 91` comment even documents), and `dom/walker.ts:100` already bounds the decode branch at `value < WalkCode.NextEnd + 1` (≤91), but `WalkRangeSize.Next` is 20, so `toCharString` (`translator/util/walks.ts:184`) only ever emits remainder codes 67..86 and codes 87..91 are dead — unlike Over/Out/Multiplier, each fully packed at size 10 (97..106, 107..116, 117..126). As a result a consecutive `next` run of 20-24 nodes emits a redundant Multiplier char (2 chars where 1 suffices), and runs of 200-249 emit 3 where 2 would do; walk strings ship in every compiled DOM template and bundle size is a tracked feature here. Setting `WalkRangeSize.Next = 25` uses the whole range: encoder and decoder both read the enum so the change is coordinated, the widest emitted `next` char becomes 91 (remainder 24), still below the reserved backslash 92, and the decoder's `WalkRangeSize.Next * currentMultiplier + value - WalkCode.Next` reconstruction stays exact. Re-verify: mirror `toCharString` and the walker's `next` branch in node with `rangeSize=25` and confirm n=20..24 each encode to one char with max charCode 91 and decode back to 20..24 (I saw 0 roundtrip mismatches and 0 reserved-code emissions over n=0..600), then flip the enum and run `npm run test:update` plus the size hook to confirm affected DOM fixtures shrink rather than break. Corroboration (2026-07-23): flipping `Next=25` and running `test:update` roundtrips green (9624 passing) but changes **zero** of the 503 fixtures — no real template has a 20+ consecutive `next` run, so the dead codes 87..91 are never emitted and the practical impact is currently nil (defensive/future-proofing only).

## Loop helpers allocate a fresh args array per iteration

`packages/runtime-tags/src/dom/control-flow.ts` › `_for_of` | 2026-07-23 | impact:low | effort:med

Each of `_for_of`/`_for_in`/`_for_to`/`_for_until` allocates a fresh args array (`[item, i]` / `[key, value]` / `[v]`) per iteration for `params?.(branch, args)`. The loop callback reads args synchronously into scope and never retains them (its only use is `params?.(branch, args)`), so this is avoidable: a 10k keyed list allocates 10k arrays per create and ~N per reconcile. Best fix: pass the loop params POSITIONALLY (`cb(key, arg0, arg1)` with a fixed-arity loop closure forwarding `params?.(branch, arg0, arg1)`) — eliminates the allocation AND lets the generated `$params` wrapper (`($scope, $params2) => row($scope, $params2[0])`) eta-collapse to the row fn when the body only reads the item, reducing bundle; a simpler single-reused-array variant works but costs ~+14 brotli of runtime. The positional form changes the branch-renderer params ABI shared by for/await/try/if (`getBranchRendererArgs`), so it needs a full regen and care. Not benchmark-visible (every keyed op profiles 89-99% native/paint on 6.3.20; the args-array is sub-noise); value is general large-list apps. Verify: `rg -n "\[item, i\]|\[key, value\]|\[v\]" packages/runtime-tags/src/dom/control-flow.ts`.
