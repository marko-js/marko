...
## Extend dynamic-native-tag compilation to bodies and custom tags

`packages/runtime-tags/src/translator/visitors/tag/dynamic-tag.ts` | 2026-07-03 | impact:med | effort:high

Stage 1 landed: a dynamic tag whose name is a non-nullable, statically enumerable set of safe native names (`link ? "a" : "span"`) now compiles as a real native element with a `_dynamic_native_tag` name-swap (rename = move attributes/children to a fresh element, `dom/dom.ts`) instead of the general `_dynamic_tag` branch runtime, dropping `createBranchWithTagNameOrRenderer`, `normalizeDynamicRenderer`, `_attrs`, the content-renderer path, and `_resume_dynamic_tag` (measured ~1.1–2.3 KB brotli, net-positive). The gate (`util/dynamic-native-tag.ts`) currently requires **no body** and excludes tag vars, spread, event/change handlers, and form-control/void/namespaced names.

Two extensions remain:

- **Bodies (stage 2).** Inline the body into the clone template (default name) and, on rename, move the children to the new element — the child DOM nodes are preserved so their signals keep working, exactly the mechanism `<show>` already uses to relocate a body without re-rendering (`_show` in `dom/control-flow.ts`). This is the ticket's full case (`<${link ? "a" : "span"}>${label}</>`), where today the body is compiled as a separate client renderer. Needs the body section's walks/content signals to attach to the inlined element rather than a branch.

- **Custom tags rendering their body unconditionally once.** The same body-inlining could apply to a custom tag proven to render its `content` unconditionally exactly once: the parent would place the body inline (reusing the `<show>`-style move, or the existing `kDirectContent`/`_dynamic_tag_content` direct-content machinery) instead of passing a separate content renderer. Needs cross-template analysis of the child's body-usage pattern — a sibling of the empty-setup elision analysis in `util/setup-statements.ts`.

Also minor: a gated dynamic native tag still emits a leading `<!>` marker in the clone template (from the surrounding section), which a native element does not need; worth checking whether it can be elided.
