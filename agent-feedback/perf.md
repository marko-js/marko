...
## Serializer scope-prop inlining is limited to numbers; extending it needs a monomorphic-safe design

`packages/runtime-tags/src/html/serializer.ts:1560` | 2026-07-19 | impact:med | effort:high

`writeObjectProps` now writes numeric props in a single buffered chunk (skipping the `writeProp` dispatch + a second `buf.push`), which measured ~15% faster on numeric-dense scope payloads (a 1500-row data grid) with no regression elsewhere. Extending the same single-push inlining to _all_ primitives (booleans, short strings) measured even better on pure-numeric grids (~19-22%) but regressed reference/string-heavy pages by ~3-5%: adding cases to the per-property branch hurts this hot, effectively-megamorphic loop (a long-lived server serializes every component shape through it), and short-string inlining via `quote` in the loop is especially fragile. A shape-adaptive fast path (detect uniform primitive-heavy scopes once and use a specialized loop) or another monomorphic-friendly approach could capture the remaining win without the regression. Separately, the `_(id)` memoization added here covers `writeObject`/`ensureId` but not the scoped registered-function form `_(id,"...")` at `serializer.ts:698`, which still rebuilds its string per emission.
