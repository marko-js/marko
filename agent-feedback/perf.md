...
pages (`scripts/measure-persisted-wire.ts`) before readopting.

## Structural-patch second-pass optimizations (post shell refactor)

`designs/persisted-pages-structural-patch-proposal.md` | 2026-07-20 | impact:med | effort:med
...
entirely, so re-run `scripts/measure-persisted-scaling.ts` and ratchet the
