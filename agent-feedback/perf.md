...
## Deduplicate structurally identical signal fns into shared declarations

`packages/runtime-tags/src/translator/util/signals.ts` › `writeSignals` | 2026-07-22 | impact:med | effort:med

Repeated template patterns (sibling loops/conditionals with the same body) emit byte-identical signal fns per section — five copies of `_render(($scope, text) => _text($scope.a, text))` plus five identical `$params` forwarders in `for-by` — and minifiers never merge function literals (verified: rolldown/oxc, terser, and esbuild all keep them; an alias const `const b = a` survives minification at ~3 bytes and is brotli-free). A prototype (orphaned commits `e6b5905d` + `b73a6677` on the `claude/marko-async-transitions-pyk8zt` reflog, dropped as not-worth-it-yet) measured −343 bytes min across fixtures at base (for-by −176, async-multi-resolve-in-order-and-update −139) with zero regressions. Minimal shape: at the single point where a top-level `const name = value` signal declaration is written, key shareable values (raw fn expressions only — renderer/registration factories like `_content`/`_for_of` carry identity) on a structural AST serialization and emit `const name = canonicalName` on a hit; plus a name→canonical alias map so forwarders referencing per-section aliases still match. Hard-won constraints for any reimplementation: key only after `traverseReplace` swaps binding reads for accessors (source names both under-dedupe — `i` vs `key` reading the same accessor — and can false-merge — same name, different accessor); identifiers in name position (non-computed member properties/object keys, labels, private names) must serialize literally or distinct-accessor effects merge (`define-tag-render-args` miscompiled); never hoist shared consts to program-body end (a template with a top-level `throw` had its const tree-shaken as unreachable, leaving a dangling reference in `lazy-tag-load-error`); walk fields via `t.NODE_FIELDS`, not `Object.keys`, so builder-vs-parser field presence doesn't silently break matching. Re-verify the claim: compile `packages/runtime-tags/src/__tests__/fixtures/for-by/template.marko` with `npm run compile -- -o dom` and count the identical `_text` arrow fns.
