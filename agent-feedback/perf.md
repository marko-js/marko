...
The always-string direction (`link ? "a" : "span"`) has an even smaller ceiling: it must keep the native/`_attrs`/form-control path (it _is_ a native element) and can only drop the renderer machinery, which is tiny. Truly compiling it "like a native tag with a dynamic name" (inlining the body as native content rather than a client-compiled renderer, the reported case of a dynamic native tag shipping the generic form-control runtime and compiling its body client-side) is a much larger change: `native-tag.ts` is built entirely around a compile-time-constant tag name (`getCanonicalTagName` → `t.stringLiteral`, `write\`<${tagName}\``, and switches on the name for void/select/textarea/head/body handling), so threading a runtime name expression through it is a rewrite, not a spot fix. A narrower version — when the name is a statically enumerable set of literals (`"a" | "span"`), desugar to `<if>`/`<else>` over static native tags — avoids the dynamic runtime entirely but duplicates the body/attrs codegen per branch and only applies to enumerable-literal names.
