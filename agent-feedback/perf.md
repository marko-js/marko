...
## Emit the bare value for a lone text-only interpolation instead of a redundant `${_to_text(x)}` template wrapper

`packages/runtime-tags/src/translator/util/body-to-text-literal.ts` › `bodyToTextLiteral` | 2026-07-20 | impact:low | effort:low

`bodyToTextLiteral` calls `buildTextLiteral(body, toText, false)` (body-to-text-literal.ts:18), forcing `bareSingle=false`, so a text-only native tag or html-comment whose body is a single interpolation `${x}` compiles (DOM) to a template literal wrapping the coerced value: `<title>${input.pageTitle}</title>` emits `_text_content($scope["#title/0"], `${_to_text(input_pageTitle)}`)` and drags in the `_to_text` import. Both the `${…}`wrapper and the explicit`_to_text`are dead weight, because the sinks`_text`/`_text_content`already re-coerce via`_to_text`internally (dom.ts:157,165) and`_to_text`always returns a string (dom.ts:45-52) —`_text_content(node, input_pageTitle)`is byte-for-byte equivalent (verified in node that`_to_text(x) === _to_text(`${_to_text(x)}`)`for`0`/`null`/`undefined`/`false`/`""`/objects/arrays). The sibling paths already do the bare thing: the standalone-placeholder DOM path emits `_text($scope["#text/0"], input_x)` (placeholder.ts:170-175) and `bodyToRawTextLiteral` passes `bareSingle=true` with the comment "a lone interpolation stays bare so the placeholder coerces it directly." Gate the bare return on a lone interpolation with no surrounding static text (the multi-part `<title>Hi ${x}</title>`still needs`_to_text`, since JS template concatenation would stringify `null`→`"null"`), and return the original placeholder value node so the render-statement keying survives for free — native-tag.ts:970 reads `textLiteral.extra?.referencedBindings`, and that node's `extra`is exactly the`placeholderExtra`the template branch copies at line 79, whereas a fresh`_to_text(child.value)`node would lack it. Re-verify:`npm run compile -- -o dom -d`on`<title>${input.pageTitle}</title>` still emits `_text_content($scope["#title/0"], `${_to_text(input_pageTitle)}`)`plus a`_to_text`import, where bare`_text_content($scope["#title/0"], input_pageTitle)` is equivalent.
