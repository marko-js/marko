...
**Compiler half RESOLVED** (2026-07-09): analyze-stage pre-registration
landed (`translator/util/preallocate-register-ids.ts`). Analyze runs once
per cached file and enumerates the full register-key union in a canonical
order -- getters, effect groups (recorded as analyze footprints beside the
translate sites they predict), pending closures, update seed/value `var`
keys, structural `update_if_`/`update_dynamic_`/`update_boundary_` keys,
content renderers, `update_globals`, and the root update key -- then
allocates every child id up front when `optimize` +
`optimizeKnownTemplates` are set. Translates only ever look up existing
slots, so compile scheduling cannot permute ids; a translate-time request
for a key analyze failed to predict throws an internal error (the
`analyzeUpdateGeneric` tripwire pattern), turning the old silent
production divergence into a loud build failure in every mode. Verified
by corpus diffing (temporary instrumentation logged every requested key
across the full suite vs the enumeration: zero unpredicted keys; eight
accepted over-enumerations -- five `update` root keys on update-generic
templates kept for `load=` parents that force a child update module, two
empty/unused content sections, one confidently-evaluated spread) and by
three consecutive `test:parallel` runs with byte-identical output. The
enumeration also surfaced and fixed a real waste: plain (non-persisted)
`load=` builds allocated an unused `<sid>_update` slot on the child
(`custom-tag.ts` now gates the load id on `isPersisted()`).
