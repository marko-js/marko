...
## Drop a failed async `<try>` body's already-written scopes from the resume payload on catch

`packages/runtime-tags/src/html/writer.ts` › `tryCatch` (async reorder-abort path) | 2026-07-20 | impact:low | effort:med

The sync-error path now drops the body's scopes (any written at/above the scope-id mark taken before the body render, since sync render is contiguous). The **async** reorder-abort cleanup (~931-938) still clears the discarded chunks' `html`/`scripts`/`effects` but leaves the body's scope entries in the shared `serializeState`, so they ship as dead fills. The sync scope-id-range trick doesn't transfer: an async body interleaves with siblings, so a before/after id snapshot would also drop sibling scopes. A complete fix needs the async body to write into an isolated `serializeState` (merged on success, dropped on abort) — which entangles with the reorder machinery, since `bodyNext = body.fork(...)` would inherit that isolated state for post-`<try>` content.
