...
A tag with `<let/open:=input.open>` at the top and `${input.submitLabel || "OK"}` referenced _only_ inside a child's attribute tag body (`<modal-view open:=open><@footer><button>${input.submitLabel || "OK"}</button></@footer></modal-view>`) compiles the DOM setter for `submitLabel` and the `let open` accessor to the same scope index: in the minified bundle `submitLabel`'s setter is `M(9, …)` and the `let` writer is `Ze(9)`, both keyed `_.j`, so passing `submitLabel="Install" open=true` renders the button as `true` (the later `open` write clobbers it). Reading the input into a `<const/submitLabel=(input.submitLabel || "OK")>` before the `<let>` avoids it, which points at slot allocation for inputs first touched inside a nested attribute-tag renderBody rather than at serialization. Re-verify with a fixture of the three-line shape above (a `prompt-dialog` wrapping a `modal-view` whose `@footer` renders `input.submitLabel`), mounted client-side with `submitLabel="Install" open=true` — the footer button reads `true`; with `submitLabel` hoisted into a top-level `<const>` it reads `Install`.
