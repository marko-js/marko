...
## CSR walk misbinds every element after a dynamic `<style>` (and after `<html-script>` following a `content=` attr)

`packages/runtime-tags/src/translator/core/style.ts` + `dom/walker.ts` | 2026-08-03 | impact:high | effort:med

While building the `render-effect-helpers` fixture, a template with a dynamic `<style>` (`color: ${expr}`) placed mid-template left every subsequently walked element unbound in CSR: `scope["#script/14"]` (an `<html-script>` after the style) was `undefined` at `$setup`, crashing `_attr_nonce`, and after reordering, the `$!{html}` marker after the style crashed `_html` on `undefined.parentNode`. SSR + resume of the same template works. Moving the `<style>` to the end of the template avoids it, which is why no existing fixture catches this: every `style-tag-*` fixture has only static content after the style. A second, possibly related shape: `<html-script>` directly after a `<div content=...>` misbound the same way. Suspect the CSR template/walks emitted for the style (or the swap between its extracted-vs-inline forms) shifts walk offsets for later nodes. Re-verify: CSR-render a template with `<style>.x { color: ${input.c}; }</style>` followed by `<div data-x=input.c/>`, and observe the div's accessor is unbound at setup.
