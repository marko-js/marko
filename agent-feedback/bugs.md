...
## Held DOM updates go stale for helpers that read live DOM state, so `_attrs` can leak an attribute once updates are held past their own flush

`packages/runtime-tags/src/dom/dom.ts` › `_attrs` | 2026-07-24 | impact:med | effort:med

`_enable_hold` (see `packages/runtime-tags/src/dom/hold.ts`) buffers observable DOM updates keyed by target node and drains them after the render flush, so between an update and its drain the live DOM is stale. Any helper that reads live DOM state to compute its next write is therefore reading a value that does not include its own pending updates. `_style_rule_item` had exactly this defect (two rules on one `<style>` in one flush: the second read the pre-hold `textContent` and dropped the first rule) and is fixed by reading back the pending text via `heldText`. `_attrs` has the same shape and is still unfixed: it iterates `el.attributes` to remove attributes absent from `nextAttrs`, but a held `_attr` write is not in `el.attributes` yet, so the removal loop cannot see it while the keyed record still carries it — at drain the stale value is written back and the attribute leaks. This is unreachable while every flush drains its own updates (the render queue coalesces `_attrs` for one element into a single render), and becomes reachable as soon as updates are held across flushes for async transitions. Suggested direction: have the removal loop consult the pending record for the element (an `heldAttrs(el)` accessor alongside `heldText`), or drop the element's pending attribute record when `_attrs` takes ownership of the full attribute set. Re-verify by holding `_attr(el,"foo","1")` in one `holdRenders` pass and calling `_attrs` with a `nextAttrs` lacking `foo` in a later pass before the drain, then asserting `el.getAttribute("foo") === null`.
