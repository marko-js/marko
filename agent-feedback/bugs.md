...
## Input attr read only inside an attribute-tag body collides with a `<let>` scope slot

`packages/runtime-tags/src/translator` › tags scope-slot allocation (`<let/x:=input.x>` + `@attrTag` body) | 2026-08-16 | impact:high | effort:med

A tag with `<let/open:=input.open>` at the top and `${input.submitLabel || "OK"}` referenced _only_ inside a child's attribute tag body (`<modal-view open:=open><@footer><button>${input.submitLabel || "OK"}</button></@footer></modal-view>`) compiles the DOM setter for `submitLabel` and the `let open` accessor to the same scope index: in the minified bundle `submitLabel`'s setter is `M(9, …)` and the `let` writer is `Ze(9)`, both keyed `_.j`, so passing `submitLabel="Install" open=true` renders the button as `true` (the later `open` write clobbers it). Hoisting that one input into a top-level `<const>` only moves the collision onto the next input read in the footer (`label` then rendered `true`); removing the `<let>` and passing `open`/`openChange` straight through to the child avoids it, which points at slot allocation for inputs first touched inside a nested attribute-tag renderBody rather than at serializationsubmitLabel || "OK")>`before the`<let>`avoids it, which points at slot allocation for inputs first touched inside a nested attribute-tag renderBody rather than at serialization. Re-verify with a fixture of the three-line shape above (a`prompt-dialog`wrapping a`modal-view`whose`@footer`renders`input.submitLabel`), mounted client-side with `submitLabel="Install" open=true`— the footer button reads`true`; with the `<let/open>`removed (open passed through) it reads`Install`.
...
## Map attribute tags to a Class-API child's declared `targetProperty` and hand them over as an array

`packages/runtime-tags/src/translator/util/nested-attribute-tags.ts` › `analyzeAttributeTags` | 2026-08-17 | impact:high | effort:med

`analyzeAttributeTags` keys the attr-tag lookup off `getTagName(child)` alone, so `translate-attrs.ts` › `translateAttrs` emits `attrTag`/`attrTags` under the literal tag name, and the compat layer (`packages/runtime-class/src/runtime/helpers/tags-compat/runtime-html.js` › the `patchDynamicTag` input loop) passes that key straight through — it only renames `content` to `renderBody`. A Marko 5 taglib that declares `"@items <item>[]"` therefore never sees `input.items`: `<@item>` arrives as `input.item`, holding an `attrTags` iterable rather than an Array. `@ebay/ebayui-core` declares exactly that for `<ebay-carousel>`/`<ebay-menu-button>`, so a Tags-API call site renders a carousel with zero slides and leaks the attr tags into the component's unknown-attribute passthrough — server output carries `item='{}'` on the root element, and the client path throws `Converting circular structure to JSON` out of `vdom/VElement.js` › `normalizeValue`. Both halves are compile-time information the translator already has via the taglib lookup (`packages/compiler/src/taglib/loader/Tag.js` › `addNestedTag` fills `nestedTag.targetProperty`), so resolve the property name from the child's taglib and materialize an Array when the child is Class API. Re-verify: add a `marko-tag.json` with `"@items <item>[]"` next to `fixtures-interop/interop-attr-tags-tags-to-class/components/class-list.marko` and iterate `input.items` there — the fixture renders an empty `<ul>`.

## Render a Class-API child on the CSR path even when nothing about it changes client side

`packages/runtime-tags/src/translator/visitors/tag/dynamic-tag.ts` › `translate.exit` | 2026-08-17 | impact:med | effort:med

A Class-API child is translated as a dynamic tag whose value is registered with `addValue(section, tagExtra.referencedBindings, signal, tagExpression)`; when neither the tag nor its input references a client-side binding, the DOM output ends up with no call that renders it, so `template.mount()` produces nothing where the server rendered the whole subtree. With a Tags parent holding `<class-child><span class="greet">hi</span></class-child>` over a `// use class` child of `<div class="wrap"><${input.renderBody}/></div>`, SSR emits `<div class=wrap><span class=greet>hi</span></div>` while CSR emits `<!----><!---->` — the compiled DOM module even defines the content renderer (`_content_resume("…", "<span class=greet>hi</span>", …)`) and then never calls it. Making the child interactive (any `class {}` state it updates itself) or making its input depend on a `<let>` restores it, which is why `fixtures-interop/interop-inert-class-with-stateful-tags-child` passes and this case is uncovered. It matters because `@marko/testing-library`'s browser `render` mounts client-side, so any page embedding an inert Class child — a layout wrapper, a header — is untestable there and silently renders empty. Re-verify: add that two-file fixture under `fixtures-interop` and compare its `render.debug.md` CSR output against SSR.
