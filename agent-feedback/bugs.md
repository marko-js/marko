...
## Forward-referenced `<const>` function calls silently drop every argument

`packages/runtime-tags/src/dom/signals.ts` › `_hoist` | 2026-08-11 | impact:high | effort:med

Calling a template `<const>` function that is declared later in the template — `<const/first=() => second("A", 42)>` above `<const/second=(name, num) => …>` — compiles the call through a hoist accessor (`$second_getter($scope)("A", 42)`), and the function `_hoist` returns is a zero-parameter getter thunk, so the call's arguments are discarded while the target still executes: every parameter arrives `undefined`. No warning at compile or runtime; in an app this surfaced as `JSON.stringify({ fullName, number })` producing `"{}"` in a fetch body. Either thread call arguments through the hoisted accessor or reject forward-referenced const _calls_ with a located compile error. Re-verify: fixture `<let/result="pending"><const/first=() => { result = second("A", 42) }><const/second=(name, num) => `${name}:${num}`><button onClick() { first() }>go</button><div>${result}</div>` with a click step — `render.md` shows the div update to `undefined:undefined` instead of `A:42`.

## Compile a located error (or support) for `async`/`await` in attribute-method handlers

`packages/compiler/src/babel-plugin/parser.js` › `onAttrMethod` | 2026-08-11 | impact:med | effort:med

`onAttrMethod` always builds `t.functionExpression(undefined, params, body)` — never async — while `parseStatements` happily parses `await` in the body, so `<button onClick() { await x() }>` emits `function () { await x(); }`: invalid JS that surfaces later as the bundler's locationless "`await` is only allowed within async functions" parse error, nowhere near the template. Writing `async onClick() {…}` is worse: the parser reads `async` as a separate boolean attribute (`<button async>` lands in the emitted template) and the handler compiles as before. Until async handlers are supported (reportedly planned), `onAttrMethod` should mark the emitted function async when its body awaits, or the translator should raise a located diagnostic on top-level `await`/an `async` modifier. Re-verify: `pnpm compile -t tags -o dom` on `<button onClick() { await Promise.resolve(1) }>go</button>` emits a plain `function () { await … }`, and the `async onClick` variant emits `<button async>` in `$template`.
