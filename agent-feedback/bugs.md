...
## Flush the pending HTML buffer before a controlled `<select>` opens, so preceding sibling markup stops being swallowed into its content arrow

`packages/runtime-tags/src/translator/visitors/tag/native-tag.ts` › `translate.html.exit (the `selectArgs` branch)` | 2026-07-27 | impact:high | effort:low

When a `<select>` carries `value`/`valueChange` (statically or via a spread — both set `htmlSelectArgs`), the HTML translator's exit calls `writer.flushInto(tag)` to move the buffered markup into the arrow it passes as `_attr_select_value`'s `content` argument, but nothing ever calls `writer.flushBefore(tag)` first the way the sibling `kTagContentAttr` branch and every statement-emitting core tag (`core/if.ts`, `core/for.ts`, `core/await.ts`) do. `flushInto` consumes the _entire_ pending buffer for the section, so all markup preceding the select is dragged inside the arrow and evaluated under `withContext(kSelectedValue, …)` — the committed snapshot `src/__tests__/fixtures/controllable-select-mutated-option/__snapshots__/html.bundle.js` shows this as `_html("<form><select>")` inside the arrow. Any `<option value>` in that preceding markup — a second, uncontrolled `<select>`, an `<optgroup>`, a `<datalist>` — is then stamped `selected` by `_attr_option_value` (`packages/runtime-tags/src/html/attrs.ts`), which the DOM runtime never does (`-o dom` emits a single `_attr_select_value_default($scope, "#select/0", …)` touching only the controlled select), so a form with two selects ships a wrong preselected option and diverges from CSR. The fix belongs in the `html` `enter` visitor: resolve `getUsedAttrs`/`staticControllable` before the ``write`<${tagName}` `` call and call `writer.flushBefore(tag)` when the select is controlled — flushing at `exit` is too late, since by then the buffer also holds the select's own open tag, body and `</select>`, and would leave the arrow empty. This is a different defect from "Render a value-carrying dynamic `<select>`'s body through `_dynamic_tag`…", which concerns the `html/dynamic-tag.ts` string-renderer path and missing branch markers, and from "Don't emit the 'no matching `<option>`' dev error…", which is a MARKO_DEBUG false positive on otherwise-correct HTML. Re-verify: `pnpm run compile -o html -d` on `<select><option value="a">A</option><option value="b">B</option></select><select value=input.v><option value="a">A</option><option value="b">B</option></select>` puts _both_ selects inside the `_attr_select_value(…, () => { _html(…) })` arrow, and server-rendering it with `v="b"` marks `<option value=b selected>` in the first, uncontrolled select; putting the controlled select first renders correctly.

## Write the controlled scope for a bodyless dynamic `<select value:=…>` so SSR resume matches CSR

`packages/runtime-tags/src/html/dynamic-tag.ts` › `_dynamic_tag` | 2026-07-27 | impact:low | effort:low

In the string-renderer branch of the HTML `_dynamic_tag`, the `renderer === "select" && ("value" in input || "valueChange" in input)` call to `_attr_select_value` sits inside `if (renderContent)`, and `_attrs`'s `case "select"` (`html/attrs.ts`) only widens `skip` to drop `value`/`valueChange` from the markup — so a dynamic tag resolving to `"select"` with no body writes neither `ControlledType`/`ControlledHandler` nor the `_dynamicTagScript` resume registration. CSR has no such guard: `dom/control-flow.ts` › `_dynamic_tag` always passes `controllableRenders[tagName]` into `_attrs`, and `dom/controllable.ts` › `_controllable_select` only tests `"value" in nextAttrs || "valueChange" in nextAttrs`, so the same template is controlled after a client mount but uncontrolled after SSR resume until the dynamic tag next re-renders (options injected into the empty select in the meantime are picked up by `_attr_select_value_script`'s `observeOnce` under CSR and ignored after resume). The select call is the only controllable gated on having a body — `tag="input"`/`"textarea"` register correctly because their helpers run outside the guard, and the static bodyless `<select value:=v/>` registers because the translator always hands `_attr_select_value` a body closure — so hoist the select case out of `if (renderContent)` and pass `renderContent` (possibly undefined) as its optional `content` argument. This is a different defect from the entry "Render a value-carrying dynamic `<select>`'s body through `_dynamic_tag` so it gets a resume branch like every other native dynamic tag", which concerns the branch-marker/`ConditionalRenderer` shape when a body _does_ exist and whose fix stays inside the same content guard. Re-verify: compile `<let/v="a"/><${input.tag} value:=v/>` with `pnpm run compile -o html -d` and server-render it twice — the `{"tag":"input"}` payload contains `"ControlledType:#input/0":2`, `"ControlledHandler:#input/0"` and `"_dynamicTagScript 2"`, the `{"tag":"select"}` payload contains none of them.
