...
## Skip regular expression literals when scanning a tag's attributes for `<!--`

`packages/compiler/src/babel-plugin/parser.js` › `findHTMLCommentInAttrs` | 2026-08-14 | impact:low | effort:med

`findHTMLCommentInAttrs` skips strings and JS comments but not regular expression literals, so an attribute value whose regex contains the sequence is rejected outright: `<div hidden=/<!--/.test(s) />` compiled before the scan was added and now fails with "HTML comments are not supported in a tag's attributes". Telling a regex from division needs the previous significant token plus character-class handling (`/[/]/`), which is why it was left out, but this is valid code the compiler now refuses. Deleting the scan is the better end state: htmljs-parser's `checkForOperators` already refuses to continue an attribute expression across `</`, so `<!--` could join it with its own error code and catch the orphan `-->` case too — though marko-js/htmljs-parser#170 raised this exact scenario and was closed as "complicates things more than it is worth", so it needs a fresh case made for the error rather than for emitting the comment. Re-verify: `compileSync('<div hidden=/<!--/.test(s) />', "/t.marko", { output: "html" })` throws.
