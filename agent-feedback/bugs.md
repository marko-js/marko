...
`packages/runtime-tags/src/common/errors.ts:65` | 2026-07-19 | impact:med | effort:low
...
## MARKO_DEBUG dynamic-tag validation misses `.default`/bare non-renderers, silently dropping the wrapper element

`packages/runtime-tags/src/common/helpers.ts:155` | 2026-07-19 | impact:med | effort:low

A dynamic tag `<${input.v}>inner</>` whose value is a truthy object/function that is not a Marko renderer — a plain function, `Promise.resolve()`, a `new Map()`, an `import * as X` namespace, or `{ default: <non-renderer> }` — renders just the body `inner` with NO wrapper element and throws nothing, even under `MARKO_DEBUG`. `normalizeDynamicRenderer` (`common/helpers.ts:150`) computes `normalized = value.content || value.default || value` (:155); the DEBUG block (:156-171) only throws for `value.content` being a non-renderer (:161) and for non-object/function primitives (:166), so a truthy object/function whose `.default` (or the value itself) is a non-renderer falls through both guards and the function returns `undefined`. `html/dynamic-tag.ts` then takes the `renderer === undefined` path `else if (content) { return content(); }` (:172-173), emitting the body with no wrapper; the shared DOM path (`dom/dom.ts` `normalizeClientRender`) behaves identically, so it is a consistent silent no-op, not a hydration mismatch. The asymmetry is stark: `{ content: nonRenderer }` throws loudly while `{ default: nonRenderer }` is silent — yet a misimport, a dynamic `import()` whose `.default` is the wrong module, or a lazily-loaded template of the wrong file is exactly the shape that produces `.default`. Widen the DEBUG assert at `helpers.ts:156-171` to throw the same "not a template/component" error whenever `normalized` lacks a `RendererProp.Id`, covering `.default` and any truthy non-renderer object/function. Distinct from the three existing dynamic-tag entries (renderer-id conflation at `control-flow.ts:535`, dynamic native tag vars at `html/dynamic-tag.ts:146`, and the perf specialization at `perf.md`), none of which touch this DEBUG gap.

## Assert `<for of>` receives an iterable instead of silently dropping falsy values, iterating strings per-char, or throwing context-free

`packages/runtime-tags/src/common/for.ts:16` | 2026-07-19 | impact:med | effort:low

`<for|item| of=input.v>` handles a non-array value three different wrong ways with no diagnostic. `forOf` (`common/for.ts:12-22`) guards `if (list)` (:16) then does a native `for (const item of list)` (:18): a falsy scalar (`0`, `-0`, `0n`, `NaN`, `""`, `false`) renders nothing silently; a string is truthy and iterable so it iterates per code point (`"Bob"` → `[B][o][b]`); and any truthy non-iterable (a plain object, a `Date`, a number like `5`, a bigint) reaches the native for-of and throws the engine's own `TypeError: list is not iterable`, whose stack points into `marko/dist/debug/html.js` with no tag name, template path, or line. So the same `<for of=x>` is silent for `x=0` but throws for `x=5` (both numbers), and the throw carries no source context. There is no `assertValidList` in `common/errors.ts`. Add a `MARKO_DEBUG` assertion — `value == null || typeof value[Symbol.iterator] === "function"` — that throws a tag/source-anchored error for the non-iterable case and optionally warns for a bare string, turning all three silent/opaque outcomes into one locatable message. Shared by html `_for_of` (`writer.ts:321`) and the DOM runtime, so behavior is universal.

## Guard `<for to>`/`<for until>` against non-finite bounds instead of hanging the render in an infinite loop

`packages/runtime-tags/src/common/for.ts:32` | 2026-07-19 | impact:med | effort:low

`<for|i| to=input.v>` with a non-finite bound hangs the render forever. `forTo` (`common/for.ts:24-35`) computes `steps = (to - start) / delta` (:32) and loops `for (i=0; i <= steps; i++)`; with `to=Infinity`, `steps=Infinity` and `i <= Infinity` is always true, so it is a synchronous infinite loop — and because the loop body is synchronous JS, in `@marko/run` SSR it blocks the entire Node event loop, not just the one request (a `Math.ceil(total/perPage)` with `perPage===0` yields `Infinity` and takes the server down). `to=NaN` instead makes `i <= NaN` false immediately and renders nothing silently. `forUntil` (:37-48, `i < steps` at :45) has the same shape. There is no finiteness guard anywhere and no assertion in `common/errors.ts`, in any mode. In `MARKO_DEBUG`, assert `Number.isFinite(to)` / `Number.isFinite(until)` and throw a tag-anchored error; a runtime clamp (or refusing to loop on a non-finite bound) would additionally prevent the production event-loop hang.

## Mount `<script>`/effect run order diverges between SSR-hydration and client-created branches

`packages/runtime-tags/src/translator/util/signals.ts:1265` | 2026-07-19 | impact:med | effort:med
...
`packages/runtime-class/src/runtime/helpers/tags-compat/runtime-html.js:193` | 2026-07-19 | impact:high | effort:med
...
`packages/runtime-class/src/runtime/helpers/tags-compat/runtime-html.js:190` | 2026-07-19 | impact:med | effort:low
...
`packages/compiler/src/babel-utils/tags.js:353` | 2026-07-19 | impact:high | effort:med
...
`docs/reference/typescript.md:175` | 2026-07-19 | impact:med | effort:low
...
`packages/runtime-tags/src/dom/schedule.ts:34` | 2026-07-19 | impact:med | effort:low
