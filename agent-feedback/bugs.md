...
## Give a dynamic tag name and sibling-visible `<for>` params tag variable cycle edges

`packages/runtime-tags/src/translator/util/references.ts` › `assertNoValueCycles` | 2026-08-12 | impact:low | effort:med

The cycle graph is `binding.upstreamAlias` plus `binding.initDeps`, recorded against `expr.downstream`, and two declaration paths contribute nothing. A dynamic tag's name is not one of its reference nodes, so `<${a}/b/><const/a=b/>` compiles, and so does `<${a}/b x=1/><const/a=b/>` — the name read is not inherited through the merged tag extra, so an attribute does not rescue it. Separately a `<for>` param is not a reference path of a sibling expression, so `<const/fns=[fn]/><for|fn| of=fns><button onClick=fn/></for>` compiles: a legal hoist of a function that is still an init cycle. A cycle wholly inside the body is already reported (`<for|item| of=items><const/items=[item]/></for>` errors), so this is only the sibling direction. Make `tag.name` an explicit edge the way `dynamic-tag.ts` › `analyze` already treats it, and give param bindings an edge to the owner tag's `of=`/`value=` expression. Re-verify: compile the three templates above with `-o dom`; all succeed.

## Restore tag variable cycle coverage for controllable `<let>` and accessor bodies

`packages/runtime-tags/src/translator/util/references.ts` › `assertNoValueCycles` | 2026-08-13 | impact:low | effort:med

Two shapes are excluded from the cycle graph to avoid rejecting valid templates, and each hides a real cycle. A controllable `<let>` merges `value=` and `valueChange=` into one tag extra, so a handler reference is indistinguishable from a value dependency; `binding.skipInitEdges` drops the tag entirely, so `<let/a=b valueChange=noop/><let/b=a valueChange=noop/>` compiles. Separately, `read.afterInit` treats every function body as post-init, including accessors, so `<const/a={ get x() { return b } }/><const/b=a.x/>` compiles even though evaluating `b` runs the getter. Fix the first by giving `<let>` a downstream expression covering only `value=`, and the second by distinguishing an accessor that is invoked during another binding's initializer from one that is merely defined. Re-verify: compile both with `-o dom`; both succeed today, while the same shapes without `valueChange=`/without the accessor report a cycle.

## Do not treat class method/field bodies as init-time reads

`packages/runtime-tags/src/translator/util/get-root.ts` › `isFunction` | 2026-08-13 | impact:low | effort:low

`read.afterInit` is set only when `getFnRoot` finds a function, and `isFunction` accepts `FunctionExpression` / `ArrowFunctionExpression` / `ObjectMethod` but not `ClassMethod` / `ClassPrivateMethod` / `ClassPrivateProperty`. A read inside `class { m() { return b } }` or `class { x = b }` is therefore eager, so `<const/a=class { m() { return b } }/><const/b=a/>` is reported as a tag-variable cycle even though constructing the class does not run the method or instance field. Object methods of the same shape (`<const/a={ m() { return b } }/><const/b=a/>`) compile. Teach `isFunction` (or `getFnRoot`) to treat class members like `ObjectMethod`, while leaving `ClassStaticBlock` and `extends` eager. Re-verify: compile the class-method template with `-o dom`; it must succeed, while `<const/a=class { static { b } }/><const/b=a/>` still reports a cycle.
