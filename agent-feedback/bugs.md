...
## Terminate `getDebugName`'s input-alias walk at the section's own params binding — reaching it crashes instead of naming it

`packages/runtime-tags/src/translator/util/references.ts` › `getDebugName` | 2026-07-29 | impact:low | effort:low

For a `BindingType.input` binding, `getDebugName` walks `root = root.upstreamAlias` while `root.upstreamAlias !== root.section.params`, i.e. it stops one hop _before_ the params binding. Handed the params binding itself it never satisfies that test — a section's `params` has `upstreamAlias === undefined` while `section.params` is the binding itself, so `undefined !== params` stays true, `root` becomes `undefined`, and the next iteration throws `TypeError: Cannot read properties of undefined (reading 'upstreamAlias')`. Nothing routes a bare params binding there today, so this is latent, but it is the first thing hit when a new analysis pass contributes `Sources.param` roots: `finalizeParamSerializeReasonGroups` (`util/sections.ts`) names every group with `getDebugNames(reason)`, so any reason whose `param` list includes a section's params binding turns a serialization decision into a compiler crash with no code frame and no mention of the binding. Guard the loop with `root !== root.section.params` as well (returning `root.name + access` for that case), so the walk terminates on the binding it is already using as its sentinel. Re-verify: in `finalizeReferences`, after the `pruneBinding` loop, add `for (const b of bindings) if (b.type !== BindingType.dom) addSerializeReason(b.section, b.sources, b);` and compile any template with a `<for|item| of=input.items>` body — it throws the `TypeError` above from `getDebugName`, and stops throwing once the loop terminates at the params binding.
