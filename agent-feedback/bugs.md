...
## Keep the `<id>` fallback memo slot out of the pruned create-time id space

`packages/runtime-tags/src/translator/core/id.ts` › `analyze` | 2026-08-08 | impact:high | effort:low

The synthetic `BindingType.local` `#id` fallback has no `reads`, so `pruneBinding` drops it from the program set before dense per-section renumbering. Translate still emits `getScopeAccessorLiteral(fallbackBinding)`, which then uses the stale create-time id. In optimize mode that accessor collides with a later dense-assigned scope key (DOM node or value) in the same section: `$scope["e"] ||= _id($scope)` overwrites/aliases `$scope.e` used as a walk node. The simple `id-tag` fixture is collision-free by accident. Either stop using a freestanding binding (prefer a prefix on the tag-var binding) or retain the binding through finalize (`pruned = false` early-return, or any path that keeps it in `section.bindings` for dense ids). Re-verify: compile with optimize `<id/z=input.z/><div id=z>${z}</div>` plus ≥15 later used `<let>`s and confirm the `||=` key is not also a `_text($scope.<key>, …)` node accessor.
