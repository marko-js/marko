...
## Rebuild taglib metadata after `clearCaches()` — a hot reload leaves core attributes untyped, and serial `pnpm test` is red on `main`

`packages/compiler/src/taglib/index.js` › `clearCaches` | 2026-07-28 | impact:high | effort:med

Once a compile has run with an **inline translator object** (one not already keyed in the module-level `loadedTranslatorsTaglibs` map), a subsequent `compiler.clearCaches()` leaves the taglib layer able to build a lookup with missing metadata: the first `buildLookup` after it resolves `buildTaglibLookup(dir).getAttribute("div", "no-update")` to an attribute whose `type` is `null` instead of `"flag"`. `clearCaches` resets `loader`, `finder` and `lookupCache`, but the module-level `registeredTaglibs` (seeded once at import by the three `register(marko*Taglib)` calls) and `loadedTranslatorsTaglibs` survive it, so taglib objects created before the loader cache was cleared get merged with ones created after — and `lookupCache`'s key is only the joined taglib **ids**, which cannot distinguish the two. This is the hot-reload path, since `packages/runtime-class/src/node-require/hot-reload.js` calls `compiler.clearCaches()` on every reload, so a dev-server reload can silently drop attribute metadata; it also makes the documented `pnpm test` red on clean `main` (`taglib-lookup/fixtures` `core-attributes` and `forEachAttribute`), while CI stays green because `@ci:test` runs `test:parallel` and shards those specs into separate workers. Note as a starting point that clearing `loadedTranslatorsTaglibs` and re-seeding `registeredTaglibs` inside `clearCaches` does **not** fix it, so the stale state lives deeper — most likely in `taglib/loader/cache` or in the `Taglib`/`Tag`/`Attribute` objects those loaders hand back. Re-verify from the repo root: `pnpm exec mocha --no-config --no-package --require ~ts --timeout 60000 packages/compiler/test/babel-utils/tags.test.js packages/runtime-class/test/hot-reload/index.test.js packages/runtime-class/test/taglib-lookup/index.test.js` reports 2 failures, and dropping either of the first two spec files makes it pass.

## Recover a controlled `<select>`'s value on resume; with no matching option at SSR time it resets to `""` and the first late-arriving option overwrites app state

`packages/runtime-tags/src/dom/controllable.ts` › `_attr_select_value_script` | 2026-07-28 | impact:med | effort:med

The `isResuming` branch rebuilds `scope[AccessorPrefix.ControlledValue + nodeAccessor]` by scanning `el.options` for `defaultSelected`, because the value signal does not re-run on resume. When the server rendered no option matching the controlled value — the common "options load after first paint" case, where the `<select>` is emitted empty — nothing is `defaultSelected`, so the controlled value becomes `""` (or `[]`) even though the app's own state still holds the real value. Once options arrive, the observer sees `el.value !== ""`, cannot re-apply `""` to any option, and reports the browser's fallback back through `valueChange`, overwriting the app's value. The client-only path no longer has this problem (options replaced after a client render keep their value, covered by fixture `src/__tests__/fixtures/controllable-select-replaced-options`), so this is now an SSR/CSR divergence rather than a uniform bug. Fixing it likely means serializing the controlled value for a controlled select rather than re-deriving it from the DOM, which is a payload tradeoff worth a maintainer call. Re-verify by adding a fixture whose `template.marko` is `<let/value="b"/><let/options=[]/><select value:=value><for|opt| of=options by=v=>v><option value=opt>${opt}</></></select><div id="result">${value}</div><button.load onClick() { options = ["a","b","c"] }>load</>` with `steps` clicking `.load`, and `equivalent: false` in `test.ts`: `render-csr.debug.md` shows no `#result` change while `render-ssr.debug.md` shows `UPDATE: #result::text "b" => "a"`.
