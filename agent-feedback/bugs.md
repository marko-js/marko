...
## Hydrate a `<try>` placeholder's scopes with the flush that registers its effects, and destroy them on the swap

`packages/runtime-tags/src/html/writer.ts` › `Chunk.flushPlaceholder` | 2026-08-17 | impact:high | effort:high

When the entry runs before a streamed `<await>` body, the placeholder flush's resume payload lists the placeholder content's effects (`…child.marko_0_input_workspace_id#5 7 …shell.marko_0…#4 4`) while `M._.r` in that flush carries only the outer scopes; the placeholder's own scopes (3–7: the placeholder branch, the shell, its `<for>` item, the child) arrive with the body flush. Resume therefore runs a nested tag's `<lifecycle>` against a scope that has no data yet (`onMount` sees `input.workspace` undefined), the direct child's effect is skipped, and when the body swaps the placeholder out none of it is destroyed — an app shell rendered as its own placeholder keeps its document click listener alive and steals every navigation from the real shell (URL changes, nothing renders), which is what work's `app-page` hit as soon as `status` streamed on every load. Either serialize the placeholder's scopes in the flush that registers their effects and run their destroy on the swap (`_try` / `placeholder.c`), or hold those effects with the pending scopes and drop them at swap; today it is half of each. Re-verify: `pnpm test -- --grep "runtime-tags/translator try-placeholder-effects-before-scopes "` — `render-ssr.debug.md` logs `"child mounted" undefined` on the first render and never `"shell destroyed" "loading"`, and `writes.debug.html` shows the effect list ahead of the scopes it names.
