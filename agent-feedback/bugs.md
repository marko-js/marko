...
## Guard getStaticDeclRefs against reference cycles so a self-referencing static/export function does not crash the compile with a stack overflow

`packages/runtime-tags/src/translator/visitors/function.ts` › `getStaticDeclRefs` | 2026-07-20 | impact:high | effort:low

`getStaticDeclRefs` (function.ts:117) recurses on every non-invoked reference of a static/export-scope declaration, but `getDeclarationRoot(ref)` (get-root.ts:80) resolves a self-reference back to the same `FunctionDeclaration`, so the walk re-reads the identical binding and `referencePaths` forever — there is no visited set threaded through. The `isInvokedFunction(ref)` guard at line 130 only excludes references that are _called_, so classic recursion (`tick()`) compiles cleanly while any reference used as a value re-enters at line 136 and overflows the stack. Because this runs in the shared `analyze` phase, a valid template such as `static function tick() { requestAnimationFrame(tick); }` — plus the `export function` variant and a mutual `ping`/`pong` `setTimeout` cycle — aborts every output (dom/html, debug/optimize) with an opaque `RangeError: Maximum call stack size exceeded` and no source location, leaving idiomatic self-scheduling, retry-loop, and self-registering-listener patterns uncompilable. Fix by threading a `Set` of already-visited bindings (or refs) through the recursion and skipping any that were seen before recursing. Re-verify: `npm run compile -- -o dom -d tick.marko` on that `tick` template above a `<div/>` throws the RangeError at function.ts:124/136, while replacing the value `tick` with the call `tick()` compiles with exit 0.

## Strip HTML's ignored leading newline from a `<textarea>` body in preAnalyze so a formatting-only newline stops throwing the value+body error and emitting value="\n"

`packages/runtime-tags/src/translator/core/textarea.ts` › `preAnalyze` | 2026-07-20 | impact:low | effort:low

`<textarea>` parses with `preserveWhitespace: true` (marko-html.json:830), so `preAnalyze` (textarea.ts:5-41) counts a body that is only a leading newline as real content: `tag.node.body.body.length` is truthy, it pushes `"\n"` into `parts`, and `normalizeStringExpression(["\n"])` returns a truthy `stringLiteral("\n")` (normalize-string-expression.ts:70-72). With a `value=` attribute also present, the `find(attr.name === "value")` check throws `A textarea cannot have both a value attribute and body content.`, pointing at the value attribute. So the natural multi-line `<textarea value=v>` + newline + `</textarea>` is rejected while the byte-equivalent single-line `<textarea value=v></textarea>` and self-close `<textarea value=v/>` compile fine — the difference being an invisible newline the author cannot see. The same root cause makes a newline-only body compile to an initial value of `"\n"` (dom emits `_attr_textarea_value_default($scope, "a", "\n")`, setting `el.defaultValue = "\n"`), whereas HTML ignores a single leading newline after the start tag and yields an empty value (SSR round-trips only because the browser re-strips on parse; CSR/hydrate reinstates the `"\n"`). Fix: in `preAnalyze`, strip one leading `\n` (or `\r\n`) from the first `MarkoText` child before building `parts`, matching the HTML parser — a newline-only body then normalizes to `undefined`, eliminating both the spurious conflict error and the `"\n"` initial value, while the intentional real-content error (fixture `error-textarea-value-and-body`, `<textarea value=input.v>fallback</textarea>`) still fires. Re-verify: compiling a `<textarea value=v>` whose `</textarea>` sits on the next line throws the value+body error at `value=v`, whereas the same template with `</textarea>` on the same line compiles cleanly.
