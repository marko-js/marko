...
## Resume misparents an `<await>` branch under a later sibling branch, so toggling the sibling kills the awaited content

`packages/runtime-tags/src/dom/resume.ts` › `createVisitBranches` | 2026-08-13 | impact:high | effort:high

The orphan-adoption loop in `createVisitBranches` (`while (i && orphanBranches[i - 1][AccessorProp.Id] > branchId)`) infers branch nesting from scope-id order, assuming ids are allocated in document order. `_await` in `packages/runtime-tags/src/html/writer.ts` breaks that invariant: the awaited body's branch id is `_peek_scope_id()` at promise-resolve time, after the sibling continuation has already allocated its ids, so the await branch gets a HIGHER id than a stateful branch that FOLLOWS it in the document. On resume the sibling branch's `BranchEnd` then adopts the await branch (`setParentBranch(orphanBranches.pop()!, branch)`), so destroying the sibling branch (e.g. toggling an `<if>`) destroys the resumed await branch even though its DOM is untouched. A fix needs either serialized parent-branch info for async-created branches plus an adoption guard for branches that already have a `ParentBranch`, or an adoption predicate based on DOM containment rather than id order. Re-verify with a fixture: `<let/show=true/><let/n=0/><button#toggle onClick() { show = !show }>toggle</button><await|v|=resolveAfter("A", 1)><button#inc onClick() { n++ }>${v}:${n}</button></await><if=show><span>shown</span></if>` with steps `[{}, flush, wait, clickToggle, clickInc]` — CSR updates `A:0` to `A:1` on the inc click, resumed SSR does not (the harness reports a `render.debug.md` snapshot conflict); dropping the toggle step makes both agree.
