...
## Replace the tag variable self-reference compile guard with an initialization-aware read

`packages/runtime-tags/src/translator/util/references.ts` › `trackReferencesForBinding` | 2026-08-13 | impact:med | effort:high

A tag variable read from inside a function in its own tag's attributes is always a compile error, so `<const/sum=(i = 0) => (i >= items.length ? 0 : items[i] + sum(i + 1))/>` is rejected even though the call happens after the tag is built. The equivalent on a native element is allowed: `trackDomVarReferences` has no such guard, and `<button/$el onClick() { $el() }/>` compiles while an eager read like `<button/$el class=$el().tagName/>` fails at runtime with the guard in `common/errors.ts`. Two implementations were measured and both fail. Dropping the guard for reads inside a function leaves a lexical read, so `<define/Child|input|><button>${input.cb()}</button><return=() => 42/></define>` with `<Child/foo cb() { return foo() }/>` throws `Cannot access 'foo' before initialization` on the server while the client renders `42`, and `<const/x=[() => x]/><div>${x.length}</div>` renders `1` on the server and blank on the client. Routing the read down the hoisted branch instead replaces both with a stated error but then rejects `${sum()}`, because a hoisted value may only be read from scripts and handlers. Forcing a scope read without marking the binding hoisted does not help either: the divergence in the second case comes from signal seeding, not from the read form. The fix is a scope-backed read whose guard means "read before initialization" rather than "outside a script or handler", shared by both outputs, after which the compile guard can go. Re-verify: compile the three templates above; the first is a compile error today and the other two are the checks any change has to pass.
