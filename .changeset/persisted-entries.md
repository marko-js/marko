...
content, and renderers from — a new `entry: "persisted"` compile (the
`?persisted` virtual module, statically imported by the generated
...
bindings and the persisted entry imports them; side-effect statements are
not re-run), and persisted entry builds never re-register main-registered
ids (`writeRegisteredFns` keeps declarations but skips the `_resume`
calls; effects compile through the new non-registering `_script_shared`
...
Requires the paired `@marko/vite` that resolves `?persisted` imports.
