...
function _lazy_template(id, load) {
  _enable_catch();
  let pending,
    params,
    getLazy = () => (pending ||= load().then((r) => ((params = r.d), r)));
  return _template(
    id,
    0,
    0,
    (branch) => {
      let awaitCounter;
      (params || (awaitCounter = addAwaitCounter(branch)),
        getLazy().then(
          (r) =>
            queueAsyncRender(
              branch,
            ...
            (mod) => {
              ((renderer = _content("", ...mod._)()),
                queueAsyncRender(
                  ...
                  awaitCounter
                    ? (child, owner) => {
                        (apply(child, owner), awaitCounter.c());
                      }
                    : apply,
                  -1,
                  owner,
