      ...
      defineRuntime({
        value: (curRenders = (renderId) => {
          ...
          return (
            (render.m = (effects) => {
              (readyIds &&
                readyIds.forEach((readyId) =>
                  processResumes(render.b?.[readyId], effects),
                ),
          ...
          (pending ||= load()).then(
            ...
            (error) => {
              (awaitCounter && (awaitCounter.i = 0),
                queueAsyncRender(branch, renderCatch, error));
            },
          ));
      },
...
function _load_setup(nodeAccessor, childScopeAccessor, load) {
  ...
  let pending,
    renderer,
    insert = (child, owner, awaitCounter) => {
      ...
      if (renderer) insert(child, owner);
        ...
        ((child.X ||= /* @__PURE__ */ new Map()),
          (pending ||= load()).then(
            ...
            (error) => {
              (awaitCounter && (awaitCounter.i = 0),
                queueAsyncRender(child, renderCatch, error));
            },
          ));
      }
    }
